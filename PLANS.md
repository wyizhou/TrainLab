...
2026-09-08用户批准VC-005/A-023：提取必要能力与数据，归档旧业务和已取消验收；
新方案须同步替换规则、代码、资源、入口、测试、CI和文档，不能保留两套有效答案。
运动位置按A-022向选定AI开放，仍不公开或进入Git。每个独立模块适用测试与全新只读验收
通过后提交、正常推送origin/main并核对CI；不包含私人数据、强推或跳过验证。

| 任务 | 状态 | 显式依赖 | Batch | 预期成果 |
| --- | --- | --- | --- | --- |
| [x] `M12-0001` 保护检查点、冻结与统一归档 | completed | 用户批准 | serial-m12 | 检查点及恢复归档完成；旧入口退役/测试映射独立PASS；旧物理代码随替代模块迁移 |
| [x] `M12-0002` FIT 新库与同步 | completed | M12-0001 | serial-m12 | 内部新库/导入/分页/同步恢复已独立PASS并推送；真实全历史同步留0006 |
| [ ] `M12-0003` 解析与 AI 边界及旧系统收敛 | active | M12-0002 | serial-m12 | 0003a–m已交付；VC-005首批资源/周输入/安全接替双平台3049项及独立验收PASS，提交推送/CI与其余旧CLI退出待完成；旧M9失败原因未确定并永久保留 |
