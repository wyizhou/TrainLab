...
### [ ] `M12` FIT 每周系统：跑步独立规划分支 — `active`

当前分支 `work/m12-running-only-planning` 于2026-09-08获用户逐字批准继续开发，
与另一条路线隔离。既有2022起FIT/新库/同步/解析底座不重写；取消健康API、日报AI和高保真邮件。
本分支以[唯一执行计划](docs/exec-plans/active/M12-fit-weekly-0001-rebuild.md)的RUN-VC-001完整合同为准。

全部运动进入总结，但跑课仅依据跑步、可验证跑步历史与用户目标/限制/时间。
每周两次独立AI，先规划后总结，最终同一周报和固定七日计划；共享每周20次、单次20分钟细读。
每天香港22:00同步当日/昨日/缺口并邮件；周日15:00先同步再执行周任务。
Python手动daemon、不装cron/开机项；错过仅补最近一期并标原周期和“补发”，不排过去课程。

采用主Agent＋全新Planner/Developer/Validator；主Agent审核拆解、持久保存证据次数并实际最终验收。
每模块独立验收和主Agent验收后，精确提交并推送 `origin/work/m12-running-only-planning`，
核对远端与CI；不main、不强推、不夹带私人/无关/未验收资料。另一分支不被修改或追溯改判。

历史M12-0001/0002和0003a-m已交付，继续参加回归；继承的0003n原受审结果只作底座证据，
不自动证明本分支新输入隔离/角色/业务闭环通过。旧合同和失败在历史快照中保留，不参与当前裁决。
真实同步、模型、邮件、课程须离线完成后另获精确预算授权；首份真实周任务等待其后下个正常周日15:00。

| 任务 | 状态 | 显式依赖 | 预期成果 |
| --- | --- | --- | --- |
| [x] `M12-R0` 协作与继承基线 | completed | 用户批准 | 保护、三角色模板、规则同步、旧必要基线；d525a39已推送，双平台CI通过 |
| [x] `M12-R1` 底座接线 | completed | M12-R0 | FIT复用/总预算/无归档运行已验收，5c67ead已推送且双平台CI通过 |
| [x] `M12-R2` 两阶段AI | completed | M12-R1 | 跑步专属规划、全运动总结、共享预算与耐久恢复；独立/Main验收及macOS/Linux各3089项全量CI通过，671f7b2已推实验分支 |
| [x] `M12-R3` 周报与课程 | completed | M12-R2 | 七部分周报、固定详细跑课与安全/证据校验；独立及Main实际验收、macOS/Linux各3158项完整CI通过，7bc2175已推送当前分支 |
| [ ] `M12-R4` PDF与修订 | validated | M12-R3 | 同源PDF/Markdown、显式修订、封存及搬迁；3189项完整独立回归、30页实视和Main实际验收通过，分支提交及同提交双平台CI尚待 |
| [ ] `M12-R5` Gmail/Garmin发布 | planned | M12-R4 | 新库单次动作、附件/课程/排期读回 |
| [ ] `M12-R6` 调度与离线交付 | planned | M12-R5 | 统一入口/停机补发/可搬迁/全部旧路径退出 |
| [ ] `M12-R7` 真实验收与切换 | planned | M12-R6+精确授权 | 正常周日两阶段AI/PDF邮件/课表、用户确认后切换 |
