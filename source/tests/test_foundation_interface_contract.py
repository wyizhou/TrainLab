...
def tree_snapshot(root: Path) -> dict[Path, tuple]:
    return {
        path.relative_to(root): (
            ("directory", path.stat().st_mtime_ns)
            if path.is_dir()
...
def test_request_schema_conditions_and_api_reject_without_side_effects(
    tmp_path: Path,
) -> None:
    schema = json.loads(
    ...
    assert not list(validator.iter_errors(valid_migrate)) and not list(
        validator.iter_errors(valid_init)
    )
...
def test_cli_service_integration_isolated_owner_config(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "owner-root"
    owner_config = config(root)
    instance = tmp_path / "instance"
    for name in ("config", "state", "logs"):
        (instance / name).mkdir(parents=True)
    monkeypatch.setenv("TRAINLAB_INSTANCE_ROOT", str(instance))
    monkeypatch.setattr(
        foundation.FoundationConfig, "load", classmethod(lambda cls, _: owner_config)
    )
    schema = Draft202012Validator(
        json.loads(resource_bytes("harness/schemas/foundation_receipt.schema.json"))
    )
    ...
    def call(arguments: list[str], expected: int) -> dict:
        ...
        assert (
            "access_token" not in encoded
            and "refresh_token" not in encoded
            and "raw/" not in encoded
        )
        return payload
    ...
    assert (
        call(
            [
                ...
                "migrate",
                "--target-version",
                str(FOUNDATION_SCHEMA_VERSION + 1),
            ],
            10,
        )["status"]
        == "incompatible"
    ...
    for rejected in (
        ["foundation", "--data-root", "/tmp", "init"],
        ["foundation", "init", "--daemon"],
    ):
        with pytest.raises(SystemExit):
            _parser().parse_args(rejected)
...
def test_all_receipt_statuses_validate_schema(tmp_path: Path) -> None:
    ...
    lock = busy_root / "state" / "locks" / "foundation.lock"
    lock.parent.mkdir(parents=True)
    lock.write_text(json.dumps({"pid": os.getpid()}))
    lock.chmod(0o600)
    receipts = [
        instance.execute(request("init")),
        instance.execute(request("init")),
        instance.execute(request("status")),
        instance.execute(request("verify")),
        instance.execute(request("migrate", FOUNDATION_SCHEMA_VERSION + 1)),
        FoundationTool(config(tmp_path / "bad")).execute(request("migrate")),
        FoundationTool(config(busy_root)).execute(request("init")),
    ]
    ...
    assert all(
        not list(schema.iter_errors(json.loads(item.json()))) for item in receipts
    )
