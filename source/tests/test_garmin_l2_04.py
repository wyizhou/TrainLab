...
def test_interrupted_full_skips_only_completed_health_items_and_rebuilds_counts(
    ...
    repository.coverage(connection, subject, "steps", "2026-04-16", "empty", None, 0)
    connection.close()
...
def test_finish_run_closes_proven_fetch_and_fails_orphaned_running_item(
    ...
    repository.item(connection, orphaned_run, "steps", "2026-04-15", "fetch", "running")
...
def test_generated_invocation_is_shared_by_lock_and_run(
    ...
    result = tool.execute(SyncRequest("incremental", through_local_date="2026-04-15"))
...
def test_gap_capability_and_continuous_cursor_state_machines(
    ...
    monkeypatch.setattr("src.garmin.utc_now", lambda: next(times))
