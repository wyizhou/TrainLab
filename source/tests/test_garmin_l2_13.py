...
from src.garmin_modes import build_collection_mode_plan
...
class ModeTransport:
    ...
    def activity_summary(self, activity_id: str) -> dict[str, Any]:
        ...
        entry = next(
            item for item in self.entries if str(item.get("activityId")) == activity_id
        )
...
def test_mode_planner_uses_full_history_independent_cursors_and_snapshot_day() -> None:
    ...
    assert {
        (item.resource_kind, item.start, item.through) for item in full.health_windows
    } == {
    ...
    starts = {item.resource_kind: item.start for item in incremental.health_windows}
    assert starts == {
    ...
    assert all(
        item.start == item.through == date(2026, 4, 17)
        for item in snapshot.health_windows
    )
...
def test_incremental_retries_at_most_one_hundred_preexisting_due_gaps(
    ...
    monkeypatch.setattr("src.garmin.HEALTH_RESOURCES", ("steps",))
...
def test_snapshot_is_partial_noop_then_revision_and_never_advances_cursor(
    ...
    config, tool, transport = _setup(tmp_path)
    monkeypatch.setattr("src.garmin.HEALTH_RESOURCES", ("user_summary",))
    ...
    with sqlite3.connect(config.database_path) as connection:
        ...
        assert (
            connection.execute("SELECT count(*) FROM garmin_sync_cursors").fetchone()[0]
            == 0
        )
...
def test_snapshot_revisits_uploaded_activity_until_fit_pipeline_can_fill_it(
    ...
    config, tool, transport = _setup(tmp_path)
    monkeypatch.setattr("src.garmin.HEALTH_RESOURCES", ())
    transport.entries = [
        {
            "activityId": 71,
    ...
    monkeypatch.setattr(tool, "_collect_activity_chart_fallback", lambda *_args: None)
    first = tool.execute(
        SyncRequest(
            "snapshot",
            snapshot_local_date="2026-04-17",
...
def test_completed_modes_reject_today_and_snapshot_rejects_future_without_login(
    ...
    with pytest.raises(ValueError, match="completed_mode_through_must_be_before_today"):
        tool.execute(
            SyncRequest(
                "incremental",
