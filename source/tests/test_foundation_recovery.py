...
def test_initialization_phases_are_observable_and_exact_phase1_or_phase2_recovers(
    tmp_path: Path,
) -> None:
    ...
    assert (
        crashing_tool(after_schema, "after_schema").execute(request()).status
        == "failed"
    )
    ...
    assert (
        crashing_tool(after_marker, "after_marker").execute(request()).status
        == "failed"
    )
...
def test_initializing_mismatch_is_preserved_and_never_rebuilt(
    ...
    if mutation == "unknown_table":
        ...
        conn.execute(
            "UPDATE schema_migrations SET content_sha256='wrong' WHERE version=1"
        )
    conn.commit()
    conn.close()
...
def test_ready_marker_and_state_disagreements_are_not_repaired(tmp_path: Path) -> None:
    ...
    tool = FoundationTool(config(other))
    assert tool.execute(request()).status == "initialized"
    marker = other / "state" / "foundation-ready.json"
    payload = json.loads(marker.read_text())
    payload["schema_version"] = FOUNDATION_SCHEMA_VERSION + 1
    marker.write_text(json.dumps(payload))
    marker.chmod(0o600)
    receipt = tool.execute(request())
    assert receipt.status == "incompatible" and receipt.next_action == "operator_review"
    assert (
        json.loads(marker.read_text())["schema_version"]
        == FOUNDATION_SCHEMA_VERSION + 1
    )

    initializing_marker = tmp_path / "initializing-marker"
    assert (
        crashing_tool(initializing_marker, "after_marker").execute(request()).status
        == "failed"
    )
    ...
    assert receipt.status == "incompatible" and receipt.next_action == "operator_review"
    assert (
        hashlib.sha256((initializing_marker / "data.db").read_bytes()).hexdigest()
        == before
    )

    third = tmp_path / "state"
    tool = FoundationTool(config(third))
    assert tool.execute(request()).status == "initialized"
    conn = sqlite3.connect(third / "data.db")
    conn.execute("UPDATE foundation_state SET state='initializing'")
    conn.commit()
    conn.close()
    receipt = tool.execute(request())
    assert (
        receipt.status == "initialized"
    )  # an otherwise complete initializing state is the sole recovery case
    assert state(third)[0] == "ready"


def test_status_is_bounded_while_verify_checks_the_complete_manifest(
    tmp_path: Path,
) -> None:
...
def test_version_corruption_permissions_and_status_matrix_preserve_evidence(
    tmp_path: Path,
) -> None:
    high = tmp_path / "high"
    tool = FoundationTool(config(high))
    assert tool.execute(request()).status == "initialized"
    conn = sqlite3.connect(high / "data.db")
    conn.execute(
        "UPDATE foundation_state SET schema_version=?", (FOUNDATION_SCHEMA_VERSION + 1,)
    )
    conn.commit()
    conn.close()
    receipt = tool.execute(request())
    assert (
        receipt.status == "incompatible" and receipt.next_action == "explicit_migrate"
    )
