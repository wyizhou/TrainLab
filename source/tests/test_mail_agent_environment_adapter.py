...
from src.gmail_environment import GmailEnvironmentStatus
from src.mail_agent.environment_adapter import (
    GmailEnvironmentAdapterError,
    GmailEnvironmentRecipientAdapter,
)
from src.mcp import MCPResponseError
...
def send(value: GmailEnvironmentRecipientAdapter):
    return value.send_html_recipient(
        run_id=KEY,
        subject=f"Re: TrainLab · {KEY}",
        plain_text="plain",
        html="<p>html</p>",
        thread_id=THREAD,
...
def test_exact_binding_is_checked_before_client_starts():
    client = Client()
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_mcp_binding_invalid$"
    ):
        send(
            adapter(
                client,
                inspector=lambda: GmailEnvironmentStatus(
                    False, None, "gmail_mcp_binding_invalid", "secret"
                ),
            )
        )
...
def test_no_match_verifies_exact_self_thread_and_uses_only_fixed_arguments():
    ...
    assert (
        receipt.already_sent is False
        and receipt.provider_message_id == "sent-1"
        and receipt.provider_thread_id == THREAD
    )
    ...
    assert client.calls == [
        ...
        (
            "send_email",
            {
                ...
                "subject": f"Re: TrainLab · {KEY}",
                "body": "plain",
                "htmlBody": f'<p>html</p><span style="display:none!important;max-height:0;overflow:hidden;opacity:0;mso-hide:all;">{marker}</span>',
...
def test_subject_need_not_expose_run_id_and_marker_is_html_only():
    client = Client(
        responses=["", self_thread(), "Email sent successfully with ID: sent-1"]
    )
    ...
    assert (
        KEY in payload["htmlBody"] and "display:none!important" in payload["htmlBody"]
    )
...
def test_caller_cannot_preinject_idempotency_marker() -> None:
    ...
    value = adapter(client)
    with pytest.raises(
        GmailEnvironmentAdapterError, match="gmail_reply_idempotency_invalid"
    ):
        value.send_html_recipient(
            run_id=KEY,
            subject="Re: 本周训练",
                ...
                "messages": [
                    {
                        ...
                        "to": SELF,
                        "cc": [],
                        "body": f"x [TrainLab idempotency: {KEY}]",
                    }
                ],
            },
        ]
...
def test_preflight_search_returns_only_a_self_verified_exact_thread_match():
    ...
    assert adapter(client).search_run_id(run_id=KEY) == (
        {"message_id": "old-1", "thread_id": THREAD},
    )
    assert [name for name, _ in client.calls] == [
        "search_emails",
        "get_thread",
...
def test_poll_search_uses_only_search_result_id_then_fixed_read_email_for_thread_id():
    ...
    assert client.calls == [
        (
            "search_emails",
            {
                "query": "label:TrainLab after:2026-07-20 before:2026-07-21",
                "maxResults": 100,
            },
        ),
...
def test_multi_party_thread_is_rejected_before_send():
    ...
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_reply_thread_participants_invalid$"
    ):
        send(adapter(client))
    assert [name for name, _ in client.calls] == [
        "search_emails",
...
def test_target_and_thread_must_match_exactly():
    ...
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_reply_thread_mismatch$"
    ):
        send(adapter(client))
...
def test_malformed_search_or_thread_protocol_fails_closed():
    ...
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_reply_protocol_invalid$"
    ):
        send(adapter(client))
...
def test_missing_capability_closes_without_any_mail_call():
    ...
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_reply_capability_mismatch$"
    ):
        send(adapter(client))
...
def test_verified_registry_binding_is_used_to_start_the_client():
    ...
    value = GmailEnvironmentRecipientAdapter(
        ...
        client_factory=lambda command, args, **kwargs: (
            started.append((command, args, kwargs)) or client
        ),
    )
    send(value)
    assert started == [
...
def test_read_only_short_rate_limit_is_retried_but_send_is_not():
    client = Client(
        responses=[
            MCPResponseError(status_code=429, retry_after_seconds=1),
            "",
            self_thread(),
            "Email sent successfully with ID: sent-1",
        ]
...
def test_thread_evidence_normalizes_provider_rfc2822_date_to_utc():
    ...
    assert (
        evidence.raw_payload["messages"][0]["internal_date_utc"]
        == "2026-07-26T12:45:49Z"
    )
