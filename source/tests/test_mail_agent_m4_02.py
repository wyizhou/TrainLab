...
from src.mail_agent.locks import MailLockBusyError, MailWriteLock
from src.mail_agent.repository import (
...
def initialize(
    tmp_path: Path,
) -> tuple[sqlite3.Connection, MailRepository, int, int, int]:
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "mail-repository-fixture", NOW))
        .status
        == "initialized"
    )
    ...
    connection.execute(
        "INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,source_revision_id,processing_state) VALUES(?,?,?,?,?,?)",
        (thread_id, "message-1", "inbound", "user", source_revision_id, "new"),
    )
...
def response_draft(
    ...
    *,
    ...
    return AcceptedResponseDraft(
        ...
        events=(
            EventDraft(
                "feedback_recorded", "user", NOW, message_id, "untrusted_content"
            ),
        ),
        facts=(
            FactDraft(
                ...
                "feedback_recorded",
                expires_at_utc=(
                    "2026-08-01T00:00:00Z" if fact_scope == "temporary" else None
                ),
            ),
        ),
...
def test_same_invocation_resumes_same_run_and_status_is_read_only(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, _, _ = initialize(tmp_path)
    created = repo.start_or_resume_run(request(subject_id))
    resumed = MailRepository(
        sqlite3.connect(tmp_path / "foundation" / "data.db"), clock=lambda: NOW
    ).start_or_resume_run(request(subject_id))
    assert created.id == resumed.id and created.run_key == resumed.run_key
    selector = MailRequest(
        mode="status",
        subject_id=subject_id,
        invocation_id="status-inv",
        requested_at_utc=NOW,
...
def test_item_state_machine_is_idempotent_and_sanitizes_error(tmp_path: Path) -> None:
    ...
    stored = connection.execute(
        "SELECT error_summary FROM mail_agent_items WHERE id=?", (failed.id,)
    ).fetchone()[0]
    assert (
        "secret@example.invalid" not in stored
        and "https://private.invalid" not in stored
    )
...
def test_accepted_publish_is_atomic_idempotent_and_keeps_exact_delivery_relation(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, thread_id, message_id = initialize(tmp_path)
    run = repo.start_or_resume_run(request(subject_id))
    published = repo.publish_accepted_response(
        run.id, response_draft(thread_id, message_id)
    )
    retried = repo.publish_accepted_response(
        run.id, response_draft(thread_id, message_id)
    )
    assert published.response_artifact_id == retried.response_artifact_id
    assert published.delivery_id == retried.delivery_id
    assert (
        connection.execute("SELECT COUNT(*) FROM mail_response_artifacts").fetchone()[0]
        == 1
    )
    assert (
        connection.execute("SELECT COUNT(*) FROM mail_response_inputs").fetchone()[0]
        == 1
    )
    assert connection.execute("SELECT COUNT(*) FROM user_facts").fetchone()[0] == 1
    relation = connection.execute(
        "SELECT mail_response_artifact_id,content_role FROM mail_delivery_artifacts"
    ).fetchone()
    assert (
        relation[0] == published.response_artifact_id and relation[1] == "mail_response"
    )
    second = repo.start_or_resume_run(request(subject_id, "inv-2"))
    newer = repo.publish_accepted_response(
        second.id, response_draft(thread_id, message_id, key="delivery-2")
    )
    rows = connection.execute(
        "SELECT id,is_current FROM mail_response_artifacts ORDER BY revision_no"
    ).fetchall()
    assert [(row[0], row[1]) for row in rows] == [
        (published.response_artifact_id, 0),
        (newer.response_artifact_id, 1),
    ]


def test_accepted_publish_rolls_back_all_rows_on_constraint_failure(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, thread_id, message_id = initialize(tmp_path)
    run = repo.start_or_resume_run(request(subject_id))
    with pytest.raises(MailRepositoryError, match="invalid_user_fact_draft"):
        repo.publish_accepted_response(
            run.id, response_draft(thread_id, message_id, fact_scope="invalid-scope")
        )
    for table in (
        "conversation_events",
        "user_facts",
...
def test_single_writer_lock_preserves_live_lock_and_recovers_only_dead_pid(
    tmp_path: Path,
) -> None:
    ...
    os.chmod(path, 0o600)
    stale = MailWriteLock(
        path, "mail:1:process:replacement", process_is_alive=lambda _: False
    )
    stale.acquire()
...
def test_lock_rejects_missing_private_parent_symlink_and_malformed_record(
    tmp_path: Path,
) -> None:
    missing = MailWriteLock(
        tmp_path / "missing" / "locks" / "mail.lock", "mail:1:poll:x"
    )
    with pytest.raises(MailLockBusyError):
        missing.acquire()
    ...
    with pytest.raises(MailLockBusyError):
        MailWriteLock(
            locks / "mail.lock", "mail:1:poll:x", process_is_alive=lambda _: False
        ).acquire()


def test_safe_error_summary_never_keeps_urls_or_email_addresses() -> None:
    assert (
        safe_error_summary("a@example.invalid https://example.invalid/a\nbody")
        == "<redacted-email> <redacted-url> body"
    )


def test_repository_rejects_unready_manifest_and_does_not_migrate(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, _, _ = initialize(tmp_path)
...
def test_cursor_and_processing_state_are_subject_scoped_and_monotonic(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, _, message_id = initialize(tmp_path)
    connection.execute(
        ...
        (subject_id, "h" * 64, NOW, NOW),
    )
    identity_id = connection.execute(
        "SELECT id FROM subject_identities WHERE identity_hmac=?", ("h" * 64,)
    ).fetchone()[0]
    connection.commit()
    run = repo.start_or_resume_run(
        MailRequest(
            mode="poll",
            subject_id=subject_id,
            invocation_id="poll-cursor",
            requested_at_utc=NOW,
        )
    )
    cursor = repo.advance_poll_cursor(
        run.id, subject_id, identity_id, "trainlab_label", NOW, "2026-07-22T00:00:00Z"
    )
...
def test_same_run_conflicting_response_draft_is_rejected(tmp_path: Path) -> None:
    ...
    conflicting = AcceptedResponseDraft(
        conflicting.mail_thread_id,
        conflicting.in_reply_to_mail_message_id,
        conflicting.response_kind,
        conflicting.schema_version,
        conflicting.structured_content_json,
        "different",
        conflicting.inputs,
        conflicting.events,
        conflicting.facts,
...
def test_cursor_rejects_verified_non_gmail_identity_and_incomplete_window(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, _, _ = initialize(tmp_path)
    connection.execute(
    ...
    identity_id = connection.execute(
        "SELECT id FROM subject_identities WHERE identity_hmac=?", ("g" * 64,)
    ).fetchone()[0]
    connection.commit()
    run = repo.start_or_resume_run(
        MailRequest(
    ...
    gmail_id = connection.execute(
        "SELECT id FROM subject_identities WHERE identity_hmac=?", ("m" * 64,)
    ).fetchone()[0]
    connection.commit()
...
def test_accepted_response_replay_checks_events_facts_and_cross_subject_ownership(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, thread_id, message_id = initialize(tmp_path)
    connection.execute(
    ...
    other_subject = connection.execute(
        "SELECT id FROM data_subjects WHERE subject_key='other'"
    ).fetchone()[0]
    connection.execute(
        "INSERT INTO mail_threads(subject_id,provider_thread_id,is_current) VALUES(?,?,1)",
        (other_subject, "thread-other"),
    )
    other_thread = connection.execute(
        "SELECT id FROM mail_threads WHERE provider_thread_id='thread-other'"
    ).fetchone()[0]
    connection.execute(
        "INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,processing_state) VALUES(?,?,?,?,?)",
        (other_thread, "message-other", "inbound", "user", "new"),
    )
    other_message = connection.execute(
        "SELECT id FROM mail_messages WHERE provider_message_id='message-other'"
    ).fetchone()[0]
    connection.commit()
    run = repo.start_or_resume_run(request(subject_id))
    ...
    bad = AcceptedResponseDraft(
        bad.mail_thread_id,
        bad.in_reply_to_mail_message_id,
        bad.response_kind,
        bad.schema_version,
        bad.structured_content_json,
        bad.user_visible_text,
        bad.inputs,
        (
            EventDraft(
                "feedback_recorded", "user", NOW, other_message, "untrusted_content"
            ),
        ),
        bad.facts,
        bad.delivery,
    ...
    changed_event = AcceptedResponseDraft(
        ...
        "mail_response",
        "1",
        json.dumps({"kind": "reply"}, sort_keys=True),
        "已记录。",
        (
            InputDraft(
                "trigger_message",
        ...
        (
            EventDraft(
                "feedback_recorded", "trainlab", NOW, message_id, "untrusted_content"
            ),
        ),
        (
            FactDraft(
...
def test_after_constructor_schema_object_tamper_fails_before_business_write(
...
def test_cursor_rejects_process_or_terminal_run_and_state_cross_step_is_zero_write(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, _, message_id = initialize(tmp_path)
    connection.execute(
        "INSERT INTO subject_identities(subject_id,provider,identity_kind,identity_hmac,is_verified,first_seen_at_utc,last_seen_at_utc) VALUES(?, 'gmail','email',?,1,?,?)",
        (subject_id, "z" * 64, NOW, NOW),
    )
    identity_id = connection.execute(
        "SELECT id FROM subject_identities WHERE identity_hmac=?", ("z" * 64,)
    ).fetchone()[0]
    connection.commit()
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "INSERT INTO schema_migrations VALUES(5,'extra','2026-07-23T00:00:00Z','foundation-v4','deadbeef')",
...
def test_repository_requires_exact_foundation_migration_receipts(
...
def test_repository_rejects_every_extra_database_object_kind(
    tmp_path: Path, ddl: str
) -> None:
    connection, _, _, _, _ = initialize(tmp_path)
    connection.execute(ddl)
...
@pytest.mark.parametrize(
    ...
    (
        (
            ...
            "CREATE INDEX idx_source_revisions_current ON source_revisions(resource_kind,provider,provider_object_id,is_current)",
        ),
        (
            "ux_source_revision_current",
            "CREATE UNIQUE INDEX ux_source_revision_current ON source_revisions(provider,resource_kind,provider_object_id,is_current)",
        ),
        (
            "ux_activity_source_role_active",
            "CREATE UNIQUE INDEX ux_activity_source_role_active ON activity_source_revisions(source_role,activity_id) WHERE is_active=1",
        ),
        (
            "ux_daily_health_current",
            "CREATE UNIQUE INDEX ux_daily_health_current ON daily_health(local_date,subject_id) WHERE is_current=1",
        ),
        (
            "ux_analysis_artifact_current",
            "CREATE UNIQUE INDEX ux_analysis_artifact_current ON analysis_artifacts(subject_id,artifact_kind,period_start_local_date,period_end_local_date) WHERE is_current=0",
        ),
        (
            "ux_mail_response_current",
            "CREATE UNIQUE INDEX ux_mail_response_current ON mail_response_artifacts(subject_id,response_kind,mail_thread_id) WHERE is_current=1",
        ),
        (
            "ux_garmin_sync_gap_unresolved",
            "CREATE UNIQUE INDEX ux_garmin_sync_gap_unresolved ON garmin_sync_gaps(subject_id,resource_kind,logical_object_key,window_start_local_date,window_end_local_date,stage) WHERE status IN ('open')",
        ),
        (
            "idx_coverage_resource_date",
            "CREATE UNIQUE INDEX idx_coverage_resource_date ON resource_coverage(resource_kind,local_date)",
        ),
        (
            "idx_activity_samples_stream",
            "CREATE INDEX idx_activity_samples_stream ON activity_samples(activity_id,sample_index,stream_kind)",
        ),
    ),
)
...
def test_repository_validates_complete_trigger_sql_not_just_manifest_token(
    tmp_path: Path,
) -> None:
    connection, _, _, _, _ = initialize(tmp_path)
...
def test_item_relations_remain_immutable_across_status_transitions(
    tmp_path: Path,
) -> None:
    connection, repo, subject_id, thread_id, message_id = initialize(tmp_path)
    connection.execute(
    ...
    other_analysis_id = connection.execute(
        "SELECT id FROM analysis_artifacts ORDER BY id DESC LIMIT 1"
    ).fetchone()[0]
    connection.execute(
...
def test_same_invocation_requires_exact_full_request_identity(
    ...
    assert (
        persisted["context_snapshot_sha256"]
...
def test_null_message_events_are_run_and_subject_scoped(tmp_path: Path) -> None:
    ...
    second = repo.publish_accepted_response(
        second_run.id,
        null_event_draft(thread_2, message_2, source_2, "null-delivery-2", "thread-2"),
    )
...
def test_fact_scope_dates_activity_and_exact_supersession(tmp_path: Path) -> None:
    ...
    first_base = response_draft(thread_id, message_id, key="fact-original-delivery")
    first_draft = replace(
        first_base,
        facts=(
            FactDraft(
    ...
    second_base = response_draft(thread_id, message_id, key="fact-new-delivery")
...
def test_delivery_unknown_requires_reconcile_and_terminal_states_need_explicit_repair(
    ...
    with pytest.raises(MailRepositoryError, match="illegal_mail_delivery_transition"):
        repo.transition_delivery_state(subject_id, published.delivery_id, "sending")
    ...
    repair_delivery = repo.publish_accepted_response(
        repair_run.id,
        response_draft(thread_id, message_id, key="delivery-repair-key"),
    ).delivery_id
...
def test_resume_validates_canonical_snapshot_digest_and_full_identity_zero_write(
    ...
    if tamper_kind == "digest":
        ...
        snapshot["mail_request_identity"]["requested_at_utc"] = "2026-07-23T00:00:01Z"
        encoded = json.dumps(
            snapshot,
            ensure_ascii=False,
            sort_keys=True,
...
def test_null_message_event_semantics_and_run_ownership_fail_closed(
    ...
    base = response_draft(thread_id, message_id, key="null-event-semantics-delivery")
...
def test_message_quarantine_and_crash_recovery_are_explicit_and_evidenced(
    ...
    before = connection.total_changes
    with pytest.raises(MailRepositoryError, match="illegal_mail_message_transition"):
        repo.set_message_processing_state(subject_id, message_id, "quarantined")
    assert connection.total_changes == before
    repo.set_message_processing_state(
        subject_id,
        message_id,
...
def test_item_relations_validate_subject_exact_relation_and_logical_identity(
    ...
    other_analysis = _add_analysis_artifact(connection, other_subject, "other-item")
    connection.commit()
    ...
    other_base = response_draft(other_thread, other_message, key="other-item-delivery")
    other_draft = replace(
        other_base,
        inputs=(
            InputDraft(
    ...
    other_published = repo.publish_accepted_response(other_run.id, other_draft)
    ...
    primary_published = repo.publish_accepted_response(
        primary_run.id,
        response_draft(thread_id, message_id, key="primary-item-relations-delivery"),
    )
    ...
    second_primary = repo.publish_accepted_response(
        ...
        response_draft(thread_id, message_id, key="second-primary-item-delivery"),
    )
    item_run = repo.start_or_resume_run(request(subject_id, "item-relation-validation"))
    bad_relations = (
        {
    ...
    for ordinal, relation in enumerate(bad_relations):
        ...
        assert (
            connection.execute("SELECT count(*) FROM mail_agent_items").fetchone()[0]
            == before_count
        )
...
def test_item_and_run_evidence_combinations_are_strict_and_idempotent(
    ...
    with pytest.raises(MailRepositoryError, match="invalid_mail_run_evidence"):
        repo.finish_run(run.id, "partial", next_retry_at_utc="not-a-time")
    assert connection.total_changes == before
    ...
    run_with_retry = repo.start_or_resume_run(request(subject_id, "strict-run-retry"))
    repo.finish_run(
        run_with_retry.id,
        "deferred",
        next_retry_at_utc="2026-07-23T00:01:00Z",
...
def test_dependency_wait_is_the_only_deferred_item_and_run_without_retry(
    ...
    connection, repo, subject_id, _, _ = initialize(tmp_path)
    dependency_id = _add_analysis_artifact(connection, subject_id, "dependency-wait")
    connection.commit()
