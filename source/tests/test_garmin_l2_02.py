...
from src.garmin import GarminError, SyncRequest

from .fixtures.synthetic_fit import SYNTHETIC_FIT_NAMES, SYNTHETIC_FIT_SHA256
...
def test_manifest_and_synthetic_fixtures_have_safe_metadata_only():
    assert len(SYNTHETIC_FIT_NAMES) == 6
    assert set(SYNTHETIC_FIT_NAMES) == set(SYNTHETIC_FIT_SHA256)
    assert all(
        re.fullmatch(r"[0-9a-f]{64}", digest)
        for digest in SYNTHETIC_FIT_SHA256.values()
    )
...
def test_receipt_and_captured_output_do_not_match_secret_patterns(tmp_path, caplog):
    ...
    receipt = t.execute(
        SyncRequest(
            "incremental", through_local_date="2026-04-15", invocation_id="scan"
        )
    )
