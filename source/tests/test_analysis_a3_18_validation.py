...
def expectation(
    *,
    ...
    red_flag: bool = False,
    zones: bool = False,
    coaching_profile: dict[str, object] | None = None,
    progression_inputs: dict[str, object] | None = None,
) -> ResultValidationExpectation:
    days = [
        (
            date.fromisoformat(PLAN["start_local_date"]) + timedelta(days=index)
...
def test_weekly_first_run_has_exact_windows_plan_items_and_host_safety_normalization() -> (
    ...
    assert all(
        item["course_contract"]["activity_kind"] == item["activity_kind"]
...
def test_host_profile_limits_hard_loads_independently_of_model_constraints() -> None:
    ...
    value["training_plan"]["constraints"].update(
        recovery_ready=False, adherence_stable=False, capacity_change="volume"
    )
    code(
        value,
        "analysis_result_hard_load_max_reached",
        expectation(
            zones=True,
            coaching_profile={
                "schema_version": "1",
...
def test_progression_decision_uses_host_inputs_not_model_constraints() -> None:
    value = output()
    value["training_plan"]["constraints"].update(
        recovery_ready=False, adherence_stable=False, capacity_change="volume"
    )
    ...
    assert accepted["training_plan"]["constraints"]["progression_decision"] == {
        "decision": "hold",
        "reason": "no_single_dimension_change",
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda value: value.__setitem__("artifacts", [None]),
        lambda value: value["training_plan"].__setitem__("items", None),
    ],
)
def test_malformed_model_containers_use_sanitized_domain_errors(mutation) -> None:
    value = output()
    mutation(value)
    with pytest.raises(AnalysisResultValidationError) as exc_info:
        AnalysisResultValidator().validate(
            json.dumps(value).encode("utf-8"), expectation()
        )
    assert exc_info.value.code == "analysis_result_schema_invalid"
