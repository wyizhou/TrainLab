...
def test_fresh_init_accepts_frozen_states_legacy_values_and_rejects_invalid(
    tmp_path: Path,
) -> None:
    root = tmp_path / "fresh"
    instance = FoundationTool(config(root))
    assert instance.execute(request("init")).status == "initialized"
    conn = sqlite3.connect(root / "data.db")
    try:
        conn.execute("PRAGMA foreign_keys=ON")
        conn.execute(
            "INSERT INTO data_subjects(subject_key,created_at_utc) VALUES('fresh-subject',?)",
            (UTC,),
        )
        ...
        assert (
            conn.execute(
                ...
                (thread, "default", "inbound", "user"),
            ).rowcount
            == 1
        )
        assert (
            conn.execute(
                "SELECT processing_state FROM mail_messages WHERE provider_message_id='default'"
            ).fetchone()[0]
            == "discovered"
        )
        for index, state in enumerate(MAIL_PROCESSING_STATES):
            conn.execute(
                ...
                (thread, f"state-{index}", "inbound", "user", state),
            )
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,processing_state) VALUES(?,?,?,?,?)",
                (thread, "invalid", "inbound", "user", "not_a_frozen_state"),
            )
    finally:
        conn.close()


def test_explicit_v1_migration_preserves_rows_is_idempotent_and_init_never_upgrades(
    tmp_path: Path,
) -> None:
    root = tmp_path / "legacy"
    instance = create_published_v1_database(root)
    seed = sqlite3.connect(root / "data.db")
    seed.execute("PRAGMA foreign_keys=ON")
    seed.execute(
        "INSERT INTO data_subjects(subject_key,created_at_utc) VALUES('legacy-subject',?)",
        (UTC,),
    )
    ...
    seed.execute(
        "INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,body_text,body_sha256,processing_state) VALUES(?,?,?,?,?,?,?)",
        (
            thread,
            "legacy-message",
            "inbound",
    ...
    assert all(
        (root / "raw" / name).stat().st_mode & 0o777 == 0o700
        for name in ("garmin", "gmail", "legacy")
    )
    assert (
        migrated.migration_start_version == 1
        and migrated.migration_end_version == FOUNDATION_SCHEMA_VERSION
    )
    conn = sqlite3.connect(root / "data.db")
    try:
        assert conn.execute(
            "SELECT body_text,processing_state FROM mail_messages WHERE provider_message_id='legacy-message'"
        ).fetchone() == ("legacy body", "new")
        assert conn.execute(
...
@pytest.mark.parametrize(
    "mutation", ("marker", "migration", "ddl", "extra_object", "missing_view")
)
def test_only_exact_published_v1_is_admitted_before_any_database_write(
    ...
    if mutation == "marker":
        ...
        if mutation == "migration":
            conn.execute(
                "UPDATE schema_migrations SET content_sha256='forged' WHERE version=1"
            )
        elif mutation == "ddl":
            conn.execute("PRAGMA writable_schema=ON")
            conn.execute(
                "UPDATE sqlite_master SET sql=replace(sql,\"DEFAULT 'new'\",\"DEFAULT 'forged'\") WHERE type='table' AND name='mail_messages'"
            )
...
def test_marker_republish_requires_completed_verified_v3_database(
    tmp_path: Path,
) -> None:
    root = tmp_path / "republish"
    instance = create_published_v1_database(root)
    assert (
        instance.execute(request("migrate", FOUNDATION_SCHEMA_VERSION)).status
        == "initialized"
    )
    marker = root / "state" / "foundation-ready.json"
    ...
    marker.write_bytes(legacy_marker)
    marker.chmod(0o600)
    repaired = instance.execute(request("migrate", FOUNDATION_SCHEMA_VERSION))
    assert (
        repaired.status == "initialized"
        and repaired.warnings[0]["code"] == "migration_marker_republished"
    )
    marker.write_bytes(legacy_marker)
    marker.chmod(0o600)
    conn = sqlite3.connect(root / "data.db")
    conn.execute("CREATE TABLE v2_forged_extra (id INTEGER)")
    conn.commit()
    conn.close()
