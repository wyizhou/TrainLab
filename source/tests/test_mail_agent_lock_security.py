...
def test_state_symlink_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(MailLockBusyError):
        MailWriteLock(
            tmp_path / "state" / "locks" / "mail.lock", "mail:1:poll:a"
        ).acquire()


def test_claim_blocker_prevents_new_acquire(tmp_path: Path) -> None:
    path = make(tmp_path)
    blocker = path.parent / ".mail.lock.claim-dead"
    blocker.write_text("evidence")
...
def test_stale_claim_in_place_mutation_is_preserved_and_blocks(
    ...
    with pytest.raises(MailLockBusyError):
        lock.acquire()
    assert any(
        item.name.startswith(".mail.lock.tombstone-") for item in path.parent.iterdir()
    )
    with pytest.raises(MailLockBusyError):
        MailWriteLock(path, "mail:1:poll:b").acquire()


def test_zero_write_cleans_own_incomplete_lock(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = make(tmp_path)
...
def test_claim_scan_error_is_busy(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = make(tmp_path)
    monkeypatch.setattr(
        os, "listdir", lambda _fd: (_ for _ in ()).throw(OSError("scan"))
    )
    with pytest.raises(MailLockBusyError):
        MailWriteLock(path, "mail:1:poll:a").acquire()


def test_create_in_place_mutation_is_preserved_as_claim(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = make(tmp_path)
    original = os.write
    ...
    def guarded(fd, data):
        ...
        os.ftruncate(fd, 0)
        original(fd, b"unknown-external-content")
...
def test_verified_claim_cleanup_is_bounded_and_reacquires(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path = make(tmp_path)
    calls: list[str] = []
...
def test_verified_dead_stale_recovery_is_bounded_for_long_running_polling(
    tmp_path: Path,
) -> None:
    path = make(tmp_path)
    before = len(os.listdir("/dev/fd"))
    for index in range(1000):
        ...
        os.chmod(path, 0o600)
        replacement = MailWriteLock(
            path, f"mail:1:poll:stale-{index}", process_is_alive=lambda _pid: False
        )
        replacement.acquire()
        replacement.release()
        assert not path.exists()
...
def test_finalizer_failure_retains_claim_and_blocks_retry(
    ...
    if operation == "release":
        ...
        os.chmod(path, 0o600)
        owner = MailWriteLock(
            path, "mail:1:poll:replacement", process_is_alive=lambda _pid: False
        )
        held_locks_fd = None
    ...
    def unlink_hook(name, *args, **kwargs):
        nonlocal fired
        if fault == "unlink" and str(os.fspath(name)).startswith(
            ".mail.lock.tombstone-"
        ):
            fired = True
    ...
    def fsync_hook(fd):
        nonlocal fired
        if fault == "post_fsync" and fd == (
            held_locks_fd if held_locks_fd is not None else locks_fd.get("fd")
        ):
            fired = True
