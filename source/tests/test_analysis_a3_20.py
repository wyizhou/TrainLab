...
from src.analysis.delivery import AnalysisDeliveryFactory
from src.analysis.publisher import AnalysisPublisher, AnalysisPublishError
from src.analysis.regenerate import RegenerateRoute


def _request(
    artifact_id: str = "1", reason: str = "explicit_user_request"
) -> AnalysisRequest:
    return AnalysisRequest(
        ...
        regeneration_reason_code=reason,
    )
...
def test_current_supported_target_maps_to_host_owned_shape(
    kind: str, run_kind: str, shape: str
) -> None:
...
def test_all_frozen_reason_codes_are_accepted(reason: str) -> None:
    assert (
        _route("daily_summary", "daily")._source(_request(reason=reason))["artifact_id"]
        == 1
    )
...
def test_regeneration_keeps_same_content_revision_and_exact_direct_lineage() -> None:
    ...
    assert (
        conn.execute(
            "SELECT is_current FROM analysis_artifacts WHERE id=100"
        ).fetchone()[0]
        == 0
    )
...
def test_regeneration_and_pending_delivery_commit_in_one_transaction() -> None:
    ...
    assert (
        conn.execute("SELECT status FROM analysis_deliveries").fetchone()[0]
        == "pending"
    )
    assert (
        conn.execute("SELECT count(*) FROM analysis_delivery_artifacts").fetchone()[0]
        == 2
    )
    assert (
        conn.execute(
...
def test_same_invocation_restores_persisted_ids_periods_and_content_comparison() -> (
    None
):
    conn = _publisher_database()
    rows = _manifest()
