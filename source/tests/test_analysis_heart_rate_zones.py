...
from src.analysis.heart_rate_zones import calculate_zone_candidates
from src.analysis.heart_rate_zones_store import (
...
def test_confirmed_revision_is_append_only_and_exposes_safety_evidence(tmp_path):
    ...
    assert (
        tool.execute(
            FoundationRequest("init", "hrr-test", "2026-08-03T00:00:00Z")
        ).status
        == "initialized"
    )
    import sqlite3
    ...
    candidate = calculate_zone_candidates(
        [
            {
                "local_date": (date(2026, 8, 3) - timedelta(days=index)).isoformat(),
                "bpm": 50,
            }
            for index in range(28)
        ],
    ...
    record_id = append_confirmed_zone_revision(
        ...
        1,
        candidate,
        effective_from_utc="2026-08-03T00:00:00Z",
        confirmation_event_id=9,
    )
    connection.commit()
    stored = latest_confirmed_zone_revision(connection, 1)
    assert (
        stored
        and stored["id"] == record_id
        and set(stored["methods"])
        == {"hrr", "historical_threshold_proxy", "tanaka_low_confidence"}
    )
    records = tuple(
        dict(row)
        for row in connection.execute(
            "SELECT * FROM physiology_records WHERE id=?", (record_id,)
        )
    )
    metrics = tuple(
        dict(row)
        for row in connection.execute(
            "SELECT * FROM physiology_metrics WHERE physiology_record_id=?",
            (record_id,),
        )
    )
    snapshot = StableSnapshot(
        ...
        subject_context=StableSubjectContext(
            1, "Asia/Hong_Kong", "garmin", "account", True
        ),
    )
...
def test_candidate_bootstrap_is_idempotent_and_remains_unconfirmed(tmp_path):
    ...
    assert (
        tool.execute(
            FoundationRequest("init", "hrr-candidate", "2026-08-03T00:00:00Z")
        ).status
        == "initialized"
    )
    import sqlite3
    ...
    first = ensure_zone_candidate(
        ...
        effective_at_utc="2026-08-04T01:00:00Z",
    ...
    second = ensure_zone_candidate(
        ...
        1,
        as_of_local_date="2026-08-03",
        effective_at_utc="2026-08-04T01:00:01Z",
        weekly_refresh=False,
    )
    connection.commit()
