...
from src.garmin import cli as garmin_cli
from src.garmin.cli import _validate_garmin_cli_args, add_root_subparser
...
def test_cli_preserves_repair_budget_in_request_before_tool_execution(
    ...
    monkeypatch.setattr(garmin_cli, "project_root", lambda: tmp_path)
    config = GarminConfig(
        tmp_path / "data.db",
        tmp_path / "raw",
        tmp_path / "state",
