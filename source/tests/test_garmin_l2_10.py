...
def _setup(tmp_path: Path, entries: list[dict[str, object]]):
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "l2-10", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
    config = GarminConfig(
...
def test_full_uses_count_complete_pages_all_history_filters_through_and_replays_noop(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute("SELECT min(local_date) FROM activities").fetchone()[0]
            == "2020-01-01"
        )
        assert (
            conn.execute(
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute("SELECT count(*) FROM source_revisions").fetchone()[0]
            == revisions_before
        )
        assert conn.execute("SELECT count(*) FROM activities").fetchone()[0] == 204
...
def test_complete_full_runs_advance_missing_and_invocation_replay_does_not(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute("SELECT count(*) FROM raw_objects").fetchone()[0] >= raw_count
        )
...
def test_incremental_is_window_bounded_and_snapshot_absence_never_marks_missing(
    ...
    present = tool.execute(
        SyncRequest(
            ...
            invocation_id="snapshot-present",
        )
    )
...
def test_nested_connect_activity_summary_is_normalized_without_changing_raw(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path, [_entry(1, "2026-04-15")])
...
def test_pinned_transport_exposes_count_pages_and_bounded_date_inventory(
    tmp_path: Path,
) -> None:
    class Session:
    ...
    assert transport.activity_page(0, 1) == {"activities": [_entry(1, "2026-04-15")]}
