...
def _run_fault_once(
    root: Path, fault: str, monkeypatch: pytest.MonkeyPatch
    ...
    def fsync_hook(descriptor: int) -> None:
        if descriptor == opened.get("create") and fault in {
            "file_fsync",
            "rename",
            "unlink",
            "post_fsync",
        ...
        if descriptor == opened.get("locks"):
            ...
            if (
                fault in {"post_fsync", "blocker_open"}
                and "file_fsync" in stages
                and "post_fsync" not in stages
            ):
    ...
    def unlink_hook(name: str | bytes | Path, *args: object, **kwargs: object) -> None:
        ...
        if (
            isinstance(text, str)
            and text.startswith(".mail.lock.tombstone-")
            and fault == "unlink"
        ):
            stages.add("unlink")
...
def test_e3a_fault_state_and_recovery(
...
def test_e3a_partial_write_success_and_release(
...
def test_e3a_fault_fd_loop(
    ...
    for index in range(20):
        with monkeypatch.context() as patch:
            path, stages, expected_busy = _run_fault_once(
                tmp_path / f"{fault}-{index}", fault, patch
            )
            assert EXPECTED_STAGES[fault] <= stages
        _assert_retry(path, expected_busy)
