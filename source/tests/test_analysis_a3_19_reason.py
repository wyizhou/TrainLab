...
def _repository(tmp_path: Path) -> tuple[sqlite3.Connection, StableViewRepository]:
    ...
    conn.execute(
        "INSERT INTO data_subjects(id,subject_key,created_at_utc) VALUES(1,'subject',?)",
        (UTC,),
    )
    conn.execute(
        "INSERT INTO subject_identities(subject_id,provider,identity_kind,identity_hmac,is_verified,first_seen_at_utc,last_seen_at_utc) VALUES(1,'garmin','account','hmac',1,?,?)",
        (UTC, UTC),
    )
    conn.execute(
        "INSERT INTO source_revisions(id,provider,resource_kind,provider_object_id,revision_no,payload_hash,is_current,parsed_at_utc) VALUES(1,'gmail','message_json','message-1',1,?,1,?)",
        ("a" * 64, UTC),
    )
    conn.execute(
        "INSERT INTO mail_threads(id,subject_id,provider_thread_id,is_current) VALUES(1,1,'thread-1',1)"
    )
    conn.execute(
        "INSERT INTO mail_messages(id,mail_thread_id,provider_message_id,direction,actor_role,received_at_utc,source_revision_id,processing_state) VALUES(1,1,'message-1','inbound','user',?,1,'awaiting_analysis')",
        (UTC,),
    )
...
def _event_payload(**changes: object) -> str:
    value: dict[str, object] = {
        ...
        "policy_version": "1",
        "subject_id": 1,
        "run_key": "mail:1:process:fixture",
        "source_mail_message_id": 1,
        "source_mail_thread_id": 1,
...
def _insert_event(conn: sqlite3.Connection, payload: str) -> None:
    conn.execute(
        "INSERT INTO conversation_events(subject_id,event_type,actor_role,occurred_at_utc,mail_message_id,structured_payload_json,trust_level,created_by) VALUES(1,'plan_revision_reason_recorded','trainlab',?,1,?,'system_generated','mail_agent')",
        (UTC, payload),
    )


def test_reason_dto_requires_current_awaiting_user_mail_and_hides_evidence(
    tmp_path: Path,
) -> None:
...
def test_reason_dto_rejects_malformed_or_unbound_payload(
