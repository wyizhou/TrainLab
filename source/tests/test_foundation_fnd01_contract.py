...
def test_fnd01_request_schema_is_exact_and_api_rejects_before_io(
    tmp_path: Path,
) -> None:
    schema = json.loads(
    ...
    for payload in invalid:
        ...
        if "unknown" not in payload and payload["mode"] in {
            "init",
            "status",
            "verify",
            "migrate",
            ...
            receipt = tool.execute(
                FoundationRequest(
                    ...
                    payload["requested_at_utc"],
                    payload["target_schema_version"],
                )
            )
            assert receipt.status == "failed" and receipt.invocation_id in {
                "",
                payload["invocation_id"],
            }
...
def test_fnd01_owner_config_and_path_rejection_has_no_foundation_write(
    tmp_path: Path, attack: str
) -> None:
    project = tmp_path / "project"
    _write_owner_project(project)
    yaml_path = project / "config" / "foundation.yaml"
    if attack == "absolute":
        yaml_path.write_text(
            yaml_path.read_text().replace("state/foundation", "/tmp/foundation")
        )
    elif attack == "escape":
        yaml_path.write_text(
            yaml_path.read_text().replace(
                "database_path: data.db", "database_path: ../escape.db"
            )
        )
    elif attack == "root_alias":
        yaml_path.write_text(
            yaml_path.read_text().replace("data_root: state/foundation", "data_root: .")
        )
    elif attack == "db_raw_overlap":
        yaml_path.write_text(
            yaml_path.read_text().replace("raw_root: raw", "raw_root: data.db")
        )
    elif attack == "middle_symlink":
        (project / "state").symlink_to(tmp_path / "outside", target_is_directory=True)
    elif attack == "config_symlink":
        target = project / "config" / "other.yaml"
        target.write_text(yaml_path.read_text())
        yaml_path.unlink()
        yaml_path.symlink_to(target)
    else:
...
def test_fnd01_cli_and_api_share_equivalent_receipt_and_no_workers(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    root = tmp_path / "foundation"
    owner = config(root)
    api_owner = config(tmp_path / "api-foundation")
    instance = tmp_path / "instance"
    for name in ("config", "state", "logs"):
        (instance / name).mkdir(parents=True)
    monkeypatch.setenv("TRAINLAB_INSTANCE_ROOT", str(instance))
    monkeypatch.setattr(
        foundation.FoundationConfig, "load", classmethod(lambda cls, _: owner)
    )
    ...
    api_receipt = json.loads(
        FoundationTool(api_owner)
        .execute(FoundationRequest("init", "same-request", UTC))
        .json()
    )
...
def test_fnd01_cli_configuration_failure_is_one_redacted_receipt(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    instance = tmp_path / "instance"
    for name in ("config", "state", "logs"):
    ...
    output = capsys.readouterr()
    assert (
        output.err == "" and output.out.count("\n") == 1 and "token=" not in output.out
    )
