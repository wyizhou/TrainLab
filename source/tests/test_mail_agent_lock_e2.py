...
def path(tmp_path: Path) -> Path:
    ...
    os.chmod(tmp_path / "state" / "locks", 0o700)
...
def test_e2_directory_rejections(tmp_path: Path, kind: str) -> None:
    ...
    if kind == "state_symlink":
        (tmp_path / "state").rename(tmp_path / "old")
        (tmp_path / "state").symlink_to(outside, target_is_directory=True)
    elif kind == "locks_symlink":
        (tmp_path / "state" / "locks").rename(tmp_path / "state" / "old")
        (tmp_path / "state" / "locks").symlink_to(outside, target_is_directory=True)
    elif kind == "state_missing":
        os.rmdir(tmp_path / "state" / "locks")
        os.rmdir(tmp_path / "state")
    elif kind == "locks_missing":
        os.rmdir(tmp_path / "state" / "locks")
    elif kind == "state_mode":
        os.chmod(tmp_path / "state", 0o755)
    else:
        os.chmod(tmp_path / "state" / "locks", 0o755)
    with pytest.raises(MailLockBusyError):
        MailWriteLock(target, "mail:1:poll:x").acquire()
...
def test_e2_success_fd_loop(tmp_path: Path) -> None:
    ...
    for _ in range(100):
        lock = MailWriteLock(target, "mail:1:poll:x")
        lock.acquire()
        lock.release()
...
def test_e2_reader_after_disappearance_busy(
    ...
    with pytest.raises(MailLockBusyError):
        lock._read_existing(lock._parent_fd or -1)
...
@pytest.mark.parametrize(
    "target,stage",
    [("state", "before"), ("state", "after"), ("locks", "before"), ("locks", "after")],
)
def test_e2_directory_swap_is_busy(
    ...
    stats = 0
    directory = (
        tmp_path / "state" if target == "state" else tmp_path / "state" / "locks"
    )
    ...
    def swap() -> None:
        ...
        if not swapped:
            swapped = True
            os.rename(directory, directory.with_name(directory.name + "-old"))
            directory.mkdir(mode=0o700)

    def guarded_open(name, *args, **kwargs):
        if stage == "before" and name == target:
            swap()
    ...
    with pytest.raises(MailLockBusyError):
        MailWriteLock(candidate, "mail:1:poll:x").acquire()
    assert swapped
...
def test_e2_reader_replacement_busy(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, kind: str
) -> None:
    candidate = path(tmp_path)
    lock = MailWriteLock(candidate, "mail:1:poll:x")
    lock.acquire()
    original = os.open
    changed = False
    ...
    def guarded(name, *args, **kwargs):
        ...
        if name == "mail.lock" and not changed:
            ...
            candidate.unlink()
            if kind == "symlink":
                candidate.symlink_to(tmp_path / "other")
            else:
    ...
    assert (
        candidate.is_symlink() if kind == "symlink" else candidate.read_bytes() == raw
    )
    if lock._parent_fd is not None:
        os.close(lock._parent_fd)
        lock._parent_fd = None
...
def test_e2_failure_stage_fd_stable(
    ...
    for index in range(20):
        ...
        def guarded_stat(name, *args, **kwargs):
            ...
            if name in {"state", "locks"} and kwargs.get("dir_fd") is not None:
                ...
                if case == f"{name}_before_stat" and counts[name] == 1:
                    fired = True
                    raise OSError("injected")
                if case == f"{name}_after_stat" and counts[name] == 2:
                    fired = True
                    raise OSError("injected")
            return original_stat(name, *args, **kwargs)
        ...
        def guarded_fstat(fd):
            ...
            if case.endswith("_fstat") and captured.get(target) == fd:
                fired = True
                raise OSError("injected")
