...
from src import cli
from src.analysis import runtime
from src.analysis.contracts import AnalysisReceipt
...
def test_analysis_daily_uses_only_its_current_runtime_and_prints_one_receipt(
    monkeypatch, capsys
):
...
def test_unique_active_subject_is_used_for_runtime_wiring(monkeypatch):
    ...
    monkeypatch.setattr(runtime, "load_analysis_config", lambda _root: config)
    ...
    from src.analysis import daily
...
def test_weekly_runtime_wires_the_weekly_route_and_closes_connection(monkeypatch):
    ...
    from src.analysis import weekly
...
def test_plan_revision_runtime_wires_route_and_closes_connection(monkeypatch):
    ...
    from src.analysis import revise_plan
...
def test_analysis_routes_are_explicit_and_never_use_historical_run_command(
    monkeypatch, capsys
):
    calls = []
    monkeypatch.setattr(
    ...
    assert calls[-1] == (
        "weekly",
        {"invocation_id": "three", "as_of_date": "2026-07-27", "deliver": True},
    )
    assert (
        cli.main(
            [
...
def test_analysis_delivery_uses_the_shared_project_recipient_config(monkeypatch):
    ...
    assert (
        runtime._execute_delivery(
            object(), config, 7, "retry_delivery", Path("/project")
        )
        == "delivered"
    )
...
def test_status_runtime_opens_only_foundation_database_and_status_service(monkeypatch):
    ...
    from src.analysis import status
