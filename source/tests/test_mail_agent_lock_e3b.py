...
def _make_claim_mutator(
    ...
    def rename_hook(
        ...
        **kwargs: object,
        ...
        if os.fspath(src) != "mail.lock" or not str(os.fspath(dst)).startswith(
            ".mail.lock.tombstone-"
        ):
            return
        ...
        if kind == "mutate":
            fd = os.open(
                claim, os.O_WRONLY | os.O_TRUNC | os.O_NOFOLLOW, dir_fd=directory_fd
            )
            try:
...
def _run_claim_race(
    root: Path, operation: str, kind: str, monkeypatch: pytest.MonkeyPatch
    ...
    fired, unknown_raw, unknown_inode_box = _make_claim_mutator(
        path, original_raw, original_inode, kind, monkeypatch
    )
    if operation == "release":
        ...
        with pytest.raises(MailLockBusyError, match="mail_lock_replaced"):
            MailWriteLock(
                path, "mail:1:poll:takeover", process_is_alive=lambda _pid: False
            ).acquire()
    assert fired
...
def test_e3b_claim_race_preserves_unknown_and_blocks_retry(
    ...
    for index in range(20):
        with monkeypatch.context() as patch:
            path, raw, inode, fired = _run_claim_race(
                tmp_path / f"{operation}-{kind}-{index}", operation, kind, patch
            )
            assert (
                "inplace_mutation" if kind == "mutate" else "new_inode_replacement"
            ) in fired
...
def _run_claim_uncertainty(
    ...
    def open_hook(
        name: str | bytes | Path, flags: int, *args: object, **kwargs: object
    ) -> int:
        if (
            fault == "claim_open"
    ...
    def stat_hook(
        name: str | bytes | Path, *args: object, **kwargs: object
    ) -> os.stat_result:
        nonlocal claim_stats
        if fault == "claim_after_stat" and str(os.fspath(name)).startswith(
            ".mail.lock.tombstone-"
        ):
            claim_stats += 1
            if claim_stats == 2:
    ...
    def unlink_hook(name: str | bytes | Path, *args: object, **kwargs: object) -> None:
        ...
        # deletion attempt.
        if fault == "restore_unlink" and str(os.fspath(name)).startswith(
            ".mail.lock.tombstone-"
        ):
...
@pytest.mark.parametrize(
    "fault", ("claim_open", "claim_after_stat", "restore_link", "restore_unlink")
)
def test_e3b_claim_uncertainty_preserves_busy_evidence(
    ...
    for index in range(20):
        with monkeypatch.context() as patch:
            proofs.append(
                _run_claim_uncertainty(
                    tmp_path / f"{operation}-{fault}-{index}", operation, fault, patch
                )
            )
