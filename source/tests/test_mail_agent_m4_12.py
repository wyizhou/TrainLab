...
from src.cli import main as root_main
from src.foundation import FoundationConfig, FoundationRequest, FoundationTool
from src.mail_agent.application import (
    MailApplicationService,
    MailApplicationStages,
)
from src.mail_agent.cli import main as mail_main
from src.mail_agent.contracts import MailCounts, MailReceipt, MailRequest, MailTool
...
class Stage:
    ...
    def execute(self, item):
        ...
        key = (
            item.mail_message_ids[0]
            if item.mail_message_ids
            else item.mail_response_artifact_ids[0]
            if item.mail_response_artifact_ids
            else item.mail_delivery_ids[0]
            if item.mail_delivery_ids
            else "default"
        )
        value = self.outcomes.get(key, self.outcomes.get("default"))
        if isinstance(value, Exception):
            raise value
        if isinstance(value, MailReceipt):
            return value
...
class Queue:
    ...
    def queued_message_ids(self, *, subject_id, limit):
        self.calls.append((subject_id, limit))
        return self.items


def app(
    *, queued=("m2", "m1"), process=None, deliver=None, reconcile=None, status=None
):
    poll = Stage("poll")
    process_stage = Stage("process", process)
    delivery = Stage("delivery", deliver)
    recovery = Stage("reconcile", reconcile)
    local_status = Stage("status", status)
    queue = Queue(queued)
    service = MailApplicationService(
        MailApplicationStages(
            poll, process_stage, delivery, recovery, local_status, queue
        ),
    ...
    service, stages = app(
        process={
            "m1": {
                ...
                "mail_response_artifact_ids": ("r1",),
                "counts": MailCounts(processed=1, responses_accepted=1),
            }
        },
        deliver={
            "r1": {
                "mail_delivery_ids": ("d1",),
                "counts": MailCounts(deliveries_sent=1),
            }
        },
    )
...
def test_run_continues_after_one_item_failure_and_reconciles_without_regeneration() -> (
    None
):
    service, stages = app(
        process={
            "m1": RuntimeError("x"),
            "m2": {
                "mail_response_artifact_ids": ("r2",),
                "counts": MailCounts(processed=1),
            },
        },
        deliver={
            "r2": {"mail_delivery_ids": ("d2",), "next_action": "reconcile_delivery"}
        },
        reconcile={"d2": {"counts": MailCounts(deliveries_already_sent=1)}},
    )
    result = service.execute(request(max_items=2, deadline_seconds=30))
...
def test_invalid_stage_receipt_and_invalid_run_limits_fail_closed() -> None:
    service, stages = app()
    stages[4].outcomes["default"] = MailReceipt(
        run_key="wrong",
        invocation_id="bad",
        mode="status",
        status="succeeded",
...
def test_cli_stdout_is_one_receipt_and_root_registration_uses_injected_tool(
    capsys,
) -> None:
    service, _ = app(status={"default": {"status": "unchanged"}})
    tool = MailTool(service)
    assert (
        mail_main(["--subject-id", "7", "--invocation-id", "cli", "status"], tool=tool)
        == 0
    )
    assert list(json.loads(capsys.readouterr().out).keys())
    assert (
        root_main(
            ["mail", "--subject-id", "7", "--invocation-id", "root", "status"],
            mail_tool=tool,
        )
        == 0
    )
...
def test_root_status_needs_no_mail_configuration_or_background_thread(
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "mail-status-fixture", NOW))
        .ready
    )
    (data_root / "config").mkdir()
    (data_root / "logs").mkdir()
    monkeypatch.setenv("TRAINLAB_INSTANCE_ROOT", str(data_root))
    monkeypatch.setattr(
        "src.mail_agent.runtime.FoundationConfig.load",
        lambda _root: foundation,
    )
    before = {item.ident for item in threading.enumerate()}
    code = root_main(
        ["mail", "--subject-id", "7", "--invocation-id", "offline", "status"]
    )
    value = json.loads(capsys.readouterr().out)
