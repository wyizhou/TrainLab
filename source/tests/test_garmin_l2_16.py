...
from src.garmin import GarminConfig, SyncReceipt, cli
...
def test_garmin_help_has_only_one_shot_command_tree() -> None:
    ...
    garmin = next(
        item for item in parser._subparsers._group_actions if item.dest == "command"
    ).choices["garmin"]
    ...
    assert (
        parser.parse_args(
            ["garmin", "sync", "full", "--health-from", "2026-01-01"]
        ).garmin_sync_mode
        == "full"
    )
    assert (
        parser.parse_args(
            ["garmin", "repair", "--resource", "activities", "--strategy", "reparse"]
        ).strategy
        == "reparse"
    )
...
def test_invalid_request_is_rejected_before_provider_or_configuration(
    monkeypatch: pytest.MonkeyPatch, args: SimpleNamespace
) -> None:
    monkeypatch.setattr(
        cli.FoundationConfig,
        "load",
        lambda _: (_ for _ in ()).throw(AssertionError("configuration")),
    )
    result = cli.garmin_cli_execute(
        args,
        transport_factory=lambda *_a, **_k: (_ for _ in ()).throw(
            AssertionError("provider")
        ),
    )
...
def test_every_receipt_status_has_stable_exit_code(
    ...
    monkeypatch.setattr(
        root_cli, "_parser", lambda: SimpleNamespace(parse_args=lambda _argv: parsed)
    )
    monkeypatch.setattr(
        root_cli, "garmin_cli_execute", lambda _args: _receipt(status=status)
    )
...
def test_valid_command_calls_unified_api_once_and_leaves_no_listener(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    monkeypatch.setattr(cli, "project_root", lambda: tmp_path)
    foundation = SimpleNamespace(state_root=tmp_path / "state")
    config = GarminConfig(
        tmp_path / "data.db", tmp_path / "raw", tmp_path / "state", "2026-01-01"
    )
    ...
    monkeypatch.setattr(
        socket,
        "socket",
        lambda *_a, **_k: (_ for _ in ()).throw(AssertionError("socket")),
    )
...
def test_status_constructs_no_transport_and_malformed_adapter_args_are_controlled(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    monkeypatch.setattr(cli, "project_root", lambda: tmp_path)
    foundation = SimpleNamespace(state_root=tmp_path / "state")
