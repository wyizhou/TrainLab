...
from src.garmin_catalog import RESOURCE_CATALOG

ADVANCED = {
    "training_readiness": (
        {
            "trainingReadinessScore": 71,
            "timestampGMT": "2026-04-15T01:00:00Z",
            "displayName": "PRIVATE",
        },
        "garmin.training_readiness.score",
    ),
    "max_metrics": (
        {"vo2Max": 44.2, "calendarDate": "2026-04-15"},
        "garmin.vo2_max.ml_per_kg_min",
    ),
    "lactate_threshold": (
        {
            "lactateThresholdHeartRate": 168,
            "lactateThresholdPower": 242,
    ...
    "training_status": (
        {
            "trainingStatusScore": 64,
            "acuteTrainingLoad": 398,
            "calendarDate": "2026-04-15",
        },
        "garmin.training_status.score",
    ),
    "running_tolerance": (
        [
            {
        ...
        "garmin.running_tolerance.score",
    ),
    "endurance_score": (
        {"enduranceScore": 5120, "calendarDate": "2026-04-15"},
        "garmin.endurance_score",
    ),
    "hill_score": (
        {"hillScore": 48, "calendarDate": "2026-04-15"},
        "garmin.hill_score",
    ),
    "race_predictions": (
        {
            "racePredictions": [
                {"predictionSeconds": 1370, "calendarDate": "2026-04-15"}
            ]
        },
        "garmin.race_prediction.seconds",
    ),
    "fitness_age": (
        {"fitnessAge": 31, "calendarDate": "2026-04-15"},
    ...
    "menstrual_day": (
        {
            ...
            "periodLength": 5,
            "calendarDate": "2026-04-15",
            "symptomNote": "preserve-in-raw",
        },
        "garmin.menstrual.cycle_length_days",
    ),
    "menstrual": (
        {
            "cycleLength": 28,
    ...
    "nutrition_food_log": (
        {
            ...
            "fat": 61,
            "calendarDate": "2026-04-15",
            "foodName": "preserve-in-raw",
        },
        "garmin.nutrition.calories_kcal",
    ),
    "nutrition_meals": (
        {
            "meals": [
    ...
    "nutrition_settings": (
        ...
        "garmin.nutrition.calorie_goal_kcal",
    ),
}
...
class AdvancedTransport:
    def __init__(self) -> None:
        self.payloads = {
            ...
            for kind, (payload, _metric) in ADVANCED.items()
        }
    ...
    def fetch_account(self, *_args):
        raise AssertionError("no account endpoint")

    def list_activities(self, *_args):
        return []

    def activity_summary(self, *_args):
        raise AssertionError("no activity")

    def activity_original(self, *_args):
        raise AssertionError("no activity")

    def activity_extra(self, *_args):
        raise AssertionError("no activity")
...
def _setup(tmp_path: Path):
    ...
    assert (
        FoundationTool(f)
        .execute(FoundationRequest("init", "l2-09b2", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
    config = GarminConfig(
...
def test_live_nested_vo2_and_weight_are_available_to_daily_analysis(
    tmp_path: Path,
) -> None:
...
def test_provider_json_quarantine_normalizes_reviewed_credentials_and_auth_values(
    payload: object,
) -> None:
...
def test_provider_json_quarantine_allows_similarly_named_health_and_profile_fields() -> (
    None
):
    payload = {
...
def test_repository_archive_boundary_rejects_wrapped_api_keys_without_payload_writes(
    ...
    with sqlite3.connect(config.database_path) as audit:
        ...
        assert (
            audit.execute("SELECT count(*) FROM source_field_catalog").fetchone()[0]
            == 0
        )
        dump = "\n".join(audit.iterdump())
        raw_files = b"".join(
...
@pytest.mark.parametrize(
    ("resource", "payload", "metric"),
    [(kind, payload, metric) for kind, (payload, metric) in ADVANCED.items()],
)
def test_each_advanced_resource_has_catalog_signature_raw_revision_and_canonical(
    tmp_path: Path, resource: str, payload: object, metric: str
) -> None:
    config, tool, transport = _setup(tmp_path)
    spec = RESOURCE_CATALOG[resource]
    assert (
        spec.scope in {"daily", "range"} and spec.conditional and spec.cursor_eligible
    )
    first = _repair(tool, resource, f"advanced-{resource}")
    expected_call = (
        (resource, "2026-04-15:2026-04-15")
        if spec.scope == "range"
        else (resource, "2026-04-15")
    )
    assert first.status == "succeeded" and transport.calls == [expected_call]
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind=? AND is_current=1",
                (resource,),
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_metrics WHERE metric_key=?", (metric,)
            ).fetchone()[0]
            >= 1
        )
        source_path = conn.execute(
            "SELECT source_path FROM physiology_metrics WHERE metric_key=? LIMIT 1",
            (metric,),
        ).fetchone()[0]
        assert conn.execute(
            "SELECT mapping_state,canonical_metric_key FROM source_field_catalog WHERE resource_kind=? AND field_path=?",
            (resource, source_path),
        ).fetchone() == ("mapped", metric)
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind=?",
                (resource,),
            ).fetchone()[0]
            == "fetched"
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_items WHERE resource_kind=? AND status='running'",
                (resource,),
            ).fetchone()[0]
            == 0
        )
...
def test_advanced_revisions_tombstones_capabilities_and_snapshot_are_deterministic(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    _repair(tool, "training_readiness", "adv-first")
    transport.payloads["training_readiness"] = {
        "trainingReadinessScore": 75,
        "calendarDate": "2026-04-15",
    }
    revised = _repair(tool, "training_readiness", "adv-revised")
    assert revised.counts["revised"] == 1
    transport.payloads["training_readiness"] = {
        "availability_state": "not_enabled",
        "email": "private@example.invalid",
    }
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind='training_readiness' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "not_enabled"
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM v_current_physiology_records WHERE record_type='training_readiness'"
            ).fetchone()[0]
            == 0
        )
    ...
    unavailable = tool.execute(
        SyncRequest(
            ...
            health_from_local_date="2026-04-15",
            through_local_date="2026-04-15",
            resource_kinds=("hill_score", "endurance_score"),
            repair_strategy="refetch",
            invocation_id="adv-capability",
        )
    )
    assert (
        unavailable.status == "partial"
        and unavailable.counts["not_available"] == 1
        and unavailable.counts["failed"] == 1
    )
    ...
    snap = tool.execute(
        SyncRequest(
            "snapshot", snapshot_local_date="2026-04-15", invocation_id="adv-snapshot"
        )
    )
...
def test_advanced_malformed_payload_rolls_back_and_sensitive_markers_stay_outside_raw(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        current = conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind='race_predictions' AND is_current=1"
        ).fetchone()[0]
    transport.payloads["race_predictions"] = {
        "racePredictions": [{"predictionSeconds": 1200, "timestampGMT": "not-a-time"}],
        "token": "PRIVATE",
    }
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind='race_predictions'"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT error_code FROM garmin_sync_items WHERE resource_kind='race_predictions' AND status='failed' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "provider_payload_quarantined"
        )
        raw = b"".join(
            (config.raw_root.parent / row[0]).read_bytes()
            for row in conn.execute("SELECT relative_path FROM raw_objects")
        )
        dump = "\n".join(conn.iterdump()).encode()
        assert b"PRIVATE" not in raw + dump + failed.json().encode()


def test_advanced_full_incremental_lookback_and_single_coverage_per_day(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    full = tool.execute(
        SyncRequest(
            "full",
            health_from_local_date="2026-04-14",
            through_local_date="2026-04-15",
            resource_kinds=("fitness_age",),
            invocation_id="adv-full",
        )
    )
    assert full.status == "succeeded"
    assert transport.calls == [
        ("fitness_age", "2026-04-14"),
        ("fitness_age", "2026-04-15"),
    ]
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='fitness_age' AND local_date='2026-04-15'"
            ).fetchone()[0]
            == 1
        )


def test_range_calendar_is_chunked_once_and_one_raw_revision_has_multi_day_lineage(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    receipt = tool.execute(
        SyncRequest(
            ...
            health_from_local_date="2026-04-14",
            through_local_date="2026-04-15",
            resource_kinds=("menstrual",),
            invocation_id="adv-calendar-range",
        )
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        revision = conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind='menstrual' AND is_current=1"
        ).fetchone()[0]
        assert list(
            conn.execute(
                "SELECT local_date,source_revision_id FROM resource_coverage WHERE resource_kind='menstrual' ORDER BY local_date"
            )
        ) == [("2026-04-14", revision), ("2026-04-15", revision)]
        raw = b"".join(
            (config.raw_root.parent / row[0]).read_bytes()
            for row in conn.execute(
                "SELECT relative_path FROM raw_objects WHERE resource_kind='menstrual'"
            )
        )
...
def test_sparse_range_is_planned_by_catalog_limit_and_closes_empty_days(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    receipt = tool.execute(
        SyncRequest(
            "full",
            health_from_local_date="2026-04-01",
            through_local_date="2026-04-29",
            resource_kinds=("menstrual",),
            invocation_id="sparse-range",
        )
    )
    assert receipt.status == "succeeded"
    assert transport.calls == [
    ...
    with sqlite3.connect(config.database_path) as conn:
        rows = list(
            conn.execute(
                "SELECT local_date,availability_state,source_revision_id FROM resource_coverage WHERE resource_kind='menstrual' ORDER BY local_date"
            )
        )
        assert len(rows) == 29
        ...
        assert (
            rows[1][1] == "empty"
        )  # sparse per-day absence is not a capability tombstone
        assert len({row[2] for row in rows[:14]}) == 1
        assert len({row[2] for row in rows[14:28]}) == 1
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_records WHERE record_type='menstrual'"
            ).fetchone()[0]
            == 3
        )
        assert conn.execute(
...
def test_range_parse_failure_keeps_received_revision_and_reparse_same_payload(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    transport.payloads["race_predictions"] = {
        "racePredictions": [{"predictionSeconds": 1300, "calendarDate": "not-a-date"}],
        "displayName": "kept-raw",
    }
    failed = _repair(tool, "race_predictions", "range-unparsed")
    assert failed.status == "partial"
    with sqlite3.connect(config.database_path) as conn:
        revision = conn.execute(
            "SELECT id,is_current,parsed_at_utc FROM source_revisions WHERE resource_kind='race_predictions' AND is_current=0 ORDER BY id DESC"
        ).fetchone()
        ...
        assert (
            conn.execute(
                "SELECT source_revision_id FROM garmin_sync_gaps WHERE resource_kind='race_predictions'"
            ).fetchone()[0]
            == revision[0]
        )
        raw = b"".join(
            (config.raw_root.parent / row[0]).read_bytes()
            for row in conn.execute(
                "SELECT relative_path FROM raw_objects WHERE resource_kind='race_predictions'"
            )
        )
        assert b"kept-raw" in raw
    transport.payloads["race_predictions"] = {
        "racePredictions": [{"predictionSeconds": 1300, "calendarDate": "2026-04-15"}],
        "displayName": "kept-raw",
    }
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='race_predictions' AND is_current=1"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_metrics WHERE metric_key='garmin.race_prediction.seconds'"
            ).fetchone()[0]
            == 1
        )
...
def test_unchanged_range_success_resolves_per_day_key_but_provider_error_does_not(
    ...
    try:
        ...
        conn.close()
    transport.errors["race_predictions"] = GarminError(
...
def test_range_long_rate_limit_defers_whole_catalog_chunks_and_does_not_close_cursor(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.errors["menstrual"] = GarminError(
        "rate_limited", http_status=429, retry_after=121
    )
    receipt = tool.execute(
        SyncRequest(
            "full",
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert list(
            conn.execute(
                "SELECT window_start_local_date,window_end_local_date,status FROM garmin_sync_gaps WHERE resource_kind='menstrual' ORDER BY id"
            )
        ) == [
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_cursors WHERE resource_kind='menstrual'"
            ).fetchone()[0]
            == 0
        )
...
def test_credential_quarantine_is_end_to_end_boundary_for_every_json_surface(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind=?",
                (resource,),
            ).fetchone()[0]
            == 0
        )
        ...
        assert (
            conn.execute("SELECT count(*) FROM physiology_records").fetchone()[0] == 0
        )
        assert (
            conn.execute("SELECT count(*) FROM physiology_metrics").fetchone()[0] == 0
        )
        assert conn.execute("SELECT count(*) FROM activities").fetchone()[0] == 0
        assert (
            conn.execute(
                "SELECT count(*) FROM source_field_catalog WHERE resource_kind=?",
                (resource,),
            ).fetchone()[0]
            == 0
        )
        ...
        durable = (
            "\n".join(conn.iterdump())
            + receipt.json()
            + raw_files.decode(errors="ignore")
        )
        assert "QUARANTINE_" not in durable and "fixture-credential" not in durable


def test_same_raw_hash_reuses_unparsed_revision_after_projector_fix(
    ...
    monkeypatch.setattr(
        tool,
        "_project_health",
        lambda *_args: (_ for _ in ()).throw(ValueError("controlled_project_failure")),
    )
    assert _repair(tool, "race_predictions", "same-hash-fail").status == "partial"
    with sqlite3.connect(config.database_path) as conn:
        received = conn.execute(
            "SELECT id,payload_hash FROM source_revisions WHERE resource_kind='race_predictions' AND is_current=0 AND parsed_at_utc IS NULL"
        ).fetchone()
    ...
    with sqlite3.connect(config.database_path) as conn:
        current = conn.execute(
            "SELECT id,payload_hash FROM source_revisions WHERE resource_kind='race_predictions' AND is_current=1"
        ).fetchone()
        assert current == (received_id, received_hash)
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='race_predictions'"
            ).fetchone()[0]
            == 2
        )


def test_malformed_range_is_validated_before_old_coverage_or_canonical_changes(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.payloads["menstrual"] = [
    ...
    assert (
        tool.execute(
            SyncRequest(
                ...
                through_local_date="2026-04-15",
                resource_kinds=("menstrual",),
                invocation_id="range-old-canonical",
            )
        ).status
        == "succeeded"
    )
    with sqlite3.connect(config.database_path) as conn:
        ...
        old_coverage = list(
            conn.execute(
                "SELECT id,source_revision_id FROM resource_coverage WHERE resource_kind='menstrual' ORDER BY local_date"
            )
        )
    transport.payloads["menstrual"] = [
        {"calendarDate": "2026-04-14", "cycleLength": 28},
        {"calendarDate": "not-a-date", "cycleLength": 29},
    ]
    failed = tool.execute(
        SyncRequest(
            "full",
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='menstrual' AND is_current=0 AND parsed_at_utc IS NULL"
            ).fetchone()[0]
            == 1
        )
        assert conn.execute(
            "SELECT window_start_local_date,window_end_local_date FROM garmin_sync_gaps WHERE resource_kind='menstrual' ORDER BY id DESC LIMIT 1"
        ).fetchone() == ("2026-04-14", "2026-04-15")


def test_duplicate_advanced_range_date_is_shape_drift_before_projection(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.payloads["menstrual"] = [
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='menstrual'"
            ).fetchone()[0]
            == 0
        )
        assert (
...
@pytest.mark.parametrize(
    ("error", "expected_state", "expected_receipt"),
    [
        (GarminError("forbidden", http_status=403), "forbidden", "partial"),
        (GarminError("missing", http_status=404), "not_available", "succeeded"),
    ],
)
def test_range_terminal_4xx_has_chunk_coverage_capability_and_gap(
    tmp_path: Path, error: GarminError, expected_state: str, expected_receipt: str
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind='menstrual'"
            ).fetchone()[0]
            == expected_state
        )
        assert (
            conn.execute(
                "SELECT capability_state FROM garmin_resource_capabilities WHERE resource_kind='menstrual'"
            ).fetchone()[0]
            == expected_state
        )
        assert conn.execute(
            "SELECT window_start_local_date,window_end_local_date FROM garmin_sync_gaps WHERE resource_kind='menstrual'"
        ).fetchone() == ("2026-04-15", "2026-04-15")


def test_range_short_rate_limit_retries_inline_and_transient_errors_stop_at_five(
    tmp_path: Path,
) -> None:
    ...
    assert failed.status == "partial"
    assert [call for call in transport.calls if call[0] == "menstrual"] == [
        ("menstrual", "2026-04-15:2026-04-15")
    ] * 5


def test_range_second_401_is_run_auth_required_after_one_refresh(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    logins = {"count": 0}
    ...
    assert (
        receipt.status == "auth_required" and logins["count"] == 2
    )  # initial login + one refresh
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT status FROM garmin_sync_runs ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "auth_required"
        )
...
def test_live_health_range_shapes_project_only_measurements_and_preserve_raw(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    weigh_ins = {
    ...
    transport.payloads.update(
        {"weigh_ins": weigh_ins, "lactate_threshold": lactate, "hill_score": hill_empty}
    )
    ...
    assert (
        _repair(tool, "lactate_threshold", "live-lactate-repeat").counts["unchanged"]
        == 1
    )
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            stable_json(weigh_ins) in raw
            and stable_json(lactate) in raw
            and stable_json(hill_empty) in raw
        )


def test_hill_score_non_null_period_average_is_not_silently_empty(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
...
def test_live_body_battery_date_and_endurance_singleton_envelopes(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
...
def test_endurance_latest_outside_requested_window_is_not_misattributed(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.payloads["endurance_score"] = {
