...
from src.mail_agent.poll import MailPollConfig, MailPollService
...
def message(
    ...
    *,
    ...
    when: str = "2026-07-22T08:00:00Z",
    html: str | None = None,
) -> dict:
    value = {
        "message_id": message_id,
        "thread_id": thread_id,
        "internal_date_utc": when,
        "from": "self@example.com",
        "to": ["self@example.com"],
        "subject": "TrainLab",
        "headers": {"message_id": f"<{message_id}>"},
        "label_ids": labels or [],
        "plain_text": body,
...
class FakePollAdapter:
    ...
    def prepare(self, connection: sqlite3.Connection, subject_id: int) -> GmailIdentity:
        self.verified_identity_id = connection.execute(
            "SELECT id FROM subject_identities WHERE subject_id=?", (subject_id,)
        ).fetchone()[0]
        return self._identity
        ...
        for item in raw["messages"]:
            ...
            values.append(
                GmailMessage(
                    str(item["message_id"]),
                    str(item["thread_id"]),
                    item.get("internal_date_utc"),
                    item.get("from"),
                    tuple(item.get("to", [])),
                    item.get("subject"),
                    headers.get("message_id"),
                    headers.get("in_reply_to"),
                    tuple(headers.get("references", [])),
                    headers.get("x_trainlab_run_id"),
                    tuple(item.get("label_ids", [])),
                    item.get("plain_text", ""),
                    bool(item.get("has_html")),
                    tuple(item.get("attachments", [])),
                )
            )
...
def env(
    tmp_path: Path, adapter: FakePollAdapter
) -> tuple[MailPollService, sqlite3.Connection, int, Path]:
    root = tmp_path / "data"
    ...
    assert (
        FoundationTool(config).execute(FoundationRequest("init", "m4-04", NOW)).status
        == "initialized"
    )
    ...
    conn.execute(
        ...
        (subject, _hmac("self@example.com"), NOW, NOW),
    )
    conn.commit()
    service = MailPollService(
        ...
        adapter,
        MailPollConfig(
            root / "raw", root / "state" / "locks" / "mail.lock", provider_page_limit=3
        ),
        clock=lambda: NOW,
    )
...
def request(
    subject: int, invocation: str, *, max_threads: int | None = None
) -> MailRequest:
...
def test_two_streams_archive_normalize_cursor_and_repeat_noop(tmp_path: Path) -> None:
    ...
    assert first.status == "succeeded" and {
        row["stream"] for row in first.poll_state
    } == {"trainlab_label", "tracked_threads"}
    ...
    assert (
        conn.execute("SELECT count(*) FROM mail_response_artifacts").fetchone()[0] == 0
    )
    assert conn.execute("SELECT count(*) FROM mail_deliveries").fetchone()[0] == 0
    assert (
        conn.execute("SELECT count(*) FROM mail_delivery_artifacts").fetchone()[0] == 0
    )
    assert {
        tuple(row) for row in conn.execute("SELECT stage,status FROM mail_agent_items")
    } >= {
        ("discover", "succeeded"),
        ("archive", "succeeded"),
        ("normalize", "succeeded"),
...
def test_initial_window_then_48_hour_overlap_and_max_threads_never_marks_complete(
    tmp_path: Path,
) -> None:
    raw = {"messages": [message("m-overlap", "toverlap", labels=["TrainLab"])]}
    adapter = FakePollAdapter(
        label_matches=[{"thread_id": "toverlap"}], threads={"toverlap": raw}
    )
    ...
    assert (
        service.execute(
            MailRequest("poll", subject, "overlap", "2026-07-24T07:00:00Z")
        ).status
        == "unchanged"
    )
    overlap_start, overlap_end = adapter.search_calls[-1]
    assert overlap_start == datetime(
        2026, 7, 21, 7, tzinfo=timezone.utc
    ) and overlap_end == datetime(2026, 7, 24, 7, tzinfo=timezone.utc)
    limited = FakePollAdapter(
        label_matches=[{"thread_id": "toverlap"}], threads={"toverlap": raw}
    )
    limited_service, limited_conn, limited_subject, _ = env(
        tmp_path / "limited", limited
    )
    receipt = limited_service.execute(
        request(limited_subject, "limited", max_threads=0)
    )
    assert (
        receipt.status == "partial"
        and limited_conn.execute("SELECT count(*) FROM mail_poll_cursors").fetchone()[0]
        == 1
    )


def test_tracked_reply_without_label_and_one_stream_failure_do_not_cross_cursor(
    tmp_path: Path,
) -> None:
    ...
    assert receipt.status == "partial" and states == {
        "trainlab_label": "succeeded",
        "tracked_threads": "partial",
    }
    assert (
        conn.execute(
...
def test_label_page_limit_splits_and_unsplittable_window_is_partial(
    tmp_path: Path,
) -> None:
...
def test_full_page_splits_into_complete_subwindows_with_shared_dedupe(
    tmp_path: Path,
) -> None:
    raws = {
    ...
    class SplitAdapter(FakePollAdapter):
        def search_trainlab_window(
            ...
            if int((end_date - start_date).total_seconds()) == 48 * 3600:
                return [
                    {"thread_id": "overflow-1"},
                    {"thread_id": "overflow-2"},
                    {"thread_id": "overflow-3"},
                ]
...
def test_long_cursor_gap_is_split_into_provider_bounded_windows(tmp_path: Path) -> None:
    ...
    assert all(
        (end - start).total_seconds() <= 7 * 86_400
        for start, end in adapter.search_calls
    )


def test_stream_stops_after_first_failed_thread_and_does_not_scan_later_items(
    tmp_path: Path,
) -> None:
    threads = {
...
def test_shared_budget_stops_both_streams_without_reading_past_limit(
    tmp_path: Path,
) -> None:
    threads = {
...
def test_labeled_continuation_with_budget_one_eventually_processes_union_and_advances(
    ...
    for ordinal in range(1, 7):
        ...
        if (
            ...
            and conn.execute("SELECT count(*) FROM mail_poll_cursors").fetchone()[0]
            == 2
        ):
            break
...
def test_label_continuation_keeps_deferred_candidate_when_next_search_omits_it(
    ...
    service, conn, subject, _ = env(tmp_path, adapter)
    assert (
        service.execute(request(subject, "candidate-first", max_threads=1)).status
        == "partial"
    )
    ...
    assert (
        service.execute(request(subject, "candidate-resume", max_threads=2)).status
        == "succeeded"
    )
...
def test_completed_window_is_scanned_fresh_and_detects_overlap_update(
    tmp_path: Path,
) -> None:
    raw = {
        "messages": [
            message("m-overlap-refresh", "overlap-refresh", labels=["TrainLab"])
        ]
    }
    adapter = FakePollAdapter(
...
def test_same_partial_invocation_replays_without_search_or_thread_read(
    tmp_path: Path,
) -> None:
    threads = {
        "a": {"messages": [message("m-a", "a", labels=["TrainLab"])]},
...
def test_cross_stream_cached_failure_stops_tracked_stream_without_second_read(
    tmp_path: Path,
) -> None:
    threads = {
        "a-fails": {"messages": [message("m-a", "a-fails", labels=["TrainLab"])]},
...
def test_unlabeled_untracked_private_thread_is_never_read(tmp_path: Path) -> None:
    ...
    adapter = FakePollAdapter(threads={"private": private})
    service, conn, subject, _ = env(tmp_path, adapter)
...
def test_invalid_or_ambiguous_search_ids_stop_before_provider_read(
    tmp_path: Path,
) -> None:
    adapter = FakePollAdapter(
...
def test_read_result_cannot_substitute_another_thread(tmp_path: Path) -> None:
    ...
    class SubstitutionAdapter(FakePollAdapter):
        def read_thread_evidence(self, thread_id: str) -> GmailThreadEvidence:
            self.read_calls.append(thread_id)
            values = FakePollAdapter(
                threads={"wrong-thread": raw}
            ).read_thread_evidence("wrong-thread")
            return values
...
def test_html_attachment_and_changed_payload_create_revision(tmp_path: Path) -> None:
    raw = {
        "messages": [
            message(
                ...
                "t3",
                body="",
                html="<p>Hello <b>world</b><script>secret()</script></p>",
                labels=["TrainLab"],
            )
        ]
    }
    ...
    raw["messages"][0]["plain_text"] = "changed"
    raw["messages"][0]["label_ids"] = ["TrainLab", "later"]
    ...
    assert (
        conn.execute(
            "SELECT body_text FROM mail_messages WHERE provider_message_id='m3'"
        ).fetchone()[0]
        == "changed"
    )


def test_later_label_and_delayed_message_create_only_affected_message_revisions(
    tmp_path: Path,
) -> None:
    raw = {"messages": [message("m-current", "t-late", labels=["TrainLab"])]}
    adapter = FakePollAdapter(
        label_matches=[{"thread_id": "t-late"}], threads={"t-late": raw}
    )
    service, conn, subject, _ = env(tmp_path, adapter)
    assert service.execute(request(subject, "baseline")).status == "succeeded"
    baseline = dict(
        conn.execute("SELECT provider_message_id,source_revision_id FROM mail_messages")
    )
    service._clock = lambda: "2026-07-24T07:00:00Z"  # type: ignore[method-assign]
    raw["messages"].insert(
        0,
...
def test_message_id_cannot_be_reparented_to_another_thread(tmp_path: Path) -> None:
    ...
    second = {
        "messages": [message("stable-message", "thread-two", labels=["TrainLab"])]
    }
...
def test_invalid_attachment_metadata_rolls_back_projection(tmp_path: Path) -> None:
    raw = {
        "messages": [
            message("bad-attachment", "attachment-thread", labels=["TrainLab"])
        ]
    }
...
def test_message_revisions_are_independent_of_thread_snapshot(tmp_path: Path) -> None:
    raw = {
        "thread_metadata": {"topic": "one"},
        "messages": [
            message("m1", "tm", labels=["TrainLab"]),
            message("m2", "tm", labels=["TrainLab"]),
        ],
    }
    ...
    attachment = conn.execute(
        "SELECT id FROM mail_attachments WHERE mail_message_id=(SELECT id FROM mail_messages WHERE provider_message_id='m1')"
    ).fetchone()[0]
    ...
    assert (
        after_add["m1"] == initial["m1"]
        and after_add["m2"] == initial["m2"]
        and "m3" in after_add
    )
    assert (
        conn.execute(
    ...
    assert (
        revised["m1"] == initial["m1"]
        and revised["m2"] != initial["m2"]
        and revised["m3"] == after_add["m3"]
    )
...
def test_shared_read_budget_deduplicates_label_and_preexisting_tracked(
    tmp_path: Path,
) -> None:
    ...
    service, conn, subject, _ = env(tmp_path, adapter)
    conn.execute(
        "INSERT INTO mail_threads(subject_id,provider_thread_id,is_current) VALUES(?, 'tb',1)",
        (subject,),
    )
    conn.commit()
...
def test_raw_survives_transaction_failure_but_never_becomes_canonical(
    tmp_path: Path, monkeypatch
) -> None:
    raw = {"messages": [message("m4", "t4", labels=["TrainLab"])]}
    service, conn, subject, root = env(
        tmp_path,
        FakePollAdapter(label_matches=[{"thread_id": "t4"}], threads={"t4": raw}),
    )
    monkeypatch.setattr(
        service,
        "_project",
        lambda *_args: (_ for _ in ()).throw(
            sqlite3.IntegrityError("no provider text")
        ),
    )
...
def test_invalid_evidence_and_symlink_raw_root_are_partial_without_final_file(
    tmp_path: Path,
) -> None:
    invalid = {"messages": [message("", "t5", labels=["TrainLab"])]}
    service, conn, subject, root = env(
        tmp_path / "invalid",
        FakePollAdapter(label_matches=[{"thread_id": "t5"}], threads={"t5": invalid}),
    )
    ...
    service, conn, subject, root = env(
        tmp_path / "symlink",
        FakePollAdapter(label_matches=[{"thread_id": "t6"}], threads={"t6": raw}),
    )
    outside = tmp_path / "outside"
    outside.mkdir()
    year_link = root / "raw" / "gmail" / "json" / "2026"
    year_link.symlink_to(outside, target_is_directory=True)
    assert service.execute(request(subject, "symlink")).status == "partial"
    assert not list(outside.iterdir())


def test_existing_raw_tamper_fails_closed_without_canonical_change(
    tmp_path: Path,
) -> None:
    raw = {"messages": [message("m7", "t7", labels=["TrainLab"])]}
    service, conn, subject, root = env(
        tmp_path,
        FakePollAdapter(label_matches=[{"thread_id": "t7"}], threads={"t7": raw}),
    )
    ...
    assert (
        conn.execute(
            "SELECT count(*) FROM source_revisions WHERE resource_kind='message_json'"
        ).fetchone()[0]
        == 1
    )
