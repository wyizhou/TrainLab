...
def request(
    mode: str, invocation: str = "analysis-input-v3", target: int | None = None
) -> FoundationRequest:
...
def _seed_v2_inputs(root: Path) -> tuple[tuple[object, ...], ...]:
    ...
    try:
        ...
        for ordinal, trust in enumerate(
            (
                "provider_fact",
                "user_asserted",
                "derived_statistic",
            ...
            conn.execute(
                ...
                (
                    ...
                    hashlib.sha256(trust.encode()).hexdigest(),
                    trust,
                    ordinal,
                ),
            )
        ...
        return tuple(
            conn.execute(
                "SELECT id,analysis_run_id,input_role,source_entity_type,source_entity_id,source_revision_id,source_window_start_utc,source_window_end_utc,input_sha256,trust_class,ordinal FROM analysis_artifact_inputs ORDER BY id"
            )
        )
    finally:
        conn.close()
...
def _schema_rows(root: Path) -> tuple[tuple[object, ...], ...]:
    ...
    try:
        return tuple(
            conn.execute(
                "SELECT type,name,sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' AND name NOT IN ('analysis_artifact_inputs','foundation_state','schema_migrations') ORDER BY type,name"
            )
        )
    finally:
        conn.close()
...
def test_explicit_v2_to_v3_rebuilds_only_analysis_inputs_and_preserves_every_old_value(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v2"
    ...
    assert (
        migrated.status,
        migrated.migration_start_version,
        migrated.migration_end_version,
        migrated.applied_migration_ids,
    ) == ("initialized", 2, 4, [4])
    ...
    try:
        ...
        run = conn.execute(
            "SELECT id FROM analysis_runs WHERE run_key='migration-run'"
        ).fetchone()[0]
        for ordinal, trust in enumerate(
            ("provider_derived", "provider_predicted", "unknown"), start=10
        ):
            conn.execute(
        ...
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO analysis_artifact_inputs(analysis_run_id,input_role,source_entity_type,input_sha256,trust_class,ordinal) VALUES(?,?,?,?,?,?)",
                (run, "bad", "fixture", "0" * 64, "not_a_trust_class", 99),
            )
    finally:
        conn.close()
    ...
    assert (
        tool.execute(request("migrate", "v3-repeat", FOUNDATION_SCHEMA_VERSION)).status
        == "already_initialized"
    )
...
def test_v2_admission_rejects_lookalike_before_any_v3_write(tmp_path: Path) -> None:
    ...
    conn.execute(
        "CREATE TABLE analysis_artifact_inputs(id INTEGER PRIMARY KEY, trust_class TEXT)"
    )
    conn.commit()
    conn.close()
    ...
    assert (
        json.loads((root / "state" / "foundation-ready.json").read_text())[
            "manifest_sha256"
        ]
        == LEGACY_V2_MANIFEST_SHA256
    )
    assert (
        sqlite3.connect(root / "data.db")
        .execute("SELECT schema_version FROM foundation_state")
        .fetchone()[0]
        == 2
    )
    assert (
        db_before != (root / "data.db").read_bytes()
    )  # the deliberate forge, not migration, changed it
...
def test_v3_transaction_failure_rolls_back_table_and_metadata(tmp_path: Path) -> None:
    ...
    receipt = FoundationTool(config(root), failpoint=fail).execute(
        request("migrate", "v3-rollback", FOUNDATION_SCHEMA_VERSION)
    )
    ...
    try:
        ...
        assert conn.execute(
            "SELECT schema_version,manifest_sha256,implementation_version FROM foundation_state"
        ).fetchone() == (2, LEGACY_V2_MANIFEST_SHA256, "foundation-v2")
    finally:
        conn.close()
