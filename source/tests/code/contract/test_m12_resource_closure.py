...
from skills._shared.fit_weekly import command_runtime, runtime_resources
...
def runtime_copy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        command_runtime,
        "__file__",
        str(source / "skills/_shared/fit_weekly/command_runtime.py"),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "fit_weekly/weekly_context.py",
        "schemas/training_goal_text_v1.schema.json",
    ],
)
def test_actual_goal_validation_dependency_changes_runtime_identity(
    ...
    runtime = runtime_copy(tmp_path, monkeypatch)
    before = runtime.identity()
    path = runtime.source / "skills/_shared" / name
...
def test_missing_actual_schema_prevents_runtime_identity(
    ...
    with pytest.raises(ValueError, match="command_runtime_unavailable"):
        runtime.identity()
...
def valid_result(body, payload):
    ...
    assert payload["schema_version"] == "fit_weekly_stage_input_v3"
...
def test_publication_is_in_current_resource_closure():
    names = {p.name for p in runtime_resources.files(SOURCE, collection=True)}
    assert {
        "publication.py",
        ...
        "gmail_rest.py",
        "gmail_message.py",
        "garmin_publication.py",
        "garmin_workouts.py",
        "fit_delivery_request_v1.schema.json",
        "fit_delivery_intent_v1.schema.json",
        "fit_delivery_call_v1.schema.json",
        "fit_delivery_result_v1.schema.json",
    } <= names
    for module in (
        "publication",
        "publication_ledger",
        ...
        text = (SOURCE / "skills/_shared/fit_weekly" / (module + ".py")).read_text()
        assert "gmail_rest_delivery" not in text and "data-backup" not in text


@pytest.mark.parametrize("mail_version", [1, 2])
def test_closed_publication_prepares_delivers_and_replays_after_source_and_instance_move(
    tmp_path, monkeypatch, mail_version
):
    from skills._shared.fit_weekly import model_job, report_artifacts, report_revisions

    monkeypatch.syspath_prepend(str(SOURCE / "tests/code/fixtures"))
    fixture = importlib.import_module("m12_publication_factory")
    instance, _, adapters = fixture.reports.setup(
        tmp_path,
        monkeypatch,
        transform=fixture.reports.supported_content,
        with_cadence=True,
    )
    revision = report_revisions.create(instance, fixture.reports.END)
    revision_sha = model_job.sha(revision)
    report_artifacts.render(instance, fixture.reports.END, "ai", revision_sha)
    source = tmp_path / "closed-publication"
    for path in runtime_resources.files(SOURCE, collection=True):
        target = source / path.relative_to(SOURCE)
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(path, target)
    assert not (source / "tests").exists()
    assert not (source / "data-backup").exists()
    assert not (source / "skills/gmail-sender/scripts/gmail_rest_delivery.py").exists()
    prefix = """
...
from skills._shared.fit_weekly import publication, publication_ledger as ledger, gmail_rest, gmail_labels, garmin_publication, storage
"""
    program = (
        prefix
        + "\n".join(
            inspect.getsource(item)
            for item in (fixture.Gmail, fixture.Auth, fixture.Garmin, fixture.curated)
        )
        + """
root, end, sha, mode = Path(sys.argv[1]), sys.argv[2], sys.argv[3], sys.argv[4]
version = int(sys.argv[5])
prepared = publication.prepare_weekly(root, end, 'ai', sha,
    sender='owner@example.invalid', recipient='runner@example.invalid' if version == 1 else 'owner@example.invalid', late=True, mail_version=version)
mail = prepared['mail']
create, schedule = prepared['workouts'][0].values()
ensure, apply = gmail_labels.ensure_key('owner@example.invalid'), gmail_labels.apply_key(mail)
grant = ledger.Authorization('closed-grant', (mail, create, schedule, ensure, apply),
    '2026-08-01', '2026-08-31', '2026-08-10T00:00:00Z', '2026-08-10T01:00:00Z',
    {'gmail.profile': 3, 'gmail.send': 1, 'gmail.get': 3, 'gmail.labels.list': 2, 'gmail.labels.create': 1, 'gmail.modify': 1,
     'upload_workout': 1, 'get_workout_by_id': 1, 'schedule_workout': 1, 'get_scheduled_workouts': 1})
http, sdk = Gmail(), Garmin()
now = lambda: '2026-08-10T00:00:00Z'
if version == 2:
    assert gmail_labels.process(root, ensure, gmail_rest.Client(Auth(), http), grant, readonly=False, now=now)['status'] == 'success'
mail_result = gmail_rest.deliver(root, mail, gmail_rest.Client(Auth(), http), grant, now=now)
if version == 2:
    assert gmail_labels.process(root, apply, gmail_rest.Client(Auth(), http), grant, readonly=False, now=now)['status'] == 'success'
course_result = asyncio.run(garmin_publication.deliver(root, create, schedule, sdk, grant, now=now))
assert mail_result['status'] == course_result['create']['status'] == course_result['schedule']['status'] == 'success'
assert mail_result['evidence']['pdf_sha256'] == prepared['seal']['pdf_sha256']
assert course_result['create']['evidence']['workout_id'] == 123
assert course_result['schedule']['evidence']['scheduled_workout_id'] == 987
if mode == 'first':
    assert len(http.calls) == (3 if version == 1 else 11) and len(sdk.calls) == 4
else:
    assert http.calls == sdk.calls == []
for name, module in tuple(sys.modules.items()):
    if name.startswith('skills.') and getattr(module, '__file__', None):
        assert Path(module.__file__).is_relative_to(Path.cwd())
...
print(json.dumps({'mode': mode, 'gmail_calls': len(http.calls), 'garmin_calls': len(sdk.calls)}))
"""
    )

    def run(source, instance, mode):
        result = subprocess.run(
            [
                sys.executable,
                "-c",
    ...
    assert run(source, instance, "first") == {
        "mode": "first",
        "gmail_calls": 3 if mail_version == 1 else 11,
        "garmin_calls": 4,
    }
    moved_source, moved_instance = (
        tmp_path / "relocated-source",
        tmp_path / "relocated-instance",
    )
    source.rename(moved_source)
    instance.rename(moved_instance)
    assert not source.exists() and not instance.exists()
    assert run(moved_source, moved_instance, "replay") == {
        "mode": "replay",
        "gmail_calls": 0,
        "garmin_calls": 0,
    }
