    ...
    for index in range(7):
        ...
        conn.execute(
            ...
            (
                ...
                day,
                "rest",
                json.dumps({"source": index}, separators=(",", ":")),
                f"旧理由 {index}",
                json.dumps([f"old-{index}"], separators=(",", ":")),
            ),
        )
    conn.commit()
    return conn
...
def accepted() -> dict[str, object]:
    plan = {
        ...
        "objective": {"focus": "adjust"},
        "constraints": {},
        "original_plan_id": 9,
        "original_artifact_id": 8,
        "reason_event_id": 31,
        "effective_local_date": "2026-07-23",
        "items": items(),
    }
    return {
        "run_key": "analysis:1:revise_plan:9:31:one",
        "subject_id": 1,
        "mode": "revise_plan",
        "status": "accepted",
        "artifacts": [
...
def manifest() -> list[dict[str, object]]:
    ...
    return [
        {
            "ordinal": 0,
            "input_role": "current_plan",
            "source_entity_type": "training_plan",
            "source_entity_id": "9",
            "source_revision_id": "plan:9",
        ...
        {
            "ordinal": 1,
            "input_role": "prior_artifact",
            "source_entity_type": "analysis_artifact",
            "source_entity_id": "8",
            "source_revision_id": "artifact:8",
            "source_window": source_window,
            "input_sha256": "b" * 64,
            "trust_class": "prior_model_output",
        },
        {
            "ordinal": 2,
            "input_role": "revision_reason",
            "source_entity_type": "conversation_event",
            "source_entity_id": "31",
            "source_revision_id": "event:31",
            "source_window": source_window,
            "input_sha256": "c" * 64,
...
def publish(conn: sqlite3.Connection):
    rows = manifest()
    return AnalysisPublisher(conn, clock=lambda: "2026-07-23T00:00:00Z").publish(
...
def test_revision_creates_full_snapshot_preserves_history_and_records_lineage() -> None:
    ...
    assert (
        conn.execute("SELECT status FROM training_plans WHERE id=9").fetchone()[0]
        == "superseded"
    )
    assert (
        conn.execute("SELECT is_current FROM analysis_artifacts WHERE id=8").fetchone()[
            0
        ]
        == 0
    )
    ...
    assert conn.execute(
        "SELECT supersedes_artifact_id,is_current FROM analysis_artifacts WHERE id=?",
        (artifact,),
    ).fetchone()[:] == (8, 1)
    assert {
        row[0]
        for row in conn.execute(
            "SELECT relation_type FROM analysis_artifact_relations WHERE from_artifact_id=?",
            (artifact,),
        )
    } == {"supersedes", "derived_from", "references_prior_plan"}
    old = conn.execute(
        "SELECT item_index,prescription_json,rationale_text,stop_conditions_json FROM training_plan_items WHERE training_plan_id=9 ORDER BY item_index"
    ).fetchall()
    new = conn.execute(
        "SELECT item_index,prescription_json,rationale_text,stop_conditions_json FROM training_plan_items WHERE training_plan_id=? ORDER BY item_index",
        (receipt.training_plan_id,),
    ).fetchall()
    assert len(new) == 7 and [tuple(row) for row in new[:3]] == [
        tuple(row) for row in old[:3]
    ]
...
@pytest.mark.parametrize(
    ...
    [
        "after_run_evidence",
        "after_inputs",
        "after_artifact:weekly_training_plan",
        "after_plan_revision_lineage",
        "after_plan_supersession",
        "after_plan",
        "after_plan_item:0",
        "after_plan_item:6",
        "after_plan_items",
...
def test_revision_rejects_invalid_suffix_or_source_without_side_effects(mutate) -> None:
    ...
    with pytest.raises(AnalysisPublishError):
        AnalysisPublisher(conn).publish(
            run_id=1, accepted=value, input_manifest=rows, run_evidence=evidence(rows)
        )
    assert (
        conn.execute("SELECT status FROM training_plans WHERE id=9").fetchone()[0]
        == "active"
...
def test_revision_requires_exact_manifest_and_unused_reason() -> None:
    ...
    conn = database()
    conn.execute(
        "INSERT INTO analysis_runs(id,run_key,subject_id,analysis_kind,status) VALUES(3,'prior',1,'plan_revision','succeeded')"
    )
    conn.execute(
        "INSERT INTO analysis_artifact_inputs(analysis_run_id,input_role,source_entity_type,source_entity_id,source_revision_id,source_window_start_utc,source_window_end_utc,input_sha256,trust_class,ordinal) VALUES(3,'revision_reason','conversation_event',31,NULL,'2026-07-13T16:00:00Z','2026-07-26T16:00:00Z',?,'user_asserted',0)",
        ("d" * 64,),
    )
    conn.commit()
    rows = manifest()
