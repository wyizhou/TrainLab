...
from src.mail_agent.delivery import (
    AcceptedDeliveryTarget,
    MailDeliveryError,
    MailResponseDeliveryService,
)
from src.mail_agent.gmail_adapter import GmailAdapterError, SendReceipt
...
class Store:
    ...
    def mark_delivery_sending(self, value):
        self.calls.append(("sending", value.delivery_id))

    def mark_delivery_sent(self, value, **kwargs):
        self.calls.append(("sent", value.delivery_id, kwargs))

    def mark_delivery_unknown(self, value, **kwargs):
        self.calls.append(("unknown", value.delivery_id, kwargs))

    def mark_label_pending(self, value, **kwargs):
        self.calls.append(("label_pending", value.delivery_id, kwargs))

    def mark_delivery_conflict(self, value, **kwargs):
        self.calls.append(("conflict", value.delivery_id, kwargs))
...
class Adapter:
    def __init__(
        ...
        *,
        ...
        self.receipt = receipt or SendReceipt("message-1", "thread-7", False)
...
def test_renderer_is_deterministic_plaintext_only_and_inert_html() -> None:
    first = render_mail_response(
        response_artifact_id=23,
        response_kind="mail_reply",
        user_visible_text="a <script>x</script>\n&",
        delivery_run_id="mail:response:23:thread-7",
    )
    second = render_mail_response(
        response_artifact_id=23,
        response_kind="mail_reply",
        user_visible_text="a <script>x</script>\n&",
        delivery_run_id="mail:response:23:thread-7",
    ...
    assert first.requested_message_id.startswith("<src.")
...
def test_delivery_sends_only_exact_verified_target_and_labels_after_send() -> None:
    ...
    assert (
        sent["thread_id"] == "thread-7"
        and sent["run_id"] == "mail:response:23:thread-7"
    )
...
def test_delivery_preflight_existing_message_labels_without_resend() -> None:
    store, adapter = (
        Store(),
        Adapter(matches=({"message_id": "message-9", "thread_id": "thread-7"},)),
    )
    result = service(store, adapter).deliver(subject_id=7, response_artifact_id=23)
...
def test_label_failure_never_resends_message() -> None:
    store, adapter = (
        Store(),
        Adapter(label_error=GmailAdapterError("gmail_transport_failed")),
    )
    result = service(store, adapter).deliver(subject_id=7, response_artifact_id=23)
    assert (
        result.status == "label_pending" and result.next_action == "reconcile_delivery"
    )
    assert [call[0] for call in adapter.calls] == ["search", "send", "label"]
...
def test_send_error_becomes_unknown_without_resend() -> None:
    store, adapter = (
        Store(),
        Adapter(send_error=GmailAdapterError("gmail_transport_failed")),
    )
    result = service(store, adapter).deliver(subject_id=7, response_artifact_id=23)
    assert (
        result.status == "delivery_unknown"
        and result.next_action == "reconcile_delivery"
    )
...
def test_search_or_label_pending_never_authorizes_a_second_send() -> None:
    store, adapter = (
        Store(),
        Adapter(search_error=GmailAdapterError("gmail_transport_failed")),
    )
    result = service(store, adapter).deliver(subject_id=7, response_artifact_id=23)
    ...
    result = service(pending, second_adapter).deliver(
        subject_id=7, response_artifact_id=23
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            (
                {"message_id": "a", "thread_id": "thread-7"},
                {"message_id": "b", "thread_id": "thread-7"},
            ),
...
def test_existing_unknown_is_reconcile_only_and_invalid_target_is_rejected() -> None:
    unknown = Store(target(delivery_status="delivery_unknown"))
    adapter = Adapter()
    assert (
        service(unknown, adapter)
        .deliver(subject_id=7, response_artifact_id=23)
        .next_action
        == "reconcile_delivery"
    )
    assert adapter.calls == []
    with pytest.raises(MailDeliveryError, match="mail_delivery_target_invalid"):
        service(Store(target(thread_verified=False)), Adapter()).deliver(
            subject_id=7, response_artifact_id=23
        )
