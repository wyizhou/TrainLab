...
def test_packaged_template_resources_load() -> None:
    for email_type in ("daily_report", "weekly_report", "mail_reply"):
        assert any(
            node.attribute_values("data-field")
            for node in _walk_elements(load_template(email_type).root)
        )
...
def test_repeat_text_is_escaped_and_empty_optional_block_is_deleted() -> None:
    html = _render_daily(
        optional={"decision_factors": []}, repeats={"decision_factors": ["<unsafe>"]}
    )
...
def _render_daily(*, title="safe title", optional=None, repeats=None) -> str:
    return _render_complete(
        "daily_report", title=title, optional=optional, repeats=repeats
    )


def _render_complete(
    email_type, *, title="safe title", optional=None, repeats=None
) -> str:
    ...
    variant = (
        "running"
        if any(
            node.attribute_values("data-variant")
            for node in _walk_elements(template.root)
        )
        else None
    )
