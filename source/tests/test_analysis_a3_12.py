...
def safety(
    item: dict[str, object] | None = None, **changes: object
) -> dict[str, object]:
...
def test_schema_is_strict_and_a_valid_daily_result_returns_only_typed_value() -> None:
    schema = json.loads(resource_bytes("harness/schemas/analysis_result.schema.json"))
    Draft202012Validator.check_schema(schema)
    accepted = validate(output())
    assert accepted.result["run_key"] == RUN_KEY
    assert set(accepted.result) == {
        "schema_version",
        "run_key",
        "mode",
        "subject_id",
...
def test_non_json_and_unknown_fields_are_rejected_without_echoing_output() -> None:
    ...
    assert caught.value.as_record() == {
        "code": "analysis_result_json_invalid",
        "path": "root",
    }
    changed = output()
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda value: value.update(run_key="other"),
            "analysis_result_run_key_mismatch",
        ),
        (lambda value: value.update(subject_id=2), "analysis_result_subject_mismatch"),
        (
            lambda value: value["artifacts"].pop(),
            "analysis_result_daily_cardinality_invalid",
        ),
        (
            lambda value: value["artifacts"][0].update(period=ADVICE),
            "analysis_result_date_mismatch",
        ),
    ],
)
def test_identity_cardinality_dates_and_manifest_bindings_fail_closed(
...
def test_simplified_training_plan_wording_passes_but_traditional_wording_fails() -> (
    None
):
    simplified = output()
    ...
    traditional["artifacts"][1]["user_visible_text"] = traditional["artifacts"][1][
        "user_visible_text"
    ].replace("主训练", "主訓練")
...
def test_primary_opening_and_supported_activity_plan_are_enforced() -> None:
    wrong_opening = output()
    wrong_opening["artifacts"][1]["user_visible_text"] = wrong_opening["artifacts"][1][
        "user_visible_text"
    ].replace("今日跑步。", "今日力量训练。")
    ...
    assert validate(climbing, expected()).result is not None
...
def test_quality_disclosures_must_equal_actual_gate_warnings() -> None:
    ...
    object.__setattr__(
        ...
        "quality_gate",
        {
            ...
            "blockers": [],
            "warnings": [
                {"code": "quality_warning", "entity": "sleep", "summary": "safe"}
            ],
        },
    )
    with pytest.raises(
        AnalysisResultValidationError, match="quality_disclosure_mismatch"
    ):
        validate(value, exp)
...
def test_conservative_a3_09_normalization_is_owned_by_host_not_model() -> None:
    ...
    assert accepted["artifacts"][1]["structured_content"]["primary_item"] == normalized
