...
from src.mail_agent import MailReceipt, MailRequest, MailTool, exit_code_for_status
from src.mail_agent.cli import _parser, main, request_from_args
...
def test_request_modes_are_valid_and_have_stable_run_keys(
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "mail_message_ids_not_allowed_for_run",
        ),
        ...
        (
            "reconcile",
            {"mail_delivery_ids": ("a", "b")},
            "reconcile_accepts_at_most_one_mail_delivery_id",
        ),
        (
            "status",
            {"mail_message_ids": ("a", "b")},
            "status_accepts_at_most_one_mail_message_id",
        ),
    ],
)
def test_request_mode_fields_are_mutually_exclusive(
    mode: str, kwargs: dict[str, object], error: str
) -> None:
...
class SpyService:
    ...
    def execute(self, item: MailRequest) -> MailReceipt:
        ...
        return MailReceipt(
            ...
            if item.mode == "status" and item.run_key
            else item.stable_run_key,
...
def test_api_and_cli_share_the_same_mail_tool_service_boundary(
    ...
    code = main(
        ["--subject-id", "7", "--invocation-id", "inv-cli", "status"], tool=tool
    )
...
def test_mail_cli_module_is_not_a_second_direct_entrypoint() -> None:
    process = subprocess.run(
        [sys.executable, "-m", "src.mail_agent.cli", "--help"],
        capture_output=True,
        text=True,
        timeout=10,
        check=False,
    ...
    status = request_from_args(
        parser.parse_args(
            [
                ...
                "status",
                "--run-key",
                "mail:7:poll:historical-invocation",
            ]
        ),
        requested_at_utc=FIXED_TIME,
    )
...
def test_request_and_receipt_schemas_are_strict_and_match_serialization() -> None:
    request_schema = json.loads(
        resource_bytes("harness/schemas/mail_request.schema.json")
    )
    receipt_schema = json.loads(
        resource_bytes("harness/schemas/mail_receipt.schema.json")
    )
    item = request("status")
