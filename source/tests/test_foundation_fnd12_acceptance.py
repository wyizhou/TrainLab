...
from src.foundation_sample import (
...
def test_fnd12_sample_is_deterministic_complete_and_privacy_clean(
    tmp_path: Path,
) -> None:
    ...
    assert (first / "sample-acceptance.json").read_bytes() == (
        second / "sample-acceptance.json"
    ).read_bytes()
    assert (first / "sample-metadata.json").read_bytes() == (
        second / "sample-metadata.json"
    ).read_bytes()
    assert len(first_manifest["tables"]) == len(TABLES) + 1
    assert set(first_manifest["views"]) == set(VIEWS) and len(VIEWS) == 21
    ...
    assert all(
        item["segments"] >= 1 and item["samples"] >= 1
        for item in first_manifest["fit_shapes"].values()
    )
    ...
    try:
        ...
        assert (
            conn.execute("SELECT COUNT(*) FROM fit_metric_definitions").fetchone()[0]
            == 1
        )
        assert (
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM daily_health WHERE is_current=0"
            ).fetchone()[0]
            == 1
        )
        assert (
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM analysis_artifacts WHERE artifact_kind='daily_training_advice'"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM v_plan_revision_reason_events"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM analysis_artifact_inputs WHERE trust_class='prior_model_output'"
            ).fetchone()[0]
            == 2
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM mail_response_artifacts WHERE is_current=0"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM mail_response_artifacts WHERE is_current=1"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute("SELECT COUNT(*) FROM analysis_deliveries").fetchone()[0] >= 1
        )
        assert conn.execute("SELECT COUNT(*) FROM mail_deliveries").fetchone()[0] >= 1
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name LIKE '%orchestrator%'"
            ).fetchone()[0]
            == 0
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name LIKE '%operational_incident%'"
            ).fetchone()[0]
            == 0
        )
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM conversation_events WHERE content_text IS NOT NULL"
            ).fetchone()[0]
            == 0
        )
        for code, forbidden in (
            ...
            conn.commit()
            assert any(
                item.startswith(f"{code}:") for item in scan_sample_privacy(first)
            )
        conn.execute(
...
def test_fnd12_encrypted_backup_restore_reconciles_rows_content_and_references(
    tmp_path: Path,
) -> None:
    ...
    assert not [
        ...
        if path.name.startswith((".backup-", ".restore-"))
    ]


def test_fnd12_shadow_rebuild_reconciles_all_rows_references_views_and_raw(
    tmp_path: Path,
) -> None:
    root = generate_sample_database(tmp_path / "sample")
    tool = FoundationTool(config(root))
...
def test_fnd12_shadow_failure_after_publication_leaves_verified_candidate(
    tmp_path: Path,
) -> None:
    root = generate_sample_database(tmp_path / "sample")
    expected = build_acceptance_manifest(root)
    ...
    assert (
        FoundationTool(config(destination)).execute(request("verify")).status == "ready"
    )


def test_fnd12_legacy_migration_rollback_coexists_with_current_sample(
    tmp_path: Path,
) -> None:
    sample = generate_sample_database(tmp_path / "sample")
    sample_before = tree_evidence(sample)
    ...
    failed = FoundationTool(legacy.config, failpoint=failpoint).execute(
        request("migrate", FOUNDATION_SCHEMA_VERSION)
    )
    assert failed.status == "failed"
    assert tree_evidence(sample) == sample_before
    migrated = FoundationTool(legacy.config).execute(
        request("migrate", FOUNDATION_SCHEMA_VERSION)
    )
...
def test_fnd12_acceptance_has_no_network_provider_model_thread_or_subprocess(
    ...
    assert result["views_verified"] == 21
    assert result["fit_shapes_verified"] == 6
    assert result["privacy_findings"] == 0


def test_fnd12_acceptance_command_is_repeat_safe_and_outputs_only_receipt(
    tmp_path: Path,
) -> None:
    ...
    command = [
        sys.executable,
        "scripts/verify_foundation_acceptance.py",
        "--output-root",
        str(output),
    ]
    ...
    assert payload["status"] == "passed" and payload["views_verified"] == 21
