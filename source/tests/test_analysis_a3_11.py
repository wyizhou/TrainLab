...
import src.analysis.runner as runner_module
from src.analysis.config import AnalysisConfig
from src.analysis.harness import HarnessBundle, HarnessFileEvidence, SchemaEvidence
...
def _bundle(root: Path) -> HarnessBundle:
    ...
    return HarnessBundle(
        ...
        files=(
            HarnessFileEvidence("harness/analysis.md", sha256(payload).hexdigest()),
        ),
...
def accepted_context(monkeypatch):
    ...
    monkeypatch.setattr(
        runner_module, "_canonical_context", lambda value, maximum: value
    )
...
def test_dynamic_extension_objects_fall_back_to_prompt_schema_validation():
    ...
    closed = (
        b'{"type":"object","additionalProperties":false,"properties":{},"required":[]}'
    )
    assert _supports_strict_structured_output(dynamic) is False
    assert _supports_strict_structured_output(closed) is True
    assert "--output-schema" not in _command(("codex",), structured_output=False)
    assert "--output-schema" in _command(("codex",), structured_output=True)
...
def test_success_is_one_generation_in_private_workspace_and_cleans_it(
    tmp_path, monkeypatch, accepted_context
):
    ...
    _fake_command(
        monkeypatch, "import sys; sys.stdout.buffer.write(b'{\\\"ok\\\":true}')"
    )
    result = AnalysisCodexRunner(config, process_factory=factory).execute(
        bundle, accepted_context
    )
...
def test_stdin_contains_complete_verified_harness_and_context(
    tmp_path, monkeypatch, accepted_context
):
    config = _config(tmp_path)
    bundle = _bundle(tmp_path)
...
def test_codex_home_uses_effective_account_when_environment_is_absent(
    tmp_path, monkeypatch, accepted_context
):
    config = _config(tmp_path)
    seen = []
...
def test_codex_home_rejects_group_writable_configured_directory(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODEX_HOME", str(codex_home))
    with pytest.raises(
        AnalysisRunnerError, match="^analysis_runner_auth_home_unavailable$"
    ):
        _trusted_codex_home()
...
def test_nonzero_is_stable_and_never_exposes_stderr(
    tmp_path, monkeypatch, accepted_context
):
    _fake_command(
        monkeypatch,
        "import sys; sys.stderr.write('sensitive error'); raise SystemExit(7)",
    )
    with pytest.raises(AnalysisRunnerError, match="^analysis_runner_process_nonzero$"):
        AnalysisCodexRunner(
            _config(tmp_path), process_factory=runner_module.subprocess.Popen
        ).execute(_bundle(tmp_path), accepted_context)


def test_output_limit_terminates_group_and_cleans_workspace(
    tmp_path, monkeypatch, accepted_context
):
    config = _config(tmp_path, maximum=8)
    _fake_command(monkeypatch, "import sys; sys.stdout.buffer.write(b'x' * 64)")
    with pytest.raises(
        AnalysisRunnerError, match="^analysis_runner_output_limit_exceeded$"
    ):
        AnalysisCodexRunner(config).execute(_bundle(tmp_path), accepted_context)
    assert list(config.temp_root.iterdir()) == []


def test_timeout_kills_descendant_process_group_and_cleans_workspace(
    tmp_path, monkeypatch, accepted_context
):
...
def test_output_schema_hash_mismatch_fails_before_process_start(
    tmp_path, accepted_context
):
    config = _config(tmp_path)
    bundle = _bundle(tmp_path)
    config.output_schema.write_bytes(b'{"type":"array"}')
    with pytest.raises(
        AnalysisRunnerError, match="^analysis_runner_output_schema_invalid$"
    ):
