...
from src.mail_agent.fact_gate import (
    ActiveFactEvidence,
...
def initialize(
    ...
    *,
    ...
    assert FoundationTool(config).execute(FoundationRequest("init", "m408", NOW)).ready
...
def test_policy_is_versioned_closed_and_schema_valid() -> None:
    policy = json.loads(resource_bytes("harness/mail/fact-gate-policy.json"))
    schema = json.loads(
        resource_bytes("harness/schemas/mail_fact_gate_policy.schema.json")
    )
...
def test_exact_replay_and_explicit_supersession_not_last_write_wins(
    ...
    replay = gate.evaluate(output, payload, evidence(payload, active_facts=(active,)))
    assert not replay.facts[0].persist
    assert replay.facts[0].replay_fact_id == 41

    conflict_db, _, conflict_subject, _, conflict_message, conflict_run = initialize(
        tmp_path / "conflict",
...
def test_plan_change_persists_only_reason_and_dependency_wait_idempotently(
    ...
    assert (
        event_payload["source_revision_id"]
        == payload["trigger_message"]["source_revision_id"]
    )
    assert (
        connection.execute(
...
def test_dependency_ready_and_wait_failed_stale_cross_period_matrix(
    ...
    connection.execute("UPDATE analysis_artifacts SET is_current=0")
    connection.execute("UPDATE training_plans SET status='superseded'")
    ...
    ready = _ready_dependency(payload, subject, message, waiting.reason_event_id)
    gate = MailFactGate()
    accepted = gate.evaluate(output, payload, evidence(payload, dependency=ready))
