...
def test_bounded_runtime_path_keeps_only_existing_absolute_directories(
    tmp_path: Path,
) -> None:
    ...
    value = os.pathsep.join(
        (str(first), "relative", "", str(first), str(tmp_path / "missing"), str(second))
    )
    assert bounded_runtime_path(value) == os.pathsep.join((str(first), str(second)))


def test_bounded_runtime_path_rejects_world_writable_but_keeps_group_writable_directories(
    tmp_path: Path,
) -> None:
    ...
    assert bounded_runtime_path(
        os.pathsep.join((str(trusted), str(group_writable), str(writable)))
    ) == os.pathsep.join((str(trusted), str(group_writable)))


def test_bounded_runtime_path_rejects_unowned_directories(
    tmp_path: Path, monkeypatch
) -> None:
    owned = tmp_path / "owned"
    owned.mkdir(mode=0o755)
    import src.runtime_environment as module
