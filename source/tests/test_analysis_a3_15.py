...
from src.analysis.gmail_delivery import (
    GmailDeliveryError,
...
def test_rejects_noncanonical_or_multiple_recipient_forms():
    ...
    for value in (
        ...
        with pytest.raises(
            GmailDeliveryError, match="^gmail_delivery_recipient_invalid$"
        ):
...
def test_invalid_current_environment_binding_fails_before_client_start():
    ...
    status = GmailEnvironmentStatus(
        False, None, "gmail_mcp_binding_invalid", "secret path"
    )
    with pytest.raises(
        GmailDeliveryError, match="^gmail_mcp_binding_invalid$"
    ) as raised:
...
def test_missing_required_capability_fails_closed_and_closes():
    client = FakeClient(tools=REQUIRED - {"modify_email"})
    with pytest.raises(
        GmailDeliveryError, match="^gmail_delivery_capability_mismatch$"
    ):
...
def test_exact_complete_subject_query_and_one_match_is_labeled_then_returned():
    ...
    assert (
        receipt.status == "already_sent" and receipt.provider_message_id == "message-1"
    )
...
def test_no_match_sends_multipart_then_creates_and_applies_trainlab_label():
    ...
    assert (
        ...
        receipt.label_id,
    ) == ("sent-1", "thread-1", "label-1")
    assert client.calls[0] == (
        "search_emails",
        {"query": 'in:sent "analysis-delivery:v1:abc"', "maxResults": 2},
    )
...
def test_multiple_matches_are_ambiguous_and_never_send():
    ...
    with pytest.raises(
        GmailDeliveryError, match="^gmail_delivery_idempotency_ambiguous$"
    ):
        deliver(gateway(client))
...
@pytest.mark.parametrize(
    "sender", [None, "other@example.com", "Name <other@example.com>"]
)
def test_search_result_must_prove_sender_is_configured_self(sender):
    ...
    with pytest.raises(
        GmailDeliveryError, match="^gmail_delivery_search_identity_unverified$"
    ) as raised:
        deliver(gateway(client))
...
def test_actual_search_text_requires_final_newline_and_rfc_date():
    client = FakeClient(
        responses=[
            f"ID: message-1\nSubject: {SUBJECT}\nFrom: TrainLab <self@example.com>\nDate: {DATE}\n",
            "Successfully found existing label:\nID: label-1\nName: TrainLab\nType: user",
            "Email message-1 labels updated successfully",
        ]
    )
    receipt = deliver(gateway(client))
    assert (
        receipt.status == "already_sent"
        and receipt.provider_sent_at_utc == "2026-07-24T00:00:00Z"
    )


def test_reconcile_never_sends_and_returns_typed_not_found():
    client = FakeClient(responses=[""])
    receipt = gateway(client).reconcile(
        subject=SUBJECT, idempotency_key="analysis-delivery:v1:abc"
    )
    assert receipt.status == "not_found" and [name for name, _ in client.calls] == [
        "search_emails"
    ]
...
def test_send_uncertainty_and_existing_label_failure_carry_safe_evidence():
    ...
    with pytest.raises(GmailDeliveryError) as raised:
        deliver(gateway(client))
    assert (
        raised.value.code == "gmail_delivery_timeout"
        and raised.value.may_have_sent is True
        and raised.value.provider_message_id is None
    )
    ...
    assert (
        labeled.value.may_have_sent is True
        and labeled.value.provider_message_id == "message-1"
    )
