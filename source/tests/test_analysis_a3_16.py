...
def pending(connection: sqlite3.Connection) -> int:
    result = AnalysisDeliveryFactory(connection, clock=lambda: NOW).create_pending(
        publish_receipt={
            "run_id": 1,
            "artifact_ids": {"daily_summary": 11, "daily_training_advice": 12},
        },
...
def test_load_is_exact_revision_and_current_drift_is_never_followed():
    connection = database()
    delivery_id = pending(connection)
    connection.execute("UPDATE analysis_artifacts SET is_current=0 WHERE id=11")
    connection.execute(
        "INSERT INTO analysis_artifacts VALUES(13,1,'daily_summary','2026-07-24','2026-07-24',2,1,'a3','new current',?,1,?)",
        (
            json.dumps(
                {
                    "overall_state": "稳定",
                    "decision_factors": ["恢复稳定"],
...
def test_crash_after_provider_acceptance_or_label_failure_is_unknown_and_never_resent():
    ...
    assert (
        ...
        and unknown.provider_message_id == "gmail-message-1"
    )
...
def test_unknown_without_provider_evidence_cannot_be_retried_and_failed_can_be_claimed():
    connection = database()
    first = pending(connection)
    repo = repository(connection)
    repo.claim_for_send(1, first)
    repo.record_delivery_unknown(
        1, first, error_code="analysis_delivery_send_ambiguous"
    )
    with pytest.raises(AnalysisDeliveryError, match="transition_illegal"):
        repo.claim_for_send(1, first)
    ...
    assert (
        other_repo.record_failed(
            1, second, error_code="analysis_delivery_provider_failed"
        ).status
        == "failed"
    )
...
def test_exact_provider_evidence_is_required_for_sent_or_search_match():
    ...
    with pytest.raises(AnalysisDeliveryError, match="evidence_required"):
        repo.transition_delivery_state(
            1, delivery_id, "sent", provider_message_id="message-1", sent_at_utc=NOW
        )
    assert repo.read_state(delivery_id).status == "sending"
...
def test_bad_relation_or_transition_rolls_back_without_changing_state():
    ...
    assert repo.read_state(delivery_id).status == "sending"
    connection.execute(
        "UPDATE analysis_delivery_artifacts SET content_role='wrong' WHERE analysis_delivery_id=?",
        (delivery_id,),
    )
    connection.commit()
    with pytest.raises(AnalysisDeliveryError, match="relation_conflict"):
        repo.record_failed(
            1, delivery_id, error_code="analysis_delivery_provider_failed"
        )
    assert (
        connection.execute(
            "SELECT status FROM analysis_deliveries WHERE id=?", (delivery_id,)
        ).fetchone()[0]
        == "sending"
    )
