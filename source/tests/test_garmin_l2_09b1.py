...
class B1Transport:
    def __init__(self) -> None:
        ...
        self.account_payloads: dict[tuple[str, str | None], object] = {
            ...
            ("device_settings", "PRIVATE-DEVICE-A"): {
                "softwareVersion": "software-a",
                "firmwareVersion": "firmware-a",
            },
            ("device_settings", "PRIVATE-DEVICE-B"): {
                "softwareVersion": "software-b",
    ...
    def identity(self) -> str:
        ...
        return "b1-fixture-account"

    def fetch_health(self, resource: str, _day: str):
        self.calls.append(f"health:{resource}")
        if resource in self.health_errors:
            raise self.health_errors[resource]
        if resource == "devices":
    ...
    def fetch_account(self, resource: str, provider_device_id: str | None = None):
        ...
        return copy.deepcopy(
            self.account_payloads.get((resource, provider_device_id), {})
        )

    def list_activities(self, *_args):
        raise AssertionError("account-only scope must not enumerate activities")

    def activity_summary(self, _id):
        raise AssertionError("no activities")

    def activity_original(self, _id):
        raise AssertionError("no activities")

    def activity_extra(self, _id, _role):
        raise AssertionError("no activities")
...
def _setup(tmp_path: Path):
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "l2-09b1", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
    config = GarminConfig(
...
def test_device_settings_uses_only_this_run_devices_and_account_scope_is_closed(
    tmp_path: Path,
) -> None:
    ...
    assert transport.calls == [
        ...
        "identity",
        "health:devices",
        "account:device_settings:PRIVATE-DEVICE-A",
        "account:device_settings:PRIVATE-DEVICE-B",
    ]
    with sqlite3.connect(config.database_path) as conn:
        ...
        paths = dict(
            conn.execute(
                "SELECT field_path,mapping_state FROM source_field_catalog WHERE resource_kind='device_settings'"
            )
        )
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_records WHERE record_type='device_settings'"
            ).fetchone()[0]
            == 2
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_metrics WHERE metric_key='garmin.device.settings.softwareVersion'"
            ).fetchone()[0]
            == 2
        )


def test_basic_fetch_stages_are_terminal_for_success_failure_noop_and_recovery(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    first = _repair(
        tool, "b1-basic", "user_profile", "user_profile_settings", "devices"
    )
    assert first.status == "succeeded"
    repeat = _repair(
        tool, "b1-basic-repeat", "user_profile", "user_profile_settings", "devices"
    )
    assert repeat.counts["unchanged"] == 3
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_items WHERE status='running'"
            ).fetchone()[0]
            == 0
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT status FROM garmin_sync_items WHERE resource_kind='devices' AND stage='fetch' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "fetched"
        )
        assert (
            conn.execute(
                "SELECT status FROM garmin_sync_items WHERE resource_kind='devices' AND stage='project' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "failed"
        )
        run = conn.execute(
            "SELECT id FROM garmin_sync_runs WHERE run_id=?", (failed.run_id,)
        ).fetchone()[0]
        conn.execute(
            "UPDATE garmin_sync_runs SET status='started',completed_at_utc=NULL WHERE id=?",
            (run,),
        )
        conn.execute(
            "INSERT INTO garmin_sync_items(garmin_sync_run_id,resource_kind,logical_object_key,stage,status,attempt_count,started_at_utc) VALUES(?,?,?,?,?,?,?)",
            (
                run,
                "devices",
                "garmin:recover",
        ...
        tool.repo.start_run(
            ...
            SyncRequest(
                "repair",
                through_local_date="2026-04-15",
                resource_kinds=("devices",),
                repair_strategy="refetch",
                invocation_id="b1-basic-project-fails",
            ),
            subject,
            recovery,
        )
...
def test_primary_last_used_have_fresh_devices_dependency_and_canonical_roles(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.calls.clear()
    ...
    assert transport.calls == [
        ...
        "account:device_last_used:-",
    ]
    with sqlite3.connect(config.database_path) as conn:
        assert set(
            row[0]
            for row in conn.execute(
                "SELECT metric_key FROM physiology_metrics WHERE metric_key LIKE 'garmin.device.role.%'"
            )
        ) == {"garmin.device.role.primary", "garmin.device.role.last_used"}
        assert all(
            b"PRIVATE" not in row[0]
            for row in conn.execute(
                "SELECT extras_json FROM physiology_records WHERE record_type IN ('primary_device','device_last_used')"
            )
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind='primary_device'"
            ).fetchone()[0]
            >= 1
        )
    ...
    transport.account_payloads[("primary_device", None)] = {
        "primaryTrainingDevice": {"deviceId": "PRIVATE-DEVICE-B"}
    }
...
def test_last_used_volatile_metadata_does_not_change_device_reference_revision(
    ...
    transport.account_payloads[("device_last_used", None)]["lastSyncTimestamp"] = (
    ...
    transport.account_payloads[("device_last_used", None)]["deviceId"] = (
        "PRIVATE-DEVICE-A"
    )
...
def test_unchanged_b1_success_resolves_only_its_account_key(
    ...
    try:
        ...
        tool.repo.gap(
            conn, subject, resource, key, day, "project", "account_project_failed"
        )
        tool.repo.gap(
            conn, subject, resource, sibling, day, "project", "account_project_failed"
        )
    finally:
        conn.close()
...
def test_b1_physiology_is_redacted_revisioned_and_noop(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        metrics = set(
            conn.execute("SELECT metric_key,canonical_unit FROM physiology_metrics")
        )
        ...
        before = dict(
            conn.execute(
                "SELECT resource_kind,count(*) FROM source_revisions WHERE resource_kind IN ('personal_records','cycling_ftp','pregnancy') GROUP BY resource_kind"
            )
        )
        everything = (
            b"".join(
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='cycling_ftp' AND is_current=1"
            ).fetchone()[0]
            == 1
        )
        assert dict(
            conn.execute(
                "SELECT resource_kind,count(*) FROM source_revisions WHERE resource_kind IN ('personal_records','pregnancy') GROUP BY resource_kind"
            )
        ) == {key: before[key] for key in ("personal_records", "pregnancy")}


def test_b1_capabilities_unknown_device_and_snapshot_are_isolated(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.account_errors[("pregnancy", None)] = GarminError(
        "not-here", http_status=404
    )
    transport.account_errors[("personal_records", None)] = GarminError(
    ...
    with sqlite3.connect(config.database_path) as conn:
        states = dict(
            conn.execute(
                "SELECT resource_kind,capability_state FROM garmin_resource_capabilities WHERE resource_kind IN ('pregnancy','personal_records')"
            )
        )
        assert states == {"pregnancy": "not_available", "personal_records": "forbidden"}
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind='primary_device' AND reason_code='unknown_device_reference'"
            ).fetchone()[0]
            == 1
        )
    ...
    transport.account_payloads[("pregnancy", None)] = {
        "availability_state": "not_enabled",
        "PRIVATE-PERSON": "must-not-persist",
    }
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        revision = conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind='pregnancy' AND is_current=1"
        ).fetchone()[0]
        assert (
            conn.execute(
                "SELECT source_revision_id FROM resource_coverage WHERE resource_kind='pregnancy' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == revision
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_records WHERE record_type='pregnancy'"
            ).fetchone()[0]
            == 0
        )
    ...
    snap = tool.execute(
        SyncRequest(
            "snapshot", snapshot_local_date="2026-04-15", invocation_id="b1-snapshot"
        )
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind='cycling_ftp' ORDER BY id DESC LIMIT 1"
            ).fetchone()[0]
            == "partial"
        )
        assert (
...
def test_one_device_failure_does_not_rollback_other_device(tmp_path: Path) -> None:
    ...
    transport.account_errors[("device_settings", "PRIVATE-DEVICE-B")] = GarminError(
        "forbidden", http_status=403
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='device_settings' AND is_current=1"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_items WHERE resource_kind='device_settings' AND status='forbidden'"
            ).fetchone()[0]
            == 1
        )
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='device_settings' AND local_date='2026-04-15'"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind='device_settings'"
            ).fetchone()[0]
            == "forbidden"
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_items WHERE resource_kind='device_settings' AND stage='fetch' AND status='running'"
            ).fetchone()[0]
            == 0
        )
...
def test_receipt_v1_folds_not_supported_into_unavailable_and_replays(
    tmp_path: Path,
) -> None:
    config, tool, _transport = _setup(tmp_path)
    ...
    request = SyncRequest(
        "repair",
        through_local_date="2026-04-15",
        resource_kinds=("cycling_ftp",),
        repair_strategy="refetch",
        invocation_id="b1-not-supported",
    )
    first = tool.execute(request)
    second = tool.execute(request)
    assert (
        first.status == "succeeded"
        and first.counts["not_available"] == 1
        and first.counts["fetched"] == 0
    )
    assert second.counts == first.counts
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT status FROM garmin_sync_items WHERE resource_kind='cycling_ftp' AND stage='fetch'"
            ).fetchone()[0]
            == "not_supported"
        )
        assert (
            conn.execute(
                "SELECT capability_state FROM garmin_resource_capabilities WHERE resource_kind='cycling_ftp'"
            ).fetchone()[0]
            == "not_supported"
        )
        assert (
            conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE resource_kind='cycling_ftp'"
            ).fetchone()[0]
            == "not_supported"
        )


def test_device_reference_aliases_resolve_to_inventory_canonical_hash(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
