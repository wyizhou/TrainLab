...
from src.garmin_smoke import (
    GarminSmokeEvidenceError,
    evidence_sha256,
    require_valid_smoke_evidence,
    validate_smoke_evidence,
)
from src.resources import resource_bytes

ROOT = Path(__file__).resolve().parents[1]
SCHEMA = json.loads(resource_bytes("harness/schemas/garmin_smoke_evidence.schema.json"))
H = "a" * 64
...
def _evidence() -> dict[str, object]:
    def operation(
        index: int, mode: str, start: str | None = None, end: str | None = None
    ) -> dict[str, object]:
        return {
            "mode": mode,
    ...
    result: dict[str, object] = {
        ...
        "approved_window": {
            ...
            "from_local_date": "2026-07-20",
            "through_local_date": "2026-07-22",
        },
        "backup_evidence_sha256": H,
        "prerequisites": {
            "e01_e04_verified": True,
            "named_operator_registered": True,
            "backup_restore_verified": True,
            "shadow_window_registered": True,
            "legacy_path_independent_verified": True,
            "credential_permissions_verified": True,
            "bounded_activity_window_verified": True,
        ...
        "repeat_no_op": {
            "incremental": {
                "first_run_id": _run_id(2),
                "repeat_run_id": _run_id(3),
                "same_request_hash": True,
                "no_op_confirmed": True,
            },
            "snapshot": {
                "first_run_id": _run_id(4),
                "repeat_run_id": _run_id(5),
                "same_request_hash": True,
                "no_op_confirmed": True,
            },
        },
        ...
        "safety_checks": {
            "receipt_log_scan_clean": True,
            "run_table_scan_clean": True,
            "temporary_zip_cleaned": True,
            "no_daemon": True,
            "no_background_threads": True,
            "no_timer": True,
            "no_listening_socket": True,
...
@pytest.mark.parametrize(
    ...
    [
        ("authorization_id",),
        ("approved_window", "through_local_date"),
...
@pytest.mark.parametrize(
    "key", ["token", "raw_response", "activity_payload", "heart_rate"]
)
...
def test_total_elapsed_must_cover_all_eight_stage_durations_with_rounding_tolerance() -> (
    None
):
    evidence = _evidence()
    evidence["total_elapsed_seconds"] = 35.94
    evidence["evidence_sha256"] = evidence_sha256(evidence)
    assert "total_elapsed_seconds_less_than_operations" in validate_smoke_evidence(
        evidence
    )
    ...
    evidence["evidence_sha256"] = evidence_sha256(evidence)
    assert "total_elapsed_seconds_less_than_operations" not in validate_smoke_evidence(
        evidence
    )
...
def test_verifier_reads_only_supplied_evidence_and_returns_safe_codes(
    tmp_path: Path,
) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts/verify_garmin_smoke.py"),
            "--evidence",
            str(evidence_path),
        ],
        cwd=ROOT,
    ...
    assert (
        ...
        and "sensitive_or_payload_field_present" in result["codes"]
    )
