...
from src.mail_agent.delivery import MailResponseDeliveryService
from src.mail_agent.delivery_repository import (
    MailDeliveryRepository,
    MailDeliveryRepositoryError,
)
from src.mail_agent.fact_gate import FactGateDecision
from src.mail_agent.gmail_adapter import SendReceipt
from src.mail_agent.publisher import MailResponsePublisher
from src.mail_agent.repository import MailRepository
from src.mail_agent.stages import ReconcileStage
...
def prepared(tmp_path: Path):
    ...
    assert (
        FoundationTool(cfg).execute(FoundationRequest("init", "m410", NOW)).status
        == "initialized"
    )
    ...
    source = conn.execute("SELECT id FROM source_revisions").fetchone()[0]
    conn.execute(
    ...
    conn.execute(
        "INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,received_at_utc,subject,body_text,body_sha256,source_revision_id,processing_state) VALUES(?,'m','inbound','user',?,'本周训练','请求',?,?, 'analyzing')",
        (thread, NOW, "c" * 64, source),
    )
    message = conn.execute("SELECT id FROM mail_messages").fetchone()[0]
    conn.commit()
    repo = MailRepository(conn, clock=lambda: NOW)
    run = repo.start_or_resume_run(
        MailRequest("process", subject, "inv", NOW, mail_message_ids=("m",))
    )
    context = {
        "run": {"id": run.id, "run_key": run.run_key},
        "trigger_message": {
            "id": message,
            "thread_id": thread,
    ...
    result = {
        ...
        "response": {
            ...
            "subject_intent": "reply",
            "structured_content": {},
            "user_visible_text": "好的",
...
def test_claim_and_receipt_are_one_exact_outbound_evidence_chain(
    tmp_path: Path,
) -> None:
    conn, subject, identity, message, delivery = prepared(tmp_path)
    store = MailDeliveryRepository(
        MailRepository(conn, clock=lambda: NOW), clock=lambda: LATER
    )
    ...
    saved = store.record_provider_receipt(
        subject,
        delivery,
        identity,
    ...
    assert (
        conn.execute(
            "SELECT count(*) FROM conversation_events WHERE event_type='mail_response_sent' AND mail_delivery_id=?",
            (delivery,),
        ).fetchone()[0]
        == 1
    )
    assert (
        conn.execute(
            "SELECT status FROM mail_agent_items WHERE mail_delivery_id=? AND stage='verify'",
            (delivery,),
        ).fetchone()[0]
        == "succeeded"
    )
...
def test_label_failure_records_verify_deferred_without_resending(
    tmp_path: Path,
) -> None:
    ...
    saved = store.record_provider_receipt(
        ...
        status="sent",
        label_applied=False,
        label_retry_at_utc="2026-07-24T00:10:00Z",
    )
    assert saved.label_pending
    row = conn.execute(
        "SELECT status,error_code,next_retry_at_utc FROM mail_agent_items WHERE mail_delivery_id=? AND stage='verify'",
        (delivery,),
    ).fetchone()
...
def test_thread_conflict_preserves_claimed_delivery_for_reconcile(
    tmp_path: Path,
) -> None:
    conn, subject, identity, message, delivery = prepared(tmp_path)
    store = MailDeliveryRepository(
    ...
    store.claim_pending_delivery(subject, delivery)
    with pytest.raises(
        MailDeliveryRepositoryError, match="mail_delivery_thread_conflict"
    ):
...
def test_delivery_service_and_sqlite_repository_form_one_real_chain(
    tmp_path: Path,
) -> None:
    conn, subject, identity, message, delivery = prepared(tmp_path)
...
def test_load_target_includes_trigger_subject_and_rejects_non_object_json(
    tmp_path: Path,
) -> None:
    conn, subject, identity, _, delivery = prepared(tmp_path)
    store = MailDeliveryRepository(
        MailRepository(conn, clock=lambda: NOW), verified_identity_id=identity
    )
    ...
    conn.execute(
        "UPDATE mail_response_artifacts SET structured_content_json='[]' WHERE id=?",
        (response,),
    )
    with pytest.raises(
        MailDeliveryRepositoryError, match="mail_delivery_target_invalid"
    ):
        store.load_accepted_delivery_target(
            subject_id=subject, response_artifact_id=response
        )


def test_unknown_delivery_reconcile_records_provider_evidence_without_reopening_send(
    tmp_path: Path,
) -> None:
    conn, subject, identity, message, delivery = prepared(tmp_path)
    store = MailDeliveryRepository(
    ...
    class Adapter:
        ...
        def search_run_id(self, *, run_id):
            assert run_id == "mail:response:1:thread-1"
    ...
    receipt = ReconcileStage(
        MailRepository(conn, clock=lambda: NOW), store, Adapter()
    ).execute(
        MailRequest(
            "reconcile", subject, "reconcile-1", NOW, mail_delivery_ids=(str(delivery),)
        )
    )
    ...
    assert conn.execute(
        "SELECT status,error_code FROM mail_agent_items WHERE mail_delivery_id=? AND stage='send'",
        (delivery,),
    ).fetchone()[:] == ("failed", "mail_item_failed")
    assert (
        conn.execute(
            "SELECT status FROM mail_agent_items WHERE mail_delivery_id=? AND logical_item_kind='delivery' AND stage='verify'",
            (delivery,),
        ).fetchone()[0]
        == "succeeded"
    )
    assert (
        conn.execute(
            "SELECT count(*) FROM conversation_events WHERE mail_delivery_id=? AND event_type='mail_response_sent'",
            (delivery,),
        ).fetchone()[0]
        == 1
    )
