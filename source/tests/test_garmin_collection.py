...
def garmin_env(tmp_path: Path) -> tuple[GarminConfig, bytes]:
    ...
    receipt = FoundationTool(foundation).execute(
        FoundationRequest("init", "foundation-test", "2026-01-01T00:00:00Z")
    )
...
def test_full_archives_projects_cursors_and_is_idempotent(
    garmin_env: tuple[GarminConfig, bytes],
) -> None:
    ...
    first = tool.execute(
        SyncRequest(
            ...
            resource_kinds=("activity_summary",),
            invocation_id="full-1",
        )
    )
    ...
    assert (
        conn.execute(
            "SELECT count(*) FROM activities WHERE provider='garmin'"
        ).fetchone()[0]
        == 1
    )
    ...
    assert (
        conn.execute(
            "SELECT count(*) FROM source_field_catalog WHERE provider='garmin'"
        ).fetchone()[0]
        > 0
    )
    conn.close()
...
def test_snapshot_does_not_advance_cursor_and_long_429_is_deferred(
    garmin_env: tuple[GarminConfig, bytes],
) -> None:
    config, fit = garmin_env
    normal = GarminCollectionTool(
        config,
        FakeGarmin(fit),
        sleep=lambda _: None,
        clock=lambda: datetime(2026, 4, 17),
    ...
    normal.execute(
        SyncRequest("incremental", through_local_date="2026-04-15", invocation_id="inc")
    )
    conn = sqlite3.connect(config.database_path)
    before = conn.execute(
        "SELECT max(complete_through_local_date) FROM garmin_sync_cursors"
    ).fetchone()[0]
    conn.close()
    snap = normal.execute(
        SyncRequest(
            "snapshot", snapshot_local_date="2026-04-17", invocation_id="snapshot"
        )
    )
    ...
    partial = conn.execute(
        "SELECT count(*) FROM resource_coverage WHERE local_date='2026-04-17' AND availability_state='partial'"
    ).fetchone()[0]
    conn.close()
    assert before == after and partial > 0
    deferred = GarminCollectionTool(
        config,
        FakeGarmin(fit, fail="429"),
        sleep=lambda _: None,
        clock=lambda: datetime(2026, 4, 17),
    ).execute(
        SyncRequest(
            "incremental", through_local_date="2026-04-16", invocation_id="limited"
        )
    )
    assert deferred.status == "deferred" and deferred.next_retry_at_utc


def test_l2_10_does_not_open_zip_and_status_is_local_only(
    garmin_env: tuple[GarminConfig, bytes],
) -> None:
    config, fit = garmin_env
    bad = io.BytesIO()
...
def test_request_boundaries() -> None:
    with pytest.raises(ValueError, match="history_start_date_required"):
        GarminCollectionTool(
            GarminConfig(Path("x"), Path("x"), Path("x"), None)
        ).execute(SyncRequest("full"))
    with pytest.raises(ValueError, match="repair_requires_scope"):
        GarminCollectionTool(
            GarminConfig(Path("x"), Path("x"), Path("x"), "2026-01-01")
        ).execute(SyncRequest("repair"))


def test_l2_10_never_downloads_representative_fit_files(
    garmin_env: tuple[GarminConfig, bytes],
) -> None:
    config, _ = garmin_env
    first_tool = GarminCollectionTool(
        config,
        FakeGarmin(next(iter(SYNTHETIC_FITS.values()))),
        sleep=lambda _: None,
        clock=lambda: datetime(2026, 4, 17),
    )
    assert first_tool.execute(SyncRequest("auth")).status == "succeeded"
    for number, (name, fit) in enumerate(SYNTHETIC_FITS.items()):
        result = GarminCollectionTool(
            config,
            FakeGarmin(fit),
            sleep=lambda _: None,
            clock=lambda: datetime(2026, 4, 17),
    ...
    assert (
        conn.execute("SELECT count(*) FROM fit_unknown_message_catalog").fetchone()[0]
        == 0
    )
    assert (
        conn.execute("SELECT count(*) FROM fit_metric_definitions").fetchone()[0] == 0
    )
    assert conn.execute("SELECT count(*) FROM strength_sets").fetchone()[0] == 0
    conn.close()


def test_auth_and_offline_repair_do_not_network(
    garmin_env: tuple[GarminConfig, bytes],
) -> None:
    config, fit = garmin_env
    provider = FakeGarmin(fit)
    tool = GarminCollectionTool(
        config, provider, sleep=lambda _: None, clock=lambda: datetime(2026, 4, 17)
    )
    assert tool.execute(SyncRequest("auth")).status == "succeeded"
    assert (
        tool.execute(
