...
from src.garmin import GarminCollectionTool, GarminConfig
from src.resources import resource_path

SCHEMA_PATH = resource_path(
    "harness/schemas/garmin_live_acceptance_receipt.schema.json"
)
ZERO_HASH = "0" * 64
...
def _preflight(
    ...
    return tool._run_live_acceptance_preflight(
        ...
        invocation_id="garmin-live-acceptance-fixture",
    )
...
def test_stable_authority_streams_database_larger_than_32mib(
    ...
    monkeypatch.setattr("src.garmin.base.os.read", bounded_read)
    result = _preflight(tool, {"database": database}, tmp_path / "stop.json")
...
def test_preflight_failure_is_redacted_and_blocks_pre_auth_boundary(
    ...
    with pytest.raises(RuntimeError, match="preflight_failed"):
        _preflight(tool, {"database": authority}, stop)
...
def test_preflight_rejects_replaced_authority_after_open(
    ...
    monkeypatch.setattr("src.garmin.base.os.read", replace_during_read)
    with pytest.raises(RuntimeError, match="preflight_failed"):
        _preflight(tool, {"database": authority}, tmp_path / "stop.json")
...
def test_preflight_stop_persistence_failure_cannot_continue(
    ...
    with pytest.raises(RuntimeError, match="preflight_stop_persist_failed"):
        _preflight(tool, {"database": missing}, tmp_path / "stop.json")
...
def test_atomic_writer_retries_short_writes_and_rejects_zero_write(
    ...
    monkeypatch.setattr("src.garmin.base.os.write", short_write)
    tool._persist_live_acceptance_document(destination, checkpoint)
    assert calls > 1
    assert json.loads(destination.read_text(encoding="utf-8")) == checkpoint

    monkeypatch.setattr("src.garmin.base.os.write", lambda _fd, _payload: 0)
...
def test_parent_fsync_stop_preserves_actual_replacement_prefix(
    ...
    monkeypatch.setattr("src.garmin.base.os.fsync", fail_only_parent_fsync)
    with pytest.raises(RuntimeError, match="rotation"):
        tool._run_verified_live_auth_refresh(
            token_path=token,
            load_token_state=lambda: {
...
def test_atomic_token_writer_completes_partial_writes_or_keeps_old_token(
    ...
    monkeypatch.setattr("src.garmin.base.os.write", partial_write)
