...
from src.foundation import TABLES, VIEWS, FoundationConfig, FoundationTool
...
def test_projection_only_sample_is_complete_private_and_repeat_safe(
    tmp_path: Path,
) -> None:
    root = generate_sample_database(tmp_path / "sample")
    with pytest.raises(FileExistsError):
        generate_sample_database(root)
    ...
    try:
        ...
        for view in VIEWS:
            conn.execute(f"SELECT * FROM {view} LIMIT 3").fetchall()
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM v_current_activities WHERE provider_state='provider_deleted'"
            ).fetchone()[0]
            == 0
        )
        assert (
            conn.execute("SELECT COUNT(*) FROM v_current_daily_health").fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM v_current_analysis_artifacts WHERE artifact_kind='weekly_summary'"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute("SELECT COUNT(*) FROM v_open_data_quality_issues").fetchone()[
                0
            ]
            == 1
        )
        ...
        assert conn.execute(
            "SELECT DISTINCT trust_class FROM v_analysis_history_context"
        ).fetchall() == [("prior_model_output",)]
        assert conn.execute(
            "SELECT DISTINCT trust_class FROM v_mail_response_history_context"
        ).fetchall() == [("prior_model_output",)]
        malicious = conn.execute(
            "SELECT content_text FROM conversation_events"
        ).fetchone()[0]
        assert (
            malicious is None
        )  # the untrusted message remains a mail fact, not promoted event content
        body = conn.execute(
            "SELECT body_text FROM mail_messages WHERE provider_message_id='reply'"
        ).fetchone()[0]
        assert "IGNORE ALL PREVIOUS" in body
        assert (
            conn.execute("SELECT trust_level FROM conversation_events").fetchone()[0]
            == "untrusted_content"
        )
    finally:
        conn.close()
    corpus = (root / "sample-metadata.json").read_bytes() + b"".join(
        path.read_bytes() for path in (root / "raw" / "synthetic").iterdir()
    )
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM activity_samples WHERE latitude IS NOT NULL OR longitude IS NOT NULL"
        ).fetchone()[0]
        == 0
    )
    conn.close()
    ...
    assert not re.search(
        r"(?:access|refresh)[_-]?token|api[_-]?key|password|authorization\s*[:=]|https?://accounts\.google",
        values,
    )


def test_sample_encrypted_backup_restore_is_lossless_and_authenticated(
    tmp_path: Path,
) -> None:
    root = generate_sample_database(tmp_path / "sample")
    ...
    temporary_before = (
        {path.name for path in destination.parent.glob("*")}
    ...
    assert not [
        ...
        if p.name == "backup.sqlite"
        or p.name.startswith(".backup-")
        or p.name.startswith(".restore-")
    ]
    assert {path.name for path in destination.parent.glob("*")} - temporary_before == {
        "sample.tlfb"
    }


def test_runbook_script_works_in_project_venv_and_refuses_existing_target(
    tmp_path: Path,
) -> None:
    ...
    command = [
        sys.executable,
        "scripts/generate_foundation_sample.py",
        "--output-root",
        str(output),
    ]
