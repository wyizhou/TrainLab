...
def _session(payload: bytes) -> tuple[str, str]:
...
def _activity_pipeline(
    ...
    *,
    ...
    start, _sport = _session(SYNTHETIC_FITS["Running.fit"])
    root = tmp_path / "data"
...
def test_six_representative_fits_pass_crc_and_session_identity_without_temp_residue(
    ...
    tool = _tool(tmp_path)
    assert len(SYNTHETIC_FITS) == 6
    for payload in SYNTHETIC_FITS.values():
        start, sport = _session(payload)
        assert tool._fit_session_identity(payload, start, sport) == (
            start,
            sport,
        )
...
def test_session_identity_time_and_sport_mismatch_are_rejected(tmp_path: Path) -> None:
    tool = _tool(tmp_path)
    sample = SYNTHETIC_FITS["Running.fit"]
    start, _sport = _session(sample)
    with pytest.raises(GarminError, match="fit_identity_mismatch"):
        tool._fit_session_identity(sample, "2020-01-01T00:00:00Z", "running")
    with pytest.raises(GarminError, match="fit_identity_mismatch"):
        tool._fit_session_identity(sample, start, "cycling")
    assert not list(tmp_path.glob(".fit-download-*"))
...
def test_ordered_multi_session_evidence_and_current_revision_are_explicit(
    ...
    fit = SYNTHETIC_FITS["Running.fit"]
...
def test_zip_slip_crc_and_multiple_candidate_boundaries_leave_no_temp_residue(
    ...
    tool = _tool(tmp_path)
    sample = next(iter(SYNTHETIC_FITS.values()))
    slipped = io.BytesIO()
...
def test_candidate_failure_matrix_records_expected_gap_without_canonical(
    ...
    running = SYNTHETIC_FITS["Running.fit"]
    bouldering = SYNTHETIC_FITS["Bouldering.fit"]
...
def test_activity_fit_gap_resolution_is_scoped_to_one_activity_key_and_day(
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert statuses[("activity_fit", key_a, "2026-05-10", "extract")] == "open"
        run_id = conn.execute(
        ...
        assert (
            conn.execute(
                """SELECT status FROM garmin_sync_gaps
                   WHERE resource_kind='activity_fit' AND window_start_local_date='2026-05-10'"""
            ).fetchone()[0]
            == "open"
        )
...
def test_default_pipeline_archives_and_projects_representative_fit_then_replays_noop(
    ...
    sample = SYNTHETIC_FITS[filename]
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert candidate_raw == [(hashlib.sha256(sample).hexdigest(),)]
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='activity_fit'"
            ).fetchone()[0]
        ...
        if expected == "hiking":
            assert conn.execute("SELECT count(*) FROM course_points").fetchone()[0] == 3
            ...
            assert 0.49 < low_lat <= high_lat < 0.53
            assert 0.74 < low_lon <= high_lon < 0.79
            assert (
                conn.execute(
            ...
            evidence = conn.execute(
                """SELECT activity_aux_messages.timestamp_utc,
                          activity_aux_messages.payload_json,course_points.route_time_utc
                   FROM activity_aux_messages JOIN course_points
                     ON course_points.activity_id=activity_aux_messages.activity_id
                    AND course_points.point_index=activity_aux_messages.message_index
                   WHERE activity_aux_messages.message_name='course_point_evidence'
                   ORDER BY activity_aux_messages.message_index"""
            ).fetchall()
            assert len(evidence) == 3
            for timestamp_utc, payload_json, route_time_utc in evidence:
                timestamp = json.loads(payload_json)["timestamp"]
                assert timestamp["field_definition_number"] == 1
                assert timestamp["value"] == timestamp_utc == route_time_utc
            aux = json.loads(
                conn.execute(
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM activity_segments WHERE segment_type='lap'"
            ).fetchone()[0]
            == 1
        )
        ...
        if expected == "bouldering":
            ...
            """).fetchall()
            assert len(routes) == 2
            assert tuple(routes[0]) == ("climb_active", "2", "font", "3", 1, 1, 7.0)
        ...
        if expected == "indoor_climbing":
            ...
            """).fetchall()
            assert len(routes) == 2
            assert tuple(routes[0]) == ("climb_active", "3", "font", "4", 1, 1, 7.0)
            assert tuple(routes[-1]) == (
                "climb_active",
                "4",
                "font",
        ...
        if expected == "strength":
            ...
            assert [tuple(row) for row in sets] == [
                (0, 0, 0, "pull_up", 0, "fixture movement alpha", 5, 17.25, 28.0),
                (1, 1, 0, "pull_up", 0, "fixture movement alpha", 0, 0.0, 12.0),
            ]
            assert sets[-1][6:8] == (0, 0.0)
            assert (
                conn.execute(
...
def test_fit_identity_hmacs_are_stable_canonical_and_absent_in_operational_text(
    ...
    assert receipt.status == "succeeded"
    application_tuple = tuple(range(16))
    application_list = list(application_tuple)
    serial_a, serial_b = "424242", "424243"
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert {serial_a_hmac, tuple_hmac} <= device_hashes
        assert serial_b_hmac not in device_hashes
        operational = "\n".join(conn.iterdump())
...
def test_fit_projection_failure_keeps_existing_current_revision_and_cleans_temp_zip(
    ...
    class Transport:
        payload = _original_zip(bouldering)
    ...
    failed_fit = _valid_fit_variant(bouldering)
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert {
            ...
            hashlib.sha256(bouldering).hexdigest(),
