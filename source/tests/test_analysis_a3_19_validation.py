...
def running(zone: int = 4) -> dict[str, object]:
    return {
        ...
        "total_volume": "interval_session_by_duration",
        "target_zone": zone,
        ...
        "stop_conditions": STOP,
        "rationale": "quality_session_appropriate",
    }
...
def expectation() -> ResultValidationExpectation:
    manifest = [
        ...
        {
            "ordinal": 1,
            "input_role": "reason.accepted",
            "source_entity_type": "conversation_event",
            "source_entity_id": "91",
            "source_revision_id": "91",
        },
    ]
    bases = {
        day: {
    ...
    return ResultValidationExpectation(
        ...
        mode="revise_plan",
        subject_id=1,
        target_periods={"plan": PERIOD},
        input_manifest=manifest,
        quality_gate={"state": "ready", "blockers": [], "warnings": []},
        safety_request_base={},
        weekly_safety_request_bases=bases,
...
def output() -> dict[str, object]:
    plan = {
        ...
        "items": [item(index, day) for index, day in enumerate(DAYS[2:])],
    }
    return {
        "schema_version": "1",
        "run_key": RUN_KEY,
        ...
        "artifacts": [
            {
                ...
                "user_visible_text": "计划已按已确认的限制调整，保留恢复空间。",
            }
        ],
        "training_plan": plan,
        "source_usage": [
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda value: value["training_plan"].update(
                effective_local_date="2026-07-19"
            ),
            "analysis_result_revision_lineage_mismatch",
        ),
        (
            lambda value: value["training_plan"]["items"].pop(),
            "analysis_result_revision_item_cardinality_invalid",
        ),
        (
        ...
        (
            lambda value: value["training_plan"]["items"][0].update(
                activity_kind="cycling", prescription={"activity_kind": "cycling"}
            ),
        ...
        (
            lambda value: value["training_plan"].update(original_plan_id="other"),
            "analysis_result_revision_lineage_mismatch",
        ),
    ],
)
...
def test_revision_rejects_historical_item_injection_and_missing_required_source_usage() -> (
    None
):
    value = deepcopy(output())
...
def test_revision_enforces_red_flag_bpm_and_clock_time_rules() -> None:
        ...
        "safety_signals": [
            {
                ...
                "source_revision_id": "injury",
                "current": True,
                "active": True,
                "effective_from_utc": "2026-07-20T00:00:00Z",
                "expires_at_utc": None,
            }
        ],
    }
    ...
    value = output()
    value["training_plan"]["items"][0]["rationale_text"] = "目标 150 bpm。"
    value["artifacts"][0]["structured_content"] = value["training_plan"]
