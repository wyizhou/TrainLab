        ...
        "artifacts": [
            {
                ...
                "user_visible_text": "本周恢复与训练已复盘。",
            },
            {
                "artifact_kind": "weekly_training_plan",
                "period": period,
                "structured_content": training_plan,
                "user_visible_text": "未来七天计划已制定。",
            },
        ],
        ...
        rows.append(
            {
                ...
                "source_entity_id": str(prior_artifact_id),
                "source_revision_id": f"artifact:{prior_artifact_id}",
                "source_window": {
...
def evidence(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "harness_version": "weekly-harness-v1",
...
def test_weekly_pair_plan_and_seven_items_publish_atomically() -> None:
    ...
    plan = conn.execute(
        "SELECT * FROM training_plans WHERE id=?", (receipt.training_plan_id,)
    ).fetchone()
    assert plan["status"] == "active" and plan["timezone"] == "Asia/Hong_Kong"
    items = conn.execute(
        "SELECT item_index,local_date,activity_kind FROM training_plan_items WHERE training_plan_id=? ORDER BY item_index",
        (receipt.training_plan_id,),
    ).fetchall()
    assert [
        (row["item_index"], row["local_date"], row["activity_kind"]) for row in items
    ] == [
        (0, "2026-07-26", "rest"),
        (1, "2026-07-27", "running"),
        (2, "2026-07-28", "strength"),
        (3, "2026-07-29", "rest"),
        (4, "2026-07-30", "climbing"),
        (5, "2026-07-31", "rest"),
        (6, "2026-08-01", "running"),
    ]
...
def test_weekly_prior_relations_and_overlapping_plans_are_preserved_then_superseded() -> (
    None
):
    conn = database()
    conn.execute(
    ...
    relations = conn.execute(
        "SELECT from_artifact_id,relation_type FROM analysis_artifact_relations WHERE to_artifact_id=8 ORDER BY from_artifact_id"
    ).fetchall()
    assert len(relations) == 2 and {row["relation_type"] for row in relations} == {
...
def test_plan_supersession_failure_restores_existing_active_plan() -> None:
    ...
    with pytest.raises(RuntimeError):
        AnalysisPublisher(conn).publish(
            ...
            failpoint=lambda stage: (
                (_ for _ in ()).throw(RuntimeError("injected"))
                if stage == "after_plan_supersession"
                else None
            ),
        )
    assert (
    ...
    assert (
        conn.execute("SELECT count(*) FROM analysis_artifacts WHERE id!=8").fetchone()[
            0
        ]
        == 0
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda payload: payload["training_plan"].update(
            {
                "prior_artifact_state": {
                    "summary": "unknown",
                    "plan": "no_prior_artifact",
        ...
        lambda payload: payload["training_plan"]["items"][1].update(
...
def test_weekly_invalid_plan_never_publishes(mutate) -> None:
    ...
    with pytest.raises(AnalysisPublishError, match="weekly_plan"):
        AnalysisPublisher(conn).publish(
            run_id=1, accepted=payload, input_manifest=rows, run_evidence=evidence(rows)
        )
    assert conn.execute("SELECT count(*) FROM analysis_artifacts").fetchone()[0] == 0
