...
def request(
    mode: str, invocation: str | None = None, target: int | None = None
) -> FoundationRequest:
...
def test_fnd11_first_init_and_ready_init_are_identity_preserving_full_tree_noop(
    tmp_path: Path,
) -> None:
    root = tmp_path / "foundation"
    tool = FoundationTool(config(root))
    first = tool.execute(request("init", "first-init"))
    assert (first.invocation_id, first.mode, first.status, first.ready) == (
        "first-init",
        "init",
        "initialized",
        True,
...
@pytest.mark.parametrize(
    ("mode", "expected"),
    [("status", "ready"), ("verify", "ready"), ("init", "already_initialized")],
)
...
@pytest.mark.parametrize(
    "mutation",
    ["implementation", "initialized_timestamp", "updated_timestamp", "foreign_key"],
)
def test_fnd11_deep_compatibility_checks_are_explicit_and_read_only(
    ...
    if mutation == "implementation":
        ...
        conn.execute(
            "UPDATE foundation_state SET initialized_at_utc='2026-01-01T00:00:00.000000Z'"
        )
    elif mutation == "updated_timestamp":
        conn.execute(
            "UPDATE foundation_state SET updated_at_utc='2026-01-01T00:00:00.000000Z'"
        )
    else:
        conn.execute(
            "INSERT INTO mail_threads(subject_id,provider_thread_id) VALUES(999,'orphan')"
        )
    conn.commit()
    conn.close()
    before = tree_evidence(root)
...
def test_fnd11_cli_exit_mapping_and_receipt_identity(
    ...
    for name in ("config", "state", "logs"):
        (instance / name).mkdir(parents=True)
    monkeypatch.setenv("TRAINLAB_INSTANCE_ROOT", str(instance))
    monkeypatch.setattr(
    ...
    def execute(self, item: FoundationRequest) -> FoundationReceipt:
        return FoundationReceipt(
            ...
            next_action="none"
            if status in {"initialized", "already_initialized", "ready", "lock_busy"}
    ...
    assert (
        foundation.main(["--invocation-id", "receipt-identity", "status"])
        == expected_exit
    )
...
def test_fnd11_four_modes_start_no_thread_process_subprocess_or_daemon(
    ...
    assert (
        tool.execute(request("migrate", target=FOUNDATION_SCHEMA_VERSION)).status
        == "already_initialized"
    )
...
@pytest.mark.parametrize(
    "phase", ["before_migration_transaction", "during_migration_transaction"]
)
def test_fnd11_precommit_migration_failure_rolls_back_last_known_good(
    tmp_path: Path, phase: str
) -> None:
    root = tmp_path / phase
    base = create_published_v1_database(root)
    ...
    recovered = FoundationTool(base.config).execute(
        request("migrate", target=FOUNDATION_SCHEMA_VERSION)
    )
    assert recovered.status == "initialized" and recovered.ready


def test_fnd11_postcommit_premarker_failure_requires_explicit_republication(
    tmp_path: Path,
) -> None:
    ...
    repaired = FoundationTool(base.config).execute(
        request("migrate", target=FOUNDATION_SCHEMA_VERSION)
    )
...
def test_fnd11_postmarker_failure_leaves_verified_last_known_good(
    tmp_path: Path,
) -> None:
    root = tmp_path / "postmarker"
    base = create_published_v1_database(root)
    ...
    assert (
        ready.execute(request("migrate", target=FOUNDATION_SCHEMA_VERSION)).status
        == "initialized"
    )
...
@pytest.mark.parametrize(
    "mutation", ["marker_timestamp", "state_timestamp", "migration_timestamp"]
)
def test_fnd11_explicit_migrate_rejects_noncanonical_v1_metadata_without_writes(
    ...
    if mutation == "marker_timestamp":
        ...
        if mutation == "state_timestamp":
            ...
            conn.execute(
                "UPDATE schema_migrations SET applied_at_utc='2026-01-01T00:00:00.000000Z'"
            )
        conn.commit()
        conn.close()
    db = root / "data.db"
...
def test_fnd11_explicit_migrate_rejects_inconsistent_current_receipt_without_data_write(
    tmp_path: Path,
) -> None:
...
def test_fnd11_bootstrap_accepts_only_already_initialized(tmp_path: Path) -> None:
    ...
    status = tool.execute(request("status", "bootstrap"))
