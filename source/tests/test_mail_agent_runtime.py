...
from src.mail_agent.runtime import (
    MailRuntimeDependencies,
    RuntimeMailApplicationService,
    _recipient_email,
)
from src.mail_agent.stages import MissingDependencyStage, PreparedStage
...
def test_recipient_config_is_strict_and_not_an_environment_variable(tmp_path) -> None:
    ...
    (config / "src.json").write_text(
        '{"schema_version":1,"mail":{"recipient_email":"bad address"}}',
        encoding="utf-8",
    )
    try:
...
def test_fast_foundation_status_allows_read_only_mail_status_during_snapshot(
    ...
    monkeypatch.setattr("src.mail_agent.runtime.project_root", lambda supplied: root)
    monkeypatch.setattr(
        "src.mail_agent.runtime.FoundationConfig.load", lambda supplied: foundation
    )
    adapter_calls = []
