...
def _setup(
    tmp_path: Path,
) -> tuple[GarminConfig, GarminCollectionTool, RepairTransport]:
    root = tmp_path / "data"
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "l2-14", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
    config = GarminConfig(
...
def test_repair_cursor_stops_at_unresolved_middle_health_gap(tmp_path: Path) -> None:
    ...
    try:
        ...
        tool.repo.gap(
            ...
            subject,
            "steps",
            "garmin:health:steps:2026-04-16:sibling",
            "2026-04-16",
            "project",
            "parse_or_project_failed",
        )
    ...
    result = tool.execute(
        SyncRequest(
            ...
            resource_kinds=("steps",),
            repair_strategy="refetch",
            invocation_id="cursor-repair-middle-gap",
        )
    )
...
def test_unchanged_daily_success_resolves_only_its_exact_gap_key(
    tmp_path: Path,
) -> None:
    config, tool, _transport = _setup(tmp_path)
    ...
    try:
        subject = tool.repo.subject(conn)
        tool.repo.gap(
            conn, subject, "steps", key, day, "project", "parse_or_project_failed"
        )
        tool.repo.gap(
            conn, subject, "steps", sibling, day, "project", "parse_or_project_failed"
        )
    finally:
        conn.close()
        ...
        with pytest.raises(ValueError, match="controlled_range_failure"):
            tool.repo.archive(
                ...
                lambda _revision: (_ for _ in ()).throw(
                    ValueError("controlled_range_failure")
                ),
            )
    ...
    try:
        with pytest.raises(ValueError, match="controlled_daily_failure"):
            tool.repo.archive(
                ...
                "application/json",
                lambda _revision: (_ for _ in ()).throw(
                    ValueError("controlled_daily_failure")
                ),
            )
        return conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind=? AND provider_object_id=? "
    ...
    try:
        with pytest.raises(ValueError, match="controlled_activity_failure"):
            tool.repo.archive(
                ...
                "activity_extra",
                provider_object_id,
                stable_json({"role": "fixture"}),
                "json",
                "application/json",
                lambda _revision: (_ for _ in ()).throw(
                    ValueError("controlled_activity_failure")
                ),
            )
        return conn.execute(
...
def test_auto_routes_parse_gap_to_offline_reparse_without_login(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        revision = conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind='steps'"
        ).fetchone()[0]
        conn.execute(
    ...
    result = tool.execute(
        SyncRequest(
            ...
            invocation_id="auto-reparse",
        )
    )
    assert result.status == "succeeded", result.json()
    assert (
        transport.login_calls == before_login and transport.health_calls == before_fetch
    )
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
...
def test_reparse_recovers_health_date_from_raw_key_without_explicit_dates(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    _refetch_steps(tool, "seed")
...
def test_reparse_selects_only_distinct_revision_linked_gaps(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        raw_before = list(
            conn.execute("SELECT sha256 FROM raw_objects ORDER BY sha256")
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            list(conn.execute("SELECT sha256 FROM raw_objects ORDER BY sha256"))
            == raw_before
        )
...
def test_explicit_reparse_without_revision_gap_rebuilds_current(tmp_path: Path) -> None:
    ...
    receipt = tool.execute(
        SyncRequest(
            ...
            resource_kinds=("steps",),
            repair_strategy="reparse",
            invocation_id="explicit-current-reparse",
        )
    )
    assert receipt.status == "succeeded" and receipt.counts["revised"] == 1
    assert (transport.login_calls, transport.health_calls) == before_calls
...
def test_activity_id_scope_does_not_match_prefix_ids_in_gap_or_fallback_paths(
    tmp_path: Path,
    with_linked_gap: bool,
) -> None:
    config, tool, _transport = _setup(tmp_path)
    revisions = {
        activity_id: _archive_activity_extra_revision(
            tool,
            f"garmin:activity:{activity_id}:extra",
        )
    ...
    request = SyncRequest(
        ...
        health_from_local_date="2026-04-15",
        through_local_date="2026-04-15",
        resource_kinds=("activity_extra",),
        activity_ids=("1",),
        repair_strategy="reparse",
        invocation_id=f"activity-id-scope-{with_linked_gap}",
    )
    conn = tool.repo.connect()
    try:
...
def test_reparse_range_revisions_offline_by_day_without_duplicate_canonical(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    payloads = {
    ...
    request = SyncRequest(
        ...
        health_from_local_date="2026-04-15",
        through_local_date="2026-04-15",
        resource_kinds=tuple(payloads),
        repair_strategy="reparse",
        invocation_id="range-reparse",
    )
    ...
    assert (
        tool.execute(replace(request, invocation_id="range-reparse-repeat")).status
        == "succeeded"
    )
    with sqlite3.connect(config.database_path) as conn:
        assert conn.execute("SELECT count(*) FROM body_measurements").fetchone()[0] == 1
...
def test_reparse_range_key_must_be_fully_within_requested_window(
    tmp_path: Path,
) -> None:
    ...
    assert tuple(
        ...
        for value in tool._repair_range_dates(
            "weigh_ins",
            "garmin:health:weigh_ins:2026-04-15:2026-04-16",
            request,
        )
    ...
    with pytest.raises(ValueError, match="repair_range_before_request"):
        tool._repair_range_dates(
            "weigh_ins",
            "garmin:health:weigh_ins:2026-04-14:2026-04-16",
            request,
        )
    with pytest.raises(ValueError, match="repair_range_after_request"):
        tool._repair_range_dates(
            "weigh_ins",
            "garmin:health:weigh_ins:2026-04-15:2026-04-17",
            request,
        )


def test_reparse_malformed_range_rolls_back_existing_current_projection(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    good = {
    ...
    transport.fetch_range = lambda resource, _start, _end: (
        good if resource == "weigh_ins" else []
    ...
    assert (
        tool.execute(
            SyncRequest(
                ...
                resource_kinds=("weigh_ins",),
                repair_strategy="refetch",
                invocation_id="range-good-current",
            )
        ).status
        == "succeeded"
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        raw_before = list(
            conn.execute("SELECT sha256,relative_path FROM raw_objects ORDER BY sha256")
        )
        raw_bytes_before = [
            (config.raw_root.parent / row[1]).read_bytes() for row in raw_before
        ]
    result = tool.execute(
        SyncRequest(
            "repair",
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert [
            (config.raw_root.parent / row[1]).read_bytes() for row in raw_after
        ] == raw_bytes_before
...
def test_refetch_is_noop_for_same_payload_and_revises_changed_payload(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    _refetch_steps(tool, "seed")
...
def test_reconcile_is_offline_and_audit_records_raw_integrity_gap(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    _refetch_steps(tool, "seed")
    before_login = transport.login_calls
    reconciled = tool.execute(
        SyncRequest(
            "repair",
            health_from_local_date="2026-04-15",
            through_local_date="2026-04-15",
            resource_kinds=("steps",),
            repair_strategy="reconcile",
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute("SELECT count(*) FROM reconciliation_results").fetchone()[0]
            >= 1
        )
        ...
        conn.execute(
            "UPDATE raw_objects SET relative_path='outside/raw.json' WHERE resource_kind='steps'"
        )
...
def test_audit_does_not_create_coverage_gap_outside_requested_window(
    tmp_path: Path,
) -> None:
    config, tool, _transport = _setup(tmp_path)
    with sqlite3.connect(config.database_path) as conn:
...
def test_audit_keeps_unknown_fields_discoverable_without_blocking_analysis(
    tmp_path: Path,
) -> None:
    config, tool, _transport = _setup(tmp_path)
    ...
    result = tool.execute(
        SyncRequest(
            ...
            health_from_local_date="2026-04-15",
            through_local_date="2026-04-15",
            invocation_id="unknown-field-audit",
        )
    )
    assert result.status == "succeeded", result.json()
...
def test_field_catalog_normalizes_only_dynamic_numeric_dictionary_keys(
    tmp_path: Path,
) -> None:
    _config, tool, _transport = _setup(tmp_path)
    conn = tool.repo.connect()
