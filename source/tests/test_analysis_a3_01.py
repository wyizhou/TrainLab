...
from src.analysis.boundary import (
...
ANALYSIS_PACKAGE = PROJECT_ROOT / "src" / "analysis"


def test_manifest_records_frozen_contracts_and_migration_baseline() -> None:
    manifest = build_module_manifest(PROJECT_ROOT)
    checked_in = json.loads(
        (ANALYSIS_PACKAGE / "module_manifest.json").read_text(encoding="utf-8")
    )

    assert {
        item.relative_path: item.sha256 for item in manifest.contracts
    } == FROZEN_CONTRACT_SHA256
    assert checked_in["contracts"] == FROZEN_CONTRACT_SHA256
    assert (
        checked_in["production_entrypoint"]
        == CURRENT_PRODUCTION_ENTRYPOINT
        == "python source/index.py analysis"
    )
    assert {
        item.relative_path: item.sha256 for item in manifest.migration_baseline
    } == INITIAL_MIGRATION_BASELINE
...
def test_initial_migration_baseline_is_not_recomputed_from_current_harness(
    tmp_path: Path,
) -> None:
...
def test_import_boundary_blocks_legacy_adapter_import(tmp_path: Path) -> None:
    (tmp_path / "bad.py").write_text("from src import ingest\n", encoding="utf-8")

    with pytest.raises(
        AnalysisBoundaryError, match="analysis_forbidden_import:.*src.ingest"
    ):
        verify_import_boundary(tmp_path)


def test_shared_mcp_exception_is_limited_to_gmail_delivery_module(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        AnalysisBoundaryError, match="analysis_forbidden_import:.*src.mcp"
    ):
        verify_import_boundary(tmp_path)
