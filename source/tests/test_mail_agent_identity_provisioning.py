...
def foundation(tmp_path: Path) -> FoundationConfig:
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "mail-identity", NOW))
        .status
        == "initialized"
    )
    with sqlite3.connect(config.database_path) as connection:
        ...
        connection.execute(
            "INSERT INTO data_subjects(id,subject_key,created_at_utc) VALUES(1,'subject-1',?)",
            (NOW,),
        )
    return config
...
def adapter(config: FoundationConfig) -> GmailEnvironmentRecipientAdapter:
    return GmailEnvironmentRecipientAdapter(
        RECIPIENT, identity_state_root=config.state_root
    )
...
def test_configured_recipient_is_hmac_anchored_idempotently_without_plaintext(
    tmp_path: Path,
) -> None:
    ...
    with sqlite3.connect(config.database_path) as connection:
        row = connection.execute(
            "SELECT subject_id,identity_hmac,is_verified FROM subject_identities"
        ).fetchone()
        assert row[0] == 1 and row[2] == 1
        assert (
            row[1]
            == hmac.new(
                key_path.read_bytes(), RECIPIENT.lower().encode("ascii"), hashlib.sha256
            ).hexdigest()
        )
...
def test_identity_existing_for_another_subject_fails_closed(tmp_path: Path) -> None:
    config = foundation(tmp_path)
    key_dir = config.state_root / "secrets"
    key_dir.mkdir(mode=0o700)
    key = b"x" * 32
    key_path = key_dir / "gmail-recipient-identity.key"
    key_path.write_bytes(key)
    os.chmod(key_path, 0o600)
    digest = hmac.new(
        key, RECIPIENT.lower().encode("ascii"), hashlib.sha256
    ).hexdigest()
    with sqlite3.connect(config.database_path) as connection:
        connection.execute(
            "INSERT INTO data_subjects(id,subject_key,created_at_utc) VALUES(2,'subject-2',?)",
            (NOW,),
        )
        connection.execute(
            "INSERT INTO subject_identities(subject_id,provider,identity_kind,identity_hmac,is_verified,first_seen_at_utc,last_seen_at_utc) VALUES(2,'gmail','email',?,1,?,?)",
            (digest, NOW, NOW),
        )
    with pytest.raises(
        GmailEnvironmentAdapterError, match="^gmail_reply_identity_conflict$"
    ):
        prepare(config)
...
def test_only_subject_one_may_bind_the_fixed_mail_recipient(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(config.database_path) as connection:
        with pytest.raises(
            GmailEnvironmentAdapterError, match="^gmail_reply_subject_invalid$"
        ):
            adapter(config).prepare(connection, 2)


def test_simultaneous_provisioning_converges_to_one_verified_identity(
    tmp_path: Path,
) -> None:
    config = foundation(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as connection:
        assert (
            connection.execute(
                "SELECT count(*) FROM subject_identities WHERE provider='gmail' AND identity_kind='email'"
            ).fetchone()[0]
            == 1
        )
...
def test_simultaneous_first_start_accepts_directory_create_winner(
    ...
    monkeypatch.setattr("src.mail_agent.environment_adapter.os.lstat", raced_lstat)
    with ThreadPoolExecutor(max_workers=2) as pool:
        identities = list(pool.map(lambda _: prepare(config), range(2)))
    assert identities[0] == identities[1]
