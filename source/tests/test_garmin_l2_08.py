...
from src.garmin import GarminCollectionTool, GarminConfig, GarminError, SyncRequest
...
class HealthFixtureTransport:
    ...
    def activity_summary(self, activity_id: str):
        raise AssertionError("activity not expected")
...
def _fixture_payloads() -> dict[str, object]:
    return json.loads(
        (Path(__file__).parent / "fixtures" / "garmin_health_l2_08.json").read_text(
            encoding="utf-8"
        )
    )
...
def _nested_fixture_payloads() -> dict[str, object]:
    ...
    return json.loads(
        (
            Path(__file__).parent / "fixtures" / "garmin_health_nested_l2_08.json"
...
def test_fixture_is_synthetic_and_every_base_spec_has_real_catalog_mapping() -> None:
    text = (Path(__file__).parent / "fixtures" / "garmin_health_l2_08.json").read_text(
        encoding="utf-8"
    )
...
def test_all_base_resources_share_raw_revision_projection_coverage_pipeline(
    tmp_path: Path,
) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        coverage = {
            ...
            for row in conn.execute(
                "SELECT resource_kind,availability_state FROM resource_coverage WHERE provider='garmin'"
            )
        }
        assert {
            resource: "fetched" for resource in BASE_RESOURCES
        }.items() <= coverage.items()
        assert conn.execute(
            "SELECT count(*) FROM raw_objects WHERE provider='garmin' AND media_type='application/json'"
        ).fetchone()[0] >= len(BASE_RESOURCES)
        assert conn.execute(
            "SELECT count(*) FROM source_revisions WHERE provider='garmin' AND is_current=1"
        ).fetchone()[0] >= len(BASE_RESOURCES)
        assert (
            conn.execute(
                "SELECT count(*) FROM daily_health WHERE is_current=1"
            ).fetchone()[0]
            == 1
        )
        values = json.loads(
            conn.execute(
                "SELECT values_json FROM daily_health WHERE is_current=1"
            ).fetchone()[0]
        )
        assert values == {
            "garmin.body.weight_kg": 70.5,
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM health_samples WHERE metric_key='garmin.steps.count' AND value_number=0"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM health_samples WHERE metric_key LIKE 'garmin.hydration.%'"
            ).fetchone()[0]
            == 0
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_records WHERE record_type IN ('all_day_events','lifestyle','body_battery_events')"
            ).fetchone()[0]
            == 3
        )
...
def test_sleep_cross_midnight_nap_stage_bounds_and_singapore_sample_day(
    tmp_path: Path,
) -> None:
    config, tool, _ = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        sessions = conn.execute(
            "SELECT session_type,start_time_utc,end_time_utc FROM sleep_sessions ORDER BY id"
        ).fetchall()
        assert sessions == [
            ("main_sleep", "2026-04-14T15:30:00Z", "2026-04-15T00:30:00Z"),
            ("nap", "2026-04-15T05:00:00Z", "2026-04-15T05:30:00Z"),
        ]
        assert conn.execute("SELECT count(*) FROM sleep_stages").fetchone()[0] == 3
        assert (
            conn.execute(
                "SELECT local_date FROM health_samples WHERE metric_key='garmin.heart_rate.bpm'"
            ).fetchone()[0]
            == "2026-04-15"
        )


def test_noop_revision_change_field_drift_and_failed_sleep_project_are_atomic(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_field_catalog WHERE resource_kind='user_summary' AND field_path='/future/series/*/metric'"
            ).fetchone()[0]
            == 1
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='user_summary' AND is_current=1"
            ).fetchone()[0]
            == 1
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        old_revision = conn.execute(
            "SELECT id FROM source_revisions WHERE resource_kind='sleep' AND is_current=1"
        ).fetchone()[0]
        old_sessions = conn.execute("SELECT count(*) FROM sleep_sessions").fetchone()[0]
    transport.payloads["sleep"]["sessions"][0]["stages"][0]["end_time_utc"] = (
        "2026-04-15T03:00:00Z"
    )
    receipt = _repair(tool, "sleep", "sleep-fail")
    assert receipt.status == "partial" and not any(
        "sessions" in error.get("summary", "") for error in receipt.errors
    )
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
        ...
        assert (
            conn.execute("SELECT count(*) FROM sleep_sessions").fetchone()[0]
            == old_sessions
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM raw_objects WHERE resource_kind='sleep'"
            ).fetchone()[0]
            == 2
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind='sleep' AND reason_code='parse_or_project_failed' AND status='open'"
            ).fetchone()[0]
            == 1
        )


def test_empty_capability_states_and_snapshot_partial_do_not_create_zero_or_cursor(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert {
            "floors": "empty",
            "hydration": "not_enabled",
            "stress": "not_supported",
            "heart_rates": "forbidden",
        }.items() <= coverage.items()
        states = dict(
            conn.execute(
        ...
        assert (
            states["hydration"] == "not_enabled"
            and states["stress"] == "not_supported"
            and states["heart_rates"] == "forbidden"
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM health_samples WHERE metric_key LIKE 'garmin.hydration.%'"
    ...
    snapshot = snapshot_tool.execute(
        SyncRequest(
            "snapshot", snapshot_local_date="2026-04-15", invocation_id="snapshot-base"
        )
    )
    ...
    with sqlite3.connect(snapshot_config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='user_summary' AND availability_state='partial'"
            ).fetchone()[0]
            == 1
        )
        assert (
            conn.execute("SELECT count(*) FROM garmin_sync_cursors").fetchone()[0] == 0
        )


def test_nested_connect_time_series_keep_each_sample_timestamp_and_local_day(
    tmp_path: Path,
) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        expected = {
            "garmin.heart_rate.bpm": [
                ("2026-04-15T00:00:00Z", "2026-04-15", 59.0),
                ("2026-04-15T01:00:00Z", "2026-04-15", 62.0),
            ],
            "garmin.stress.score": [
                ("2026-04-15T00:00:00Z", "2026-04-15", 21.0),
                ("2026-04-15T02:00:00Z", "2026-04-15", 34.0),
            ],
            "garmin.respiration.breaths_per_minute": [
                ("2026-04-15T01:00:00Z", "2026-04-15", 13.2),
                ("2026-04-15T03:00:00Z", "2026-04-15", 14.1),
            ],
            "garmin.body_battery.score": [
                ("2026-04-15T00:00:00Z", "2026-04-15", 66.0),
                ("2026-04-15T03:00:00Z", "2026-04-15", 61.0),
            ],
            "garmin.spo2.percent": [
                ("2026-04-15T00:00:00Z", "2026-04-15", 97.0),
                ("2026-04-15T02:00:00Z", "2026-04-15", 98.0),
            ],
        }
        ...
        assert bp_metrics == [
            ("garmin.blood_pressure.diastolic_mmhg", 76.0),
            ("garmin.blood_pressure.systolic_mmhg", 118.0),
            ("garmin.blood_pressure.diastolic_mmhg", 77.0),
            ("garmin.blood_pressure.systolic_mmhg", 119.0),
        ]
...
def test_live_nested_hrv_summary_and_overnight_readings_are_canonical(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            source_map["garmin.hrv.last_night_average_ms"]["source_path"]
...
def test_selected_health_summaries_project_from_live_connect_shapes(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
...
def test_known_health_signal_cannot_silently_become_zero_projection(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    with sqlite3.connect(tool.config.database_path) as conn:
        with pytest.raises(ValueError, match="reviewed_projection_empty:heart_rates"):
            tool._project_health(
                ...
                "heart_rates",
                "2026-04-15",
                {"heartRateValues": [[1776211200000, 70]]},
                1,
            )
...
def test_respiration_accepts_only_the_closed_interval_next_midnight_boundary(
    ...
    assert (
        _repair(tool, "respiration", "respiration-midnight-boundary").status
        == "succeeded"
    )
    with sqlite3.connect(config.database_path) as conn:
        assert conn.execute(
...
def test_unchanged_zero_record_snapshot_promotes_to_empty_not_fetched(
    ...
    assert (
        tool.execute(
            SyncRequest(
                "snapshot",
                snapshot_local_date="2026-04-15",
...
def test_sleep_levels_map_parses_string_epochs_and_iso_keys_into_bounded_stages(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        session = conn.execute(
            "SELECT start_time_utc,end_time_utc FROM sleep_sessions"
        ).fetchone()
...
def test_semantic_empty_tombstone_supersedes_current_and_snapshot_stays_partial(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM health_samples WHERE metric_key='garmin.steps.count'"
            ).fetchone()[0]
            == 1
        )
        ...
        assert [row[0] for row in coverage] == [
            "fetched",
            "empty",
            "not_enabled",
            "fetched",
    ...
    snapshot_config, snapshot_tool, snapshot_transport = _setup(
        tmp_path / "snapshot-empty"
    )
    snapshot_transport.payloads["steps"] = []
    receipt = snapshot_tool.execute(
        SyncRequest(
            "snapshot", snapshot_local_date="2026-04-15", invocation_id="snapshot-empty"
        )
    )
    assert receipt.status == "succeeded" and receipt.coverage_state == "partial"
    with sqlite3.connect(snapshot_config.database_path) as conn:
        row = conn.execute(
            "SELECT availability_state,source_revision_id FROM resource_coverage WHERE resource_kind='steps'"
        ).fetchone()
...
def test_snapshot_optional_404_and_forbidden_observations_are_always_partial(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        rows = dict(
            conn.execute(
                "SELECT resource_kind,availability_state FROM resource_coverage WHERE resource_kind IN ('floors','heart_rates')"
            )
        )
...
def test_invalid_high_frequency_time_or_tuple_arity_rolls_back_and_creates_gap(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM raw_objects WHERE resource_kind='heart_rates'"
            ).fetchone()[0]
            == 2
        )
        assert (
            conn.execute(
                "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind='heart_rates' AND reason_code='parse_or_project_failed' AND status='open'"
            ).fetchone()[0]
            == 1
        )


def test_canonical_registry_units_and_catalog_mapping_are_bounded(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert values == {
            "garmin.daily.resting_heart_rate_bpm": 51.0,
            "garmin.daily.steps": 1200.0,
            "garmin.heart_rate.daily_average_bpm": 60.5,
        }
        assert all(not isinstance(value, (list, dict)) for value in values.values())


def test_date_only_timestamp_is_rejected_for_samples_and_allowed_only_for_day_boundary() -> (
    None
):
    ...
    assert (
        GarminCollectionTool._timestamp_utc(
...
def test_snapshot_forbidden_uses_one_partial_coverage_and_no_revision_growth(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.errors["heart_rates"] = GarminError("denied", http_status=403)

    def request(invocation: str) -> SyncRequest:
        return SyncRequest(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='heart_rates'"
            ).fetchone()[0]
            == 0
        )
        assert (
...
def test_snapshot_parse_and_project_failures_have_partial_coverage_gap_item_and_no_cursor(
    ...
    parsed = tool.execute(
        SyncRequest(
            "snapshot",
            snapshot_local_date="2026-04-15",
            invocation_id="snapshot-parse-failure",
        )
    )
    ...
    transport.payloads["steps"][0]["steps"] = (
        1201  # force publisher/projector, not snapshot no-op
    )
    projected = tool.execute(
        SyncRequest(
            "snapshot",
            snapshot_local_date="2026-04-15",
            invocation_id="snapshot-project-failure",
        )
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        for resource in ("heart_rates", "steps"):
            assert (
                conn.execute(
                    "SELECT count(*) FROM resource_coverage WHERE resource_kind=? AND availability_state='partial'",
                    (resource,),
                ).fetchone()[0]
                == 1
            )
            assert (
                conn.execute(
                    "SELECT count(*) FROM garmin_sync_gaps WHERE resource_kind=? AND reason_code='parse_or_project_failed'",
                    (resource,),
                ).fetchone()[0]
                == 1
            )
            assert (
                conn.execute(
                    "SELECT count(*) FROM garmin_sync_items WHERE resource_kind=? AND stage='project' AND status='failed'",
                    (resource,),
                ).fetchone()[0]
                >= 1
            )
...
def test_meaningful_timestampless_sample_is_not_fetched_with_zero_projection(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    assert (
        _repair(tool, "heart_rates", "heart-good-before-missing-time").status
        == "succeeded"
    )
    with sqlite3.connect(config.database_path) as conn:
        old_revision = conn.execute(
        ...
        old_samples = conn.execute(
            "SELECT count(*) FROM health_samples WHERE metric_key='garmin.heart_rate.bpm'"
        ).fetchone()[0]
...
def test_live_daily_aliases_and_missing_sensor_points_do_not_drop_resource(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.payloads["steps"] = [
...
def test_daily_capabilities_record_supported_empty_and_explicit_state_in_configured_region(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.payloads["floors"] = []
...
def test_audit_reconciles_legacy_default_and_backfills_completed_coverage_idempotently(
    tmp_path: Path,
) -> None:
    ...
    try:
        ...
        conn.close()
    assert tool.execute(
        SyncRequest("audit", invocation_id="capability-audit")
    ).status in {"succeeded", "partial"}
    assert tool.execute(
        SyncRequest("audit", invocation_id="capability-audit-repeat")
    ).status in {"succeeded", "partial"}
    with sqlite3.connect(config.database_path) as conn:
        assert conn.execute(
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='floors'"
            ).fetchone()[0]
            == 1
        )
        assert conn.execute(
...
def test_zero_record_coverage_reconciliation_is_region_independent(
    tmp_path: Path,
) -> None:
    _config, tool, _transport = _setup(tmp_path)
    conn = tool.repo.connect()
