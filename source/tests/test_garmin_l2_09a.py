...
class AccountTransport:
    ...
    def list_activities(self, _start, _through):
        self.calls.append("activities")
        return []

    def activity_summary(self, _activity_id):
        raise AssertionError("no activity")

    def activity_original(self, _activity_id):
        raise AssertionError("no activity")

    def activity_extra(self, _activity_id, _role):
        raise AssertionError("no activity")
...
def _setup(tmp_path: Path):
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "l2-09a", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
...
def _account_sync(
    tool: GarminCollectionTool, invocation: str, *resources: str, mode: str = "repair"
):
    return tool.execute(
        SyncRequest(
            mode,
            through_local_date="2026-04-15" if mode == "repair" else None,
...
def test_account_profile_devices_are_redacted_revisioned_and_idempotent(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    first = _account_sync(
        tool, "account-one", "user_profile", "user_profile_settings", "devices"
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM physiology_records WHERE record_type IN ('user_profile','user_profile_settings')"
            ).fetchone()[0]
            == 2
        )
        raw = b"".join(
            ...
            for row in conn.execute(
                "SELECT relative_path FROM raw_objects WHERE resource_kind IN ('user_profile','user_profile_settings','devices')"
            )
        )
        assert (
            b"PRIVATE DISPLAY" in raw
            and b"private@example" in raw
            and b"PRIVATE-SERIAL" in raw
            and b"PRIVATE_TOKEN" not in raw
        )
        assert b"PRIVATE DISPLAY" not in "\n".join(conn.iterdump()).encode()
        before = dict(
            conn.execute(
                "SELECT resource_kind,count(*) FROM source_revisions WHERE resource_kind IN ('user_profile','user_profile_settings','devices') GROUP BY resource_kind"
            )
        )
        uid = conn.execute("SELECT device_uid_hash FROM devices").fetchone()[0]
        assert len(uid) == 64
        assert conn.execute(
            "SELECT mapping_state,canonical_metric_key FROM source_field_catalog WHERE resource_kind='devices' AND field_path='/devices/*/hardwareVersion'"
        ).fetchone() == ("mapped", "garmin.device.hardware_version")
    second = _account_sync(
        tool, "account-repeat", "user_profile", "user_profile_settings", "devices"
    )
    ...
    transport.payloads["devices"].append(
        {
            "deviceId": "new-device",
            "manufacturer": "Garmin",
            "product": "Edge",
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='devices' AND is_current=1"
            ).fetchone()[0]
            == 1
        )
...
def test_user_profile_volatile_raw_fields_do_not_create_semantic_revisions(
    ...
    transport.payloads["user_profile"]["lastLoginTimestamp"] = "2026-04-17T00:00:00Z"
    second = _account_sync(tool, "profile-semantic-volatile-one", "user_profile")
    transport.payloads["user_profile"]["lastLoginTimestamp"] = "2026-04-17T00:01:00Z"
...
def test_account_json_credentials_are_quarantined_before_all_payload_writes(
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert (
            conn.execute("SELECT count(*) FROM source_field_catalog").fetchone()[0] == 0
        )
        assert (
            conn.execute(
...
def test_explicit_account_repair_is_closed_scope_and_software_never_becomes_hardware(
    tmp_path: Path,
) -> None:
    config, tool, transport = _setup(tmp_path)
    transport.calls.clear()
    transport.payloads["devices"] = [
        {"deviceId": "device-only", "softwareVersion": "software-99"}
    ]
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute("SELECT hardware_version FROM devices").fetchone()[0] is None
        )
        raw = b"".join(
            (config.raw_root.parent / row[0]).read_bytes()
            for row in conn.execute(
                "SELECT relative_path FROM raw_objects WHERE resource_kind='devices'"
            )
        )
        assert b"softwareVersion" in raw and b"software-99" in raw
        mappings = dict(
            conn.execute(
                "SELECT field_path,mapping_state FROM source_field_catalog WHERE resource_kind='devices'"
            )
        )
...
def test_account_clock_and_sensitive_errors_are_deterministic(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(config.database_path) as conn:
        stamp = conn.execute(
            "SELECT effective_at_utc FROM physiology_records WHERE record_type='user_profile_settings'"
        ).fetchone()[0]
    ...
    for index, marker in enumerate(markers):
        ...
        receipt = _account_sync(
            tool, f"private-error-{index}", "user_profile", "devices"
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        ...
        assert set(
            row[0]
            for row in conn.execute(
                "SELECT error_code FROM garmin_sync_items WHERE error_code IS NOT NULL"
            )
        ) <= {"provider_error"}
        assert set(
            row[0]
            for row in conn.execute(
                "SELECT reason_code FROM garmin_sync_gaps WHERE resource_kind='user_profile'"
            )
        ) <= {"provider_error"}
        assert set(
            row[0]
            for row in conn.execute(
                "SELECT reason_code FROM garmin_resource_capabilities WHERE resource_kind='user_profile'"
            )
        ) <= {"provider_error"}
        assert set(
            row[0]
            for row in conn.execute(
                "SELECT error_summary FROM garmin_sync_runs WHERE error_summary IS NOT NULL"
            )
        ) <= {"provider_error"}
...
def test_account_identity_conflict_and_hmac_secret_isolation(tmp_path: Path) -> None:
    ...
    other = GarminCollectionTool(
        GarminConfig(config.database_path, config.raw_root, other_state, "2026-04-15"),
        transport,
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        conn.execute(
            "INSERT INTO data_subjects(subject_key,timezone,created_at_utc) VALUES('other','Asia/Hong_Kong','2026-01-01T00:00:00Z')"
        )
        ...
        conn.execute(
            "INSERT INTO subject_identities(subject_id,provider,identity_kind,identity_hmac,is_verified,first_seen_at_utc,last_seen_at_utc) VALUES(?, 'garmin','account',?,1,'2026-01-01T00:00:00Z','2026-01-01T00:00:00Z')",
            (other_subject, same),
        )
        conn.commit()
...
def test_capability_reprobe_single_failure_and_snapshot_partial(tmp_path: Path) -> None:
    ...
    failed = _account_sync(
        tool, "settings-missing", "user_profile", "user_profile_settings", "devices"
    )
    assert failed.status == "partial"
    with sqlite3.connect(config.database_path) as conn:
        state, probe = conn.execute(
            "SELECT capability_state,next_probe_at_utc FROM garmin_resource_capabilities WHERE resource_kind='user_profile_settings'"
        ).fetchone()
        assert state == "not_available" and probe is not None
        assert (
            conn.execute(
                "SELECT count(*) FROM source_revisions WHERE resource_kind='devices'"
            ).fetchone()[0]
            == 1
        )
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert {
            row[0]
            for row in conn.execute(
                "SELECT environment_key FROM garmin_resource_capabilities WHERE resource_kind='devices'"
            )
        } == {"global", "cn"}
    ...
    snap = tool.execute(
        SyncRequest(
            ...
            invocation_id="account-snapshot",
        )
    )
    ...
    with sqlite3.connect(config.database_path) as conn:
        assert (
            conn.execute(
                "SELECT count(*) FROM resource_coverage WHERE resource_kind='devices' AND availability_state='partial'"
            ).fetchone()[0]
            == 1
        )
        assert (
