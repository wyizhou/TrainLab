...
from src.mail_agent.gmail_adapter import (
    GmailAdapterError,
    GmailMCPAdapter,
    GmailMCPConfig,
    MCPCallError,
...
from src.mcp import MCPResponseError, StdioMCPClient, _safe_error_metadata
...
class FakeMCP:
    def __init__(
        self,
        *,
        email: str = "self@example.com",
        outcomes: dict[str, list[object]] | None = None,
        ...
        defaults = {
            ...
            "read_thread": {
                "messages": [
                    {
                        "message_id": "m1",
                        "thread_id": arguments.get("thread_id", "thread-1"),
                        "internal_date_utc": NOW,
                        "from": self.email,
                        "to": [self.email],
                        "subject": "test",
                        "headers": {
...
class FakeStdioClient:
    ...
    def __init__(
        ...
        *,
        ...
        call_outcomes: dict[str, list[object]] | None = None,
    ) -> None:
    ...
    def call_tool(self, name: str, arguments: dict):
        ...
        value = (
            scripted.pop(0)
            if scripted
            else {"structuredContent": {"email": "self@example.com"}}
        )
        if isinstance(value, Exception):
            raise value
        return value
...
def config(tmp_path: Path, *, configured: bool = True) -> GmailMCPConfig:
    token_dir = tmp_path / "secrets"
    token_dir.mkdir(mode=0o700, parents=True, exist_ok=True)
    oauth = token_dir / "oauth.json"
    token = token_dir / "token.json"
    oauth.write_text("not-read")
    token.write_text("not-read")
    oauth.chmod(0o600)
    token.chmod(0o600)
    value = {
        "schema_version": 1,
        ...
        "authenticated_self": "self@example.com",
...
def identity_db(
    tmp_path: Path, email: str = "self@example.com"
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "adapter-fixture", NOW))
        .status
        == "initialized"
    )
    ...
    conn.execute(
        "INSERT INTO subject_identities(subject_id,provider,identity_kind,identity_hmac,is_verified,first_seen_at_utc,last_seen_at_utc) VALUES(?,?,?,?,1,?,?)",
        (subject_id, "gmail", "email", hmac_identity(email), NOW, NOW),
    )
    conn.commit()
...
def adapter(tmp_path: Path, fake: FakeMCP) -> GmailMCPAdapter:
    return GmailMCPAdapter(
        config(tmp_path),
        transport_factory=lambda _: fake,
        identity_hmac=hmac_identity,
        sleep=lambda _: None,
        inline_retry_after_seconds=3,
    )
...
def test_mapping_identity_and_dto_allow_only_verified_self(tmp_path: Path) -> None:
    ...
    instance.apply_trainlab_label(
        message_id=sent.provider_message_id, thread_id=sent.provider_thread_id
    )
...
@pytest.mark.parametrize(
    "mutation", ["wrong_email", "missing_identity", "missing_tool", "bad_mapping"]
)
def test_prepare_failure_never_calls_search_read_or_send(
    tmp_path: Path, mutation: str
) -> None:
    conn, subject_id = identity_db(tmp_path)
    fake = FakeMCP(
        email="other@example.com" if mutation == "wrong_email" else "self@example.com",
        tool_names={"get_self", "search_messages", "read_thread", "send_html_self"}
        if mutation == "missing_tool"
        else None,
    )
    cfg = config(tmp_path)
    if mutation == "missing_identity":
        conn.execute("DELETE FROM subject_identities")
        conn.commit()
    if mutation == "bad_mapping":
        cfg.mapping["capabilities"]["search"]["tool"] = "delete_messages"
    instance = GmailMCPAdapter(
        cfg,
        transport_factory=lambda _: fake,
        identity_hmac=hmac_identity,
        sleep=lambda _: None,
    ...
    assert [
        ...
        if name
        in {"search_messages", "read_thread", "send_html_self", "create_or_apply_label"}
    ] == []


def test_read_only_errors_retry_at_most_three_and_401_refreshes_once(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
...
def test_rate_limit_and_forbidden_are_typed_and_do_not_retry_writes(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    waits: list[float] = []
    fake = FakeMCP(
        outcomes={
            "search_messages": [
                MCPCallError(429, retry_after_seconds=2),
                {"matches": []},
            ]
        }
    )
    ...
    assert [name for name, _ in fake.calls].count("search_messages") == 2
    fake = FakeMCP(
        outcomes={"search_messages": [MCPCallError(429, retry_after_seconds=9)]}
    )
    instance = adapter(tmp_path, fake)
    instance.prepare(conn, subject_id)
...
def test_bad_token_permissions_and_errors_do_not_disclose_token_or_body(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    cfg = config(tmp_path)
    ...
    fake = FakeMCP()
    instance = GmailMCPAdapter(
        cfg, transport_factory=lambda _: fake, identity_hmac=hmac_identity
    )
    with pytest.raises(GmailAdapterError) as error:
        instance.prepare(conn, subject_id)
    rendered = str(error.value).lower()
    assert (
        "token.json" not in rendered
        and "not-read" not in rendered
        and "private body" not in rendered
    )
    assert fake.calls == []
...
def test_provider_extra_tool_is_strictly_rejected_before_get_self(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    fake = FakeMCP(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [MCPCallError(429, retry_after_seconds=2), {"email": "self@example.com"}],
            None,
            2,
            0,
            [2.0],
...
def test_probe_is_persistable_and_contains_no_paths_or_secret_content(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    ...
    assert (
        probe.mapping_valid
        and probe.credential_paths_checked
        and probe.credential_permissions_ok
    )
    ...
    assert (
        ...
        and "token.json" not in rendered
        and "not-read" not in rendered
    )
...
def test_only_typed_search_intents_can_construct_queries(tmp_path: Path) -> None:
    ...
    assert fake.calls[-1][1] == {
        "query": 'subject:"mail:response:42"',
        "max_results": 10,
    }
    assert (
        instance.search_trainlab_window(
            start_date=date(2026, 7, 20), end_date=date(2026, 7, 23), max_results=50
        )
        == ()
    )
    assert fake.calls[-1][1] == {
        "query": "label:TrainLab after:2026/07/20 before:2026/07/23",
        "max_results": 50,
    }
    ...
    with pytest.raises(TypeError):
        instance.search_trainlab_window(
            start_date=date(2026, 7, 20), end_date=date(2026, 7, 21), label="Inbox"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            MCPResponseError(status_code=429, retry_after_seconds=17, kind="tool"),
            429,
            17,
            "tool",
        ),
        ...
        (
            RuntimeError("provider stderr token=secret body=private"),
            None,
            None,
            "transport",
        ),
...
def test_stdio_transport_preserves_only_structured_error_metadata(
    ...
    transport = StdioMailTransport(
        config(tmp_path), client_factory=lambda *_args, **_kwargs: client
    )
    with pytest.raises(MCPCallError) as raised:
        transport.call_tool("get_self", {})
    error = raised.value
    assert (error.status_code, error.retry_after_seconds, error.kind) == (
        status,
        retry_after,
        expected_kind,
    )
...
def test_stdio_client_converts_jsonrpc_and_tool_iserror_without_provider_text(
    ...
    class Pipe:
        ...
        def readline(self) -> str:
            return '{"jsonrpc":"2.0","id":1,"error":{"message":"token=secret", "data":{"status":429,"retryAfter":12}}}\n'
    ...
    client._next_id = 1
    monkeypatch.setattr(
        "src.mcp.select.select", lambda *_args: ([client.process.stdout], [], [])
    )
    ...
    client._request = lambda *_args: {
        "isError": True,
        "structuredContent": {"status": 403, "message": "private body"},
...
def test_stdio_client_allows_each_explicit_preamble_line_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def client(lines: list[str]) -> StdioMCPClient:
        value = object.__new__(StdioMCPClient)
        value.timeout = 1
        value.process = Process(lines)
        value._next_id = 1
        value._stdout_preamble_lines = {notice}
        return value

    monkeypatch.setattr(
        "src.mcp.select.select", lambda value, *_args: ([value[0]], [], [])
    )
    accepted = client(
    ...
    with pytest.raises(MCPResponseError) as raised:
        repeated._request("initialize", {})
...
def test_stdio_client_closes_child_when_initialize_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class Child:
    ...
    child = Child()
    monkeypatch.setattr("src.mcp.subprocess.Popen", lambda *_args, **_kwargs: child)
...
class _ProcessDouble:
    def __init__(
        self, *, exited: bool = False, kill_after_timeout: bool = False
    ) -> None:
        self.exited = exited
        ...
        self.stdin = _PipeDouble()
        self.stdout = _PipeDouble()
        self.stderr = _PipeDouble()
...
def test_stdio_client_close_reaps_and_closes_pipes_idempotently(
    ...
    client.close()
    assert (
        process.terminate_calls == expected_terminate
        and process.kill_calls == expected_kill
    )
...
def test_stdio_client_uses_devnull_for_provider_stderr(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    process = _ProcessDouble(exited=True)
    popen_calls: list[dict] = []
    monkeypatch.setattr(
        "src.mcp.subprocess.Popen",
...
def test_stdio_client_close_terminates_and_reaps_its_linux_process_group(
    ...
    monkeypatch.setattr(
        "src.mcp.os.killpg", lambda group, sig: signals.append((group, sig))
    )
    monkeypatch.setattr(
        "src.mcp._reap_process_group", lambda group: reaped.append(group)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"structuredContent": {"status": "503", "retry_after_seconds": "00003"}},
            (503, 3),
        ),
...
def test_safe_error_metadata_accepts_only_bounded_ascii_decimal_strings(
    payload: dict, expected: tuple[int | None, int | None]
) -> None:
    assert _safe_error_metadata(payload) == expected


def test_prepare_closes_real_transport_on_list_and_get_self_failures(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    changes_before = conn.total_changes
    list_client = FakeStdioClient(
        list_error=RuntimeError("provider stderr token=secret")
    )
    list_transport = StdioMailTransport(
        config(tmp_path), client_factory=lambda *_args, **_kwargs: list_client
    )
    instance = GmailMCPAdapter(
        config(tmp_path),
        transport_factory=lambda _cfg: list_transport,
        identity_hmac=hmac_identity,
    )
    ...
    assert (
        instance.capability_probe is None
        and instance._identity is None
        and instance._transport is None
    )
    assert (
        "secret" not in str(list_error.value).lower()
        and conn.total_changes == changes_before
    )
    self_client = FakeStdioClient(
        call_outcomes={"get_self": [MCPResponseError(status_code=401)]}
    )
    self_transport = StdioMailTransport(
        config(tmp_path), client_factory=lambda *_args, **_kwargs: self_client
    )
    instance = GmailMCPAdapter(
        config(tmp_path),
        transport_factory=lambda _cfg: self_transport,
        identity_hmac=hmac_identity,
    )
    with pytest.raises(GmailAdapterError, match="auth_required"):
        instance.prepare(conn, subject_id)
...
def test_close_and_failed_rebuild_clear_all_cached_boundary_state(
    tmp_path: Path,
) -> None:
    conn, subject_id = identity_db(tmp_path)
    healthy = FakeMCP()
...
def test_credential_paths_reject_symlink_non_file_and_non_directory_parent(
    tmp_path: Path,
) -> None:
    cfg = config(tmp_path)
    token = Path(cfg.mapping["credentials"]["token_path"])
    token.unlink()
    token.symlink_to(Path(cfg.mapping["credentials"]["oauth_client_path"]))
    ...
    alias.symlink_to(tmp_path / "ancestor" / "secrets", target_is_directory=True)
    ...
    parent_file.write_text("not-read")
