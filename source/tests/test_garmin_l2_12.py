...
def _fit_start(payload: bytes = FIT_BYTES) -> str:
...
class L212Transport:
    def __init__(
        self, *, sport: str = "running", fit: bytes | GarminError | None = None
    ) -> None:
        self.sport = sport
...
def _setup(
    tmp_path: Path, transport: L212Transport
) -> tuple[GarminCollectionTool, FoundationConfig]:
    root = tmp_path / "data"
    ...
    assert (
        ...
        .execute(FoundationRequest("init", "l2-12", "2026-01-01T00:00:00Z"))
        .status
        == "initialized"
    )
...
def test_applicable_enrichments_have_exact_resource_revision_role_and_reviewed_projection(
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        extras = json.loads(
            conn.execute("SELECT extras_json FROM activities").fetchone()[0]
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "split_summaries_json",
            GarminError("http_404", http_status=404),
            "not_available",
            0,
        ),
...
def test_empty_and_endpoint_failures_are_isolated_to_exact_enrichment(
    ...
    resource = dict(
        (role_value, resource_value)
        for resource_value, role_value in ACTIVITY_ENRICHMENTS
    )[role]
    tool, foundation = _setup(tmp_path, transport)
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert conn.execute(
            ...
            0 if resource == "activity_weather" and terminal not in {"revised"} else 1
        )
...
def test_no_fit_chart_is_bounded_partial_then_fit_atomically_becomes_canonical(
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        source_map = json.loads(
            conn.execute("SELECT source_map_json FROM activities").fetchone()[0]
        )
...
def test_reconciliation_tolerance_tri_state_end_inference_and_replay(
    ...
    transport = L212Transport(fit=FIT_BYTES)
    transport.summary.update(
        {
            "elapsedDuration": 4321.0,
            "duration": 4200.0,
            "movingDuration": 4100.0,
            "distance": 12344.0,
            "calories": 0,
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert activity[3] == pytest.approx(4321.0)
        assert activity[4] == pytest.approx(4200.0)
        ...
        assert postprocessed["fit"]["values"]["calories_kcal"]["value"] == 678
        count = conn.execute("SELECT count(*) FROM reconciliation_results").fetchone()[
            0
        ]
...
def test_conflicting_end_evidence_fails_closed_and_records_quality(
    ...
    with sqlite3.connect(foundation.database_path) as conn:
        ...
        assert (
            json.loads(activity[1])["end_time_inference"]["decision"] == "fail_closed"
        )
...
def test_json_routes_and_sets_are_revision_bound_but_fit_remains_canonical(
    ...
    fit_payload = SYNTHETIC_FITS[filename]
    transport = L212Transport(sport=sport, fit=fit_payload)
    transport.summary["startTimeGMT"] = _fit_start(fit_payload)
    tool, foundation = _setup(tmp_path, transport)
    assert (
        _sync(tool, f"fit-priority-{resource}", ("activity_fit", resource)).status
        == "succeeded"
    )
    with sqlite3.connect(foundation.database_path) as conn:
        source_map = json.loads(
...
def test_transient_fit_failure_does_not_implicitly_fetch_chart(
    ...
    result = tool.execute(
        SyncRequest(
            ...
            through_local_date="2026-07-19",
            resource_kinds=(),
            invocation_id="transient-no-chart",
        )
    )
    assert result.status == "partial"
...
def test_adapter_details_uses_reviewed_chart_bounds_and_missing_method_is_not_supported(
    ...
    client = Client()
    monkeypatch.setattr(
        GarminConnectTransport, "_apply_request_timeout", lambda self: None
    )
    adapter = GarminConnectTransport(
...
def test_unknown_only_json_correction_never_displaces_fit_canonical(
    ...
    fit_payload = SYNTHETIC_FITS["Bouldering.fit"]
    transport = L212Transport(sport="bouldering", fit=fit_payload)
    transport.summary["startTimeGMT"] = _fit_start(fit_payload)
    tool, foundation = _setup(tmp_path, transport)
    assert (
        _sync(
            tool,
            "fit-and-json-valid",
            ("activity_fit", "activity_typed_splits"),
        ).status
        == "succeeded"
    )
    transport.extras["typed_splits_json"] = {
