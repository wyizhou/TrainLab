...
def request() -> FoundationRequest:
    return FoundationRequest(
        mode="init",
        invocation_id="concurrency-test",
...
def test_initializing_recovery_competes_under_the_same_writer_lock(
    tmp_path: Path,
) -> None:
    ...
    assert (
        FoundationTool(config(root), failpoint=crash_after_schema)
        .execute(request())
        .status
        == "failed"
    )
