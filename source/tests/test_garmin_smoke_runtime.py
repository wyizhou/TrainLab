...
from src import garmin_smoke_runtime as runtime
from src.foundation_sample import generate_sample_database
from src.garmin_smoke_runtime import (
    SmokeRuntimeError,
    SmokeRuntimeRequest,
    _window,
    run_smoke,
...
def receipt(
    ...
    return {
        ...
        else f"gr-00000000-0000-4000-8000-{number:012x}",
        "mode": mode,
        "status": status,
        "requested_range": {
            "from": start if mode == "full" else None,
            "through": through,
        },
        "effective_range": {"from": start, "through": through},
        "coverage_state": "partial" if mode == "snapshot" else "complete",
        "counts": counts
        or {
            "fetched": 0,
            "empty": 0,
...
def setup(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (tokens / "token").write_text("x")
    os.chmod(tokens / "token", 0o600)
    config = tmp_path / "garmin.yaml"
    config.write_text(
        "garmin:\n  region: CN\n  request_min_interval_ms: 0\n  request_interval_jitter_ms: 0\n  request_timeout_seconds: 30\n  max_attempts: 1\n  retry_base_seconds: 1\n  retry_max_seconds: 1\n  inline_retry_after_max_seconds: 0\n  rate_limit_fallback_seconds: 1\n  max_repair_items_per_incremental: 1\n",
        encoding="utf-8",
    )
    return root, tokens
...
def request(root: Path, tokens: Path, **changes: object) -> SmokeRuntimeRequest:
    values: dict[str, object] = dict(
        isolated_root=root,
        production_token_store=tokens,
        approved_from="2026-07-22",
        approved_through="2026-07-23",
        invocation_ids=tuple(f"inv-{i:032x}" for i in range(8)),
        production_garmin_config=root.parent / "garmin.yaml",
        authorization_id="authz-" + "a" * 32,
        approved_modes=("auth", "incremental", "snapshot", "repair", "audit", "status"),
        bounded_activity_window_authorized=True,
        e04_backup_receipt_sha256="b" * 64,
        operator_registered=True,
        shadow_attested=True,
        canonical_owner_unchanged=True,
    )
    values.update(changes)
    return SmokeRuntimeRequest(**values)  # type: ignore[arg-type]
    ...
    def run(command, **_kwargs):
        ...
        start = (
            ...
            if mode in {"auth", "status"}
                ...
                actual["health_from_local_date"]
                or actual["snapshot_local_date"]
                or payload["config"]["approved_from"]
            )
        )
        through = (
            None
            if mode in {"auth", "status"}
            else (actual["through_local_date"] or actual["snapshot_local_date"])
        )
        number += 1
        return type(
            "Result",
            (),
            {
                "stdout": json.dumps(receipt(mode, start, through, number=number))
                + "\n",
                "returncode": 0,
            },
        )()
...
def test_clone_order_cleanup_and_safe_publish(tmp_path: Path):
    ...
    config_before = (root.parent / "garmin.yaml").read_bytes()
    ...
    assert (root.parent / "garmin.yaml").read_bytes() == config_before
    operations = json.loads((published / "operations.json").read_text())
    assert (
        operations["repeat_no_op"] is True
        and operations["snapshot_cursor_unchanged"] is True
    )
    assert (
        operations["incremental_repeat_no_op"] is True
        and operations["snapshot_repeat_no_op"] is True
    )
    assert (
        operations["snapshot_cursor_before_sha256"]
        == operations["snapshot_cursor_after_sha256"]
    )
    assert (
        operations["operations"][1]["request_sha256"]
        == operations["operations"][2]["request_sha256"]
    )
    assert (
        operations["operations"][3]["request_sha256"]
        == operations["operations"][4]["request_sha256"]
    )
...
def test_authz_missing_does_not_copy_token(tmp_path: Path):
    ...
    with pytest.raises(SmokeRuntimeError, match="smoke_authorization_incomplete"):
        run_smoke(
            request(root, tokens, bounded_activity_window_authorized=False),
...
def test_symlink_and_malicious_receipt_fail_without_result(tmp_path: Path):
    root, tokens = setup(tmp_path)
    shutil.rmtree(root / "foundation-shadow/raw")
    (root / "foundation-shadow/raw").symlink_to(tmp_path)
    with pytest.raises(SmokeRuntimeError):
        run_smoke(request(root, tokens), execute=lambda *_a, **_k: None)
    ...
    def bad(command, **_kwargs):
        ...
        return type(
            ...
            {
                "stdout": json.dumps(
                    receipt(payload["mode"], None, None, "succeeded")
                    | {"run_id": "raw-token"}
                )
                + "\n",
                "returncode": 0,
    ...
    with pytest.raises(SmokeRuntimeError):
        run_smoke(request(root, tokens), execute=bad)
...
def test_window_and_retry_after_failure(tmp_path: Path):
    ...
    with pytest.raises(SmokeRuntimeError):
        run_smoke(
            request(root, tokens, approved_through="2026-08-05"),
            execute=lambda *_a, **_k: None,
        )
    ...
    with pytest.raises(SmokeRuntimeError):
        run_smoke(
            request(root, tokens),
            execute=lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("unknown")),
        )
...
def test_fixed_bounded_sequence_has_no_full_and_shares_approved_start(tmp_path: Path):
    ...
    run_smoke(
        request(
            root, tokens, approved_from="2026-07-20", approved_through="2026-08-02"
        ),
        execute=fake(calls),
    )
    payloads = [
        json.loads(path.read_text())
        for path in sorted((root / "smoke-result").glob("request-*.json"))
    ]
    ...
    assert all(
        payload["request"]["through_local_date"] == "2026-08-02"
        and payload["config"]["history_start_date"] == "2026-07-20"
        and payload["config"]["approved_from"] == "2026-07-20"
        for payload in incrementals
    )
    snapshots = [payload for payload in payloads if payload["mode"] == "snapshot"]
    assert all(
        payload["request"]["snapshot_local_date"] == "2026-08-02"
...
def test_worker_refuses_tampered_full_payload(tmp_path: Path):
    ...
    payload.write_text(json.dumps({"mode": "full"}))
...
def test_progress_reports_all_stages_durations_and_total_elapsed(tmp_path: Path):
    ...
    result = run_smoke(
        request(root, tokens), execute=fake([]), monotonic=clock, progress=events.append
    )
    assert [event["event"] for event in events] == [
        value for _ in range(8) for value in ("stage_started", "stage_completed")
    ] + ["smoke_completed"]
    ...
    assert all(
        event["duration_seconds"] == 1.0 and event["status"] == "succeeded"
...
def test_progress_failure_is_safe_and_cli_keeps_stdout_receipt(
    ...
    spec = importlib.util.spec_from_file_location(
        "run_garmin_smoke_test", ROOT / "scripts/run_garmin_smoke.py"
    )
    ...
    args = [
        ...
        str(tokens),
        "--production-garmin-config",
        str(root.parent / "garmin.yaml"),
        "--from-date",
        "2026-07-22",
        "--through-date",
        "2026-07-23",
        "--authorization-id",
        "authz-" + "a" * 32,
        "--e04-backup-receipt-sha256",
        "b" * 64,
        "--operator-registered",
        "--shadow-attested",
        "--canonical-owner-unchanged",
        "--bounded-activity-window-authorized",
    ]
    for index in range(8):
        args.extend(["--invocation-id", f"inv-{index:032x}"])
    monkeypatch.setattr(sys, "argv", args)
    monkeypatch.setattr(
        module,
        "run_smoke",
        lambda _request, progress: (
            progress({"event": "smoke_completed", "total_elapsed_seconds": 1.0})
            or {"status": "succeeded", "total_elapsed_seconds": 1.0}
        ),
    )
    assert module.main() == 0
    captured = capsys.readouterr()
    assert captured.out.splitlines() == [
        '{"status": "succeeded", "total_elapsed_seconds": 1.0}'
    ]
    assert captured.err.splitlines() == [
        '{"event":"smoke_completed","total_elapsed_seconds":1.0}'
    ]
    monkeypatch.setattr(
        module,
        "run_smoke",
        lambda *_a, **_k: (_ for _ in ()).throw(RuntimeError("secret /private/path")),
    )
    ...
    assert (
        len(captured.err.splitlines()) == 1
        and "secret" not in captured.err
        and "path" not in captured.err
    )
    monkeypatch.setattr(
        module,
        "run_smoke",
...
def test_non_success_receipt_emits_failed_not_completed(tmp_path: Path):
    ...
    assert all(
        set(item) == {"resource", "code", "count", "status"}
        for item in events[-1]["gap_breakdown"]
    )
    assert all(
        set(item) == {"resource", "code", "count"}
        for item in events[-1]["failed_item_breakdown"]
    )
...
def test_duplicate_run_id_and_repeat_mutation_are_rejected(tmp_path: Path):
    ...
    def duplicate(command, **_kwargs):
        ...
        return type(
            ...
            (),
            {
                "stdout": json.dumps(receipt(mode, start, through, number=1)) + "\n",
                "returncode": 0,
            },
        )()
...
def test_real_timeout_kills_and_reaps_child(
    ...
    with pytest.raises(SmokeRuntimeError, match="child_timeout"):
        runtime._run_child(
            [sys.executable, "-c", "import time; time.sleep(3)"], tmp_path, 1
        )
    assert killed


def test_child_exception_also_kills_and_reaps(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    events: list[str] = []
...
def test_actual_request_is_validated_before_worker_and_publish_scans_token_hashes(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    root, tokens = setup(tmp_path)
    calls: list[object] = []
    ...
    root, tokens = setup(tmp_path / "scan")
    calls2: list[list[str]] = []
    ordinary = fake(calls2)
    ...
    def similar_but_not_credential_path(command, **kwargs):
        ...
        if len(calls5) == 1:
            directory = Path(kwargs["cwd"]) / "foundation/state/secrets/garmin-metadata"
            directory.mkdir()
    ...
    assert succeeded["status"] == "succeeded"
    assert (
        root / "smoke-result/foundation/state/secrets/garmin-metadata/marker.json"
    ).read_text() == "metadata"
