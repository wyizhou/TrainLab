...
from src.analysis.delivery import (
...
def accepted() -> dict[str, object]:
    return {
        "run_key": "analysis:1:daily:2026-07-24:one",
        "subject_id": 1,
        "mode": "daily",
        "status": "accepted",
        "artifacts": [
...
def published(connection: sqlite3.Connection):
    ...
    return AnalysisPublisher(connection, clock=lambda: "2026-07-24T00:00:00Z").publish(
        run_id=1,
        accepted=accepted(),
...
def test_accepted_artifacts_commit_before_pending_delivery_and_render_is_ephemeral():
    ...
    pending, rendered = AnalysisDeliveryFactory(
        connection, clock=lambda: "2026-07-24T00:01:00Z"
    ).prepare(publish_receipt=receipt, delivery_kind="daily_report")
    assert pending.delivery_kind == "daily_report" and pending.delivery_id > 0
    assert (
        connection.execute("SELECT status FROM analysis_deliveries").fetchone()[0]
        == "pending"
    )
    assert (
        connection.execute(
            "SELECT count(*) FROM analysis_delivery_artifacts"
        ).fetchone()[0]
        == 2
    )
    assert (
        connection.execute("SELECT count(*) FROM analysis_artifacts").fetchone()[0] == 2
    )
    assert (
        rendered.subject
        == "TrainLab｜每日训练简报｜回顾2026年7月23日｜安排2026年7月24日"
    )
...
def test_same_exact_revision_is_idempotent_and_does_not_duplicate_pending_delivery():
    ...
    second = factory.create_pending(
        publish_receipt=receipt, delivery_kind="daily_report"
    )
    assert (
        second.delivery_id == first.delivery_id
        and second.idempotency_key == first.idempotency_key
    )
    assert (
...
def test_delivery_remains_bound_to_exact_revision_after_current_drifts():
    ...
    connection.execute(
        ...
        (receipt.artifact_ids["daily_summary"],),
    )
    connection.execute(
        "INSERT INTO analysis_artifacts(subject_id,artifact_kind,period_start_local_date,period_end_local_date,revision_no,generated_by_run_id,schema_version,structured_content_json,user_visible_text,content_sha256,is_current,created_at_utc) VALUES(1,'daily_summary','2026-07-23','2026-07-23',2,1,'1','{}','new current','b',1,'2026-07-24T01:00:00Z')"
    )
    rows = connection.execute(
        "SELECT analysis_artifact_id FROM analysis_delivery_artifacts WHERE analysis_delivery_id=? ORDER BY ordinal",
        (pending.delivery_id,),
    ).fetchall()
    assert [row[0] for row in rows] == [
        receipt.artifact_ids["daily_summary"],
        receipt.artifact_ids["daily_training_advice"],
    ]
...
def test_html_renderer_escapes_untrusted_text_and_emits_no_links_or_remote_content():
    ...
    receipt = published(connection)
    connection.execute(
        "UPDATE analysis_artifacts SET user_visible_text=? WHERE id=?",
        (
            "<script>alert(1)</script><img src='https://bad.invalid/x'> <a href='https://bad.invalid'>x</a>",
            receipt.artifact_ids["daily_summary"],
        ),
    )
    connection.commit()
    ...
    rendered = render_delivery(pending)
    assert (
        "<script" not in rendered.html
        and "<img" not in rendered.html
        and "<a " not in rendered.html
    )
...
def test_render_failure_does_not_roll_back_published_artifacts_or_pending_delivery():
    ...
    with pytest.raises(RuntimeError, match="render failed"):
        AnalysisDeliveryFactory(connection).prepare(
...
def test_rest_day_omits_running_steps_and_malformed_structured_content_fails_closed() -> (
    None
):
    connection = database()
    receipt = published(connection)
    rest = {
        "primary_item": {"activity_kind": "rest"},
        "configured_difficulty_level": 2,
        "selected_session_difficulty_level": 1,
        "difficulty_adjustment_reason": "恢复优先",
        "confidence": "一般",
        "confidence_reason": "恢复优先",
        "data_limitation": None,
    }
    connection.execute(
        "UPDATE analysis_artifacts SET structured_content_json=? WHERE id=?",
        (json.dumps(rest), receipt.artifact_ids["daily_training_advice"]),
    )
    connection.commit()
    rendered = render_delivery(
        AnalysisDeliveryFactory(connection).create_pending(
            publish_receipt=receipt, delivery_kind="daily_report"
        )
    )
    assert "休息日" in rendered.html
    assert (
        "热身</td>" not in rendered.html
        and "主训练</td>" not in rendered.html
        and "放松</td>" not in rendered.html
    )
    connection.execute(
        "UPDATE analysis_artifacts SET structured_content_json='[' WHERE id=?",
        (receipt.artifact_ids["daily_summary"],),
...
def test_plan_revision_uses_weekly_design_for_only_its_existing_plan_items() -> None:
    connection = database()
    connection.execute(
        "UPDATE analysis_runs SET run_key='analysis:1:weekly:2026-07-26:revision',analysis_kind='weekly' WHERE id=1"
    )
    plan = {
        ...
        "items": [
            {
                "item_index": index,
                "local_date": f"2026-07-{26 + index:02d}"
                if index < 6
                else "2026-08-01",
                "activity_kind": "rest",
                "prescription": {"activity_kind": "rest"},
                "rationale_text": "修订后恢复",
                "stop_conditions": [],
            }
    ...
    connection.commit()
    pending = AnalysisDeliveryFactory(connection).create_pending(
        publish_receipt={"run_id": 1, "artifact_ids": {"weekly_training_plan": 12}},
        ...
        "items": [
            {
                ...
                "local_date": (
                    "2026-08-01"
                    if index == 5
                    else (f"2026-07-{26 + index:02d}" if index < 6 else "2026-07-31")
                ),
                "activity_kind": "rest",
                "prescription": {"activity_kind": "rest"},
                "rationale_text": "恢复",
