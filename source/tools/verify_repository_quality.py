...
_RUNTIME_TEMPLATES = (
    "src/email_templates/templates/daily_report.html",
    "src/email_templates/templates/weekly_report.html",
    "src/email_templates/templates/mail_reply.html",
)
...
def check_frozen_contracts(root: Path = ROOT) -> int:
    manifest = root / "src" / "resources" / "contracts" / "frozen_layer_contracts.json"
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    hashes = payload.get("contract_hashes", {})
    if not isinstance(hashes, dict) or any("orchestration" in key for key in hashes):
        raise QualityCheckError("retired_orchestration_contract_present")
    return len(hashes)


def check_email_templates(root: Path = ROOT) -> int:
    failures: list[str] = []
    for runtime_name in _RUNTIME_TEMPLATES:
        runtime = root / runtime_name
        if not runtime.is_file() or runtime.is_symlink():
            failures.append(f"template_missing_or_unsafe:{runtime_name}")
            continue
        payload = runtime.read_text(encoding="utf-8")
        if "<!doctype html>" not in payload.lower() or "<html" not in payload.lower():
            failures.append(f"template_html_invalid:{runtime_name}")
    if failures:
        raise QualityCheckError("email_template_check_failed\n" + "\n".join(failures))
    return len(_RUNTIME_TEMPLATES)
