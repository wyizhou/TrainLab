...
_RUN_STATUSES = frozenset(
    {"succeeded", "partial", "deferred", "failed", "auth_required", "lock_busy"}
)
_EXIT_BY_STATUS = {
    "succeeded": 0,
    "partial": 10,
...
def _scan_forbidden(value: object, path: str = "$") -> list[str]:
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            ...
            if lowered not in _APPROVED_SECURITY_WORDS_IN_KEYS and any(
                term in lowered for term in _FORBIDDEN_TERMS
            ):
...
def validate_smoke_evidence(evidence: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if (
        ...
        or window.get("timezone") != "Asia/Hong_Kong"
        or not _is_date(window.get("from_local_date"))
        or not _is_date(window.get("through_local_date"))
    ):
    ...
    if not isinstance(prerequisites, Mapping) or any(
        prerequisites.get(key) is not True for key in prerequisite_keys
    ):
    ...
    if not isinstance(operations, list) or not operations:
        ...
        tuple(
            operation.get("mode") if isinstance(operation, Mapping) else None
    ...
    for operation in operations:
        ...
        run_id_valid = mode_valid and (
            run_id is None if mode in {"auth", "status"} else _matches(run_id, _RUN_ID)
        )
        if (
            not mode_valid
            or not run_id_valid
        ...
        if not _is_nonnegative_finite_number(duration):
            ...
            operation_durations.append(float(cast(float, duration)))
        if (
            run_id is not None and run_id in by_run_id
        ...
        if not isinstance(effective, Mapping):
            ...
            start, end = (
                effective.get("from_local_date"),
                effective.get("through_local_date"),
            )
            if mode in {"incremental", "snapshot", "repair", "audit"}:
                if not _is_date(start) or not _is_date(end) or str(start) > str(end):
                    ...
                    and _is_date(window.get("from_local_date"))
                    and _is_date(window.get("through_local_date"))
                    and (
                        str(start) < str(window["from_local_date"])
                        or str(end) > str(window["through_local_date"])
                    )
                ):
                    errors.append("operation_outside_approved_window")
        ...
        if (
            ...
            and operation.get("outcome_handed_off") is not True
        ):
    ...
    if not _is_nonnegative_finite_number(total_elapsed):
        ...
        if float(
            cast(float, total_elapsed)
        ) + _DURATION_ROUNDING_TOLERANCE_SECONDS < sum(operation_durations):
    ...
    if not isinstance(repeat, Mapping) or set(repeat) != {"incremental", "snapshot"}:
        ...
        for repeat_mode in ("incremental", "snapshot"):
            ...
            if (
                not _matches(first, _RUN_ID)
                or not _matches(second, _RUN_ID)
                or first == second
            ):
                errors.append("repeat_run_ids_invalid")
                continue
            if (
                item.get("same_request_hash") is not True
                or item.get("no_op_confirmed") is not True
            ):
                errors.append("repeat_no_op_not_confirmed")
            if first not in by_run_id or second not in by_run_id:
                errors.append("repeat_run_not_recorded")
                continue
            one, two = by_run_id[first], by_run_id[second]
            if (
                one.get("mode") != repeat_mode
                or two.get("mode") != repeat_mode
                or one.get("request_sha256") != two.get("request_sha256")
                or one.get("status") != "succeeded"
                or two.get("status") != "succeeded"
            ):
                errors.append("repeat_request_mismatch")
    ...
    if (
        ...
        or any(
            type(structure.get(key)) is not int or structure[key] < 1
    ...
    capability_state = (
        capability.get("state") if isinstance(capability, Mapping) else None
    )
    if (
        not isinstance(capability, Mapping)
        or capability.get("checked") is not True
        or type(capability_state) is not str
        or capability_state
        not in {"not_available", "not_enabled", "not_supported", "available"}
    ):
        errors.append("capability_check_incomplete")

    safety = evidence.get("safety_checks")
    safety_keys = {
        "receipt_log_scan_clean",
        "run_table_scan_clean",
        "temporary_zip_cleaned",
        "no_daemon",
    ...
    if not isinstance(safety, Mapping) or any(
        safety.get(key) is not True for key in safety_keys
    ):
