...
def _safe_error_metadata(value: Any) -> tuple[int | None, int | None]:
    ...
    if isinstance(value, dict):
        candidates.extend(
            item
            for item in (value.get("data"), value.get("structuredContent"))
...
class StdioMCPClient:
    ...
    def __init__(
        ...
        timeout: int = 30,
        stdout_preamble_lines: tuple[str, ...] = (),
    ):
        ...
        if any(
            ...
            for line in stdout_preamble_lines
        ):
    ...
    def call_tool(self, name: str, arguments: dict[str, Any]) -> Any:
        ...
        if isinstance(result, dict) and result.get("isError") is True:
            ...
            raise MCPResponseError(
                status_code=status, retry_after_seconds=retry_after, kind="tool"
            )
        return result
