...
climbing or rest. Rest is a formal item. Do not schedule standalone strength,
although every recorded activity still contributes to total load and recovery
analysis. Do not prescribe a clock time. Do not consider a
...
`hansons_session_role`; rest items do not. Return a progression decision of
`advance`, `hold` or `deload`, with evidence and exactly one changed dimension
for the next week. Never increase distance and intensity together. Running SOS
and high-load climbing must respect the hard-load count and recovery interval.
