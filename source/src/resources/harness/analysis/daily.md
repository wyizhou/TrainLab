...
Advice must bind to the unique current plan item for the advice date and may
return `as_planned`, `modify_today`, `postpone` or `cancel`; it must not mutate
the formal plan or create a user fact. Missing activity evidence
...
The advice has exactly one prescribed item: running, climbing or rest, with
structured reason and stop conditions. Running includes source-qualified
heart-rate or RPE/talk-test guidance. A climbing plan item is represented only
at course level with duration, RPE, load and recovery cost; it is never mapped
to a Garmin running/aerobic course. Standalone strength and unplanned
cross-training are not prescribed. Red flags require rest or suspension and
professional-care guidance. Record the configured target difficulty and the session difficulty
...
- The advice starts with exactly `今日跑步。`, `今日攀岩。` or `今日休息。`; no
  gym-strength or cross-training instruction appears unless it is an explicitly
  configured future activity.
