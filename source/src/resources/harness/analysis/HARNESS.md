...
status. Each date may have at most one prescribed primary item: running,
climbing or rest. Standalone strength is not scheduled in v2. Do not choose a
clock time.
...
- Analyze every recorded activity type, including climbing, cycling and gym
  ...
  recovery. Future plans may contain running, climbing or rest, but never a
  standalone strength item. Climbing is a course-level load entry and is never
  disguised as running or mapped automatically to a Garmin aerobic workout.
- Running SOS and high-load climbing are both hard-load sessions. Enforce the
  configured hard-session maximum and recovery gap deterministically; the
  model may explain the decision but may not override the gate.
