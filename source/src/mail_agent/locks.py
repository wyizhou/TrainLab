...
class MailWriteLock:
    ...
    def _directory_fd(self) -> int:
        ...
        state = parent.parent
        state_fd = _private_directory(state)
        locks_fd = -1
        try:
            try:
                ...
                info, after = (
                    os.fstat(locks_fd),
                    os.stat(parent.name, dir_fd=state_fd, follow_symlinks=False),
                )
    ...
    def _payload(self) -> bytes:
        if (
            not isinstance(self.run_key, str)
            or not self.run_key
            or len(self.run_key) > 256
        ):
    ...
    def _read_existing(
        self, directory_fd: int, name: str | None = None
    ...
    def _claim_owned(
        self, directory_fd: int, raw: bytes, inode: tuple[int, int]
    ) -> str | None:
        ...
        try:
            os.rename(
                self.path.name, claim, src_dir_fd=directory_fd, dst_dir_fd=directory_fd
            )
            renamed = True
    ...
    def _finalize_verified_claim(
        self, directory_fd: int, claim: str, raw: bytes, inode: tuple[int, int]
    ) -> None:
    ...
    def _cleanup_created(
        self, directory_fd: int, inode: tuple[int, int], raw: bytes
    ) -> None:
    ...
    def _has_claim_blocker(self, directory_fd: int) -> bool:
        try:
            info = os.fstat(directory_fd)
            if (info.st_dev, info.st_ino) in _POISONED_LOCK_DIRS:
                return True
    ...
    def acquire(self) -> None:
        ...
        try:
            ...
            if created_inode is not None:
                self._cleanup_created(directory_fd, created_inode, created_raw)
