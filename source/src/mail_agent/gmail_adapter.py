...
_ARGUMENT_KEYS = {
    ...
    "send_html_self": frozenset(
        {"run_id", "subject", "plain_text", "html", "thread_id"}
    ),
...
class GmailMCPConfig:
    ...
    def load(cls, config_path: Path, schema_path: Path) -> "GmailMCPConfig":
        ...
        if not isinstance(value, dict) or list(
    ...
    def validate(self) -> None:
        value = self.mapping
        if (
            not value.get("configured")
            or value.get("server_name") != "gmail"
            or value.get("provider") != "trainlab_gmail_v1"
        ):
            raise GmailAdapterError("gmail_mapping_invalid")
        ...
        if (
            ...
            or transport.get("env") != {}
        ):
            raise GmailAdapterError("gmail_mapping_invalid")
        requirements = value.get("requirements")
        if (
            not isinstance(requirements, dict)
            or requirements.get("authenticated_self_only") is not True
            or requirements.get("label") != "TrainLab"
        ):
            raise GmailAdapterError("gmail_mapping_invalid")
        ...
        for capability, expected_tool in _TOOLS.items():
            ...
            if (
                ...
                or item.get("tool") != expected_tool
        ...
        if (
            ...
            or capabilities["search"]["arguments"]
            != {"query": "{query}", "max_results": "{max_results}"}
            or capabilities["create_or_apply_label"]["arguments"].get("label")
            != "{label}"
        ):
            raise GmailAdapterError("gmail_tool_mapping_invalid")
        ...
        *,
        ...
        try:
            self._client = client_factory(
                ...
                [str(item) for item in transport["args"]],
                env={},
...
class GmailMCPAdapter:
    def __init__(
        ...
        *,
        transport_factory: Callable[
            [GmailMCPConfig], RestrictedMCPTransport
        ] = StdioMailTransport,
        identity_hmac: Callable[[str], str],
        sleep: Callable[[float], None] = time.sleep,
        inline_retry_after_seconds: int = 5,
    ) -> None:
        self.config = config
    ...
    def prepare(self, connection: sqlite3.Connection, subject_id: int) -> GmailIdentity:
        ...
        try:
            ...
            oauth_mode, oauth_dir_mode, token_mode, token_dir_mode = (
                self.config.credential_permission_modes()
            )
            self._transport = self._transport_factory(self.config)
            available = {
                str(item.get("name"))
                for item in self._transport.list_tools()
                if item.get("name")
            }
            ...
            row = connection.execute(
                "SELECT id FROM subject_identities WHERE subject_id=? AND provider='gmail' AND identity_kind='email' AND identity_hmac=? AND is_verified=1",
                (subject_id, actual_hmac),
            ).fetchone()
            ...
            self._capability_probe = GmailCapabilityProbe(
                ...
                config_path_sha256=hashlib.sha256(
                    str(self.config.config_path.resolve()).encode("utf-8")
                ).hexdigest(),
                mapping_valid=True,
    ...
    @staticmethod
    def _raise_terminal_transport_error(error: MCPCallError) -> Never:
        ...
        if error.status_code == 429:
            raise GmailAdapterError(
                "rate_limited", retry_at_seconds=error.retry_after_seconds
            ) from None
        raise GmailAdapterError("gmail_transport_failed") from None
        ...
        *,
        ...
        if isinstance(start_date, datetime):
            ...
            if days < 1 or days > _MAX_DISCOVERY_WINDOW_DAYS:
                raise GmailAdapterError("gmail_search_intent_invalid")
            query = (
                f"label:TrainLab after:{start_date:%Y/%m/%d} before:{end_date:%Y/%m/%d}"
            )
    ...
    def read_thread_evidence(self, provider_thread_id: str) -> GmailThreadEvidence:
        ...
        if not isinstance(raw_messages, list):
            raise GmailAdapterError("gmail_thread_result_invalid")
        messages = tuple(
            self._message(item, provider_thread_id)
    ...
    def send_html_self(
        ...
        *,
        ...
        return SendReceipt(
            ...
            bool(result.get("already_sent", False)),
        )
    ...
    def _get_self(self) -> GmailIdentity:
        ...
        email = (
            str(result.get("email", "")).strip().lower()
            if isinstance(result, dict)
        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                return dotted_get(
                    normalize_tool_result(
                        self._transport.call_tool(mapping["tool"], arguments)
                    ),
                    mapping.get("result_path"),
                )
            except MCPCallError as error:
                if error.status_code == 401:
                ...
                if error.status_code == 429:
                    ...
                    if wait is None or wait > self._inline_retry_after_seconds:
                        raise GmailAdapterError(
                            "rate_limited", retry_at_seconds=wait
                        ) from error
                ...
                if (
                    ...
                    and attempts < 3
                    and (
                        error.kind in {"timeout", "transport"}
    ...
    def _message(value: dict[str, Any], fallback_thread_id: str) -> GmailMessage:
        ...
        attachments: list[object] = (
            attachments_value if isinstance(attachments_value, list) else []
        )
        recipients_value = value.get("to")
        if isinstance(recipients_value, list):
            recipients: list[object] = recipients_value
        elif recipients_value:
            recipients = [recipients_value]
        else:
            recipients = []
        references_value = headers.get("references")
        references = references_value if isinstance(references_value, list) else []
        label_values = value.get("label_ids")
        label_ids = label_values if isinstance(label_values, list) else []
        return GmailMessage(
            provider_message_id=str(value.get("message_id") or value.get("id") or ""),
            provider_thread_id=str(
                value.get("thread_id") or value.get("threadId") or fallback_thread_id
            ),
            received_at_utc=value.get("received_at_utc")
            or value.get("internal_date_utc"),
            sender=value.get("from"),
            recipients=tuple(str(item) for item in recipients if item),
            subject=value.get("subject"),
            message_id_header=headers.get("message_id"),
            in_reply_to=headers.get("in_reply_to"),
            references=tuple(str(item) for item in references),
            trainlab_run_id=headers.get("x_trainlab_run_id"),
            label_ids=tuple(str(item) for item in label_ids if item),
            body_text=str(value.get("plain_text") or value.get("body_text") or ""),
            has_html=bool(value.get("has_html")),
