...
def _sum_counts(receipts: list[MailReceipt]) -> MailCounts:
    ...
    return MailCounts(
        **{
            name: sum(getattr(item.counts, name) for item in receipts)
            for name in fields
        }
    )
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        ...
        return MailReceipt(
            ...
            errors=(
                {
                    "stage": "application",
                    "code": "mail_application_dependencies_unavailable",
                    "summary": "mail tool is not configured",
                },
            ),
...
class MailApplicationService:
    ...
    def _execute_stage(self, stage: MailStage, request: MailRequest) -> MailReceipt:
        ...
        if (
            ...
            return self._failed_receipt(
                request, "stage", "mail_application_stage_receipt_invalid"
            )
        return receipt
    ...
    def _run(self, request: MailRequest) -> MailReceipt:
        ...
        if self._deadline_reached(begun, deadline):
            errors.append(
                {
                    "stage": "run",
                    "code": "mail_run_deadline_reached",
                    "summary": "mail run stopped at deadline",
                }
            )
            return self._run_receipt(
                request,
        ...
        try:
            queued = self._stages.queue.queued_message_ids(
                subject_id=request.subject_id, limit=max_items
            )
        except Exception as error:
            errors.append(
                {
                    ...
                    "summary": "queued items unavailable",
                }
            )
            return self._run_receipt(
                request,
        ...
        if not isinstance(queued, tuple) or any(
            not isinstance(item, str) or not item for item in queued
        ):
            errors.append(
                {
                    "stage": "queue",
                    "code": "mail_application_queue_invalid",
                    "summary": "queued items unavailable",
                }
            )
            return self._run_receipt(
        ...
        for message_id in sorted(queued)[:max_items]:
            if self._deadline_reached(begun, deadline):
                ...
                break
            process_request = self._child(
                request, "process", mail_message_ids=(message_id,)
            )
            ...
            for response_id in process_receipt.mail_response_artifact_ids:
                if self._deadline_reached(begun, deadline):
                    ...
                    break
                delivery_request = self._child(
                    request,
                    "deliver_response",
                    mail_response_artifact_ids=(response_id,),
                )
                delivery_receipt = self._execute_stage(
                    self._stages.deliver, delivery_request
                )
                ...
                if delivery_receipt.next_action == "reconcile_delivery":
                    for delivery_id in delivery_receipt.mail_delivery_ids:
                        reconcile_request = self._child(
                            request, "reconcile", mail_delivery_ids=(delivery_id,)
                        )
                        reconcile_receipt = self._execute_stage(
                            self._stages.reconcile, reconcile_request
                        )
        ...
        return self._run_receipt(
            ...
            poll_state,
            errors,
            force_partial=self._deadline_reached(begun, deadline),
        )

    def _run_limits(self, request: MailRequest) -> tuple[int, int]:
        max_items = (
            self._policy.default_max_items
            if request.max_items is None
            else request.max_items
        )
        deadline = (
            self._policy.default_deadline_seconds
            if request.deadline_seconds is None
            else request.deadline_seconds
        )
        if (
            not isinstance(max_items, int)
            or not 1 <= max_items <= self._policy.max_max_items
        ):
            raise MailApplicationError("mail_run_max_items_invalid")
        if (
            not isinstance(deadline, int)
            or not 1 <= deadline <= self._policy.max_deadline_seconds
        ):
    ...
    def _run_receipt(
        ...
        *,
        ...
        if force_partial or (
            statuses - {"succeeded", "unchanged"}
            and statuses & {"succeeded", "unchanged"}
        ):
            status = "partial"
    ...
    def _failed_receipt(
        self, request: MailRequest, stage: str, code: str
    ) -> MailReceipt:
        run_key = (
            request.run_key
            if request.mode == "status" and request.run_key
            ...
            errors=(
                {
                    "stage": stage,
                    "code": code,
                    "summary": "mail stage did not complete",
                },
            ),
            started_at_utc=request.requested_at_utc,
            completed_at_utc=self._clock(),
