...
from src.integrations.project_config import configured_recipient_email
...
class MailEnvironmentFactory(Protocol):
    ...
    def __call__(
        self, foundation: FoundationConfig, *, recipient_address: str
    ) -> Any: ...
        ...
        errors=(
            {
                "stage": "prepare",
                "code": code,
                "summary": "mail environment is not ready",
            },
        ),
...
def _foundation_lock_busy_receipt(
    ...
    requested = datetime.fromisoformat(observed_at_utc.replace("Z", "+00:00"))
    retry_at = (requested + _FOUNDATION_RETRY_DELAY).isoformat().replace("+00:00", "Z")
    run_key = (
        request.run_key
        if request.mode == "status" and request.run_key
        else request.stable_run_key
...
def _default_environment_factory() -> MailEnvironmentFactory | None:
    ...
    try:
        ...
        return None
    return create_environment_adapter
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        try:
            ...
            status = FoundationTool(foundation).execute(
                FoundationRequest(
                    "status", "mail-runtime-status", request.requested_at_utc
                )
            )
        ...
        try:
            ...
            try:
                ...
                return _receipt(request, "mail_runtime_configuration_invalid")
            factory = (
                self._dependencies.environment_factory or _default_environment_factory()
            )
            if factory is None:
    ...
    def _compose(
        ...
        poll = PollStage(
            MailPollService(
                ...
                adapter,
                MailPollConfig(
                    foundation.raw_root, foundation.state_root / "locks" / "mail.lock"
                ),
        ...
        if request.mode in {"process", "run"}:
            process: Any = ProcessStage(
                repository,
                self._dependencies.context_factory(repository.connection),
                self._dependencies.runner_factory(),
                self._dependencies.gate_factory(),
                MailResponsePublisher(repository),
                self._local_today,
            )
        else:
        ...
        return MailApplicationService(
            MailApplicationStages(
                ...
                status=StatusStage(repository),
                queue=QueueStage(repository),
            ),
            clock=self._dependencies.clock,
        )
