...
from .repository import EventDraft, MailRepository, utc_now
...
class MailDeliveryRepository:
    ...
    def _load_by_delivery_id(
        ...
        if (
            row["status"]
            not in {"pending", "sending", "sent", "already_sent", "delivery_unknown"}
    ...
    def record_provider_receipt(
        ...
        *,
        ...
        if status not in {"sent", "already_sent"} or not isinstance(
            label_applied, bool
        ):
        ...
        with self.repository._write_transaction():
            ...
            if label_applied:
                self._upsert_item(
                    target, "verify", "succeeded", outbound_id=outbound_id
                )
                pending = False
            else:
                if not self._canonical_utc(label_retry_at_utc):
                    raise MailDeliveryRepositoryError(
                        "mail_delivery_label_retry_required"
                    )
                self._upsert_item(
                    target,
                    "verify",
            ...
            return PersistedDeliveryReceipt(
                target.delivery_id, outbound_id, status, pending
            )
        ...
        *,
        ...
        with self.repository._write_transaction():
            ...
            if target.status == "delivery_unknown":
                ...
                if changed != 1:
                    raise MailDeliveryRepositoryError(
                        "mail_delivery_trigger_state_invalid"
                    )
                ...
                raise MailDeliveryRepositoryError(
                    "mail_delivery_reconcile_state_invalid"
                )
            ...
            self._assert_verified_delivery(target)
            return PersistedDeliveryReceipt(
                target.delivery_id, outbound_id, "sent", False
            )
    ...
    def mark_delivery_conflict(
        ...
        with self.repository._write_transaction():
            ...
            if persisted.status in {"sent", "already_sent", "delivery_unknown"}:
                raise MailDeliveryRepositoryError(
                    "mail_delivery_conflict_state_invalid"
                )
            self.connection.execute(
            ...
            self._upsert_item(persisted, "send", "failed", error_code=error_code)

    def _persist_outbound_message(
        self, target: AcceptedDeliveryTarget, provider_message_id: str, sent_at_utc: str
    ) -> int:
        rows = self.connection.execute(
    ...
    def _record_sent_event(
        ...
        run = self.connection.execute(
            "SELECT * FROM mail_agent_runs WHERE run_key=?", (target.run_key,)
        ).fetchone()
        if run is None:
        ...
        event_id = self.repository._create_or_recover_event(
            ...
            EventDraft(
                ...
                outbound_id,
                "system_generated",
                related_run_key=target.run_key,
    ...
    def _upsert_item(
        ...
        *,
        ...
        run = self.connection.execute(
            "SELECT id FROM mail_agent_runs WHERE run_key=?", (target.run_key,)
        ).fetchone()
        if run is None:
            raise MailDeliveryRepositoryError("mail_delivery_run_missing")
        row = self.connection.execute(
        ...
        if row is not None:
            ...
            self.connection.execute(
                ...
                (
                    status,
                    error_code,
                    "mail delivery did not complete" if error_code else None,
                    next_retry_at_utc,
                    self._clock() if status != "running" else None,
                    row["id"],
                ),
            )
            return
    ...
    def _assert_verified_delivery(self, target: AcceptedDeliveryTarget) -> None:
        if not self.repository._message_has_verified_delivery(
            target.subject_id, target.trigger_message_id
        ):
