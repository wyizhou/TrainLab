...
from .contracts import MailCounts, MailReceipt, MailRequest, mail_status, utc_now
...
def _failure(request: MailRequest, code: str, *, stage: str = "runtime") -> MailReceipt:
    ...
    return MailReceipt(
        ...
        errors=(
            {"stage": stage, "code": code, "summary": "mail stage did not complete"},
        ),
...
class ProcessStage:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        try:
            ...
            if row is None:
                return _failure(
                    request, "mail_process_message_not_found", stage="process"
                )
            ...
            self.repository.set_message_processing_state(
                request.subject_id, message_id, "analyzing"
            )
            command = parse_fact_browser_command(
                context["trigger_message"].get("latest_authored_text")
            )
            ...
            if decision.status == "dependency_wait":
                return self._dependency_wait(
                    request, message_id, decision.reason_event_id, run_id=run.id
                )
            ...
            return MailReceipt(
                ...
                invocation_id=request.invocation_id,
                mode="process",
                status="unchanged" if published.unchanged else "succeeded",
                counts=MailCounts(
                    processed=1, responses_accepted=0 if published.unchanged else 1
                ),
                processed_message_ids=request.mail_message_ids,
    ...
    def _code(error: Exception) -> str:
        ...
        return (
            value
            if isinstance(value, str) and value.startswith("mail_")
            else "mail_process_failed"
        )
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        try:
            ...
            result = self.service.deliver(
                subject_id=request.subject_id, response_artifact_id=response_id
            )
        except Exception as error:
            return _failure(
                request,
                getattr(error, "code", "mail_delivery_failed"),
                stage="delivery",
            )
        status = (
            "succeeded"
        ...
        return MailReceipt(
            ...
            if result.error_code is None
            else (
                {
                    ...
                    "summary": "delivery requires recovery",
                },
            ),
            started_at_utc=request.requested_at_utc,
            completed_at_utc=utc_now(),
...
class PreparedStage:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        try:
            ...
            return _failure(
                request,
                getattr(error, "code", "mail_delivery_identity_unverified"),
                stage="prepare",
            )
...
class ReconcileStage:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        try:
            ...
            delivery_id = (
                int(request.mail_delivery_ids[0]) if request.mail_delivery_ids else 0
            )
            if delivery_id <= 0:
            ...
            target = self.delivery_repository._load_by_delivery_id(
                request.subject_id, delivery_id
            )
            ...
            if visible.delivery_status == "label_pending":
                decision = recover_label_only(
                    ...
                    provider_message_id=self._stored_provider_message_id(delivery_id),
                )
                if (
                    decision.action != "apply_label"
                    or decision.provider_message_id is None
                ):
            ...
            reconcile_decision = reconcile_unknown_delivery(
                expected_run_id=target.idempotency_key,
                expected_thread_id=target.provider_thread_id,
                candidates=candidates,
            )
            if (
                reconcile_decision.action != "record_sent"
                or reconcile_decision.provider_message_id is None
            ):
                return self._deferred(request, delivery_id, reconcile_decision.code)
            self.adapter.apply_trainlab_label(
                message_id=reconcile_decision.provider_message_id,
                thread_id=target.provider_thread_id,
            )
            self.delivery_repository.reconcile_provider_receipt(
                ...
                delivery_id,
                identity_id,
                provider_message_id=reconcile_decision.provider_message_id,
                provider_thread_id=target.provider_thread_id,
                sent_at_utc=utc_now(),
            )
            return self._success(request, delivery_id, "succeeded")
            ...
            return self._deferred(
                request,
                request.mail_delivery_ids[0] if request.mail_delivery_ids else "",
                getattr(error, "code", "mail_reconcile_failed"),
            )
    ...
    def _success(request: MailRequest, delivery_id: int, status: str) -> MailReceipt:
        return MailReceipt(
            ...
            mode="reconcile",
            status=mail_status(status),
            mail_delivery_ids=(str(delivery_id),),
            next_action="none",
            started_at_utc=request.requested_at_utc,
            completed_at_utc=utc_now(),
        )
    ...
    @staticmethod
    def _deferred(
        request: MailRequest, delivery_id: object, code: object
    ) -> MailReceipt:
        safe = (
            code
        ...
        return MailReceipt(
            ...
            status="deferred",
            counts=MailCounts(deferred=1),
            mail_delivery_ids=() if not delivery_id else (str(delivery_id),),
            next_action="operator_review",
            errors=(
                {
                    "stage": "reconcile",
                    "code": safe,
                    "summary": "delivery recovery requires review",
                },
            ),
            started_at_utc=request.requested_at_utc,
            completed_at_utc=utc_now(),
