...
def _search_ids(value: Any) -> tuple[str, ...]:
    ...
    for row_value in rows:
        if not isinstance(row_value, dict):
            raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
        candidate_value = row_value.get(
            "id", row_value.get("messageId", row_value.get("message_id"))
        )
        message_ids.append(
            _identifier(candidate_value, code="gmail_reply_protocol_invalid")
        )
...
def _addresses(value: object, *, required: bool = False) -> tuple[str, ...]:
    ...
    if value in (None, "") or value == []:
        if required:
            raise GmailEnvironmentAdapterError(
                "gmail_reply_thread_participants_invalid"
            )
        return ()
...
def _thread_has_authorized_recipient_target(
    ...
    *,
    ...
    if (
        ...
        or value.get("threadId", value.get("thread_id")) != thread_id
    ):
    ...
    for message in messages:
        ...
        observed_message = message.get(
            "messageId", message.get("message_id", message.get("id"))
        )
        ...
        if observed_thread != thread_id:
            raise GmailEnvironmentAdapterError("gmail_reply_thread_mismatch")
        sender = _addresses(
            message.get("from"), required=observed_message == message_id
        )
        recipients = _addresses(
            message.get("to"), required=observed_message == message_id
        )
        ...
        if observed_message == message_id:
            ...
            if marker is not None:
                ...
                if not any(isinstance(body, str) and marker in body for body in bodies):
                    raise GmailEnvironmentAdapterError(
                        "gmail_reply_idempotency_unverified"
                    )
    if not exact_found:
...
def _sent_message(value: Any) -> str:
    ...
    if not isinstance(value, dict):
        raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
    return _identifier(
        value.get("id", value.get("messageId", value.get("message_id"))),
        code="gmail_reply_protocol_invalid",
    )
...
def _label_id(value: Any) -> str:
    ...
    if not isinstance(value, dict):
        raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
    return _identifier(
        value.get("id", value.get("labelId", value.get("label_id"))),
        code="gmail_reply_protocol_invalid",
    )
...
def _modified(value: Any, *, message_id: str) -> None:
    ...
    if (
        not isinstance(value, dict)
        or value.get("id", value.get("messageId", value.get("message_id")))
        != message_id
    ):
        raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
...
class GmailEnvironmentRecipientAdapter:
    ...
    def _provision_identity(self, connection: sqlite3.Connection) -> int:
        ...
        try:
            digest = hmac.new(
                self._identity_key(), self._recipient.encode("ascii"), hashlib.sha256
            ).hexdigest()
            connection.execute("BEGIN IMMEDIATE")
            subject = connection.execute(
                "SELECT id FROM data_subjects WHERE id=?", (_MAIL_SUBJECT_ID,)
            ).fetchone()
            if subject is None:
                raise GmailEnvironmentAdapterError("gmail_reply_identity_unavailable")
            ...
            if connection.in_transaction:
                connection.rollback()
            raise GmailEnvironmentAdapterError(
                "gmail_reply_identity_unavailable"
            ) from None
    ...
    def _identity_key(self) -> bytes:
        ...
        try:
            ...
            try:
                ...
                temporary = (
                    secrets_dir
                    / f".{_IDENTITY_KEY_FILENAME}.{secrets.token_hex(16)}.tmp"
                )
    ...
    def read_thread_evidence(self, provider_thread_id: str) -> GmailThreadEvidence:
        thread_id = _identifier(
            provider_thread_id, code="gmail_reply_thread_id_invalid"
        )
        client = self._open_client()
        try:
            raw = _result(
                self._read_call(
                    client, "get_thread", {"threadId": thread_id, "format": "full"}
                )
            )
            if not isinstance(raw, dict):
                raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
            canonical_raw = self._canonical_raw_thread(
                raw, expected_thread_id=thread_id
            )
            return GmailThreadEvidence(
                self._normalized_thread(canonical_raw, expected_thread_id=thread_id),
                canonical_raw,
            )
        except GmailEnvironmentAdapterError:
            raise
    ...
    def search_run_id(
        ...
        try:
            matches = _search_ids(
                _result(
                    self._read_call(
                        client,
                        "search_emails",
                        {"query": f'in:sent "{marker}"', "maxResults": max_results},
                    )
                )
            )
            if len(matches) > 1:
            ...
            self._validate_thread(
                client, thread_id=expected_thread, message_id=matches[0], marker=marker
            )
    ...
    def send_html_recipient(
        ...
        *,
        ...
        message_id = _identifier(
            in_reply_to_provider_message_id, code="gmail_reply_message_id_invalid"
        )
            ...
            matches = _search_ids(
                _result(
                    self._read_call(
                        client,
                        "search_emails",
                        {"query": f'in:sent "{marker}"', "maxResults": 2},
                    )
                )
            )
            if len(matches) > 1:
    ...
    def apply_trainlab_label(
        ...
        try:
            label = _label_id(
                _result(client.call_tool("get_or_create_label", {"name": "TrainLab"}))
            )
            _modified(
                _result(
                    client.call_tool(
                        "modify_email",
                        {"messageId": exact_message, "addLabelIds": [label]},
                    )
                ),
    ...
    def _open_client(self) -> GmailEnvironmentClient:
        ...
        if not isinstance(status, GmailEnvironmentStatus) or not status.available:
            if isinstance(status, GmailEnvironmentStatus) and status.code in {
                "gmail_mcp_not_configured",
                "gmail_mcp_binding_invalid",
            }:
    ...
    def _validate_thread(
        ...
        *,
        ...
        _thread_has_authorized_recipient_target(
            thread,
            thread_id=thread_id,
    ...
    def _thread_search_results(
        self, client: GmailEnvironmentClient, value: Any
    ) -> tuple[dict[str, Any], ...]:
        rows = (
            value.get("emails", value.get("messages", value.get("results")))
        ...
        for row in rows:
            if not isinstance(row, dict):
                raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
            message = _identifier(
                row.get("id", row.get("messageId", row.get("message_id"))),
                code="gmail_reply_protocol_invalid",
            )
            candidate_thread = row.get("threadId", row.get("thread_id"))
            if candidate_thread is None:
                detail = _result(
                    self._read_call(client, "read_email", {"messageId": message})
                )
                if isinstance(detail, dict):
                    candidate_thread = detail.get("threadId", detail.get("thread_id"))
                elif isinstance(detail, str):
                    match = re.search(
                        r"^Thread ID: ([A-Za-z0-9_-]{1,256})$", detail, re.MULTILINE
                    )
    ...
    @staticmethod
    def _normalized_thread(
        raw: dict[str, Any], *, expected_thread_id: str
    ) -> GmailThread:
        thread_id = _identifier(
            raw.get("threadId", raw.get("thread_id")),
            code="gmail_reply_protocol_invalid",
        )
        ...
        for row in raw["messages"]:
            if not isinstance(row, dict):
                raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
            message_id = _identifier(
                row.get("messageId", row.get("message_id", row.get("id"))),
                code="gmail_reply_protocol_invalid",
            )
            row_thread = _identifier(
                row.get("threadId", row.get("thread_id")),
                code="gmail_reply_protocol_invalid",
            )
            ...
            recipient_values = (
                ...
                else tuple(recipients)
                if isinstance(recipients, list)
                else ()
            )
            if (
                sender is not None
                and not isinstance(sender, str)
                or any(not isinstance(item, str) for item in recipient_values)
            ):
                raise GmailEnvironmentAdapterError("gmail_reply_protocol_invalid")
            ...
            messages.append(
                GmailMessage(
                    ...
                    row.get("headers", {}).get("message_id")
                    if isinstance(row.get("headers", {}).get("message_id"), str)
                    else None,
                    row.get("headers", {}).get("in_reply_to")
                    if isinstance(row.get("headers", {}).get("in_reply_to"), str)
                    else None,
                    tuple(row.get("headers", {}).get("references", ()))
                    if isinstance(row.get("headers", {}).get("references", ()), list)
                    else (),
                    row.get("headers", {}).get("x_trainlab_run_id")
                    if isinstance(row.get("headers", {}).get("x_trainlab_run_id"), str)
                    else None,
                    label_values,
                    body,
                    bool(row.get("has_html")),
                    (),
                )
            )
        return GmailThread(thread_id, tuple(messages))

    def _canonical_raw_thread(
        self, raw: dict[str, Any], *, expected_thread_id: str
    ) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            received = row.get(
                "internal_date_utc",
                row.get("received_at_utc", row.get("receivedAt", row.get("date"))),
            )
            ...
            if (
                (sender is not None and not isinstance(sender, str))
                or not isinstance(recipients, list)
                or any(not isinstance(v, str) for v in recipients)
            ...
            if (
                (message_header is not None and not isinstance(message_header, str))
            ...
            output.append(
                {
                    "message_id": message_id,
                    "thread_id": thread_id,
                    "internal_date_utc": canonical_time,
                    "from": sender,
                    "to": recipients,
                    ...
                    "headers": {
                        ...
                        "x_trainlab_run_id": run_id,
                    },
                    "label_ids": labels,
                    "plain_text": body,
                    "has_html": bool(row.get("has_html", row.get("htmlBody"))),
    ...
    def _read_call(
        self, client: GmailEnvironmentClient, name: str, arguments: dict[str, Any]
    ) -> Any:
        ...
        for attempt in range(5):
            try:
                ...
                retryable = translated.code in {
                    "gmail_reply_transport_error",
                    "gmail_reply_timeout",
                    "gmail_reply_rate_limited",
                    "gmail_reply_provider_error",
                }
                ...
                if (
                    ...
                    or attempt == 4
                    or (
                        translated.code == "gmail_reply_rate_limited"
                        and (wait is None or wait > 120)
                    )
                ):
                ...
                if translated.code == "gmail_reply_rate_limited" and wait:
                    ...
                    self._sleep(
                        float(2**attempt)
                        + max(0.0, float(self._jitter(float(2**attempt))))
                    )
    ...
    def _thread_from_run_id(run_id: str) -> str:
        match = re.fullmatch(
            r"mail:response:[1-9][0-9]*:([A-Za-z0-9_-]{1,256})", run_id
        )
        if match is None:
            raise GmailEnvironmentAdapterError("gmail_reply_idempotency_invalid")
        return match.group(1)
    ...
    def _translate(error: Exception) -> GmailEnvironmentAdapterError:
        ...
        if isinstance(error, MCPResponseError):
            ...
            if error.status_code == 429:
                return GmailEnvironmentAdapterError(
                    "gmail_reply_rate_limited",
                    retry_at_seconds=error.retry_after_seconds,
                )
...
def create_environment_adapter(
    ...
    return GmailEnvironmentRecipientAdapter(
        recipient_address, identity_state_root=state_root
    )
