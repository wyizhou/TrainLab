...
def _parser() -> argparse.ArgumentParser:
    return _add_commands(
        argparse.ArgumentParser(
            prog="trainlab mail", description="TrainLab fourth-layer mail tool"
        )
    )


def add_root_subparser(
...
def request_from_args(
    args: argparse.Namespace, *, requested_at_utc: str | None = None
) -> MailRequest:
    common = {
    ...
    if args.mode == "process":
        return MailRequest(
            mode="process",
            mail_message_ids=(args.message_id,),
            dependency_analysis_artifact_ids=tuple(args.dependency_artifact_id),
            regeneration_reason_code=args.regenerate_reason,
            **common,
        )
    if args.mode == "deliver-response":
        return MailRequest(
            mode="deliver_response",
            mail_response_artifact_ids=(args.response_id,),
            **common,
        )
    ...
    if args.mode == "status":
        ...
        return MailRequest(
            mode="status", mail_message_ids=message_ids, run_key=args.run_key, **common
        )
    raise ValueError("invalid_mail_mode")


def mail_cli_execute(
    args: argparse.Namespace,
    *,
    tool: MailTool | None = None,
    requested_at_utc: str | None = None,
) -> MailReceipt:
    return (tool or MailTool()).execute(
        request_from_args(args, requested_at_utc=requested_at_utc)
    )
