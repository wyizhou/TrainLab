...
from src.process_liveness import process_group_has_live_members
from src.resources import resource_root
from src.runtime_environment import bounded_runtime_path
...
class MailRunnerError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        safe = (
            code
            if isinstance(code, str) and code.startswith("mail_")
            else "mail_runner_internal"
        )
        super().__init__(safe)
        self.code = safe
        self.stage = stage
...
class _Process(Protocol):
    ...
    def communicate(
        self, input: str | bytes | None, timeout: float | None = None
    ...
    def store(self, invocation_id: str, record: AcceptedRecord) -> None:
        with self._lock:
            ...
            if existing is not None and existing != record:
                raise MailRunnerError(
                    "mail_codex_cache_identity_conflict", stage="recovery"
                )
...
def _safe_read(
    ...
    *,
    ...
    try:
        ...
        if root_info.st_uid not in {0, os.geteuid()} or not _secure_mode(
...
def _stable_secure_read(
    path: Path, identity: _FileIdentity, limit: int, code: str
) -> bytes:
    ...
    try:
        ...
        try:
            ...
            confirmed_total = 0
            while True:
                block = os.read(fd, min(65_536, limit + 1 - confirmed_total))
                if not block:
                    break
    ...
    try:
        ...
        try:
            ...
            while offset < len(data):
                ...
                if written <= 0:
                    raise MailRunnerError(
                        "mail_secure_write_failed", stage="filesystem"
                    )
                offset += written
            os.fsync(fd)
            info = os.fstat(fd)
...
def _private_directories(root: Path, relative_parent: Path) -> None:
    ...
    try:
        ...
        raise MailRunnerError(
            "mail_secure_directory_failed", stage="filesystem"
        ) from None
...
class MailHarnessResolver:
    ...
    def package(self) -> tuple[MailHarnessBundle, dict[str, bytes]]:
        ...
        identity = b"".join(
            path.encode() + b"\0" + files[path] + b"\0" for path in _PACKAGE
        )
        return MailHarnessBundle(
            _PACKAGE, hashes, _hash(identity), "mail-package-v3"
        ), files
...
class MailResultValidator:
    def __init__(
        ...
        *,
        ...
        try:
            ...
            input_roles = tuple(
                input_schema["$defs"]["manifest"]["properties"]["input_role"]["enum"]
            )
            output_roles = tuple(
                output_schema["$defs"]["source_usage"]["properties"]["input_role"][
                    "enum"
                ]
            )
            ...
            self.input = Draft202012Validator(
                input_schema, format_checker=FormatChecker()
            )
            self.output = Draft202012Validator(
    ...
    def input_context(self, value: Any) -> None:
        ...
        try:
            ...
            raise MailRunnerError(
                "mail_input_semantic_invalid", stage="input"
            ) from None

    def result(self, value: Any, context: dict[str, Any]) -> None:
        self._require(self.output, value, "mail_result_schema_invalid")
        if (
            value["run_key"] != context["run"]["run_key"]
            or value["trigger_message_id"] != context["trigger_message"]["id"]
        ):
        ...
        if (
            ...
            or (
                action not in {"reply", "await_analysis"}
                and (response is not None or request is not None)
            )
        ):
        ...
        for item in value["source_usage"]:
            ...
            if (
                ...
                raise MailRunnerError(
                    "mail_result_source_usage_invalid", stage="output"
                )
            used.add(item["ordinal"])
        if not any(
            by_ordinal[number]["input_role"] == "trigger_message" for number in used
        ):
            raise MailRunnerError(
                "mail_result_source_usage_trigger_missing", stage="output"
            )
        used_mail_ids = {
        ...
        authored: dict[int, list[str]] = {
            context["trigger_message"]["id"]: [
                context["trigger_message"]["latest_authored_text"]
            ]
        }
        for message in context["thread_context"]:
            if (
                message["actor_role"] == "user"
                and message["value_origin"] == "user_asserted"
            ):
        ...
        def validate_evidence(candidate: dict[str, Any], code: str) -> None:
            ...
            if source_id not in used_mail_ids or not any(
                ...
                raise MailRunnerError(
                    code, stage="output", field_path=("evidence_text_span",)
                )
...
class MailCodexRunner:
    ...
    def __init__(
        ...
        *,
        ...
        if executable is not None and (
            not isinstance(executable, Path) or not executable.is_absolute()
        ):
            raise MailRunnerError(
                "mail_codex_executable_invalid", stage="configuration"
            )
        if auth_source is not None and (
            not isinstance(auth_source, Path) or not auth_source.is_absolute()
        ):
    ...
    def _prepare_auth(self, home: Path) -> None:
        try:
            ...
            raise MailRunnerError(
                "mail_codex_auth_invalid", stage="configuration"
            ) from None
    ...
    def _argv(self, output_schema: Path, output: Path) -> list[str]:
        executable = (
            "__test_codex__" if self.process_factory else self._validated_executable()
        )
        return [
            executable,
            "exec",
    ...
    def _spawn(
        ...
        if self.process_factory is not None:
            try:
                ...
                if not isinstance(process.pid, int) or not hasattr(
                    process, "communicate"
                ):
                ...
                raise MailRunnerError(
                    "mail_codex_start_failed", stage="process", retryable=True
                ) from None
        executable_fd = -1
        try:
    ...
    def _force_terminate_group(process: _Process) -> None:
        ...
        if not isinstance(process, subprocess.Popen):
            try:
                ...
                raise MailRunnerError(
                    "mail_codex_cleanup_failed", stage="process"
                ) from None
        ...
        try:
            if self.process_factory is not None:
                try:
                    ...
                    raise MailRunnerError(
                        "mail_codex_timeout", stage="process", retryable=True
                    ) from None
                if (
                    ...
                    raise MailRunnerError("mail_codex_process_invalid", stage="process")
                text_streams = cast(tuple[str, str], streams)
                encoded = [item.encode("utf-8") for item in text_streams]
                if (
                    any(len(item) > _MAX_CAPTURE for item in encoded)
                    or sum(map(len, encoded)) > _MAX_CAPTURE_TOTAL
                ):
                ...
                try:
                    ...
                    while offset < len(data):
                        if capture_reader_failed.is_set():
                            raise MailRunnerError(
                                "mail_codex_capture_read_failed", stage="process"
                            )
                        if capture_overflow.is_set():
                            raise MailRunnerError(
                                "mail_codex_capture_limit", stage="process"
                            )
                    ...
                    raise MailRunnerError(
                        "mail_codex_stdin_failed", stage="process", retryable=True
                    ) from None
                while True:
                    if capture_reader_failed.is_set():
                        raise MailRunnerError(
    ...
    @staticmethod
    def _build_prompt(
        files: dict[str, bytes], bundle: MailHarnessBundle, context: dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            raise MailRunnerError(
                "mail_harness_encoding_invalid", stage="package"
            ) from None
    ...
    def accepted_store_result(
        self, invocation_id: str, identity: str, generation: MailGeneration
    ) -> None:
        try:
            self.accepted_store.store(
                invocation_id, AcceptedRecord(identity, generation)
            )
        except MailRunnerError:
            raise
        except Exception:
    ...
    def _cleanup_work(work: Path) -> None:
        try:
            ...
            raise MailRunnerError(
                "mail_codex_temp_cleanup_failed", stage="cleanup"
            ) from None
        ...
        *,
        ...
        try:
            ...
            self.rejections.append(
                MailRejection(exc.stage, exc.code, exc.retryable, exc.field_path)
            )
            raise
        except Exception:
    ...
    def _generate(
        ...
        *,
        ...
        if regeneration_reason is not None and (
            ...
            or not 1 <= len(regeneration_reason) <= 256
        ):
            raise MailRunnerError(
                "mail_codex_regeneration_reason_invalid", stage="input"
            )
        bundle, files = self.resolver.package()
        validator = MailResultValidator(
            input_bytes=files[_INPUT], output_bytes=files[_OUTPUT]
        )
        ...
        if invocation_id != context_invocation_id:
            raise MailRunnerError(
                "mail_codex_invocation_identity_invalid", stage="input"
            )
        ...
        try:
            try:
                ...
                raise MailRunnerError(
                    "mail_codex_temp_create_failed", stage="filesystem"
                ) from None
            for relative, data in files.items():
                ...
                copied = _safe_read(
                    work, relative, exact_mode=mode, max_bytes=len(data)
                )
                if copied != data or _hash(copied) != _hash(data):
                    raise MailRunnerError(
                        "mail_package_copy_invalid", stage="filesystem"
                    )
            ...
            if returncode != 0:
                raise MailRunnerError(
                    "mail_codex_nonzero", stage="process", retryable=True
                )
            raw = _safe_output_read(output, output_identity)
            result = _strict_json_bytes(
                raw, "mail_codex_output_not_single_json", max_bytes=_MAX_OUTPUT
            )
            validator.result(result, context)
            generation = MailGeneration(
                result, _hash(_canonical(result).encode()), bundle
            )
        finally:
            if work is not None:
