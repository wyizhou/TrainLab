...
from src.util import require_lastrowid
...
if TYPE_CHECKING:
    from .fact_gate import FactGateDecision, FactGateEvidence, MailFactGate
...
_FOUNDATION_INDEX_METADATA = {
    "idx_source_revisions_current": (
        "source_revisions",
        False,
        False,
        ("provider", "resource_kind", "provider_object_id", "is_current"),
    ),
    "ux_source_revision_current": (
        "source_revisions",
        True,
        True,
        ("provider", "resource_kind", "provider_object_id"),
    ),
    "ux_activity_source_role_active": (
        "activity_source_revisions",
        True,
        True,
        ("activity_id", "source_role"),
    ),
    "ux_daily_health_current": (
        "daily_health",
        True,
        True,
        ("subject_id", "local_date"),
    ),
    "ux_analysis_artifact_current": (
        "analysis_artifacts",
        True,
        True,
        (
    ...
    "ux_mail_response_current": (
        "mail_response_artifacts",
        True,
        True,
        ("subject_id", "mail_thread_id", "response_kind"),
    ),
    "ux_garmin_sync_gap_unresolved": (
        "garmin_sync_gaps",
        True,
        True,
        (
    ...
    "idx_coverage_resource_date": (
        "resource_coverage",
        False,
        False,
        ("resource_kind", "local_date"),
    ),
    "idx_activity_samples_stream": (
        "activity_samples",
        False,
        False,
...
_RECEIPT_CODES = frozenset(
    {
        ...
        "provider_page_limit_unsplittable",
        "provider_thread_id_invalid",
        "gmail_message_time_invalid",
        "mail_cursor_invalid",
        "raw_archive_failed",
        "max_threads_reached",
        "archive_or_normalize_failed",
    }
)
...
_RECEIPT_STAGES = frozenset(
    {"lock", "prepare", "poll", "replay", "discover", "archive", "normalize", "tracked"}
)
...
_DEFERRED_EVIDENCE = frozenset(
    {
        ("discover", "max_threads_reached"),
        ("discover", "provider_page_limit_unsplittable"),
        ("discover", "rate_limited"),
        ("archive", "rate_limited"),
    }
)
...
_FAILED_EVIDENCE = frozenset(
    {
        ("poll", "poll_failed"),
        ("poll", "terminal_persistence_failed"),
        ("prepare", "gmail_adapter_failed"),
        ("prepare", "gmail_transport_failed"),
        ("prepare", "forbidden"),
        ("prepare", "rate_limited"),
        ("prepare", "identity_mismatch"),
        ("prepare", "gmail_mapping_invalid"),
        ("prepare", "gmail_tools_unavailable"),
        ("prepare", "verified_identity_required"),
        ("archive", "archive_or_normalize_failed"),
        ("archive", "gmail_adapter_failed"),
        ("archive", "gmail_transport_failed"),
        ("archive", "forbidden"),
    }
)
_PARTIAL_EVIDENCE = frozenset(
    {
        ("discover", "max_threads_reached"),
        ("discover", "provider_page_limit_unsplittable"),
        ("discover", "provider_thread_id_invalid"),
        ("discover", "gmail_adapter_failed"),
        ("discover", "gmail_transport_failed"),
        ("discover", "forbidden"),
        ("discover", "rate_limited"),
        ("archive", "archive_or_normalize_failed"),
        ("archive", "gmail_adapter_failed"),
...
def _utc(value: object) -> datetime | None:
    ...
    if (
        ...
        or instant.astimezone(timezone.utc).isoformat().replace("+00:00", "Z") != value
    ):
        return None
...
def _canonical_snapshot(value: dict[str, object]) -> tuple[str, str]:
    try:
        ...
        digest = hashlib.sha256(encoded.encode("utf-8", errors="strict")).hexdigest()
...
def _canonical_request_identity(
    ...
    encoded, digest = _canonical_snapshot({"mail_request_identity": identity})
...
def validate_poll_receipt(receipt: dict[str, object]) -> None:
    ...
    for name in ("run_key", "mail_agent_run_id", "invocation_id"):
        if not isinstance(receipt.get(name), str) or not _SAFE_ID.fullmatch(
            cast(str, receipt[name])
        ):
            raise MailRepositoryError("mail_poll_receipt_shape_invalid")
    if (
        receipt.get("status") not in _POLL_PUBLIC_TO_RUN_STATUS
        or receipt.get("next_action") not in _NEXT_ACTIONS
    ):
        raise MailRepositoryError("mail_poll_receipt_shape_invalid")
    ...
    for name in ("warnings", "errors"):
        ...
        for item in values:
            if not isinstance(item, dict) or set(item) != {"stage", "code", "summary"}:
                raise MailRepositoryError("mail_poll_receipt_shape_invalid")
            if (
                item["stage"] not in _RECEIPT_STAGES
                or item["code"] not in _RECEIPT_CODES
                or _PUBLIC_SUMMARIES.get((item["stage"], item["code"]))
                != item["summary"]
            ):
                raise MailRepositoryError("mail_poll_receipt_shape_invalid")
    ...
    for item in states:
        if not isinstance(item, dict) or set(item) != {
            ...
            raise MailRepositoryError("mail_poll_receipt_shape_invalid")
        if (
            item["stream"] not in {"trainlab_label", "tracked_threads"}
            or item["status"] not in {"succeeded", "partial", "deferred"}
            or not _canonical_utc(item["window_start_utc"])
            or not _canonical_utc(item["window_end_utc"])
        ):
            raise MailRepositoryError("mail_poll_receipt_shape_invalid")
        window_start = _utc(item["window_start_utc"])
        window_end = _utc(item["window_end_utc"])
        if (
            item["stream"] in streams
    ...
    for item in pending:
        if (
            ...
            or set(item) != {"mode", "reason_event_id", "artifact_id"}
            or item["mode"] not in {"revise-plan"}
            or not all(
                isinstance(item[key], str) and _SAFE_ID.fullmatch(item[key])
                for key in item
            )
        ):
            raise MailRepositoryError("mail_poll_receipt_shape_invalid")
    if receipt.get("next_retry_at_utc") is not None and not _canonical_utc(
        receipt.get("next_retry_at_utc")
    ):
        raise MailRepositoryError("mail_poll_receipt_shape_invalid")
    started, completed = (
        _utc(receipt.get("started_at_utc")),
        _utc(receipt.get("completed_at_utc")),
    )
    ...
    if terminal_success:
        if (
            streams != {"trainlab_label", "tracked_threads"}
            or any(item["status"] != "succeeded" for item in states)
            or errors
            or receipt["warnings"]
            or action != "none"
            or receipt["next_retry_at_utc"] is not None
            or counts["failed"]
            or counts["deferred"]
        ):
        ...
        if (
            streams != {"trainlab_label", "tracked_threads"}
            or not any(item["status"] == "partial" for item in states)
            or not errors
            or not evidence <= _PARTIAL_EVIDENCE
            or counts["failed"] + counts["deferred"] == 0
            or action != "continue_poll"
            or receipt["next_retry_at_utc"] is not None
        ):
            raise MailRepositoryError("mail_poll_receipt_semantics_invalid")
        ...
        if (
            not errors
            or not evidence <= _DEFERRED_EVIDENCE
            or counts["deferred"] == 0
            or counts["failed"] != 0
            or action != "continue_poll"
            or receipt["next_retry_at_utc"] is None
            or (states and not any(item["status"] == "deferred" for item in states))
        ):
            raise MailRepositoryError("mail_poll_receipt_semantics_invalid")
        ...
        if (
            streams
            or not errors
            or evidence != _AUTH_EVIDENCE
            or counts["failed"] == 0
            or action != "reauthenticate"
            or receipt["next_retry_at_utc"] is not None
        ):
            raise MailRepositoryError("mail_poll_receipt_semantics_invalid")
    elif status == "failed":
        if (
            streams
            or not errors
            or not evidence <= _FAILED_EVIDENCE
            or counts["failed"] == 0
            or action != "operator_review"
            or receipt["next_retry_at_utc"] is not None
        ):
            raise MailRepositoryError("mail_poll_receipt_semantics_invalid")
...
def safe_error_summary(value: str | None) -> str | None:
    ...
    cleaned = _URL.sub(
        "<redacted-url>",
        _EMAIL.sub("<redacted-email>", value.replace("\n", " ").replace("\r", " ")),
    )
    return cleaned[:160]
...
class MailRepository:
    ...
    def _verify_foundation_schema(self) -> None:
        ...
        try:
            state = self.connection.execute(
                "SELECT state,schema_version,manifest_sha256 FROM foundation_state WHERE id=1"
            ).fetchone()
            manifest = json.loads(
                resource_bytes("harness/schemas/foundation_schema_manifest.json")
            )
            ...
            for name, definition in manifest["tables"].items():
                if name == "schema_migrations":
                    expected_definition: str | None = (
                        "version INTEGER PRIMARY KEY, description TEXT NOT NULL, applied_at_utc TEXT NOT NULL, code_revision TEXT NOT NULL DEFAULT 'foundation-v1', content_sha256 TEXT NOT NULL"
                    )
                else:
                    expected_definition = _FOUNDATION_TABLES.get(name)
                if (
                    expected_definition is None
                    or objects.get(name, (None, None, None))[0] != "table"
                ):
                    raise MailRepositoryError("foundation_schema_incompatible")
                actual_sql = objects[name][2]
                if not isinstance(actual_sql, str) or self._normal_sql(
                    actual_sql
                ) != self._normal_sql(f"CREATE TABLE {name} ({expected_definition})"):
                    raise MailRepositoryError("foundation_schema_incompatible")
            for name, query in _FOUNDATION_VIEWS.items():
                ...
                if (
                    ...
                    or self._normal_sql(actual[2])
                    != self._normal_sql(f"CREATE VIEW {name} AS {query}")
                ):
                    raise MailRepositoryError("foundation_schema_incompatible")
        ...
        if (
            ...
            raise MailRepositoryError("foundation_schema_incompatible")
        if not _FoundationTool._migration_receipt_valid(
            self.connection, state["manifest_sha256"]
        ):
        ...
        try:
            ...
            try:
                ...
                raise MailRepositoryError(
                    "mail_repository_constraint_violation"
                ) from exc
            except Exception:
                if self.connection.in_transaction:
                    self.connection.execute("ROLLBACK")
    ...
    def _item(row: sqlite3.Row) -> MailItem:
        return MailItem(
            row["id"],
            row["mail_agent_run_id"],
            row["logical_item_kind"],
            row["logical_item_id"],
            row["stage"],
            row["status"],
            row["attempt_count"],
    ...
    def start_or_resume_run(self, request: MailRequest) -> MailRun:
        ...
        with self._write_transaction():
            existing = self.connection.execute(
                "SELECT * FROM mail_agent_runs WHERE invocation_id=?",
    ...
    def get_run(self, run_key: str) -> MailRun | None:
        self._verify_foundation_schema()
        row = self.connection.execute(
            "SELECT * FROM mail_agent_runs WHERE run_key=?", (run_key,)
        ).fetchone()
        ...
        with self._write_transaction():
            ...
            if row["status"] != "started":
                if (
                    ...
                    or row["context_snapshot_json"] != encoded
                ):
    ...
    def finish_poll_with_receipt(
        self, run_id: int, status: str, receipt: dict[str, object]
    ) -> MailRun:
        ...
        with self._write_transaction():
            ...
            if (
                not isinstance(public_status, str)
                or _POLL_PUBLIC_TO_RUN_STATUS.get(public_status) != status
            ):
            ...
            if row["status"] != "started":
                if (
                    ...
                    or row["completed_at_utc"] != completed
                    or row["next_retry_at_utc"] != next_retry
                ):
                    raise MailRepositoryError("conflicting_mail_poll_receipt")
                return self._run(row)
            if existing_receipt is not None and existing_receipt != receipt:
    ...
    def get_poll_cursor(
        self, subject_id: int, identity_id: int, stream_kind: str
    ) -> MailPollCursor | None:
        self._verify_foundation_schema()
        if (
            stream_kind not in {"trainlab_label", "tracked_threads"}
            or subject_id <= 0
            or identity_id <= 0
        ):
        ...
        if row is None:
            return None
        observed = _utc(row["observed_through_utc"])
        overlap = _utc(row["overlap_start_utc"])
        if (
            observed is None
            or overlap is None
            or overlap > observed
            or not isinstance(row["last_successful_run_id"], int)
        ):
            raise MailRepositoryError("invalid_mail_poll_cursor")
        return MailPollCursor(*tuple(row))

    def advance_poll_cursor(
        self,
        run_id: int,
        subject_id: int,
        identity_id: int,
        ...
        *,
        ...
        if (
            not isinstance(stream_completed, bool)
            or not stream_completed
            or stream_kind not in {"trainlab_label", "tracked_threads"}
            or not _canonical_utc(observed_through_utc)
            or not _canonical_utc(overlap_start_utc)
            or (
                (overlap := _utc(overlap_start_utc)) is None
                or (observed := _utc(observed_through_utc)) is None
                or overlap > observed
            )
        ):
            raise MailRepositoryError("invalid_mail_poll_cursor")
        with self._write_transaction():
            run = self.connection.execute(
                "SELECT subject_id,request_kind,status FROM mail_agent_runs WHERE id=?",
                (run_id,),
            ).fetchone()
            identity = self.connection.execute(
                "SELECT subject_id,provider,identity_kind,is_verified FROM subject_identities WHERE id=?",
                (identity_id,),
            ).fetchone()
            if (
                run is None
                or run["subject_id"] != subject_id
                or run["request_kind"] not in {"poll", "run"}
                or run["status"] not in {"started", "partial", "deferred"}
                or identity is None
                or identity["subject_id"] != subject_id
                or identity["provider"] != "gmail"
                or identity["identity_kind"] != "email"
                or identity["is_verified"] != 1
            ):
            ...
            if previous is not None:
                ...
                current_observed = _utc(observed_through_utc)
                if (
    ...
    def set_message_processing_state(
        ...
        *,
        ...
        with self._write_transaction():
            row = self.connection.execute(
                "SELECT m.id,m.processing_state FROM mail_messages m JOIN mail_threads t ON t.id=m.mail_thread_id WHERE m.id=? AND t.subject_id=?",
            ...
            graph = {
                "new": {"discovered"},
                "discovered": {"archived", "ignored", "failed"},
                "archived": {"normalized", "ignored", "failed"},
                "normalized": {
                    "queued",
                    "store_only",
                    "ignored",
                ...
                "queued": {"analyzing", "deferred", "rejected", "failed"},
                "analyzing": {"response_accepted", "deferred", "rejected", "failed"},
                "awaiting_analysis": {"queued", "deferred", "rejected", "failed"},
                "deferred": {
                    "queued",
                    "analyzing",
                    "awaiting_analysis",
                ...
                "response_accepted": {"ready_to_send", "store_only", "failed"},
                "ready_to_send": {"sending", "store_only", "failed"},
            ...
            if current == "delivery_unknown":
                permitted = recovery_kind == "reconcile" and state in {
                    "sent",
                    "deferred",
                    "failed",
                }
                ...
                permitted = recovery_kind == "normal" and state in graph.get(
                    current, set()
                )
            if not permitted:
        ...
        *,
        ...
        if (
            ...
            or (
                ...
                and not _SAFE_ID.fullmatch(provider_thread_id)
            )
            or (sent_at_utc is not None and not _canonical_utc(sent_at_utc))
            or (
                last_verified_at_utc is not None
                and not _canonical_utc(last_verified_at_utc)
            )
        ...
        with self._write_transaction():
            ...
            if current == "delivery_unknown":
                permitted = recovery_kind == "reconcile" and status in {
                    "sent",
                    "already_sent",
                    "failed",
                }
                ...
                permitted = recovery_kind == "normal" and status in normal.get(
                    current, set()
                )
            if not permitted:
            ...
            if row["mail_message_id"] is not None and mail_message_id not in {
                None,
                row["mail_message_id"],
            }:
                raise MailRepositoryError("conflicting_mail_delivery_message")
            if row["provider_thread_id"] is not None and provider_thread_id not in {
                None,
                row["provider_thread_id"],
            }:
                raise MailRepositoryError("conflicting_mail_delivery_thread")
            if effective_thread_id is not None and row["response_thread_id"] not in {
                None,
                effective_thread_id,
            }:
            ...
            stored_summary = (
                None if error_code is None else "mail delivery did not complete"
            )
            self.connection.execute(
            ...
            return MailDeliveryState(
                mail_delivery_id, status, effective_message_id, effective_thread_id
            )
    ...
    def classify_normalized_message(
        ...
        """
        with self._write_transaction():
            return self._classify_normalized_message_txn(
                run_id,
                subject_id,
                mail_message_id,
                message,
                evidence.tracked_thread,
                evidence.has_run_header,
                evidence.trainlab_label,
            )
    ...
    def _classify_normalized_message_txn(
        ...
        subject_id: int,
        mail_message_id: int,
        supplied: CanonicalMessage,
        tracked_thread: bool,
        has_run_header: bool,
        thread_or_message_label: bool = False,
    ) -> Classification:
        ...
        run = self.connection.execute(
            "SELECT subject_id FROM mail_agent_runs WHERE id=?", (run_id,)
        ).fetchone()
        ...
        if (
            supplied.provider_message_id != row["provider_message_id"]
            or supplied.labels != labels
            or supplied.subject != row["subject"]
            or (supplied.body_text or "") != (row["body_text"] or "")
        ):
            raise MailRepositoryError(
                "classification_evidence_conflicts_with_canonical_message"
            )
        attachment_count = self.connection.execute(
            "SELECT count(*) FROM mail_attachments WHERE mail_message_id=?",
            (mail_message_id,),
        ).fetchone()[0]
        local = bool(
            self.connection.execute(
        ...
        canonical = CanonicalMessage(
            row["provider_message_id"],
            supplied.sender_is_self,
            supplied.recipient_is_self,
            labels,
            row["subject"],
            row["body_text"],
            attachment_count,
            supplied.auto_submitted,
            supplied.is_bounce,
            supplied.is_complete,
        )
        result = classify(
            canonical,
            ActorEvidence(
                local,
                tracked_thread,
                TRAINLAB_LABEL in labels or thread_or_message_label,
                has_run_header,
            ),
        )
        if row["processing_state"] != "normalized":
            return Classification(
                ...
                "mail_classification_replay",
                "already_terminal",
                False,
            )
        self.connection.execute(
            "UPDATE mail_messages SET actor_role=?,direction=?,processing_state=? WHERE id=?",
            (
                result.actor_role,
                result.direction,
        ...
        self.connection.execute(
            ...
            (
                ...
                mail_message_id,
                run_id,
                json.dumps({"reason_code": result.reason_code}, sort_keys=True),
                event_trust,
            ),
        )
        self.connection.execute(
    ...
    def select_queued_messages(
        self, subject_id: int, *, limit: int = 100
    ) -> tuple[int, ...]:
    ...
    def finish_run(
        self, run_id: int, status: str, *, next_retry_at_utc: str | None = None
    ) -> MailRun:
        if (
        ...
        with self._write_transaction():
            ...
            if status != current:
                self.connection.execute(
                    "UPDATE mail_agent_runs SET status=?,next_retry_at_utc=?,completed_at_utc=? WHERE id=?",
                    (
                        status,
                        next_retry_at_utc,
                        self._clock() if status != "started" else None,
                        run_id,
                    ),
                )
            updated = self.connection.execute(
    ...
    def record_item(
        ...
        *,
        ...
        status: str,
        mail_message_id: int | None = None,
        dependency_analysis_artifact_id: int | None = None,
        mail_response_artifact_id: int | None = None,
        mail_delivery_id: int | None = None,
        error_code: str | None = None,
        error_summary: str | None = None,
        ...
        if (
            ...
            or isinstance(run_id, bool)
            or run_id <= 0
            or not isinstance(logical_item_kind, str)
            or not _SAFE_CODE.fullmatch(logical_item_kind)
            or not isinstance(logical_item_id, str)
            or not _SAFE_ID.fullmatch(logical_item_id)
            or not isinstance(stage, str)
            or stage not in _STAGES
            or not isinstance(status, str)
            or status not in _ITEM_TRANSITIONS
            or (
                error_code is not None
                and (not isinstance(error_code, str) or error_code not in _ITEM_CODES)
            )
        ...
        if (
            ...
            and (error_code is None or next_retry_at_utc is not None)
            or status == "deferred"
            and (
        ...
        with self._write_transaction():
            ...
            row = self.connection.execute(
                "SELECT * FROM mail_agent_items WHERE mail_agent_run_id=? AND logical_item_kind=? AND logical_item_id=? AND stage=?",
                (run_id, logical_item_kind, logical_item_id, stage),
            ).fetchone()
            requested_relations = (
                mail_message_id,
                dependency_analysis_artifact_id,
            ...
            if row is None:
                self.connection.execute(
                    ...
                    (
                        ...
                        now
                        if status
                        in {"succeeded", "failed", "deferred", "unchanged", "rejected"}
                        else None,
                    ),
                )
            else:
                ...
                if (
                    status == current
                    and existing_status_evidence != requested_status_evidence
                ):
                    raise MailRepositoryError("conflicting_mail_item_replay")
                if status != current:
                    self.connection.execute(
                        "UPDATE mail_agent_items SET status=?,attempt_count=attempt_count+1,error_code=?,error_summary=?,next_retry_at_utc=?,completed_at_utc=? WHERE id=?",
                        (
                            status,
                            error_code,
                            safe_error_summary(error_summary),
    ...
    def _validate_item_relations(
        ...
        if mail_response_artifact_id is not None:
            ...
            if (
                ...
                or (
                    ...
                    and response["mail_thread_id"] != response["trigger_thread_id"]
                )
                or (
                    mail_message_id is not None
                    and response["in_reply_to_mail_message_id"] != mail_message_id
                )
            ):
        ...
        if mail_delivery_id is not None:
            ...
            if (
                ...
                or delivery_rows[0]["related_run_key"] != delivery_rows[0]["run_key"]
                or (
                ...
                or (
                    ...
                    and delivery_rows[0]["response_id"] != mail_response_artifact_id
                )
                or (
                    mail_message_id is not None
        ...
        *,
        ...
        for row in rows:
            value, _ = _strict_json(
                ...
                require_object=False,
                max_bytes=16_384,
                error_code="mail_fact_value_invalid",
            )
    ...
    def gate_mail_result(
        ...
        *,
        ...
        evidence = self._fact_gate_evidence_txn(run_id, context, as_of_utc=as_of_utc)
        ...
        with self._write_transaction():
            ...
            if existing is None:
                ...
                raise MailRepositoryError("mail_fact_gate_dependency_replay_conflict")
    ...
    def publish_accepted_response(
        ...
        *,
        ...
        with self._write_transaction():
            ...
            if run is None:
                raise MailRepositoryError("mail_run_not_found")
            structured_content_json, fact_value_json = self._validate_response_draft(
                run, draft
            )
            existing = self.connection.execute(
            ...
            digest = hashlib.sha256(
                (structured_content_json + "\n" + draft.user_visible_text).encode(
                    "utf-8"
                )
            ).hexdigest()
            self.connection.execute(
            ...
            response_id = self.connection.execute(
                "SELECT id FROM mail_response_artifacts WHERE generated_by_mail_agent_run_id=?",
                (run_id,),
            ).fetchone()["id"]
            for ordinal, item in enumerate(draft.inputs):
                self.connection.execute(
                    "INSERT INTO mail_response_inputs(mail_agent_run_id,input_role,source_entity_type,source_entity_id,source_revision_id,input_sha256,trust_class,ordinal) VALUES(?,?,?,?,?,?,?,?)",
                    (
                        run_id,
                        item.input_role,
            ...
            if draft.delivery is not None:
                ...
                if delivery_key == _AUTO_RESPONSE_DELIVERY_KEY:
                    ...
                    delivery_key = f"mail:response:{response_id}:{draft.delivery.provider_thread_id}"
                self.connection.execute(
                ...
                delivery_id = self.connection.execute(
                    "SELECT id FROM mail_deliveries WHERE idempotency_key=?",
                    (delivery_key,),
                ).fetchone()["id"]
                self.connection.execute(
                    "INSERT INTO mail_delivery_artifacts(mail_delivery_id,mail_response_artifact_id,content_role,ordinal) VALUES(?,?,?,0)",
                    (delivery_id, response_id, "mail_response"),
                )
    ...
    def _record_response_publication_txn(
        ...
        if item is None:
            ...
            (
                item["mail_message_id"],
                item["dependency_analysis_artifact_id"],
                item["mail_response_artifact_id"],
                item["mail_delivery_id"],
            )
            != relations
    ...
    def _validate_response_draft(
        ...
        if draft.mail_thread_id is not None:
            if not isinstance(draft.mail_thread_id, int) or isinstance(
                draft.mail_thread_id, bool
            ):
        ...
        if draft.in_reply_to_mail_message_id is not None:
            if not isinstance(draft.in_reply_to_mail_message_id, int) or isinstance(
                draft.in_reply_to_mail_message_id, bool
            ):
                raise MailRepositoryError("mail_message_ownership_invalid")
        ...
        for event in draft.events:
            ...
            if (
                ...
                or event.trust_level not in {"untrusted_content", "system_generated"}
        ...
        for fact in draft.facts:
            if (
                ...
                or (
                    ...
                    and (
                        (effective := _utc(fact.effective_from_utc)) is not None
                        and (expires := _utc(fact.expires_at_utc)) is not None
                        and effective >= expires
                    )
                )
                ...
                or (fact.scope != "message_only" and not fact.is_active)
            ):
            ...
            if fact.supersedes_fact_id is not None:
                ...
                if (
                    ...
                    or (old["is_active"] == 0 and old["superseded_by_fact_id"] is None)
                ):
    ...
    def _validate_input_relation(
        ...
        if (
            ...
            or (specification[1] is not None and item.trust_class != specification[1])
        ):
    ...
    def _create_or_recover_event(self, run: sqlite3.Row, event: EventDraft) -> int:
    ...
    def _create_or_recover_fact(
        ...
        fact_id = require_lastrowid(cursor)
        if fact.supersedes_fact_id is not None:
            changed = self.connection.execute(
    ...
    def _resolve_fact_for_replay(
        ...
        if len(rows) != 1 or not self._fact_row_matches(
            rows[0], subject_id, event_id, fact, canonical_fact
        ):
