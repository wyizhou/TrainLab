    ...
    *,
    ...
    for index, row in enumerate(response_rows):
        ...
        if (
            ...
            or _row_id(row, "subject_id") is None
            or not isinstance(kind, str)
            or not kind
            or run_id is None
        ):
            findings.append(_finding("response_identity_invalid", entity))
            continue
        response_key = (subject, thread, kind, revision)
        if response_key in by_response_key or response_id in response_by_id:
            findings.append(_finding("response_revision_duplicate", entity))
        by_response_key[response_key] = row
    ...
    for key, current_count in by_current.items():
        if current_count > 1:
            findings.append(
                _finding("response_current_ambiguous", f"response_current:{key}")
            )
    ...
    for index, row in enumerate(delivery_rows):
        ...
        if delivery_id is None or not isinstance(delivery_key, str) or not delivery_key:
            findings.append(_finding("delivery_identity_invalid", entity))
            continue
        if delivery_id in delivery_by_id or delivery_key in idempotency:
            findings.append(_finding("delivery_idempotency_duplicate", entity))
        delivery_by_id[delivery_id] = row
        idempotency.add(delivery_key)
    ...
    for index, row in enumerate(relation_rows):
        delivery_id, response_id = (
            _row_id(row, "mail_delivery_id"),
            _row_id(row, "mail_response_artifact_id"),
        )
        if (
            delivery_id is None
            or response_id is None
            or delivery_id not in delivery_by_id
            or response_id not in response_by_id
        ):
            findings.append(
                _finding("delivery_lineage_invalid", f"delivery_artifact[{index}]")
            )
            continue
    ...
    for delivery_id in delivery_by_id:
        ...
        if (
            len(links) != 1
            or links[0].get("content_role") != "mail_response"
            or links[0].get("ordinal") != 0
        ):
            findings.append(
                _finding("delivery_lineage_invalid", f"delivery:{delivery_id}")
            )
    ...
    for index, row in enumerate(input_rows):
        ...
        if run_id is None or run_id not in response_runs:
            findings.append(
                _finding("response_input_lineage_invalid", f"input[{index}]")
            )
            continue
    ...
    for run_id in response_runs:
        ...
        if (
            ...
            or sorted(cast(list[int], ordinals)) != list(range(len(rows)))
            or any(
                not isinstance(row.get("input_sha256"), str)
                or not _SHA256.fullmatch(cast(str, row["input_sha256"]))
                for row in rows
            )
        ):
            findings.append(
                _finding("response_input_lineage_invalid", f"response_run:{run_id}")
            )
    return AuditReport(tuple(findings))
