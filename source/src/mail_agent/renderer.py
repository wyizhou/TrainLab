...
from src.email_templates import EmailTemplateError, render_template
...
_ZONE_STATUSES = frozenset(
    {"available", "unavailable", "insufficient_evidence", "max_only"}
)
...
def _local_timestamp(value: object) -> tuple[str, str]:
    ...
    return f"{local.year}年{local.month}月{local.day}日", local.strftime(
        "%Y-%m-%d %H:%M"
    )
...
def _zone_ranges(value: object) -> str:
    ...
    for row in value[:5]:
        ...
        if (
            isinstance(zone, int)
            and not isinstance(zone, bool)
            and 1 <= zone <= 5
            and isinstance(bpm, Sequence)
            and not isinstance(bpm, (str, bytes))
            and len(bpm) == 2
            and all(
                isinstance(item, int)
...
def _heart_rate_zone_design(value: object) -> tuple[str, str, str, str] | None:
    ...
    hrr = _zone_method(
        value.get("hrr"),
        label="HRR（主方法）",
        bpm_key="resting_heart_rate_bpm",
        bpm_label="静息心率",
        max_key="max_heart_rate_bpm",
    )
    threshold = _zone_method(
        value.get("historical_threshold_proxy"),
        label="历史阈值代理",
        bpm_key="heart_rate_bpm",
        bpm_label="候选阈值心率",
    )
    tanaka = _zone_method(
        value.get("tanaka_low_confidence"),
        label="Tanaka（低置信度备用）",
        bpm_key="predicted_max_heart_rate_bpm",
        bpm_label="预测最大心率",
    )
    return (
        "跑步心率区间候选（待确认）",
...
def _design_values(
    ...
    if (
        ...
        and values["revision_status"] not in _REVISION_STATUSES
    ):
        raise MailRenderError("mail_response_revision_status_invalid")
...
def render_mail_response(
    *,
    ...
    plain_parts.extend(repeats["recommendations"])
    plain_parts.extend(
        fields[key]
        for key in (
            "next_steps",
    ...
    plain_text = "\n\n".join(dict.fromkeys(part for part in plain_parts if part))
    ...
    requested_message_id = f"<src.{message_token}@local>"
    digest = hashlib.sha256(
        (subject + "\n" + plain_text + "\n" + rendered_html).encode("utf-8")
    ).hexdigest()
    return RenderedMail(
        run_id, subject, plain_text, rendered_html, requested_message_id, digest
    )
