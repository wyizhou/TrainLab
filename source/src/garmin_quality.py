...
def _latest_capabilities(
    connection: sqlite3.Connection, subject_id: int
) -> dict[str, str | None]:
...
def _coverage_for_day(
    ...
    candidates = {
        (str(row["availability_state"]), int(row["record_count"]))
        for row in rows
        if row["observed_at_utc"] == newest
    }
    return next(iter(candidates)) if len(candidates) == 1 else ("ambiguous", 0)


def _cursor_complete(
    connection: sqlite3.Connection, subject_id: int, resource_kind: str
) -> str | None:
    rows = _rows(
        connection,
    ...
    if (
        len(rows) != 1
        or rows[0]["cursor_grain"] != "local_date"
        or rows[0]["catalog_version"] != CATALOG_VERSION
    ):
        return None
...
def _activity_reasons(
    connection: sqlite3.Connection, subject_id: int, start: str, through: str
) -> list[QualityReason]:
    rows = _rows(
        connection,
    ...
    for row in rows:
        ...
        if row["provider_state"] != "active":
            _append_unique(
                reasons, _reason("activity_provider_state_incomplete", entity)
            )
        ...
        if int(row["fit_count"]) != 1 and int(row["fallback_count"]) != 1:
            _append_unique(
                reasons, _reason("activity_fit_or_fallback_incomplete", entity)
            )
    return reasons
...
def _gap_reasons(
    ...
    for row in rows:
        ...
        _append_unique(
            reasons,
            _reason(
                "gap_deferred" if row["status"] == "deferred" else "gap_open", entity
            ),
        )
    return reasons


def _quality_issue_reasons(
    connection: sqlite3.Connection, subject_id: int, start: str, through: str
) -> tuple[list[QualityReason], list[QualityReason]]:
...
def evaluate_week_readiness(
    connection: sqlite3.Connection, subject_id: int, through: str
) -> WeekReadiness:
    ...
    try:
        ...
        for kind, capability in capabilities.items():
            ...
            if capability is None:
                ...
                continue
            if capability == "conflict" or capability not in {
                "supported",
                *_OPTIONAL,
                "forbidden",
                "unknown",
            ...
            if capability in _OPTIONAL:
                if spec.required:
                    _append_unique(
                        blockers, _reason("capability_required_unavailable", kind)
                    )
                else:
                    _append_unique(
                        warnings, _reason("capability_optional_unavailable", kind)
                    )
                continue
            ...
            for offset in range(7):
                ...
                if coverage is None:
                    ...
                    coverage[0] in {"error", "forbidden", "ambiguous"}
                    or coverage[0] not in _CLOSED
                ):
        ...
        for reason in _activity_reasons(connection, subject_id, start_text, end_text):
            _append_unique(blockers, reason)
        issue_blockers, issue_warnings = _quality_issue_reasons(
            connection, subject_id, start_text, end_text
        )
...
def weekly_quality_gate(
    database_path: str, subject_id: int, through: str
) -> QualityGate:
...
def collection_integrity_facts(
    database_path: str, subject_id: int, through: str
) -> tuple[IntegrityFact, ...]:
    ...
    window_start_utc = (
        datetime.combine(start_date, time.min, _LOCAL_TZ)
        .astimezone(UTC)
        .isoformat()
        .replace("+00:00", "Z")
    )
    window_end_utc = (
        datetime.combine(end + timedelta(days=1), time.min, _LOCAL_TZ)
        .astimezone(UTC)
        .isoformat()
        .replace("+00:00", "Z")
    )
    ...
    try:
        ...
        for check, sql, prefix, params in checks:
            for row in _rows(connection, sql, params):
                ...
                entity = (
                    f"{prefix}:{value}"
                    if QualityReason._safe(value)
                    else f"{prefix}:unknown"
                )
