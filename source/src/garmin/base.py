...
class GarminCollectionBase(GarminCollectionHost):
    def __init__(
        self,
        config: GarminConfig,
    ...
    def _execute_locked(
        ...
        try:
            ...
            if receipt.status != "started":
                self._normalize_terminal_replay_coverage(conn, run, request, receipt)
                return receipt
    ...
    def _normalize_terminal_replay_coverage(
        self,
        ...
        """Reconstruct collection completion from persisted health cursors."""
        if request.mode not in {"full", "incremental"} or receipt.status not in {
            "succeeded",
            "partial",
        }:
            return
        ...
        through = receipt.effective_range.get("through")
        if (
            not resources
            or through is None
            or any(
                (receipt.complete_through_by_resource.get(resource) or "") < through
    ...
    def _stable_regular_file_sha256(
        ...
        try:
            ...
            if not stat.S_ISREG(before.st_mode):
                raise _LiveAcceptancePreflightError("stable_file_not_regular")
            if facts(named_before) != facts(before):
            ...
            if not stat.S_ISREG(after.st_mode) or stat.S_ISLNK(named_after.st_mode):
                raise _LiveAcceptancePreflightError("stable_file_not_regular")
            if facts(before) != facts(after):
                raise _LiveAcceptancePreflightError("stable_file_metadata_drift")
            if facts(after) != facts(named_after):
                raise _LiveAcceptancePreflightError("stable_file_replaced")
    ...
    def _live_acceptance_schema() -> dict[str, Any]:
        return json.loads(
            resource_bytes("harness/schemas/garmin_live_acceptance_receipt.schema.json")
        )
    ...
    def _validate_live_acceptance_preflight_stop(document: Mapping[str, Any]) -> None:
        if document["failure_stage"] != "preflight" or any(
            document[name] != 0
            for name in (
