...
class FitCollectionMixin(GarminCollectionHost):
    ...
    @staticmethod
    def _fit_unknown_timestamp(
        fields: Iterable[Any], fallback: str | None
    ) -> str | None:
        for field in fields:
        ...
        try:
            ...
            try:
                with zipfile.ZipFile(zip_path) as archive:
                    ...
                    if (
                        len(infos) > self._FIT_MAX_MEMBERS
                        or sum(item.file_size for item in infos)
                        > self._FIT_MAX_TOTAL_BYTES
                    ):
                        raise GarminError("fit_zip_limits_exceeded")
    ...
    def _activity_declared_original_format(
        ...
        row = conn.execute(
            "SELECT extras_json FROM activities WHERE id=?",
            (activity,),
        ).fetchone()
        if row is None:
            return None
        value = json.loads(row["extras_json"] or "{}").get("connect_original_format")
    ...
    def _collect_nonfit_activity_original(
        ...
        try:
            if self._has_archived_activity_original(
                ...
                unique = {digest(candidate): candidate for candidate in candidates}
                if not unique:
    ...
    def _validate_fit_sessions(
        ...
        for index, session in enumerate(sessions):
            stamp = self._fit_timestamp(
                session.get("start_time") or session.get("timestamp")
            )
        ...
        activity_types = {
            str(activity.get("type") or "").strip() for activity in activities
        }
    ...
    def _fit_session_identity(
        self, fit: bytes, start_utc: str, sport: str
    ) -> tuple[str, str]:
        directory = self._fit_temp_dir()
        ...
        try:
            ...
            if not valid:
                ...
                code = (
                    min(
                        invalid_codes,
                        key=lambda item: (failure_priority.get(item, 99), item),
                    )
                    if invalid_codes
                    else "fit_missing"
                )
            ...
            if len(unique) != 1:
                ...
                details = json.dumps(
                    {
                        ...
                        "candidates": [
                            {
                                "hash": candidate_hash,
                                "error_code": validation_errors.get(candidate_hash),
                            }
                ...
                issue = conn.execute(
                    "SELECT id FROM data_quality_issues WHERE entity_type='activity' AND entity_id=? AND issue_code='ambiguous_activity_fit' AND status='open'",
                    (activity,),
                ).fetchone()
                if issue:
                    conn.execute(
                        "UPDATE data_quality_issues SET last_seen_at_utc=?,details_json=? WHERE id=?",
                        (now, details, int(issue[0])),
                    )
                else:
                    conn.execute(
                        "INSERT INTO data_quality_issues(entity_type,entity_id,issue_code,severity,details_json,status,first_seen_at_utc,last_seen_at_utc,source_revision_id) VALUES(?,?,?,?,?,'open',?,?,?)",
                        (
                            "activity",
                            activity,
                            "ambiguous_activity_fit",
                            "warning",
                            details,
                            now,
                            now,
            ...
            def projector(revision: int) -> None:
                ...
                conn.execute(
                    "DELETE FROM activity_metric_sources WHERE activity_id=? AND source_kind IN ('standard_fit','developer_fit')",
                    (activity,),
                )
                conn.execute(
                ...
                conn.execute(
                    "UPDATE activity_source_revisions SET is_active=0 WHERE activity_id=? AND source_role='activity_fit'",
                    (activity,),
                )
                conn.execute(
                    "INSERT OR IGNORE INTO activity_source_revisions(activity_id,source_revision_id,source_role,is_active) VALUES(?,?,?,1)",
            ...
            conn.execute(
                "UPDATE data_quality_issues SET status='resolved',resolved_at_utc=?,last_seen_at_utc=? WHERE entity_type='activity' AND entity_id=? AND issue_code='ambiguous_activity_fit' AND status='open'",
                (utc_now(), utc_now(), activity),
            )
            self.repo.resolve_gaps(
                conn,
                subject,
            ...
            terminal = (
                ...
                if outcome.status == "not_available"
                    ...
                    "forbidden"
                    if outcome.status == "forbidden"
                    else ("deferred" if outcome.status == "deferred" else "failed")
                )
            )
            ...
            self.repo.gap(
                conn, subject, "activity_fit", key, day.isoformat(), "parse", error.code
            )
            receipt.counts["failed"] += 1
    ...
    def _store_fit_session_evidence(
        ...
        session_fields = {
            field.name: self._fit_field_evidence(field) for field in fields
        }
        row = conn.execute(
    ...
    def _project_fit(
        self, conn: sqlite3.Connection, activity: int, fit: bytes, revision: int
    ) -> None:
        directory = self._fit_temp_dir()
        path = directory / "project.fit"
        try:
            self._write_temp_file(path, fit)
            ...
            with fitdecode.FitReader(path, check_crc=True) as reader:
                for frame in reader:
                    if not isinstance(frame, fitdecode.FitDataMessage):
                        continue
                    values = {field.name: field.value for field in frame.fields}
                    if frame.name == "workout_step" and isinstance(
                        values.get("message_index"), int
                    ):
                        ...
                        if category is not None and isinstance(title, str) and title:
                            exercise_titles[
                                (str(category), values.get("exercise_name"))
                            ] = title
            ...
            with fitdecode.FitReader(path, check_crc=True) as reader:
                for frame in reader:
                    ...
                    if name == "session":
                        ...
                        self._store_fit_session_evidence(
                            conn, activity, revision, frame.fields
                        )
                    if name == "record":
                        index = indexes.get("record", 0)
                        indexes["record"] = index + 1
                        generic_fields = {
                            "timestamp",
                            "position_lat",
                            "position_long",
                        ...
                        for field in frame.fields:
                            ...
                            if (
                                ...
                                and not field.name.startswith("unknown_")
                                and field.value is not None
                            ):
                                is_developer = evidence["is_developer"]
                                source_kind = (
                                    "developer_fit" if is_developer else "standard_fit"
                                )
                                developer_index = (
                                    evidence["developer_data_index"]
                                    if is_developer
                                    else None
                                )
                                attribution = (
                                    "explicit_developer" if is_developer else "unknown"
                                )
                                key = (
                                    field.name,
                                    source_kind,
                        ...
                        if index % sample_stride == 0:
                            conn.execute(
                                "INSERT OR IGNORE INTO activity_samples(activity_id,source_revision_id,stream_kind,sample_index,timestamp_utc,latitude,longitude,distance_m,speed_mps,altitude_m,heart_rate_bpm,cadence_rpm,power_w,temperature_c,extras_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                (
                                    activity,
                                    revision,
                                    "fit_record",
                        ...
                        if name == "set":
                            kind = (
                                "strength_rest"
                                if str(values.get("set_type") or "").endswith("rest")
                                else "strength_active"
                            )
                        elif name == "split" and sport == "rock_climbing":
                            kind = (
                                "climb_rest"
                                if str(values.get("split_type") or "").endswith("rest")
                                else "climb_active"
                            )
                        else:
                            kind = name
                        ...
                        conn.execute(
                            "INSERT OR IGNORE INTO activity_segments(activity_id,segment_type,segment_index,start_time_utc,end_time_utc,duration_seconds,distance_m,extras_json,source_revision_id) VALUES(?,?,?,?,?,?,?,?,?)",
                            (
                                activity,
                                kind,
                                index,
                                self._fit_timestamp(values.get("start_time")),
                                stamp,
                                self._fit_optional(
                                    values.get("total_timer_time"),
                                    values.get("duration"),
                                ),
                                values.get("total_distance"),
                                json.dumps(
                                    {
                                        field.name: self._fit_json_value(field.value)
                                        for field in frame.fields
                                    },
                                    sort_keys=True,
                                    allow_nan=False,
                        ...
                        row = conn.execute(
                            "SELECT id FROM activity_segments WHERE activity_id=? AND source_revision_id=? AND segment_type=? AND segment_index=?",
                            (activity, revision, kind, index),
                        ).fetchone()
                        segment = int(row[0])
                        if kind == "strength_active":
                            step_index = values.get("wkt_step_index")
                            step = (
                                workout_steps.get(step_index, {})
                            ...
                            title = (
                                exercise_titles.get(
                                    (str(step_category), raw_exercise_number)
                                )
                            ...
                            exercise_category = (
                                ...
                                else (step_category or (str(title) if title else None))
                            )
                            exercise_name = (
                                step.get("notes")
                                or title
                                or step.get("wkt_step_name")
                                or exercise_category
                            )
                            conn.execute(
                                "INSERT OR IGNORE INTO strength_sets(segment_id,workout_step_index,set_type,exercise_category,raw_exercise_number,exercise_name,repetitions,weight_kg,duration_seconds) VALUES(?,?,?,?,?,?,?,?,?)",
                                (
                                    segment,
                                    step_index,
                                    str(values.get("set_type") or "active"),
                                    exercise_category,
                                    raw_exercise_number,
                                    exercise_name,
                            ...
                            conn.execute(
                                "INSERT OR IGNORE INTO climbing_routes(segment_id,grade_raw,grade_system,grade_display,completed,falls,ascent_meters) VALUES(?,?,?,?,?,?,?)",
                                (
                                    segment,
                                    str(grade) if grade is not None else None,
                                    "font" if grade is not None else None,
                                    _FONT_GRADE_DISPLAY.get(
                                        grade if isinstance(grade, int) else -1
                                    ),
                                    completed,
                                    values.get("unknown_72")
                                    if values.get("unknown_72") is not None
                                    else values.get("falls"),
                                    values.get("total_ascent"),
                                ),
                            )
                    elif name == "field_description":
                        conn.execute(
                            "INSERT OR IGNORE INTO fit_metric_definitions(source_revision_id,developer_data_index,native_mesg_num,field_definition_number,field_name,base_type,raw_unit,canonical_metric_key) VALUES(?,?,?,?,?,?,?,?)",
                            (
                                revision,
                                int(values.get("developer_data_index") or 0),
                                values.get("native_mesg_num"),
                                int(values.get("field_definition_number") or 0),
                                values.get("field_name"),
                                values.get("fit_base_type_id"),
                                values.get("units"),
                                None,
                            ),
                        )
                        ...
                        if (
                            ...
                            uid = self._identity_hmac(
                                f"garmin-developer:{developer_index}:{self._fit_json_value(application_id)}"
                            )
                            now = utc_now()
                            conn.execute(
                                "INSERT INTO devices(device_uid_hash,manufacturer,product,device_type,first_seen_at_utc,last_seen_at_utc) VALUES(?,?,?,?,?,?) ON CONFLICT(device_uid_hash) DO UPDATE SET last_seen_at_utc=excluded.last_seen_at_utc",
                                (
                                    uid,
                                    "garmin_developer",
                                    f"developer_data_index:{developer_index}",
                                    "developer_app",
                                    now,
                                    now,
                                ),
                            ...
                            device = conn.execute(
                                "SELECT id FROM devices WHERE device_uid_hash=?", (uid,)
                            ).fetchone()
                            conn.execute(
                                "INSERT OR IGNORE INTO activity_devices(activity_id,device_id,device_role,source_revision_id) VALUES(?,?,?,?)",
                                (activity, int(device[0]), "developer_app", revision),
                            )
                    elif name == "device_info":
                        serial = values.get("serial_number")
                        if serial is not None:
                            uid = self._identity_hmac(f"garmin:{serial}")
                            now = utc_now()
                            conn.execute(
                                "INSERT INTO devices(device_uid_hash,manufacturer,product,device_type,first_seen_at_utc,last_seen_at_utc) VALUES(?,?,?,?,?,?) ON CONFLICT(device_uid_hash) DO UPDATE SET last_seen_at_utc=excluded.last_seen_at_utc",
                                (
                                    uid,
                                    str(values.get("manufacturer") or "garmin"),
                                    str(values.get("product") or ""),
                                    "fit_device",
                                    now,
                                    now,
                                ),
                            ...
                            conn.execute(
                                "INSERT OR IGNORE INTO activity_devices(activity_id,device_id,device_role,source_revision_id) VALUES(?,?,?,?)",
                                (activity, int(device[0]), "unknown", revision),
                            )
                    elif name == "course_point":
                        index = indexes.get("course_point", 0)
                        indexes["course_point"] = index + 1
                        lat = values.get("position_lat")
                        lon = values.get("position_long")
                        lat = (
                            lat * 180.0 / (2**31)
                        ...
                        conn.execute(
                            "INSERT OR IGNORE INTO course_points(activity_id,course_identity,point_index,name,point_type,distance_m,latitude,longitude,route_time_utc) VALUES(?,?,?,?,?,?,?,?,?)",
                            (
                                activity,
                                "fit",
                                index,
                        ...
                        conn.execute(
                            "INSERT OR IGNORE INTO activity_aux_messages(activity_id,source_revision_id,global_message_number,message_name,message_index,timestamp_utc,payload_json) VALUES(?,?,?,?,?,?,?)",
                            (
                                activity,
                                revision,
                                int(getattr(frame, "global_mesg_num", -1)),
                                "course_point_evidence",
                                index,
                                stamp,
                                json.dumps(
                                    {
                                        field.name: self._fit_field_evidence(field)
                                        for field in frame.fields
                                    },
                                    sort_keys=True,
                                    allow_nan=False,
                        ...
                        unknown_stamp = self._fit_unknown_timestamp(frame.fields, stamp)
                                ...
                                (
                                    {
                                        ...
                                        "field_definition_number": field.def_num,
                                        "unit": field.units,
                                        "developer_data_index": getattr(
                                            getattr(field, "field_def", None),
                                            "dev_data_index",
                                            None,
                                        )
                                        if getattr(field, "field_type", None)
                                        == "devfield"
                                        else None,
                                        "is_developer": getattr(
            ...
            for (metric_key, source_kind, developer_index, attribution), (
                valid_from,
                valid_to,
            ) in metric_ranges.items():
                exists = conn.execute(
                    "SELECT 1 FROM activity_metric_sources WHERE activity_id=? AND metric_key=? AND valid_from_utc IS ? AND valid_to_utc IS ? AND source_kind=? AND device_id IS NULL AND developer_data_index IS ? AND attribution_method=?",
                    (
                        activity,
                        metric_key,
                        valid_from,
                ...
                if exists is None:
                    conn.execute(
                        "INSERT INTO activity_metric_sources(activity_id,metric_key,valid_from_utc,valid_to_utc,source_kind,device_id,developer_data_index,attribution_method,confidence) VALUES(?,?,?,?,?,?,?,?,?)",
                        (
                            activity,
                            metric_key,
                            valid_from,
                            ...
                            developer_index,
                            attribution,
                            1.0 if attribution == "explicit_developer" else None,
                        ),
                    )
        finally:
            shutil.rmtree(directory, ignore_errors=True)
