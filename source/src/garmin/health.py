...
class HealthCollectionMixin(GarminCollectionHost):
    def _account_basics(
        self,
        ...
        resources = tuple(
            resource
            for resource in self._ACCOUNT_BASIC_RESOURCES
            if not selected
            or resource in selected
            or (resource == "devices" and bool(selected & device_dependents))
        )
        if not resources:
            return
        ...
        for resource in resources:
            ...
            try:
                payload = self._call(
                    lambda r=resource: self._transport().fetch_health(
                        r, day.isoformat()
                    ),
                    conn=conn,
                    run=run,
                ...
                if state is not None:
                    if state != "empty":
                        self.repo.capability(
                            ...
                            resource,
                            state,
                            reason="provider_empty_or_capability",
                            next_probe=self._account_next_probe(state),
                ...
                def projector(
                    revision: int, *, safe_payload: Any = safe, kind: str = resource
                ) -> None:
                    if kind == "devices":
                        ...
                        count = self._project_profile_settings(
                            conn, subject, kind, safe_payload, revision
                        )
                    self.repo.coverage(
                        conn,
                        subject,
                        kind,
                        day.isoformat(),
                        "partial" if request.mode == "snapshot" else "fetched",
                        revision,
                        count,
                        snapshot=request.mode == "snapshot",
                    )
                ...
                if outcome.status == "not_available" or exc.code == "not_supported":
                    ...
                    self.repo.coverage(
                        ...
                        resource,
                        day.isoformat(),
                        "partial" if request.mode == "snapshot" else state,
                        None,
                        0,
                        snapshot=request.mode == "snapshot",
                    )
                    ...
                    self.repo.capability(
                        ...
                        "forbidden",
                        reason=exc.code,
                        next_probe=self._account_next_probe("forbidden"),
                        environment_key=self.config.region,
                    )
                    ...
                    self.repo.coverage(
                        ...
                        resource,
                        day.isoformat(),
                        "partial" if request.mode == "snapshot" else "forbidden",
                        None,
                        0,
                        snapshot=request.mode == "snapshot",
                    )
                ...
                self.repo.item(
                    conn, run, resource, key, "project", "failed", error=error
                )
                self.repo.gap(
    ...
    def _account_b1(
        ...
        resources = tuple(
            resource
            for resource in self._ACCOUNT_B1_RESOURCES
        ...
        for resource in resources:
            ...
            if resource in {"primary_device", "device_last_used"} and not getattr(
                self, "_account_devices_ready", False
            ):
                ...
                self._publish_b1_coverage(
                    conn, subject, resource, day.isoformat(), request, outcomes
                )
                continue
            if resource == "device_settings" and not keys:
                self.repo.gap(
                    ...
                    subject,
                    resource,
                    "garmin:account:device_settings",
                    day.isoformat(),
                    "discover",
                    "not_available",
                )
            ...
            for device_hash, provider_id in keys:
                ...
                try:
                    ...
                    if state is not None:
                        ...
                        provider_key = self._identity_hmac(
                            f"account-b1:{resource}:{device_hash or 'account'}"
                        )
                        tombstone = {"availability_state": state}

                        def tombstone_projector(
                            revision: int, *, kind: str = resource
                        ) -> None:
                    ...
                    def projector(
                        ...
                        *,
                        ...
                        if kind in {
                            ...
                            self._project_device_b1(
                                conn, subject, kind, safe_payload, hashed, revision
                            )
                        else:
                            self._supersede_b1_projection(conn, subject, kind)
                            self.repo.fields(conn, kind, safe_payload)
                            self._project_b1_physiology(
                                conn, subject, kind, safe_payload, revision
                            )
                    ...
                    error = GarminError(
                        ...
                        if str(exc) == "unknown_device_reference"
                        else "account_project_failed"
                    )
                    self.repo.item(
                        conn, run, resource, key, "project", "failed", error=error
                    )
    ...
    def _device_id_map(self, payload: Any) -> dict[str, str]:
        ...
        for entry in records:
            ...
            if provider_id is not None:
                result[self._identity_hmac(f"device:{provider_id}")] = str(provider_id)
        return result
        ...
        *,
        ...
        if resource in {"primary_device", "device_last_used"}:
            ...
            for field in (
                ...
                if provider_id is None:
                    continue
                canonical = aliases.get(self._identity_hmac(f"device:{provider_id}"))
        ...
        if resource == "device_settings":
            if device_hash is None or device_hash not in device_ids:
                raise ValueError("unknown_device_reference")
    ...
    def _project_device_b1(
        ...
        hashed = (
            payload.get("device_uid_hash") if isinstance(payload, dict) else device_hash
        )
        if (
            not hashed
            or conn.execute(
                "SELECT 1 FROM devices WHERE device_uid_hash=?", (hashed,)
            ).fetchone()
            is None
        ):
            raise ValueError("unknown_device_reference")
        if resource == "device_settings":
            catalog = {
                key: value for key, value in payload.items() if key != "device_uid_hash"
            }
        ...
        conn.execute(
            "DELETE FROM physiology_records WHERE subject_id=? AND domain='garmin' AND record_type=? AND provider_record_id=?",
            (subject, resource, hashed),
        )
        ...
        cursor = conn.execute(
            ...
            (
                ...
                hashed,
                stamp,
                self._local_day(stamp),
                "provider_derived",
                stable_json({"device_uid_hash": hashed}),
                revision,
            ),
        )
        record = require_lastrowid(cursor)
        values: dict[str, Any] = {}
        if resource == "primary_device":
            values["garmin.device.role.primary"] = True
        elif resource == "device_last_used":
            values["garmin.device.role.last_used"] = True
        else:
            values = {
                f"garmin.device.settings.{key}": value
                for key, value in payload.items()
                if key != "device_uid_hash"
            }
        for metric_key, value in values.items():
            conn.execute(
                ...
                (
                    ...
                    None,
                    "provider_derived",
                    "/" + metric_key.rsplit(".", 1)[-1],
                ),
            )
        return 1

    def _publish_b1_coverage(
        self,
        conn: sqlite3.Connection,
        subject: int,
        resource: str,
        ...
        state, revision, _count, reason, _changed = max(
            outcomes, key=lambda outcome: severity.get(outcome[0], 6)
        )
        total = sum(item[2] for item in outcomes) if state == "fetched" else 0
        coverage_state = (
            "partial"
            if request.mode == "snapshot"
            else ({"failed": "error", "deferred": "partial"}.get(state, state))
        )
        latest = conn.execute(
        ...
        # device response.
        conn.execute(
            "DELETE FROM resource_coverage WHERE subject_id=? AND provider='garmin' AND resource_kind=? AND local_date=?",
            (subject, resource, day),
        )
        self.repo.coverage(
            conn,
    ...
    @staticmethod
    def _supersede_b1_projection(
        conn: sqlite3.Connection, subject: int, resource: str
    ) -> None:
        ...
        if resource in {"personal_records", "cycling_ftp", "pregnancy"}:
            ...
            conn.execute(
                "DELETE FROM physiology_records WHERE subject_id=? AND domain='garmin' AND record_type=?",
                (subject, resource),
            )

    def _project_b1_physiology(
        self,
        conn: sqlite3.Connection,
        subject: int,
        resource: str,
    ...
    def _safe_account_payload(self, resource: str, payload: Any) -> Any:
        ...
        if resource == "devices":
            source = (
                payload.get("devices", []) if isinstance(payload, dict) else payload
            )
            ...
            for entry in source:
                ...
                devices.append(safe)
            return {
                "devices": sorted(devices, key=lambda device: device["device_uid_hash"])
            }
    ...
    def _project_profile_settings(
        ...
        allowed = {
            key: value
            for key, value in payload.items()
            if key in self._PROFILE_SETTING_FIELDS
        }
        if not allowed:
            return 0
        ...
        cursor = conn.execute(
            ...
            (
                ...
                "garmin",
                resource,
                self._now_utc().isoformat().replace("+00:00", "Z"),
                self._now_utc().astimezone(TZ).date().isoformat(),
                "profile_setting",
                "{}",
                revision,
            ),
    ...
    def _health_item_completed(
        ...
        if fetch in {
            ...
            return True
        return fetch == "fetched" and stages.get("project") in {
            "revised",
            "unchanged",
            "succeeded",
        }
        ...
        range_resources = (
            {
                ...
                if resource in RESOURCE_CATALOG
                and RESOURCE_CATALOG[resource].scope == "range"
            }
            if callable(getattr(self._transport(), "fetch_range", None))
            else set()
        )
        for resource in sorted(range_resources):
            self._health_range(
                conn, run, subject, resource, start, through, request, receipt
            )
        ...
        for day in (start + timedelta(i) for i in range((through - start).days + 1)):
            for resource in COLLECTED_HEALTH_RESOURCES:
                ...
                try:
                    spec = RESOURCE_CATALOG[resource]
                    payload = self._call(
                        lambda r=resource, d=day: self._transport().fetch_health(
                            r, d.isoformat()
                        ),
                        conn=conn,
                        run=run,
                    ...
                    payload_state = self._health_payload_state(
                        stored_payload, spec.empty_state
                    )
                    if payload_state is not None:
                        capability = (
                            "supported" if payload_state == "empty" else payload_state
                        )
                        self.repo.capability(
                            conn,
                            subject,
                        ...
                        _, revision, changed = self.repo.archive(
                            ...
                            resource,
                            key,
                            canonical_provider_json(stored_payload),
                            "json",
                            "application/json",
                            tombstone_projector,
                        )
                        ...
                        if payload_state in receipt.counts:
                            receipt.counts[payload_state] += 1
                        continue
                    self.repo.item(
                        conn,
                        run,
                    ...
                    def projector(revision: int):
                        ...
                        projected = self._project_health(
                            conn,
                            subject,
                            resource,
                            day.isoformat(),
                    ...
                    if outcome.status == "not_available" or exc.code == "not_supported":
                        ...
                        if state in receipt.counts:
                            receipt.counts[state] += 1
                        continue
                    ...
                    receipt.counts[
                        "deferred" if terminal == "deferred" else "failed"
                    ] += 1
                    receipt.next_retry_at_utc = retry or receipt.next_retry_at_utc
                except Exception:
                    ...
                    if resource in ADVANCED_RESOURCES:
                        self._consolidate_advanced_coverage(
    ...
    def _health_range(
        ...
        while cursor <= through:
            ...
            try:
                payload = self._call(
                    lambda s=cursor, e=end: self._transport().fetch_range(
                        resource, s.isoformat(), e.isoformat()
                    ),
                    conn=conn,
                    run=run,
                    subject=subject,
                ...
                if state is not None:
                    ...
                    _, revision, changed = self.repo.archive(
                        ...
                        resource,
                        logical_key,
                        canonical_provider_json(stored),
                        "json",
                        "application/json",
                        tombstone,
                    )
                ...
                def projector(revision: int) -> None:
                    ...
                    for current_day in days:
                        ...
                        state_for_day = (
                            "partial"
                            if request.mode == "snapshot"
                            else ("fetched" if count > 0 else "empty")
                        )
                        self.repo.coverage(
                            conn,
                            subject,
                ...
                if not changed:
                    ...
                    for current_day in days:
                        state_for_day = (
                            "fetched" if by_day[current_day.isoformat()] else "empty"
                        )
                        self._promote_partial_coverage(
                            conn,
                            subject,
                ...
                if outcome.status == "auth_required":
                    raise GarminError("auth_required", http_status=401) from None
                retry = (
                    self._next_retry(exc, 0) if outcome.status == "deferred" else None
                )
                terminal = (
                    "not_supported"
                    if exc.code == "not_supported"
                        ...
                        if outcome.status == "not_available"
                            ...
                            if outcome.status == "forbidden"
                                ...
                                "deferred" if outcome.status == "deferred" else "failed"
                            )
                        )
                    )
                )
                ...
                if terminal in {"not_available", "not_supported", "forbidden"}:
                    self.repo.capability(
                        ...
                        terminal,
                        reason=exc.code,
                        next_probe=self._account_next_probe(terminal),
                        environment_key=self.config.region,
                    )
                for current_day in [
                    cursor + timedelta(index)
                    ...
                    coverage_state = (
                        ...
                        if request.mode == "snapshot"
                            ...
                            terminal
                            if terminal
                            in {"not_available", "not_supported", "forbidden"}
                            else "error"
                        )
                    )
                    self.repo.coverage(
    ...
    def _range_payload_by_day(
        self, payload: Any, start: date, end: date, *, resource: str | None = None
        ...
        for record in records:
            ...
            stamp = next(
                (record.get(key) for key in date_keys if record.get(key) is not None),
                None,
            )
            if stamp is None:
                raise ValueError("range_record_missing_date")
            parsed = self._timestamp_utc(
                stamp, start.isoformat(), allow_day_boundary=True
            )
            ...
            if (
                resource in ADVANCED_RESOURCES
                and resource != "lactate_threshold"
                and selected[local_day]
            ):
    ...
    def _range_payload_for_day(self, payload: Any, day: str, span: int) -> Any:
        ...
        return self._range_payload_by_day(
            payload, start, start + timedelta(days=span - 1)
        )[day]

    @staticmethod
    def _unparsed_revision(
        conn: sqlite3.Connection, resource: str, provider_id: str
    ) -> int | None:
        row = conn.execute(
    ...
    def _range_records(
        ...
        if isinstance(payload, dict):
            if resource == "weigh_ins" and isinstance(
                payload.get("dailyWeightSummaries"), list
            ):
                ...
                for summary in summaries:
                    if not isinstance(summary, dict):
                        continue
                    values = summary.get("allWeightMetrics")
            ...
            if selected_wrappers is not None and any(
                ...
                return items
            if resource == "endurance_score" and "enduranceScoreDTO" in payload:
            ...
            if (
                ...
                return []
        records = (
            self._advanced_records(payload) if isinstance(payload, (dict, list)) else []
        )
    ...
    def _supersede_range_projection(
        self, conn: sqlite3.Connection, subject: int, resource: str, day: str
    ) -> None:
        conn.execute(
        ...
        records = list(
            conn.execute(
                "SELECT id FROM physiology_records WHERE subject_id=? AND domain='garmin' AND record_type=? AND local_date=?",
                (subject, resource, day),
            )
        )
        if records:
            ...
            conn.execute(
                "DELETE FROM physiology_metrics WHERE physiology_record_id IN (%s)"
                % ",".join("?" for _ in ids),
                ids,
            )
            conn.execute(
                "DELETE FROM physiology_records WHERE id IN (%s)"
                % ",".join("?" for _ in ids),
                ids,
            )
    ...
    @staticmethod
    def _consolidate_advanced_coverage(
        conn: sqlite3.Connection, subject: int, resource: str, day: str
    ) -> None:
        ...
        if len(rows) > 1:
            conn.execute(
                "DELETE FROM resource_coverage WHERE id IN (%s)"
    ...
    def _timestamp_utc(
        ...
        *,
        ...
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            ...
            seconds = (
                float(value) / 1000 if abs(value) > 10_000_000_000 else float(value)
            )
            try:
                return (
        ...
        if isinstance(value, str) and value:
            ...
            try:
                if (
                    candidate_number
                    and candidate_number.lstrip("+-").replace(".", "", 1).isdigit()
                ):
                    seconds = float(candidate_number)
                    seconds = (
                        seconds / 1000 if abs(seconds) > 10_000_000_000 else seconds
                    )
                    return (
                        datetime.fromtimestamp(seconds, UTC)
                        .isoformat()
            ...
            if len(value) == 10:
                ...
                if parsed_day is not None:
                    ...
                    return (
                        datetime.combine(parsed_day, datetime.min.time(), TZ)
                        .astimezone(UTC)
                        .isoformat()
                        .replace("+00:00", "Z")
                    )
        ...
        return (
            datetime.combine(date.fromisoformat(fallback_day), datetime.min.time(), TZ)
            .astimezone(UTC)
            .isoformat()
            .replace("+00:00", "Z")
        )
        ...
        if isinstance(value, dict):
            for key, child in value.items():
                if key.lower() in {
                    ...
                    continue
                yield from HealthCollectionMixin._numeric_fields(
    ...
    def _upsert_daily_health(
        ...
        prior = conn.execute(
            "SELECT values_json,extras_json,source_map_json FROM daily_health WHERE subject_id=? AND local_date=? AND is_current=1",
            (subject, day),
        ).fetchone()
        ...
        if (
            ...
            metric_map = (
                metrics.get("metricsMap") if isinstance(metrics, dict) else None
            )
            candidates = (
                metric_map.get("WELLNESS_RESTING_HEART_RATE")
            ...
            readings = (
                [
                    item[1]
                    for item in payload.get("heartRateValues", [])
                    if isinstance(item, list)
                    and len(item) >= 2
                    and isinstance(item[1], (int, float))
                    and not isinstance(item[1], bool)
                ]
                if isinstance(payload.get("heartRateValues"), list)
                else []
            )
            if readings:
                scalar_payload["dailyAverageHeartRate"] = sum(
            ...
            if isinstance(scalar_payload, dict) and isinstance(
                scalar_payload.get("weight"), (int, float)
            ):
        ...
        if isinstance(scalar_payload, dict):
            for source, metric in DAILY_SCALAR_METRICS.get(resource, {}).items():
                ...
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    ...
                    if (
                        resource == "hrv"
                        and not nested_hrv_summary
                        and source_path.startswith("/hrvSummary/")
                    ):
        ...
        conn.execute(
            "UPDATE daily_health SET is_current=0 WHERE subject_id=? AND local_date=? AND is_current=1",
            (subject, day),
        )
        if values:
            conn.execute(
                "INSERT INTO daily_health(subject_id,local_date,values_json,extras_json,source_map_json,source_revision_id,is_current) VALUES(?,?,?,?,?,?,1)",
                (
                    subject,
                    day,
                    stable_json(values).decode("utf-8"),
    ...
    def _clear_daily_health_resource(
        self, conn: sqlite3.Connection, subject: int, day: str, resource: str
    ) -> None:
        prior = conn.execute(
            "SELECT values_json,extras_json,source_map_json,source_revision_id FROM daily_health WHERE subject_id=? AND local_date=? AND is_current=1",
            (subject, day),
        ).fetchone()
        if prior is None:
            return
        values, extras, source_map = (
            json.loads(prior[column])
            for column in ("values_json", "extras_json", "source_map_json")
        )
        for metric in DAILY_SCALAR_METRICS.get(resource, {}).values():
            values.pop(metric[0], None)
            source_map.pop(metric[0], None)
    ...
    def _assert_sample_day(
        ...
        *,
        ...
        observed = datetime.fromisoformat(stamp.replace("Z", "+00:00")).astimezone(TZ)
    ...
    def _project_samples(
        ...
        for index, record in enumerate(records):
                ...
                for point_index, point in enumerate(readings):
                    ...
                    conn.execute(
                        ...
                        (
                            ...
                            "garmin.hrv.reading_ms",
                            float(value),
                            json.dumps(
                                {"sample_index": index, "series_index": point_index},
                                sort_keys=True,
                                allow_nan=False,
                            ),
                            "ms",
                ...
                if values is not None:
                    ...
                    for point_index, point in enumerate(values):
                        ...
                        if point[1] is None:
                            ...
                            continue
                        if not isinstance(point[1], (int, float)) or isinstance(
                            point[1], bool
                        ):
            ...
            generic_record = {
                key: value for key, value in record.items() if key not in recognised
            }
            if direct_metrics:
                timestamp_fields = (
                    "timestamp",
                ...
                if meaningful_direct and timestamp is not None:
                    stamp = self._timestamp_utc(
                        ...
                        assume_utc=bool(
                            timestamp_key
                            and (
                                "GMT" in timestamp_key or timestamp_key.endswith("UTC")
                            )
                        ),
                    )
    ...
    def _project_physiology(
        ...
        excluded_containers = {
            ...
            "bloodPressureSummaries",
            "bloodPressureList",
            "bodyCompositionSummaries",
            "measurements",
        }
        ...
        for record in records:
            ...
            if not scalar_items and (isinstance(payload, list) or resource in {"hrv"}):
                continue
            daily_stamp = (
            ...
            stamp = self._timestamp_utc(
                timestamp_value,
                day,
                allow_day_boundary=bool(daily_stamp)
                or resource
                in {
                    "intensity_minutes",
                    "all_day_events",
                ...
                assume_utc=bool(
                    not daily_stamp
                    and timestamp_key
                    and ("GMT" in timestamp_key or timestamp_key.endswith("UTC"))
                ),
            )
            if has_timestamp:
                self._assert_sample_day(stamp, day)
            cursor = conn.execute(
                ...
                (
                    ...
                    self._local_day(stamp),
                    "provider_derived",
                    stable_json(record).decode("utf-8"),
                    revision,
                ),
            )
    ...
    def _project_body_measurements(
        self,
        conn: sqlite3.Connection,
        subject: int,
        resource: str,
        ...
        for item in records:
            ...
            stamp = self._timestamp_utc(
                ...
                assume_utc=bool(
                    timestamp_key
                    and ("GMT" in timestamp_key or timestamp_key == "gmtTimestamp")
                ),
            )
            self._assert_sample_day(stamp, day)
            conn.execute(
                "INSERT INTO body_measurements(subject_id,observed_at_utc,local_date,values_json,extras_json,source_revision_id) VALUES(?,?,?,?,?,?)",
                (
                    subject,
                    stamp,
                    self._local_day(stamp),
                    stable_json(item).decode("utf-8"),
                    "{}",
                    revision,
                ),
            )
    ...
    def _project_sleep(
        ...
        if isinstance(naps, list):
            sessions = [
                *sessions,
                *[
                    {**nap, "session_type": nap.get("session_type", "nap")}
                    for nap in naps
        ...
        if not sessions and any(
            key in payload for key in ("sleepStartTimestampGMT", "startTimeGMT")
        ):
            sessions = [payload]
        inserted = 0
        for session_index, item in enumerate(sessions):
            if not isinstance(item, dict):
                continue
            start = self._timestamp_utc(
                item.get("start_time_utc")
                or item.get("startTimeGMT")
                or item.get("sleepStartTimestampGMT"),
                day,
            )
            end = self._timestamp_utc(
                item.get("end_time_utc")
                or item.get("endTimeGMT")
                or item.get("sleepEndTimestampGMT"),
                day,
            )
            if end < start:
            ...
            kind = (
                ...
                or item.get("sleepType")
                or ("nap" if item.get("isNap") else "main_sleep")
            )
            kind = kind if kind in {"main_sleep", "nap"} else "unknown"
            cursor = conn.execute(
                "INSERT INTO sleep_sessions(subject_id,session_type,start_time_utc,end_time_utc,values_json,extras_json,source_map_json,source_revision_id) VALUES(?,?,?,?,?,?,?,?)",
                (
                    subject,
                    kind,
                    start,
                    ...
                    stable_json(item).decode("utf-8"),
                    "{}",
                    json.dumps({"source_revision_id": revision}, allow_nan=False),
                    revision,
                ),
            )
            session_id = require_lastrowid(cursor)
            ...
            if not stages and isinstance(item.get("sleepLevelsMap"), dict):
                points = sorted(
                    (self._timestamp_utc(mark, day), level)
                    for mark, level in item["sleepLevelsMap"].items()
                )
                stages = [
                    {
                        "stage_type": str(level),
                        "start_time_utc": begin,
            ...
            for stage_index, stage in enumerate(stages):
                if not isinstance(stage, dict):
                    continue
                stage_start = self._timestamp_utc(
                    stage.get("start_time_utc")
                    or stage.get("startTimeGMT")
                    or stage.get("startTimestampGMT"),
                    day,
                )
                stage_end = self._timestamp_utc(
                    stage.get("end_time_utc")
                    or stage.get("endTimeGMT")
                    or stage.get("endTimestampGMT"),
                    day,
                )
                if stage_end < stage_start or stage_start < start or stage_end > end:
                    raise ValueError("sleep_stage_outside_session")
                duration = (
                    datetime.fromisoformat(stage_end.replace("Z", "+00:00"))
                    - datetime.fromisoformat(stage_start.replace("Z", "+00:00"))
                ).total_seconds()
                conn.execute(
                    "INSERT INTO sleep_stages(sleep_session_id,stage_index,stage_type,start_time_utc,end_time_utc,duration_seconds,source_revision_id) VALUES(?,?,?,?,?,?,?)",
                    (
                        session_id,
                        stage_index,
                        str(
                            stage.get("stage_type")
                            or stage.get("stageType")
                            or "unknown"
                        ),
                        stage_start,
                        stage_end,
    ...
    def _supersede_health_projection(
        self,
        conn: sqlite3.Connection,
        subject: int,
        resource: str,
    ...
    def _project_advanced(
        ...
        if resource == "max_metrics":
            ...
            for index, outer in enumerate(records):
                generic_value = outer.get("generic")
                generic: dict[str, Any] = (
                    generic_value if isinstance(generic_value, dict) else outer
                )
                chosen = next(
                    (
                        field
                        for field in ("vo2MaxPreciseValue", "vo2MaxValue", "vo2Max")
                ...
                spec = specs[chosen]
                stamp = self._timestamp_utc(
                    generic.get("calendarDate") or outer.get("calendarDate"),
                    day,
                    allow_day_boundary=True,
                )
                self._assert_sample_day(stamp, day)
                cursor = conn.execute(
                    """INSERT INTO physiology_records(subject_id,domain,record_type,provider_record_id,effective_at_utc,local_date,value_origin,extras_json,source_revision_id)
                ...
                source_path = (
                    f"/*/generic/{chosen}" if generic is not outer else f"/{chosen}"
                )
                conn.execute(
                    """INSERT INTO physiology_metrics(physiology_record_id,metric_key,value_number,raw_unit,canonical_unit,value_origin,source_path)
                       VALUES(?,?,?,?,?,?,?)""",
                    (
                        require_lastrowid(cursor),
                        spec[0],
                        float(generic[chosen]),
                        spec[1],
                        spec[2],
                        spec[3],
                        source_path,
        ...
        if (
            ...
            and any(
                isinstance(record, dict) and "__trainlab_lactate_family" in record
            ...
            for index, record in enumerate(payload):
                ...
                if not isinstance(family, str):
                    continue
                lactate_spec = lactate_specs.get(family)
                value = record.get("value")
                if (
                    lactate_spec is None
                    or not isinstance(value, (int, float))
                    or isinstance(value, bool)
                ):
                    continue
                ...
                self.repo.map_field(conn, resource, source_path, lactate_spec[0])
                inserted += 1
            return inserted
        records = self._advanced_records(payload)
    ...
    def _project_health(
        ...
        if resource in daily_resources or resource == "weigh_ins":
            ...
            if any(
                ...
                projected += 1
        if resource in sampled_resources:
            projected += self._project_samples(
                conn, subject, resource, day, payload, revision
            )
        if resource == "sleep":
            projected += self._project_sleep(conn, subject, day, payload, revision)
        if resource in body_resources:
            projected += self._project_body_measurements(
                conn, subject, resource, day, payload, revision
            )
        if (
            resource in physiology_resources
            or resource
            not in daily_resources | sampled_resources | body_resources | {"sleep"}
        ):
        ...
        if resource == "hrv" and isinstance(payload, dict):
            ...
            return (
                ...
                and any(
                    ...
                    for key in ("lastNightAvg", "weeklyAvg", "lastNight5MinHigh")
                )
            ) or (
                isinstance(readings, list)
                and any(
                    isinstance(item, dict) and numeric(item.get("hrvValue"))
                    for item in readings
                )
            )
        if resource == "heart_rates" and isinstance(payload, dict):
            return any(
                numeric(payload.get(key))
                for key in DAILY_SCALAR_METRICS[resource]
                if key != "dailyAverageHeartRate"
            ) or (
                isinstance(payload.get("heartRateValues"), list)
                and any(
                    isinstance(item, list) and len(item) >= 2 and numeric(item[1])
                    for item in payload["heartRateValues"]
                )
            )
        if resource == "spo2" and isinstance(payload, dict):
            return any(
                numeric(payload.get(key)) for key in DAILY_SCALAR_METRICS[resource]
            ) or (
                isinstance(payload.get("spO2HourlyAverages"), list)
                and any(
                    isinstance(item, list) and len(item) >= 2 and numeric(item[1])
                    for item in payload["spO2HourlyAverages"]
                )
            )
        if resource == "max_metrics":
            records = payload if isinstance(payload, list) else [payload]
            for record in records:
                if not isinstance(record, dict):
                    continue
                generic_value = record.get("generic")
                generic = generic_value if isinstance(generic_value, dict) else record
                if any(
                    numeric(generic.get(key))
