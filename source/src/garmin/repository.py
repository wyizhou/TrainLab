...
class GarminRepository:
    ...
    def connect(self, *, readonly: bool = False) -> sqlite3.Connection:
        conn = sqlite3.connect(
            f"file:{self.config.database_path}?mode=ro"
            if readonly
            else self.config.database_path,
    ...
    def start_run(
        ...
        existing = conn.execute(
            "SELECT id,run_id,status FROM garmin_sync_runs WHERE invocation_id=?",
            (invocation,),
        ).fetchone()
        if existing:
            full = conn.execute(
                "SELECT * FROM garmin_sync_runs WHERE id=?", (existing["id"],)
            ).fetchone()
            receipt.run_id, receipt.status = full["run_id"], full["status"]
            receipt.started_at_utc = full["started_at_utc"]
            receipt.requested_range = {
                "from": full["requested_from_local_date"],
                "through": full["requested_through_local_date"],
            }
            receipt.effective_range = {
                "from": full["actual_from_local_date"],
                "through": full["actual_through_local_date"],
            }
            receipt.counts.update(
                {
                    "fetched": full["fetched_count"],
                    "empty": full["empty_count"],
                    "unchanged": full["unchanged_count"],
                    "revised": full["revised_count"],
                    "failed": full["failed_count"],
                    "deferred": full["deferred_count"],
                }
            )
            for item_status, count in conn.execute(
                ...
                counter = (
                    "not_available" if item_status == "not_supported" else item_status
                )
            ...
            receipt.next_retry_at_utc, receipt.completed_at_utc = (
                full["next_retry_at_utc"],
                full["completed_at_utc"],
            )
            if full["error_summary"]:
                receipt.errors.append(
                    {
                        "code": full["error_summary"],
                        "resource": "garmin",
                        "logical_object_key": "garmin:run",
                        "summary": full["error_summary"],
                    }
                )
        ...
        conn.execute(
            """INSERT INTO garmin_sync_runs(run_id,invocation_id,subject_id,mode,requested_from_local_date,requested_through_local_date,resource_catalog_version,collector_version,garminconnect_version,parser_version,status,started_at_utc)
                     VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                run_id,
                invocation,
                subject_id,
                request.mode
                if request.mode != "auth" and request.mode != "status"
                else "audit",
                request.health_from_local_date,
                request.through_local_date or request.snapshot_local_date,
                CATALOG_VERSION,
                COLLECTOR_VERSION,
    ...
    def finish_run(
        ...
        conn.execute(
            """UPDATE garmin_sync_runs SET status=?,actual_from_local_date=?,actual_through_local_date=?,fetched_count=?,empty_count=?,unchanged_count=?,revised_count=?,failed_count=?,deferred_count=?,next_retry_at_utc=?,error_summary=?,completed_at_utc=? WHERE id=?""",
            (
                receipt.status,
                actual_from,
                actual_through,
                receipt.counts["fetched"],
                receipt.counts["empty"],
                receipt.counts["unchanged"],
                receipt.counts["revised"],
                receipt.counts["failed"],
                receipt.counts["deferred"],
                receipt.next_retry_at_utc,
                safe_provider_error_code(receipt.errors[0]["code"])
                if receipt.errors
                else None,
    ...
    def item(
        ...
        *,
        ...
        error: GarminError | None = None,
        next_retry: str | None = None,
        increment_attempt: bool = True,
    ) -> None:
        allowed = {
            "pending": {"running", "failed", "deferred"},
            "running": {
                "fetched",
                "empty",
                "unchanged",
        ...
        row = conn.execute(
            "SELECT status FROM garmin_sync_items WHERE garmin_sync_run_id=? AND resource_kind=? AND logical_object_key=? AND stage=?",
            (run_id, resource, key, stage),
        ).fetchone()
        terminal = {
            "fetched",
            "empty",
        ...
        if (
            ...
            and status not in allowed.get(row["status"], set())
            and not replay_allowed
        ):
            raise ValueError("invalid_item_transition")
        now = utc_now()
        completed = (
            now
            if status
        ...
        conn.execute(
            """INSERT INTO garmin_sync_items(garmin_sync_run_id,resource_kind,logical_object_key,stage,status,attempt_count,http_status,error_code,error_summary,next_retry_at_utc,source_revision_id,started_at_utc,completed_at_utc)
    ...
    def gap(
        ...
        *,
        ...
        now = utc_now()
        status = "deferred" if deferred else "open"
        reason = safe_provider_error_code(reason)
        conn.execute(
            """INSERT INTO garmin_sync_gaps(subject_id,resource_kind,logical_object_key,window_start_local_date,window_end_local_date,stage,reason_code,status,next_retry_at_utc,first_seen_at_utc,last_attempt_at_utc,source_revision_id)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(subject_id,resource_kind,logical_object_key,window_start_local_date,window_end_local_date,stage) WHERE status IN ('open','deferred') DO UPDATE SET reason_code=excluded.reason_code,status=excluded.status,next_retry_at_utc=excluded.next_retry_at_utc,last_attempt_at_utc=excluded.last_attempt_at_utc,attempt_count=garmin_sync_gaps.attempt_count+1""",
            (
                subject,
                resource,
                key,
    ...
    def capability(
        ...
        *,
        ...
        conn.execute(
            ...
            (subject, environment_key, resource, state, reason, now, now, next_probe),
        )
    ...
    def reconcile_legacy_capability_environment(
        ...
        try:
            ...
            for legacy in legacy_rows:
                ...
                newest = max(
                    (legacy, current),
                    key=lambda row: (row["last_checked_at_utc"], row["id"]),
                )
                first_checked = min(
                    legacy["first_checked_at_utc"], current["first_checked_at_utc"]
                )
                last_checked = max(
                    legacy["last_checked_at_utc"], current["last_checked_at_utc"]
                )
                conn.execute(
                    """UPDATE garmin_resource_capabilities
                       ...
                       WHERE id=?""",
                    (
                        newest["capability_state"],
                        newest["reason_code"],
                        newest["reason_summary"],
                        first_checked,
                        last_checked,
                        newest["next_probe_at_utc"],
                        newest["source_revision_id"],
                        current["id"],
                    ),
                )
                conn.execute(
                    "DELETE FROM garmin_resource_capabilities WHERE id=?",
                    (legacy["id"],),
                )
            completed = {
                "fetched": "supported",
            ...
            for resource, coverage in latest_coverage.items():
                ...
                if (
                    current is not None
                    and current["last_checked_at_utc"] >= coverage["observed_at_utc"]
                ):
                    continue
    ...
    @staticmethod
    def _verify_filefd(
        fd: int, payload: bytes, *, expected_size: int | None = None
    ) -> None:
        info = os.fstat(fd)
        if (
    ...
    def _cleanup_owned_temp(
        ...
        *,
        ...
        try:
            ...
            os.rename(
                temporary_name, claim, src_dir_fd=directory_fd, dst_dir_fd=directory_fd
            )
        ...
        try:
            ...
            fd = os.open(
                claim, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0), dir_fd=directory_fd
            )
            try:
                opened = os.fstat(fd)
    ...
    def store_raw(
        self, resource: str, payload: bytes, suffix: str, media_type: str
    ) -> tuple[int, str]:
        ...
        if existing is not None:
            try:
                ...
                try:
                    self._verify_filefd(
                        stored_fd, payload, expected_size=int(existing["size_bytes"])
                    )
                finally:
                    os.close(stored_fd)
        ...
        try:
            ...
            if before_final is not None:
                ...
                try:
                    ...
                    if (before_final.st_dev, before_final.st_ino) != (
                        opened_final.st_dev,
                        opened_final.st_ino,
                    ) or (after_final.st_dev, after_final.st_ino) != (
                        opened_final.st_dev,
                        opened_final.st_ino,
                    ):
                        raise ValueError("raw_object_corrupt")
                ...
                try:
                    ...
                    if (
                        not stat.S_ISREG(tmp_info.st_mode)
                        or tmp_info.st_uid != os.getuid()
                    ):
                        raise ValueError("unsafe_raw_path")
                    ...
                    try:
                        ...
                        if (before_final.st_dev, before_final.st_ino) != (
                            ...
                            raise ValueError("raw_object_corrupt")
                        if won and (final_stat.st_dev, final_stat.st_ino) != (
                            tmp_device,
                            tmp_inode,
                        ):
                            raise ValueError("raw_object_corrupt")
        ...
        try:
            ...
            conn.execute("BEGIN IMMEDIATE")
            conn.execute(
                "INSERT OR IGNORE INTO raw_objects(sha256,relative_path,media_type,size_bytes,provider,resource_kind,original_name,fetched_at_utc) VALUES(?,?,?,?,?,?,?,?)",
                (sha, rel, media_type, len(payload), "garmin", resource, None, now),
            )
            row = conn.execute(
                """SELECT id,relative_path,size_bytes,media_type
                   FROM raw_objects WHERE sha256=?""",
            ...
            try:
                self._verify_filefd(
                    stored_fd, payload, expected_size=int(row["size_bytes"])
                )
            finally:
                os.close(stored_fd)
            if row["size_bytes"] != len(payload) or row["media_type"] != media_type:
                raise ValueError("raw_object_corrupt")
            conn.execute("COMMIT")
        except Exception:
            if conn.in_transaction:
    ...
    def publish_revision(
        ...
        *,
        ...
        current = conn.execute(
            "SELECT id,payload_hash,profile_version,revision_no FROM source_revisions WHERE provider='garmin' AND resource_kind=? AND provider_object_id=? AND is_current=1",
            (resource, provider_id),
        ).fetchone()
        if (
            current
            and current["payload_hash"] == semantic_hash
        ...
        try:
            current = conn.execute(
                "SELECT id,payload_hash,profile_version FROM source_revisions WHERE provider='garmin' AND resource_kind=? AND provider_object_id=? AND is_current=1",
                (resource, provider_id),
            ).fetchone()
            if (
                current
            ...
            received = conn.execute(
                "SELECT id FROM source_revisions WHERE provider='garmin' AND resource_kind=? AND provider_object_id=? AND payload_hash=? AND profile_version IS ? AND parsed_at_utc IS NULL ORDER BY revision_no DESC LIMIT 1",
                (resource, provider_id, semantic_hash, profile_version),
            ).fetchone()
            if received is None:
                rev = int(
                    conn.execute(
                        "SELECT coalesce(max(revision_no),0)+1 FROM source_revisions WHERE provider='garmin' AND resource_kind=? AND provider_object_id=?",
                        (resource, provider_id),
                    ).fetchone()[0]
                )
                conn.execute(
                    "INSERT INTO source_revisions(provider,resource_kind,provider_object_id,revision_no,raw_object_id,payload_hash,parser_name,parser_version,profile_version,is_current,parsed_at_utc) VALUES(?,?,?,?,?,?,?,?,?,0,NULL)",
                    (
                        "garmin",
                        resource,
                        provider_id,
        ...
        try:
            ...
            conn.execute(
                "UPDATE source_revisions SET is_current=1,parsed_at_utc=? WHERE id=?",
                (utc_now(), revision),
            )
            conn.execute("COMMIT")
        ...
        *,
        ...
        if snapshot:
            ...
            if row is not None:
                ...
                return
        conn.execute(
            "INSERT INTO resource_coverage(subject_id,provider,resource_kind,local_date,availability_state,record_count,source_revision_id,observed_at_utc) VALUES(?,?,?,?,?,?,?,?)",
            (subject, "garmin", resource, day, state, count, revision, utc_now()),
        )
        ...
        def walk(value: Any, path: str = ""):
            if isinstance(value, dict):
                for key, child in value.items():
                    yield from walk(child, f"{path}/{segment(key)}")
            elif isinstance(value, list):
                for child in value:
                    yield from walk(child, f"{path}/*")
            else:
                yield path or "/", type(value).__name__

        now = utc_now()
        for path, kind in walk(payload):
            conn.execute(
                "INSERT INTO source_field_catalog(provider,resource_kind,field_path,observed_type,first_seen_at_utc,last_seen_at_utc,mapping_state) VALUES(?,?,?,?,?,?,?) ON CONFLICT(provider,resource_kind,field_path) DO UPDATE SET last_seen_at_utc=excluded.last_seen_at_utc,observed_type=excluded.observed_type",
                ("garmin", resource, path, kind, now, now, "unknown"),
            )
    ...
    def passthrough_field(
        self, conn: sqlite3.Connection, resource: str, field_path: str
    ) -> None:
        conn.execute(
    ...
    def advance_cursor(
        ...
        *,
        ...
        if partial:
            return
        current = conn.execute(
            "SELECT complete_through_local_date FROM garmin_sync_cursors WHERE subject_id=? AND resource_kind=? AND cursor_grain='local_date'",
            (subject, resource),
        ).fetchone()
        current_candidate: date | None
        if current and current[0]:
            current_candidate = date.fromisoformat(current[0]) + timedelta(days=1)
        else:
            first = conn.execute(
                "SELECT min(local_date) FROM resource_coverage WHERE subject_id=? AND provider='garmin' AND resource_kind=? AND local_date<=?",
                (subject, resource, through),
            ).fetchone()[0]
            current_candidate = date.fromisoformat(first) if first else None
        latest = None
        candidate: date | None = current_candidate
        while candidate and candidate.isoformat() <= through:
            day = candidate.isoformat()
            coverage = conn.execute(
                "SELECT availability_state FROM resource_coverage WHERE subject_id=? AND provider='garmin' AND resource_kind=? AND local_date=? ORDER BY id DESC LIMIT 1",
                (subject, resource, day),
            ).fetchone()
            gap = conn.execute(
                "SELECT 1 FROM garmin_sync_gaps WHERE subject_id=? AND resource_kind=? AND window_start_local_date<=? AND window_end_local_date>=? AND status IN ('open','deferred')",
                (subject, resource, day, day),
            ).fetchone()
            if (
                not coverage
                or coverage[0]
        ...
        if latest:
            conn.execute(
                "INSERT INTO garmin_sync_cursors(subject_id,resource_kind,cursor_grain,complete_through_local_date,last_success_at_utc,last_run_id,catalog_version) VALUES(?,?,?,?,?,?,?) ON CONFLICT(subject_id,resource_kind,cursor_grain) DO UPDATE SET complete_through_local_date=excluded.complete_through_local_date,last_success_at_utc=excluded.last_success_at_utc,last_run_id=excluded.last_run_id,catalog_version=excluded.catalog_version",
                (
                    subject,
                    resource,
                    "local_date",
