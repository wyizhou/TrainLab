...
def _garmin_failed_receipt(mode: str, code: str):
    safe_mode = cast(GarminMode, mode if mode in _GARMIN_MODES else "status")
...
def _garmin_mode(args: Any) -> GarminMode:
    value = (
        getattr(args, "garmin_sync_mode", None)
        if getattr(args, "garmin_mode", None) == "sync"
        else getattr(args, "garmin_mode", "status")
    )
    if not isinstance(value, str) or value not in _GARMIN_MODES:
        raise ValueError("invalid_mode")
    return cast(GarminMode, value)
...
def garmin_cli_execute(
    ...
    *,
    ...
    try:
        ...
        return _garmin_failed_receipt("status", "invalid_request")
    try:
