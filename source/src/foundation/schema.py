...
def validate_schema_manifest(
    ...
    for name, spec in manifest["tables"].items():
        ...
        foreign = {
            row[3]: row[2] for row in conn.execute(f"PRAGMA foreign_key_list({name})")
        }
        if any(
            foreign.get(column) != target
        ...
        for item in conn.execute(f"PRAGMA index_list({name})"):
            ...
            indexes.append(
                (
                    ...
                    bool(unique),
                    bool(partial),
                    "" if not index_sql_row else index_sql_row[0] or "",
                )
            )
        for fields in spec.get("unique", []):
            if fields != primary_key and not any(
                unique and cols == fields
                for cols, unique, _partial, _statement in indexes
            ):
                errors.append(f"unique:{name}:{','.join(fields)}")
        for kind, marker in (
            ("current_unique", "whereis_current=1"),
            ("active_unique", "whereis_active=1"),
        ):
            for fields in spec.get(kind, []):
                if not any(
                    ...
                    and partial
                    and cols == fields
                    and marker in re.sub(r"\s+", "", statement).lower()
                    for cols, unique, partial, statement in indexes
                ):
                    errors.append(
                        f"{kind.split('_')[0]}_unique:{name}:{','.join(fields)}"
                    )
        for partial_spec in spec.get("partial_unique", []):
            ...
            if not any(
                ...
                and partial
                and cols == fields
                and f"where{where}" in re.sub(r"\s+", "", statement).lower()
                for cols, unique, partial, statement in indexes
            ):
                errors.append(f"partial_unique:{name}:{','.join(fields)}")
        for column, choices in spec.get("enums", {}).items():
            match = re.search(
                re.escape(column.lower()) + r"text.*?check\(([^)]*)\)", sql
            )
            if match is None or not all(
                f"'{choice}'" in match.group(1) for choice in choices
            ):
    ...
    for name, spec in manifest["views"].items():
        ...
        if (
            normal(spec["source"]) not in sql
            or ("filter" in spec and normal(spec["filter"]) not in sql)
            or ("order" in spec and normal(spec["order"]) not in sql)
            or ("trust_marker" in spec and normal(spec["trust_marker"]) not in sql)
        ):
    ...
    for name, spec in manifest.get("triggers", {}).items():
        ...
        if (
            row is None
            or row[0] != spec["table"]
            or re.sub(r"\s+", "", spec["token"]).lower()
            not in re.sub(r"\s+", "", row[1]).lower()
        ):
