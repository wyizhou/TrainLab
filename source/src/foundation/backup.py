...
def encrypt_container(plaintext: bytes, key: bytes | Any, nonce: bytes) -> bytes:
    return (
        ...
        + VERSION
        + nonce
        + AESGCM(require_key(key)).encrypt(nonce, plaintext, MAGIC + VERSION)
    )


def decrypt_container(payload: bytes, key: bytes | Any) -> bytes:
    if (
        len(payload) < len(MAGIC) + 1 + 12
        or payload[:4] != MAGIC
        or payload[4:5] != VERSION
    ):
        raise ValueError("invalid_backup_container")
    return AESGCM(require_key(key)).decrypt(
        payload[5:17], payload[17:], MAGIC + VERSION
    )
