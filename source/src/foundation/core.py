        ...
        if snapshot and self._foundation_parent_fd >= 0:
            ...
            try:
                snapshot_fd = os.open(
                    ...
                    dir_fd=self._foundation_parent_fd,
                )
                ...
                for child, identity in expected.items():
                    ...
                    os.rename(
                        child, claim, src_dir_fd=snapshot_fd, dst_dir_fd=snapshot_fd
                    )
                    claimed = os.stat(claim, dir_fd=snapshot_fd, follow_symlinks=False)
                    if not (
                        stat.S_ISREG(claimed.st_mode)
                    ...
                    rechecked = os.stat(
                        claim, dir_fd=snapshot_fd, follow_symlinks=False
                    )
...
LEGACY_V1_MANIFEST_SHA256 = (
    "e406492dc044dcc17c5e08776614b9f73ec08580b6c56c14898d9085708e6e88"
)
LEGACY_V2_MANIFEST_SHA256 = (
    "24b94ca0cffbc61a512fd29a968917020724c276c15e82413485549564b01c4c"
)
# The historical v2 compatibility fixtures are reconstructed against the
# current four-layer table set.  Orchestration tables are intentionally not
# part of that set, so these fingerprints are the no-orchestration v2 shape.
LEGACY_V2_SCHEMA_OBJECTS_SHA256 = (
    "e6505d8f81c99e4a0a7484f3b021c601df1db31da3cf89729ec6d36aafbeee03"
)
LEGACY_V2_FROM_V1_SCHEMA_OBJECTS_SHA256 = (
    "76d9c7f6e7648c20fe0a29440ef0c2bfc0d9f94301dc488399380a13ee1f4b07"
)
LEGACY_V1_SCHEMA_OBJECTS_SHA256 = (
    "5d3237e3d2688c66b9f363bc02335908d0d1a6392f4be7afed10e5eed85e499f"
)
...
class FoundationConfig:
    ...
    def load(cls, project_root: Path) -> "FoundationConfig":
        ...
        for directory in (config_path.parent,):
        ...
        schema = json.loads(resource_bytes("harness/schemas/foundation.schema.json"))
        ...
        result = cls(
            root,
            inside(raw["database_path"], role="database"),
            inside(raw["raw_root"], role="raw"),
            inside(raw["state_root"], role="state"),
            inside(raw["ready_marker"], role="marker"),
            inside(raw["lock_path"], role="lock"),
            allowed,
        )
    ...
    def validate_paths(self) -> None:
        ...
        for role, path in targets.items():
            ...
            if any(part in {"", ".", ".."} for part in relative.parts):
                raise ValueError(f"unsafe_foundation_{role}_path")
        if (
            self.database_path in {self.raw_root, self.state_root}
            or self.raw_root == self.state_root
        ):
        ...
        for target in (root, *targets.values()):
            ...
            for part in parts:
                ...
                if stat.S_ISLNK(info.st_mode) or (
                    current != target and not stat.S_ISDIR(info.st_mode)
                ):
...
class FoundationTool:
    ...
    def execute(self, request: FoundationRequest) -> FoundationReceipt:
        ...
        safe_invocation = (
            ...
            and _INVOCATION_RE.fullmatch(raw_invocation)
            else ""
        )
        receipt = FoundationReceipt(
            invocation_id=safe_invocation, mode=raw_mode if raw_mode in _ALLOWED else ""
        )
        try:
            ...
            receipt.next_action = (
                "explicit_migrate"
                if str(exc).startswith("higher_schema_version")
            ...
            receipt.errors.append(
                {"code": type(exc).__name__, "summary": "foundation_operation_failed"}
            )
    ...
    def _validate_request(request: FoundationRequest) -> None:
        ...
        if (
            not isinstance(request, FoundationRequest)
            or request.mode not in _ALLOWED
            or not isinstance(request.invocation_id, str)
            or _INVOCATION_RE.fullmatch(request.invocation_id) is None
            or not isinstance(request.requested_at_utc, str)
        ):
    ...
    def _paths(self, root: Path) -> dict[str, Path]:
        return {
            ...
            "raw": self.config.raw_root,
            "state": self.config.state_root,
            "ready": self.config.ready_marker,
    ...
    def _open_checked_directory(self, directory: Path, *, create: bool = False) -> int:
        ...
        def acceptable(info: os.stat_result, location: Path) -> bool:
            ...
            return (
                ...
                and (
                    ...
                    if strict
                    else not (stat.S_IMODE(info.st_mode) & 0o022)
                )
            )
        ...
        try:
            ...
            if (opened.st_dev, opened.st_ino) != (before.st_dev, before.st_ino):
                raise OSError("foundation_directory_replaced")
            current = anchor
            for part in relative.parts:
                ...
                try:
                    ...
                    if not create or (
                        self.config.allowed_root is not None
                        and not (
                            current == self.data_root
                            or self.data_root in current.parents
                        )
                    ):
                        raise OSError("foundation_directory_missing")
                ...
                if not (
                    acceptable(child_info, current)
                    and (child_info.st_dev, child_info.st_ino)
    ...
    def _secure_read_file(self, path: Path, limit: int = 8192) -> bytes:
        ...
        try:
            try:
                ...
                raise OSError("foundation_file_missing")
            if not (
                stat.S_ISREG(before.st_mode)
                and before.st_uid == os.getuid()
                and stat.S_IMODE(before.st_mode) == 0o600
            ):
                raise OSError("unsafe_foundation_file")
            fd = os.open(
        ...
        try:
            for part in relative.parts:
                ...
                if not (
                    ...
                    os.close(child)
                    raise ValueError("unsafe_foundation_directory")
                os.close(fd)
                fd = child
        finally:
            os.close(fd)
    ...
    def _read_lock_at(
        ...
        if (
            len(raw) > 512
            or (opened.st_dev, opened.st_ino) != ident
            or (after.st_dev, after.st_ino) != ident
        ):
            raise BlockingIOError("foundation_lock_busy")
        try:
    ...
    def _claim_lock_at(
        ...
        try:
            ...
            if actual != identity or actual_raw != raw:
    ...
    def _delete_owned_claim_at(
        self, parent_fd: int, claim: str, identity: tuple[int, int], raw: bytes
    ) -> None:
        _, actual, seen = self._read_lock_at(parent_fd, claim)
        if actual != identity or seen != raw:
            raise BlockingIOError("foundation_lock_replaced")
        os.unlink(claim, dir_fd=parent_fd)
        os.fsync(parent_fd)

    @staticmethod
    def _claim_created_inode_at(
    ...
    def _append_lock_audit(self, paths: dict[str, Path]) -> None:
        state_fd = self._open_checked_directory(paths["state"])
        audit_fd = -1
        name = "foundation-lock-recoveries.jsonl"
        try:
            try:
            ...
            after = os.stat(name, dir_fd=state_fd, follow_symlinks=False)
            if not (
                stat.S_ISREG(opened.st_mode)
                and opened.st_uid == os.getuid()
                and stat.S_IMODE(opened.st_mode) == 0o600
            ...
            self._write_all(
                audit_fd,
                json.dumps(
                    {"recovered_at_utc": _utc(), "event": "stale_lock_recovered"},
                    sort_keys=True,
                ).encode()
                + b"\n",
            )
    ...
    def _has_unresolved_lock_claim_at(parent_fd: int, name: str) -> bool:
        ...
        return any(
            entry.startswith(prefix) and len(entry) > len(prefix) for entry in entries
        )
    ...
    def _lock(self, root: Path):
        ...
        try:
            ...
            try:
                payload, identity, data = self._read_lock_at(parent_fd, name)
                datetime.fromisoformat(payload["started_at_utc"][:-1] + "+00:00")
                try:
                    os.kill(payload["pid"], 0)
                except ProcessLookupError:
                    claimed = self._claim_lock_at(
                        parent_fd, name, identity, data, "stale"
                    )
        ...
        record = json.dumps(
            {"pid": os.getpid(), "started_at_utc": _utc(), "uid": os.getuid()},
            sort_keys=True,
        ).encode()
        try:
            opened = os.fstat(fd)
            ...
            if verified != inode or seen != record:
                raise BlockingIOError("foundation_lock_replaced")
        except Exception as exc:
            try:
                os.close(fd)
        ...
        try:
            ...
            try:
                ...
                if identity != inode or raw != record:
                    raise BlockingIOError("foundation_lock_replaced")
                released = self._claim_lock_at(
                    parent_fd, name, inode, record, "release"
                )
    ...
    def _connect(self, path: Path, *, readonly: bool = False) -> sqlite3.Connection:
        ...
        name = path.name
        cleanup_prefix = f".{name}.create-cleanup-"
        intent_prefix = f".{name}.create-intent-"
        snapshot_prefix = ".foundation-readonly-"
        try:
            if any(
                (
                    entry.startswith(cleanup_prefix)
                    or entry.startswith(intent_prefix)
                    or entry.startswith(snapshot_prefix)
                )
            ...
            os.close(parent_fd)
            raise IncompatibleError("sqlite_create_cleanup_scan_failed") from exc
        ...
        def checked_stat(item: str) -> os.stat_result:
            ...
            if not (
                ...
                raise IncompatibleError("sqlite_path_unsafe")
            return value
        ...
        def _strict_source(
            source_name: str, source_fd: int | None = None
        ) -> tuple[int, tuple[int, int, int, int, int, bytes]]:
            ...
            if (pre.st_dev, pre.st_ino) != (opened.st_dev, opened.st_ino) or (
                post.st_dev,
                post.st_ino,
        ...
        def _snapshot_children(snapshot_fd: int) -> dict[str, tuple[int, int]]:
            ...
            for child in os.listdir(snapshot_fd):
                ...
                if not stat.S_ISREG(info.st_mode):
                    raise IncompatibleError("sqlite_snapshot_unsafe")
                values[child] = (info.st_dev, info.st_ino)
            return values

        def _discard_snapshot(
            snapshot_name: str, snapshot_fd: int, expected: dict[str, tuple[int, int]]
        ) -> bool:
            ...
            try:
                ...
                os.close(snapshot_fd)
                snapshot_fd = -1
                os.rmdir(snapshot_name, dir_fd=parent_fd)
                os.fsync(parent_fd)
                return True
            except Exception:
                return False
        ...
        def make_wal_snapshot() -> tuple[
            str,
            Path,
            dict[str, tuple[int, int]],
            dict[str, tuple[int, int, int, int, int, bytes]],
        ]:
            """Create a stable, source-zero-write read-only WAL snapshot."""
            snapshot_name = f".foundation-readonly-{secrets.token_hex(16)}"
            snapshot_fd = -1
            sources: list[int] = []
            ...
            try:
                ...
                # at the source path.
                for source_name, source_fd in (
                    (name, fd),
                    (name + "-wal", None),
                    (name + "-shm", None),
                ):
                    actual_fd, evidence = _strict_source(source_name, source_fd)
                    sources.append(actual_fd) if source_fd is None else None
                    source_evidence[source_name] = evidence
                    if source_name == name + "-wal":
                        os.lseek(actual_fd, 0, os.SEEK_SET)
                        header = os.read(actual_fd, 32)
                    ...
                    try:
                        ...
                        copy_to_snapshot(actual_fd, destination)
                ...
                for source_name, source_fd in (
                    ...
                    recheck_fd: int | None = source_fd
                    if recheck_fd is None:
                        recheck_fd = next(
                            item
                            for item in sources
                            if (os.fstat(item).st_dev, os.fstat(item).st_ino)
                            == source_evidence[source_name][:2]
                        )
                    _, evidence = _strict_source(source_name, recheck_fd)
                    if evidence != source_evidence[source_name]:
                        raise IncompatibleError("sqlite_wal_state_unsafe")
                ...
                return (
                    snapshot_name,
                    path.parent / snapshot_name / name,
                    children,
                    source_evidence,
                )
            except Exception:
                if snapshot_fd >= 0:
                    try:
                        ...
                        _discard_snapshot(snapshot_name, snapshot_fd, expected)
                        snapshot_fd = -1
                raise
            finally:
        ...
        snapshot_evidence: dict[str, tuple[int, int, int, int, int, bytes]] | None = (
            None
        )
        try:
            try:
                before = checked_stat(name)
            except FileNotFoundError:
                if readonly:
                    raise IncompatibleError("sqlite_path_unsafe")
                created: tuple[int, int] | None = None
                created_raw: bytes | None = None
                created_name_owned = False
                parent_persisted = False
                ...
                try:
                    ...
                    created_raw = os.read(fd, 1024)
                    if created_raw != b"" or os.fstat(fd).st_size != 0:
                        raise IncompatibleError("sqlite_create_initial_content_unsafe")
                    os.fchmod(fd, 0o600)
                    os.fsync(fd)
                    os.fsync(parent_fd)
                    ...
                    if created_name_owned:
                        ...
                        try:
                            os.rename(
                                name, claim, src_dir_fd=parent_fd, dst_dir_fd=parent_fd
                            )
                            if (
                                created is None
                                or created_raw is None
                                or not parent_persisted
                            ):
                                # Cannot prove bytes or durable publication:
                                # retain the claim as the fail-closed record.
                                raise IncompatibleError("sqlite_create_cleanup_blocker")
                            claim_info = os.stat(
                                claim, dir_fd=parent_fd, follow_symlinks=False
                            )
                            cfd = os.open(
                                claim,
                                os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0),
                            ...
                            try:
                                ...
                                os.close(cfd)
                            if not (
                                stat.S_ISREG(claim_info.st_mode)
                                and claim_info.st_uid == os.getuid()
                                and stat.S_IMODE(claim_info.st_mode) == 0o600
                                and (claim_info.st_dev, claim_info.st_ino) == created
                                and (opened.st_dev, opened.st_ino) == created
                                and (after.st_dev, after.st_ino) == created
                                and raw == created_raw
                                and claim_info.st_size == len(created_raw)
                            ):
                ...
                os.rename(
                    intent, completion, src_dir_fd=parent_fd, dst_dir_fd=parent_fd
                )
            ...
            flags = (os.O_RDONLY if readonly else os.O_RDWR) | getattr(
                os, "O_NOFOLLOW", 0
            )
            fd = os.open(name, flags, dir_fd=parent_fd)
            opened = os.fstat(fd)
            after = checked_stat(name)
            ...
            if readonly:
                ...
                try:
                    wal = checked_stat(name + "-wal")
                    wal_nonempty = wal.st_size > 0
                    if wal_nonempty:
                        wal_identity = (wal.st_dev, wal.st_ino, wal.st_size)
                except FileNotFoundError:
                    wal_nonempty = False
                if wal_nonempty:
                    (
                ...
                conn = sqlite3.connect(
                    target, isolation_level=None, uri=True, factory=_PinnedConnection
                )
            else:
                conn = sqlite3.connect(
                    str(path), isolation_level=None, factory=_PinnedConnection
                )
            ...
            if not readonly:
                try:
                    final = checked_stat(name)
                except FileNotFoundError as exc:
                    raise IncompatibleError("sqlite_path_replaced") from exc
                if (final.st_dev, final.st_ino) != identity:
                    raise IncompatibleError("sqlite_path_replaced")
            try:
                post = checked_stat(name)
            except FileNotFoundError as exc:
                raise IncompatibleError("sqlite_path_replaced") from exc
            if (post.st_dev, post.st_ino) != identity:
                raise IncompatibleError("sqlite_path_replaced")
            ...
            for row in database_list:
                ...
                if (
                    ...
                    or not isinstance(database_file, str)
                ):
                    raise IncompatibleError("sqlite_database_list_unsafe")
            ...
            if (
                ...
                or not main
                or not Path(main).is_absolute()
                or Path(main).resolve() != expected_main
            ):
                raise IncompatibleError("sqlite_database_list_unsafe")
            if readonly and "wal_identity" in locals() and wal_identity is not None:
                try:
                    after_wal = checked_stat(name + "-wal")
                except (FileNotFoundError, IncompatibleError) as exc:
                    raise IncompatibleError("sqlite_wal_state_unsafe") from exc
                if (
                    after_wal.st_dev,
                    after_wal.st_ino,
            ...
            if conn is not None:
                ...
                if snapshot_name is not None and snapshot_children is not None:
                    ...
                    try:
                        ...
                        _discard_snapshot(snapshot_name, snapshot_fd, snapshot_children)
                        snapshot_fd = -1
                    except Exception:
                        if snapshot_fd >= 0:
                            try:
    ...
    def _manifest_path() -> Path:
        return resource_path("harness/schemas/foundation_schema_manifest.json")
    ...
    def _manifest_hash(self) -> str:
        ...
        canonical = json.dumps(
            self._manifest(), ensure_ascii=False, sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
    ...
    def _required_objects_and_permissions(self, paths: dict[str, Path]) -> list[str]:
        ...
        for path in required:
            directory = (
                path == paths["root"]
                or path in self._raw_directory_paths(paths)
                or path in {paths["state"], paths["lock"].parent}
            )
    ...
    def _migration_receipt_valid(conn: sqlite3.Connection, manifest_hash: str) -> bool:
        ...
        expected = (
            (
                ...
                "foundation_v1",
                "foundation-v1",
                {_FOUNDATION_V1_MIGRATION_SHA256, LEGACY_V1_MANIFEST_SHA256},
            ),
            (
                2,
                "mail_processing_state_v2",
                "foundation-v2",
                {LEGACY_V2_MANIFEST_SHA256},
            ),
            (4, "garmin_offline_rebuild_v4", "foundation-v4", {manifest_hash}),
        )
        for row, wanted in zip(rows, expected):
            ...
            if (
                ...
                != (expected_version, expected_description, expected_revision)
                or content not in expected_contents
                or not _canonical_utc(applied)
            ):
                return False
        return True
    ...
    def _read_verification(self, root: Path, receipt: FoundationReceipt) -> None:
    ...
    def _init(self, root: Path, receipt: FoundationReceipt) -> None:
        ...
        if paths["db"].exists():
            ...
            if (
                receipt.status == "incompatible"
                and receipt.next_action == "explicit_migrate"
            ):
        ...
        with self._lock(root):
            ...
            try:
                ...
                conn.execute(
                    "INSERT INTO foundation_state VALUES(1,'initializing',?,?,?,?,?)",
                    (
            ...
            try:
                ...
                if schema_errors or not self._migration_receipt_valid(
                    audit, manifest_hash
                ):
            ...
            try:
                conn.execute(
                    "UPDATE foundation_state SET state='ready', initialized_at_utc=?, updated_at_utc=?, implementation_version='foundation-v4' WHERE id=1",
                    (_utc(), _utc()),
                )
            finally:
                conn.close()
            receipt.status, receipt.ready, receipt.next_action = (
                "initialized",
                True,
                "none",
            )
            ...
            receipt.applied_migration_ids = [1, 2, 4]
            receipt.migration_start_version, receipt.migration_end_version = (
                0,
                FOUNDATION_SCHEMA_VERSION,
            )
    ...
    def _phase1_admissible(
        self, conn: sqlite3.Connection, paths: dict[str, Path], expected_hash: str
    ) -> bool:
        """Recognise only our committed phase-1 checkpoint, never a lookalike."""
        row = conn.execute(
            "SELECT id,state,schema_version,manifest_sha256,initialized_at_utc,updated_at_utc,implementation_version FROM foundation_state WHERE id=1"
        ).fetchone()
        if (
            row is None
            or tuple(row[:5])
            != (1, "initializing", FOUNDATION_SCHEMA_VERSION, expected_hash, None)
            or row[6] != "foundation-v1"
        ):
            return False
        updated = row[5]
        ...
        try:
            if (
                datetime.fromisoformat(updated[:-1] + "+00:00")
                .isoformat()
                .replace("+00:00", "Z")
                != updated
            ):
        ...
        if {(kind, name) for kind, name, _ in entries} != {
        ...
        if any(
            canonical(actual.get(name)) != canonical(ddl)
            for name, ddl in _PHASE1_DDL.items()
        ):
            return False
        ...
        expected_columns = {
            "foundation_state": [
                ...
                ("manifest_sha256", "TEXT", 1, 0, None),
                ("initialized_at_utc", "TEXT", 0, 0, None),
                ("updated_at_utc", "TEXT", 1, 0, None),
                ("implementation_version", "TEXT", 1, 0, None),
            ],
            "schema_migrations": [
                ("version", "INTEGER", 0, 1, None),
                ("description", "TEXT", 1, 0, None),
                ("applied_at_utc", "TEXT", 1, 0, None),
                ("code_revision", "TEXT", 1, 0, "'foundation-v1'"),
                ("content_sha256", "TEXT", 1, 0, None),
            ],
        }
        ...
        return (
            not [item for item in required if not item.endswith(paths["ready"].name)]
            and conn.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
        )

    def _apply_phase2_schema(
        self, conn: sqlite3.Connection, manifest_hash: str, *, emit_failpoint: bool
    ) -> None:
        ...
        for ordinal, (name, columns) in enumerate(TABLES.items()):
            ...
            if emit_failpoint and ordinal == 1:
                self._fire_failpoint("during_schema")
        ...
        conn.execute(
            ...
            ("foundation_v1", _utc(), "foundation-v1", _FOUNDATION_V1_MIGRATION_SHA256),
        )
        conn.execute(
        ...
        conn.execute(
            ...
            ("garmin_offline_rebuild_v4", _utc(), "foundation-v4", manifest_hash),
        )

    def _complete_phase1_recovery(
        self, paths: dict[str, Path], expected_hash: str, receipt: FoundationReceipt
    ) -> None:
        ...
        try:
            if (
                validate_schema_manifest(audit, self._manifest())
                or not self._migration_receipt_valid(audit, expected_hash)
                or audit.execute("PRAGMA integrity_check").fetchone()[0] != "ok"
            ):
                raise IncompatibleError("initial_schema_verification_failed")
        finally:
            audit.close()
        ...
        receipt.foundation_schema_version = FOUNDATION_SCHEMA_VERSION
        receipt.applied_migration_ids = [1, 2, FOUNDATION_SCHEMA_VERSION]
        receipt.warnings.append(
            {
                "code": "initialization_recovered",
                "summary": "continued verified phase-1 initialization",
            }
        )

    def _resume_or_reject_existing(
        self, paths: dict[str, Path], receipt: FoundationReceipt
    ) -> None:
        ...
        """
        try:
            conn: sqlite3.Connection | None = self._connect(paths["db"])
            assert conn is not None
        except sqlite3.DatabaseError as exc:
        ...
        try:
            ...
            if schema_version != FOUNDATION_SCHEMA_VERSION:
                if (
                    state["state"] == "ready"
                    and schema_version < FOUNDATION_SCHEMA_VERSION
                ):
                    conn.close()
            ...
            if (
                ...
                or state["manifest_sha256"] != expected_hash
            ):
            ...
            errors.extend(
                ...
                if not error.endswith(paths["ready"].name)
            )
            if errors:
            ...
            conn.execute(
                "UPDATE foundation_state SET state='ready', initialized_at_utc=COALESCE(initialized_at_utc,?), updated_at_utc=?, implementation_version='foundation-v4' WHERE id=1",
                (_utc(), _utc()),
            )
            receipt.status, receipt.ready, receipt.next_action = (
                "initialized",
            ...
            receipt.warnings.append(
                {
                    "code": "initialization_recovered",
                    "summary": "completed verified initializing state",
                }
            )
        except sqlite3.DatabaseError as exc:
    ...
    def _migrate(
        self, root: Path, target: int | None, receipt: FoundationReceipt
    ) -> None:
        ...
        with self._lock(root):
            ...
            try:
                ...
                if current == FOUNDATION_SCHEMA_VERSION:
                    ...
                    if state["implementation_version"] != "foundation-v4":
                        errors.append("database_implementation_version_mismatch")
                    if not _canonical_utc(
                        state["initialized_at_utc"]
                    ...
                    if [row[0] for row in conn.execute("PRAGMA integrity_check")] != [
                        "ok"
                    ]:
                    ...
                    self._atomic_json(
                        ...
                        expected_existing={
                            "schema_version": 2,
                            "manifest_sha256": LEGACY_V2_MANIFEST_SHA256,
                            "ready": True,
                        },
                    )
                    receipt.status, receipt.ready, receipt.next_action = (
                    ...
                    receipt.warnings.append(
                        {
                            "code": "migration_marker_republished",
                            "summary": "completed explicit migration publication",
                        }
                    )
                    return
                ...
                source_error = (
                    self._published_v1_source_error(conn, paths)
                    if current == 1
                    else self._published_v2_source_error(conn, paths)
                )
                if source_error:
                    raise IncompatibleError(source_error)
                if current == 1:
                    ...
                    conn.close()
                    conn = None
                    self._migrate_v2_analysis_input_trust(
                        paths, receipt, migration_start=1
                    )
                else:
                    conn.close()
                    conn = None
                    self._migrate_v2_analysis_input_trust(
                        paths, receipt, migration_start=2
                    )
            finally:
                if conn is not None:
                    conn.close()
    ...
    def _repair_v3_timezone_contract(self, conn: sqlite3.Connection) -> None:
        ...
        subject_count = int(
            conn.execute("SELECT COUNT(*) FROM data_subjects").fetchone()[0]
        )
        plan_count = int(
            conn.execute("SELECT COUNT(*) FROM training_plans").fetchone()[0]
        )
        ...
        try:
            ...
            if (
                conn.execute(
                    "SELECT COUNT(*) FROM data_subjects__timezone_v3"
                ).fetchone()[0]
                != subject_count
                or conn.execute(
                    "SELECT COUNT(*) FROM training_plans__timezone_v3"
                ).fetchone()[0]
                != plan_count
            ):
            ...
            for statement in related_triggers:
                conn.execute(statement)
            if conn.execute("PRAGMA foreign_key_check").fetchone() is not None or [
    ...
    def _is_supported_v1_marker(self, paths: dict[str, Path]) -> bool:
        ...
        return (
            set(marker)
            == {"schema_version", "manifest_sha256", "ready", "initialized_at_utc"}
    ...
    def _schema_objects_sha256(conn: sqlite3.Connection) -> str:
        ...
        normalized = [
            (kind, name, re.sub(r"\s+", "", statement).lower())
            for kind, name, statement in rows
        ]
        return hashlib.sha256(
            json.dumps(normalized, separators=(",", ":")).encode("utf-8")
        ).hexdigest()

    def _published_v1_source_error(
        self, conn: sqlite3.Connection, paths: dict[str, Path]
    ) -> str | None:
    ...
    def _published_v2_source_error(
        self, conn: sqlite3.Connection, paths: dict[str, Path]
    ) -> str | None:
        """Prove the input is the exact released v2 store before v3 writes."""
        if not (
            self._is_supported_v2_marker(paths) or self._is_supported_v1_marker(paths)
        ):
        ...
        if (
            ...
            or migrations[0][3:]
            not in (
                ("foundation-v1", _FOUNDATION_V1_MIGRATION_SHA256),
                ("foundation-v1", LEGACY_V1_MANIFEST_SHA256),
            )
            or not _canonical_utc(migrations[0][2])
        ...
        if self._schema_objects_sha256(conn) not in {
            LEGACY_V2_SCHEMA_OBJECTS_SHA256,
            LEGACY_V2_FROM_V1_SCHEMA_OBJECTS_SHA256,
        }:
    ...
    def _migrate_v1_mail_processing_state(
        self,
        ...
        try:
            ...
            conn.execute(
                f"CREATE TABLE mail_messages__foundation_v2 ({TABLES['mail_messages']})"
            )
            fields = ",".join(columns)
            conn.execute(
                f"INSERT INTO mail_messages__foundation_v2 ({fields}) SELECT {fields} FROM mail_messages"
            )
            if (
                conn.execute(
                    "SELECT COUNT(*) FROM mail_messages__foundation_v2"
                ).fetchone()[0]
                != before_count
            ):
            ...
            conn.execute(
                "ALTER TABLE mail_messages__foundation_v2 RENAME TO mail_messages"
            )
            ...
            for name, query in VIEWS.items():
                conn.execute(f"CREATE VIEW {name} AS {query}")
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_source_revisions_current ON source_revisions(provider,resource_kind,provider_object_id,is_current)"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_source_revision_current ON source_revisions(provider,resource_kind,provider_object_id) WHERE is_current=1"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_activity_source_role_active ON activity_source_revisions(activity_id,source_role) WHERE is_active=1"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_daily_health_current ON daily_health(subject_id,local_date) WHERE is_current=1"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_analysis_artifact_current ON analysis_artifacts(subject_id,artifact_kind,period_start_local_date,period_end_local_date) WHERE is_current=1"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_mail_response_current ON mail_response_artifacts(subject_id,mail_thread_id,response_kind) WHERE is_current=1"
            )
            conn.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS ux_garmin_sync_gap_unresolved ON garmin_sync_gaps(subject_id,resource_kind,logical_object_key,window_start_local_date,window_end_local_date,stage) WHERE status IN ('open','deferred')"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_coverage_resource_date ON resource_coverage(resource_kind,local_date)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_activity_samples_stream ON activity_samples(activity_id,stream_kind,sample_index)"
            )
            conn.execute(
                "UPDATE foundation_state SET schema_version=?,manifest_sha256=?,updated_at_utc=?,implementation_version=? WHERE id=1",
                (2, LEGACY_V2_MANIFEST_SHA256, _utc(), "foundation-v2"),
            )
            conn.execute(
                "INSERT OR IGNORE INTO schema_migrations(version,description,applied_at_utc,code_revision,content_sha256) VALUES(2,?,?,?,?)",
                (
                    "mail_processing_state_v2",
                    _utc(),
                    "foundation-v2",
            ...
            if (
                self._schema_objects_sha256(conn)
                not in {
                    LEGACY_V2_SCHEMA_OBJECTS_SHA256,
                    LEGACY_V2_FROM_V1_SCHEMA_OBJECTS_SHA256,
                }
        ...
        if tightened:
            receipt.warnings.append(
                {
                    "code": "legacy_directory_permissions_tightened",
                    "summary": "secured released v1 raw directories",
                }
            )
    ...
    @classmethod
    def _analysis_artifact_input_evidence(
    ...
    def _migrate_v2_analysis_input_trust(
        self,
        paths: dict[str, Path],
        receipt: FoundationReceipt,
        *,
        ...
        try:
            actual = [
                row[1]
                for row in conn.execute("PRAGMA table_info(analysis_artifact_inputs)")
            ]
            if actual != list(columns):
                raise IncompatibleError(
                    "migration_analysis_artifact_inputs_columns_unrecognized"
                )
            if (
                conn.execute(
                ...
                raise IncompatibleError(
                    "migration_analysis_artifact_inputs_values_unrecognized"
                )
            before = self._analysis_artifact_input_evidence(
                conn, "analysis_artifact_inputs"
            )
            conn.execute("PRAGMA foreign_keys=OFF")
            ...
            transaction_open = True
            conn.execute(
                f"CREATE TABLE analysis_artifact_inputs__foundation_v3 ({TABLES['analysis_artifact_inputs']})"
            )
            fields = ",".join(columns)
            conn.execute(
                f"INSERT INTO analysis_artifact_inputs__foundation_v3 ({fields}) SELECT {fields} FROM analysis_artifact_inputs"
            )
            after_copy = self._analysis_artifact_input_evidence(
                conn, "analysis_artifact_inputs__foundation_v3"
            )
            if after_copy != before:
                raise RuntimeError(
                    "migration_analysis_artifact_inputs_evidence_mismatch"
                )
            conn.execute("DROP TABLE analysis_artifact_inputs")
            conn.execute(
                "ALTER TABLE analysis_artifact_inputs__foundation_v3 RENAME TO analysis_artifact_inputs"
            )
            if (
                self._analysis_artifact_input_evidence(conn, "analysis_artifact_inputs")
                != before
            ):
                raise RuntimeError(
                    "migration_analysis_artifact_inputs_publish_mismatch"
                )
            conn.execute(
                "UPDATE foundation_state SET schema_version=?,manifest_sha256=?,updated_at_utc=?,implementation_version=? WHERE id=1",
                (FOUNDATION_SCHEMA_VERSION, manifest_hash, _utc(), "foundation-v4"),
            )
            # The current offline rebuild is Foundation v4.  Keep the
            # historical v1/v2 admission checks above, but publish a single
            # v4 receipt for the rebuilt schema; version 3 was an old
            # analysis-only migration and must not remain an active contract.
            conn.execute(
                "INSERT INTO schema_migrations(version,description,applied_at_utc,code_revision,content_sha256) VALUES(4,?,?,?,?)",
                ("garmin_offline_rebuild_v4", _utc(), "foundation-v4", manifest_hash),
            )
        ...
        receipt.applied_migration_ids = [2, 4] if migration_start == 1 else [4]
        if republished_v2_marker:
            receipt.warnings.append(
                {
                    "code": "migration_marker_republished",
    ...
    def _atomic_json(
        ...
        *,
        ...
        parent_fd = self._open_checked_directory(path.parent)
        target = path.name
        temp = f".{target}.tmp-{secrets.token_hex(16)}"
        fd = -1
        ...
        def read_at(name: str) -> tuple[dict[str, Any], tuple[int, int], bytes]:
            ...
            if not (
                ...
                raise IncompatibleError("atomic_json_target_unsafe")
            rfd = os.open(
            ...
            try:
                ...
                raise IncompatibleError("atomic_json_invalid_json") from exc
            if not isinstance(parsed, dict):
                raise IncompatibleError("atomic_json_invalid_json")
            return parsed, (before.st_dev, before.st_ino), raw

        prefix = f".{target}.marker-"
        try:
            if any(
                entry.startswith(prefix) and len(entry) > len(prefix)
                for entry in os.listdir(parent_fd)
            ):
                os.close(parent_fd)
                raise IncompatibleError("atomic_json_unresolved_claim")
        except OSError as exc:
            os.close(parent_fd)
            raise IncompatibleError("atomic_json_claim_scan_failed") from exc
        ...
        # the existing marker.
        try:
            old, existing_inode, old_raw = read_at(target)
            existing = "ok"
        except FileNotFoundError:
            existing = None
            existing_inode = None
            ...
            os.close(parent_fd)
            raise IncompatibleError("atomic_json_target_unrecognized") from exc
        if existing == "ok" and old is not None:
            if expected_existing is None:
                os.close(parent_fd)
                raise IncompatibleError("atomic_json_target_exists")
            if any(old.get(key) != value for key, value in expected_existing.items()):
                os.close(parent_fd)
                raise IncompatibleError("atomic_json_target_unrecognized")
        name = temp
        try:
            fd = os.open(
                name,
            ...
            while len(readback) < len(encoded):
                ...
                if not part:
                    raise OSError("atomic_json_temp_readback_short")
                readback += part
            if readback != encoded:
                raise OSError("atomic_json_temp_readback_failed")
            opened = os.fstat(fd)
            if (
            ...
            if (named.st_dev, named.st_ino) != temp_inode:
                raise IncompatibleError("atomic_json_temp_replaced")
            # Do not replace a concurrently introduced unsafe object.
            try:
                _, now_inode, _ = read_at(target)
            except FileNotFoundError:
                now_inode = None
            if (existing is None and now_inode is not None) or (
                existing == "ok" and now_inode != existing_inode
            ):
                raise IncompatibleError("atomic_json_target_replaced")
            if existing is None:
                ...
                if final_inode != temp_inode or final_raw != encoded:
                    raise IncompatibleError("atomic_json_publish_failed")
            else:
                oldclaim = f".{target}.marker-old-{secrets.token_hex(16)}"
                os.rename(target, oldclaim, src_dir_fd=parent_fd, dst_dir_fd=parent_fd)
                claimed, claim_inode, claim_raw = read_at(oldclaim)
                if claim_inode != existing_inode or claim_raw != old_raw:
                    raise IncompatibleError("atomic_json_old_replaced")
                os.link(
                    name,
                    target,
                    src_dir_fd=parent_fd,
            ...
            if existing == "ok":
                ...
                if again_inode != existing_inode or again_raw != old_raw:
                    raise IncompatibleError("atomic_json_old_replaced")
                os.unlink(oldclaim, dir_fd=parent_fd)
                os.fsync(parent_fd)
        finally:
            if fd >= 0:
                os.close(fd)
            try:
                ...
                if (
                    "temp_inode" in locals()
                    and cleanup_inode == temp_inode
                    and cleanup_raw == encoded
                ):
    ...
    def _copy_raw_tree(source: Path, destination: Path) -> None:
        ...
        for path in sorted(
            source.rglob("*"), key=lambda item: (len(item.parts), str(item))
        ):
    ...
    def rebuild_into(self, destination_root: Path) -> Path:
        ...
        source_status = FoundationReceipt(
            invocation_id="rebuild-source-verify", mode="verify"
        )
        ...
        with tempfile.TemporaryDirectory(
            prefix=".foundation-shadow-", dir=destination_root.parent
        ) as workspace:
            shadow = Path(workspace) / "candidate"
            candidate = FoundationTool(
                FoundationConfig(
                    shadow,
                    shadow / "data.db",
                    shadow / "raw",
            ...
            receipt = candidate.execute(
                FoundationRequest("init", "explicit-shadow-rebuild", _utc())
            )
            ...
            try:
                ...
                for ordinal, table in enumerate(("schema_migrations", *sorted(TABLES))):
                    ...
                    rows = [
                        tuple(row) for row in source.execute(f'SELECT * FROM "{table}"')
                    ]
                    if rows:
                ...
                if [row[0] for row in target.execute("PRAGMA integrity_check")] != [
                    "ok"
            ...
            if self.raw_tree_fingerprints(
                self._paths(self.data_root)["raw"]
            ) != self.raw_tree_fingerprints(shadow / "raw"):
                raise RuntimeError("rebuild_raw_reconciliation_failed")
            self._fire_failpoint("shadow_after_copy")
            verification = candidate.execute(
                FoundationRequest("verify", "explicit-shadow-rebuild-verify", _utc())
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--invocation-id", default=f"foundation-{int(time.time() * 1000)}"
    )
    ...
    safe_invocation = (
        args.invocation_id
        if isinstance(args.invocation_id, str)
        and _INVOCATION_RE.fullmatch(args.invocation_id)
        else ""
    )
    try:
            ...
            errors=[
                {
                    ...
                    "summary": "foundation_configuration_rejected",
                }
            ],
    ...
    receipt = tool.execute(
        FoundationRequest(
            ...
            target_schema_version=getattr(args, "target_version", None),
        )
    )
    print(receipt.json())
    return (
        ...
        else {"incompatible": 10, "lock_busy": 11}.get(receipt.status, 20)
    )
