...
class Element:
    ...
    def discard_attribute(self, name: str) -> None:
        self.attributes = [
            (key, value) for key, value in self.attributes if key != name
        ]
...
def _serialize(node: Element | Text | _Comment) -> str:
    ...
    attrs = "".join(
        f' {key}="{escape(value, quote=True)}"' for key, value in node.attributes
    )
    if node.tag in _VOID_ELEMENTS:
        return f"<{node.tag}{attrs}>"
    return (
        f"<{node.tag}{attrs}>"
        + "".join(_serialize(child) for child in node.children)
        + f"</{node.tag}>"
    )
...
class Template:
    ...
    def replace_repeats(
        self, values: Mapping[str, Sequence[str | Element | Text]]
    ) -> Template:
    ...
    unresolved = sorted(
        {
            ...
            for attribute, _ in node.attributes
            if attribute in _UNRESOLVED_ATTRIBUTES
        }
    )
    if unresolved:
...
def load_template(email_type: str) -> Template:
    ...
    source = (
        files("src.email_templates.templates")
        .joinpath(filename)
