...
class HarnessBundle:
    ...
    def audit_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "files": [
                {"path_id": item.path_id, "sha256": item.sha256} for item in self.files
            ],
...
def _manifest() -> dict[str, Any]:
    return json.loads(
        Path(__file__).with_name("harness_manifest.json").read_text(encoding="utf-8")
    )
...
def _manifest_schema() -> dict[str, Any]:
    return json.loads(
        (
            Path(__file__).with_name("schemas") / "harness_bundle_manifest.schema.json"
...
def _regular_private_readonly_file(path: Path, path_id: str) -> None:
    ...
    if not path.is_file():
        raise HarnessResolutionError(
            f"analysis_harness_file_missing_or_not_regular:{path_id}"
        )
    mode = stat.S_IMODE(path.stat().st_mode)
    if mode & 0o022:
        raise HarnessResolutionError(
            f"analysis_harness_file_writable_by_group_or_other:{path_id}"
        )
...
def resolve_harness_bundle(
    config: AnalysisConfig, route: HarnessRoute, schema_evidence: SchemaEvidence
) -> HarnessBundle:
    ...
    _verify_schema(
        config.input_schema, schema_evidence.input_schema_sha256, "input_schema"
    )
    _verify_schema(
        config.output_schema, schema_evidence.output_schema_sha256, "output_schema"
    )
