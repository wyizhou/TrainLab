...
class AnalysisStatusQueryService:
    ...
    def execute(self, request: AnalysisRequest) -> AnalysisReceipt:
        ...
        try:
            ...
            return AnalysisReceipt(
                ...
                if snapshot["selected_run"] is None
                else snapshot["selected_run"]["id"],
                quality_gate_state=analysis_quality_state("blocked")
                if snapshot["quality_blocker_codes"]
                else analysis_quality_state("ready"),
                artifact_ids=tuple(
                    item["id"] for item in snapshot["current_artifacts"]
                ),
                training_plan_id=None
                if snapshot["current_plan"] is None
                else snapshot["current_plan"]["id"],
                delivery=delivery,
                next_action=analysis_next_action(action),
    ...
    def _snapshot(
        self, subject_id: int, run_key: str | None
    ) -> tuple[dict[str, Any], AnalysisDelivery | None]:
    ...
    def _selected_run(
        ...
        if run_key is None:
            ...
            row = self._connection.execute(
                "SELECT id,run_key,status,started_at_utc,completed_at_utc FROM analysis_runs "
                "WHERE subject_id=? AND run_key=? LIMIT 1",
                (subject_id, run_key),
            ).fetchone()
        if row is None:
            return None
        ...
        return [
            {
                ...
                "period_end_local_date": row["period_end_local_date"],
    ...
    def _plan(self, subject_id: int) -> dict[str, Any] | None:
        ...
        if (
            len(rows) > 1
            and rows[1]["plan_end_local_date"] >= rows[0]["plan_start_local_date"]
        ):
        ...
        return {
            "id": str(row["id"]),
            "start_local_date": row["plan_start_local_date"],
    ...
    def _latest_delivery(
        self, subject_id: int
    ) -> tuple[dict[str, Any] | None, AnalysisDelivery | None]:
        row = self._connection.execute(
        ...
        artifact_rows = self._connection.execute(
            ...
            "WHERE d.id=? AND d.subject_id=? ORDER BY da.ordinal",
            (row["id"], subject_id),
        )
        artifacts = tuple(str(item["analysis_artifact_id"]) for item in artifact_rows)
        status = row["status"]
        return (
            {"id": str(row["id"]), "status": status},
            AnalysisDelivery(
                str(row["id"]), analysis_delivery_status(str(status)), artifacts
            ),
        )
    ...
    def _actionable_delivery_ids(self, subject_id: int) -> dict[str, list[str]]:
        rows = self._connection.execute(
            ...
            f"AND {_ACTIONABLE_DELIVERY} "
            "ORDER BY d.updated_at_utc DESC,d.id DESC LIMIT 64",
            (subject_id,),
        )
        result: dict[str, list[str]] = {"retry_delivery": [], "reconcile_delivery": []}
        for row in rows:
            if row["status"] in _RETRY_STATUSES and len(result["retry_delivery"]) < 32:
                result["retry_delivery"].append(str(row["id"]))
            elif (
                row["status"] in _RECONCILE_STATUSES
                and len(result["reconcile_delivery"]) < 32
            ):
    ...
    def _terminal_times(self, subject_id: int) -> dict[str, str | None]:
        ...
        rows = self._connection.execute(
            ...
            "WHERE subject_id=? AND status IN ('succeeded','failed','rejected') GROUP BY status",
            (subject_id,),
        )
        for row in rows:
    ...
    def _quality_blockers(self, subject_id: int) -> list[str]:
        ...
        rows = self._connection.execute(
            ...
            ") ORDER BY q.issue_code LIMIT 64",
