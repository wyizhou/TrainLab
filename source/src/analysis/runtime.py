...
from .delivery_service import AnalysisDeliveryService, DeliveryExecution, DeliveryMode
...
def _active_subject_key(connection: sqlite3.Connection) -> str:
...
def _active_subject_id(connection: sqlite3.Connection) -> int:
    rows = list(
    ...
    *,
    ...
    try:
        ...
        if isinstance(connection, sqlite3.Connection):
            ...
            assert request.summary_local_date is not None
        ...
        if delivery_id is None:
            return replace(
                ...
                next_action=analysis_next_action("retry_delivery"),
            )
        ...
        return _merge_daily_delivery(
            receipt,
            _execute_delivery(connection, config, delivery_id, "retry_delivery", root),
            artifact_ids,
        )
    finally:
        connection.close()
...
def run_delivery_recovery(
    *,
    ...
    connection = FoundationTool(foundation)._connect(foundation.database_path)
    started = _utc_now()
    mode: DeliveryMode = "reconcile_delivery" if reconcile else "retry_delivery"
    try:
        ...
        result = _execute_delivery(connection, config, delivery_id, mode, root)
        ...
        return AnalysisReceipt(
            ...
            invocation_id=invocation_id,
            mode=mode,
            status=analysis_status("succeeded" if succeeded else "partial"),
            started_at_utc=started,
            completed_at_utc=_utc_now(),
            analysis_run_id=str(pending.analysis_run_id),
            quality_gate_state=analysis_quality_state("ready"),
            artifact_ids=delivery.artifact_ids,
            delivery=delivery,
            errors=() if result.error_code is None else (_error(result.error_code),),
            next_action=analysis_next_action(result.next_action),
        )
    finally:
        connection.close()
...
def _execute_delivery(
    ...
    try:
        recipient = _recipient_email(root)
    except ValueError:
        state = AnalysisDeliveryRepository(connection).read_state(delivery_id)
    ...
    service = AnalysisDeliveryService(AnalysisDeliveryRepository(connection), gateway)
    return service.execute(delivery_id, mode)


def _daily_delivery_id(
    connection: sqlite3.Connection, receipt: AnalysisReceipt
) -> int | None:
    return _analysis_delivery_id(connection, receipt, "daily_report")


def _analysis_delivery_id(
    connection: sqlite3.Connection, receipt: AnalysisReceipt, delivery_kind: str
) -> int | None:
...
def _contract_delivery(
    ...
    return AnalysisDelivery(
        ...
        analysis_delivery_status(result.status),
        artifact_ids,
...
def _regeneration_content_same(
