...
_REQUIRED: dict[str, bool] = {
    item["resource_kind"]: bool(item["cursor_required"])
    for item in _POLICY["required_resources"]
}
_CONDITIONAL: dict[str, bool] = {
    item["resource_kind"]: bool(item["cursor_required"])
    for item in _POLICY["conditional_resources"]
}
...
class QualityGateRequest:
    ...
    def validated(self) -> tuple[tuple[str, ...], datetime]:
        ...
        if self.route == "revise_plan":
            if (
                len(dates) != 1
                or not _positive(self.plan_id)
                or not _positive(self.reason_event_id)
            ):
                raise QualityGateError("analysis_quality_revision_request_invalid")
        if self.route != "revise_plan" and (
            self.plan_id is not None or self.reason_event_id is not None
        ):
            raise QualityGateError("analysis_quality_request_invalid")
...
def _canonical_utc(
    value: object, code: str = "analysis_quality_timestamp_invalid"
) -> datetime:
...
class QualityGate:
    ...
    def evaluate(
        self, request: QualityGateRequest, snapshot: StableSnapshot
    ) -> QualityGateResult:
        ...
        try:
            ...
            self._activities(
                snapshot, dates, coverage, capabilities, blockers, warnings
            )
    ...
    def _capabilities(
        ...
        for kind in sorted(_RESOURCES):
            ...
            if (
                ...
                continue
            try:
                checked_date = date.fromisoformat(
                    _local_day(row.get("last_checked_at_utc"))
                )
                probe = row.get("next_probe_at_utc")
                probe_date = (
                    date.fromisoformat(_local_day(probe)) if probe is not None else None
                )
        ...
        for kind in sorted(_RESOURCES):
            ...
            for day in dates:
                ...
                if not rows:
                    ...
                    continue
                candidates: list[tuple[datetime, tuple[str, str, str, int, str]]] = []
                for row in rows:
            ...
            if capability == "supported" and cursor_required:
                ...
                if not cursor_rows:
                    ...
                    complete = _canonical_date(
                        cursor.get("complete_through_local_date")
                    )
                    if (
    ...
    def _sleep(
        ...
        for day in dates:
            coverage_row = coverage.get(("sleep", day))
            if (
                coverage_row is not None
                and coverage_row.get("availability_state") == "fetched"
            ):
    ...
    def _activities(
        ...
        if capabilities.get("activity_inventory") != "supported":
            blockers.append(
                _reason("activity_inventory_incomplete", "activity_inventory")
            )
            return
        activities = [
            row for row in snapshot.activity_stages if row.get("local_date") in dates
        ]
        if not activities:
            warnings.append(
                _reason("activity_absent_unconfirmed", f"{dates[0]}:{dates[-1]}")
            )
            return
        activity_counts = {
        ...
        for row in activities:
            ...
            inventory_matches = (
                ...
                and row.get("inventory_coverage_state")
                == inventory.get("availability_state")
            ...
            if (
                ...
                or (row.get("fit_core_ready") != 1 and row.get("fallback_ready") != 1)
            ):
    ...
    def _plans(
        ...
        for plan in relevant_plans:
            ...
            if (
                ...
                continue
            matching_artifacts = [
                row for row in artifacts if row.get("id") == artifact_id
            ]
            ...
            plan_items = [
                row for row in items if row.get("training_plan_id") == plan_id
            ]
            indices: set[int] = set()
        ...
        if request.route == "revise_plan":
            ...
            if len(reasons) != 1:
                blockers.append(
                    _reason("plan_reason_invalid", f"reason:{request.reason_event_id}")
                )
                return
            reason = reasons[0]
            try:
