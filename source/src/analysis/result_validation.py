...
from .coaching_policy import hard_load_decision, progression_decision
from .safety_rules import SafetyRuleError, evaluate_training_safety

RESULT_SCHEMA_VERSION = "1"
_MAX_OUTPUT_BYTES = 250_000
_SCHEMA = json.loads(resource_bytes("harness/schemas/analysis_result.schema.json"))
RESULT_VALIDATOR = Draft202012Validator(_SCHEMA, format_checker=FormatChecker())
_COURSE_SCHEMA = json.loads(
    resource_bytes("harness/schemas/course_contract_v2.schema.json")
)
COURSE_VALIDATOR = Draft202012Validator(_COURSE_SCHEMA, format_checker=FormatChecker())

_CJK = re.compile(r"[\u4e00-\u9fff]")
_TRADITIONAL = re.compile(r"[體臺訓練醫療處總結數據這個與為於後當週計畫]")
_IMPERIAL = re.compile(
    r"(?:\b\d+(?:\.\d+)?\s*(?:mi|mile|miles|ft|feet|lb|lbs|pound|pounds)\b|英里|英尺|磅)",
    re.IGNORECASE,
)
_INTERNAL_PATH = re.compile(
    r"(?:^|\s)(?:/+(?:Users|home|Volumes|tmp|private)/|[A-Za-z]:\\)"
)
_SECRET = re.compile(
    r"(?:api[_ -]?key|access[_ -]?token|refresh[_ -]?token|client[_ -]?secret|password|oauth)\s*[:=]",
    re.IGNORECASE,
)
_DELIVERY_CLAIM = re.compile(
    r"(?:已(?:发送|寄出|投递)|发送成功|投递成功|\b(?:sent|delivered)\b)", re.IGNORECASE
)
_MEDICAL = re.compile(
...
_ALL_OUT = re.compile(
    r"(?:\bRPE\s*10\b|10\s*/\s*10|all[ -]?out|力竭|全力冲刺)", re.IGNORECASE
)
...
class ResultValidationExpectation:
    ...
    # Parsed by the host configuration loader.  The model-visible contract is
    # deliberately not trusted for safety decisions.
    coaching_profile: Mapping[str, Any] | None = None
    # Host-owned deterministic evidence for weekly progression.  Model
    # ``training_plan.constraints`` never supplies these values.
    progression_inputs: Mapping[str, Any] | None = None
    # Daily analysis is an evaluation of the host-selected plan item.  These
    # fields are deliberately optional for the pure legacy fixture validator;
...
def _course_contract_for_item(
    item: Mapping[str, Any], *, local_date: str
) -> dict[str, Any] | None:
    """Build the host-owned v2 course envelope around a safe prescription."""

    kind = item.get("activity_kind")
    if kind not in {"running", "climbing", "rest"}:
        return None
    hard = bool(
        item.get("hansons_session_role")
        in {"long", "tempo", "speed", "running_strength"}
        or item.get("hard_session") is True
    )
    ...
    target_range = item.get("target_bpm_range")
    heart_rate = None
    if isinstance(target_range, Mapping):
        minimum = target_range.get("minimum_bpm")
        maximum = target_range.get("maximum_bpm")
        if all(
            isinstance(value, (int, float)) and not isinstance(value, bool)
            for value in (minimum, maximum)
        ):
            heart_rate = {"min": minimum, "max": maximum}
    rpe = item.get("prescribed_rpe")
    rpe_range = (
        {"min": max(0, float(rpe) - 1), "max": min(10, float(rpe) + 1)}
    ...
    if distance_m is None:
        value = item.get("planned_distance_km")
        if (
            isinstance(value, (int, float))
            and not isinstance(value, bool)
            and value >= 0
    ...
    pace_range = None
    pace = item.get("pace_seconds_per_km", item.get("target_pace_seconds_per_km"))
    if isinstance(pace, Mapping):
        minimum, maximum = pace.get("min"), pace.get("max")
        if all(
            isinstance(value, (int, float)) and not isinstance(value, bool)
            for value in (minimum, maximum)
        ):
            pace_range = {"min": minimum, "max": maximum}
    elif isinstance(pace, (int, float)) and not isinstance(pace, bool) and pace >= 0:
        pace_range = {"min": float(pace), "max": float(pace)}
    intervals = item.get("work_intervals")
    if isinstance(intervals, list) and intervals:
        ...
        steps = [
            ...
            {
                "name": "主训练间歇",
                "end_condition": "repetitions",
                "end_value": int(interval.get("repetitions", 1))
                if isinstance(interval, Mapping)
                and isinstance(interval.get("repetitions", 1), int)
                else 1,
                "notes": "按每组工作与恢复时间完成。",
            },
            {
                ...
                "end_value": max(
                    0,
                    int(interval.get("recovery_seconds", 0))
                    if isinstance(interval, Mapping)
                    and isinstance(interval.get("recovery_seconds", 0), (int, float))
                    else 0,
                ),
                "notes": "恢复至可控呼吸，不补偿遗漏组数。",
            },
            {
                "name": "冷身",
                "end_condition": "duration_seconds",
        ...
        steps = [
            {
                "name": "恢复日",
                "end_condition": "until_symptom_or_guardrail",
                "end_value": "当日不安排训练",
            }
        ]
    else:
        steps = [
            ...
            },
            {
                "name": "主训练",
                "end_condition": "duration_seconds",
                "end_value": max(0, duration_seconds - 600),
                "notes": "按课程目的执行，不额外增加强度。",
            },
            {
                "name": "冷身",
                "end_condition": "duration_seconds",
    ...
    return {
        ...
        "activity_kind": kind,
        "course_name": str(item.get("course_name") or item.get("course_type") or kind),
        "purpose": str(
            item.get("rationale")
            or item.get("rationale_text")
            or "按当前恢复状态执行。"
        ),
        "dose": {
            "distance_m": 0 if kind == "rest" else distance_m,
            "duration_seconds": 0 if kind == "rest" else max(0, duration_seconds),
            "hard_session": hard,
        },
        "steps": steps,
        "targets": {
            "pace_seconds_per_km": pace_range if kind == "running" else None,
            "heart_rate_bpm": heart_rate,
            "rpe": rpe_range,
            "talk_test": item.get("talk_test")
            if isinstance(item.get("talk_test"), str)
            else None,
        },
        "guardrails": ["不补偿错过的课程。", "如出现停止条件，立即停止。"],
        ...
        "degradation_rule": "睡眠或恢复不足时缩短课程并降为轻松强度；攀岩不自动映射为 Garmin 跑步课程。",
        "stop_conditions": list(item.get("stop_conditions", ())) or ["acute_pain"],
...
def _validate_course_contract(
    contract: Mapping[str, Any], path: str, expected_date: str
) -> None:
    if contract.get("local_date") != expected_date:
        _reject("analysis_result_course_contract_date_mismatch", path)
    errors = sorted(
        COURSE_VALIDATOR.iter_errors(contract),
        key=lambda error: tuple(str(part) for part in error.absolute_path),
    )
    if errors:
        _reject("analysis_result_course_contract_invalid", path)


def _ensure_course_contracts(
    payload: dict[str, Any], expected: ResultValidationExpectation
) -> None:
    """Materialize and validate v2 contracts before the result schema gate."""

    # The model response is untrusted.  Do this small structural pass before
    # touching nested values so malformed JSON is always reported through the
    # sanitised domain error rather than leaking an AttributeError/TypeError.
    artifacts = payload.get("artifacts")
    if not isinstance(artifacts, list):
        _reject("analysis_result_schema_invalid", "artifacts")
    for index, artifact in enumerate(artifacts):
        if not isinstance(artifact, dict):
            _reject("analysis_result_schema_invalid", f"artifacts.{index}")
        structured = artifact.get("structured_content")
        if structured is not None and not isinstance(structured, dict):
            _reject(
                "analysis_result_schema_invalid",
                f"artifacts.{index}.structured_content",
            )
    training_plan = payload.get("training_plan")
    if training_plan is not None:
        if not isinstance(training_plan, dict):
            _reject("analysis_result_schema_invalid", "training_plan")
        items = training_plan.get("items")
        if "items" in training_plan and not isinstance(items, list):
            _reject("analysis_result_schema_invalid", "training_plan.items")
        if isinstance(items, list):
            for index, item in enumerate(items):
                if not isinstance(item, dict):
                    _reject(
                        "analysis_result_schema_invalid",
                        f"training_plan.items.{index}",
                    )

    mode = expected.mode
    if mode == "daily" or (
        mode == "regenerate" and expected.regeneration_source_shape == "daily"
    ):
        advice = next(
            (
                row
                for row in artifacts
                if row.get("artifact_kind") == "daily_training_advice"
            ),
            None,
        )
        if isinstance(advice, Mapping) and isinstance(
            advice.get("structured_content"), Mapping
        ):
            ...
            if isinstance(primary, dict):
                ...
                if contract is not None:
                    ...
                    _validate_course_contract(
                        ...
                        str(advice["period"]["start_local_date"]),
                    )
...
def _validate_text_safety(payload: Mapping[str, Any]) -> None:
    for path, value in _strings(payload):
        ...
        if path.endswith("user_visible_text") and _RAW_DEVICE_NARRATION.search(value):
    ...
    for index, artifact in enumerate(payload["artifacts"]):
        ...
        if not _CJK.search(text) or _TRADITIONAL.search(text):
            _reject(
                "analysis_result_simplified_chinese_required",
                f"artifacts.{index}.user_visible_text",
            )
...
def _validate_daily_shape(
    ...
    if len(artifacts) != 2 or sorted(kinds) != [
        "daily_summary",
    ...
    for index, artifact in enumerate(artifacts):
        wanted = (
            summary_period
            if artifact["artifact_kind"] == "daily_summary"
            else advice_period
        )
...
def _validate_daily_content(
    ...
    confidence_lines = [line for line in lines if line.startswith("判断置信度：")]
    limitation_lines = [line for line in lines if line.startswith("数据说明：")]
    body_lines = [
        line for line in lines if not line.startswith(("判断置信度：", "数据说明："))
    ]
    body = "\n".join(body_lines)
    ...
    activity_kind = primary.get("activity_kind")
    ...
    if available is not None and primary.get("activity_kind") == "running":
        advice_day = date.fromisoformat(
            str(
                advice_content.get(
                    "advice_local_date",
                    expected.target_periods["advice"]["start_local_date"],
                )
            )
        )
        if advice_day.isoweekday() not in available:
            _reject(
                "analysis_result_training_weekday_forbidden",
    ...
    if summary_content["activity_evidence"] in {
        "partial",
        "unconfirmed",
...
def _weekly_prior_state(expected: ResultValidationExpectation) -> dict[str, str]:
    ...
    if not isinstance(value, Mapping) or set(value) != {"summary", "plan"}:
        _reject(
            "analysis_result_weekly_prior_artifact_state_invalid",
...
def _canonicalize_plan_item_kind(item: dict[str, Any], *, path: str) -> None:
    """Canonicalize duplicated activity kind before safety validation."""
    ...
    elif inner in {"running", "climbing", "rest"}:
        item["activity_kind"] = inner
    else:
        _reject("analysis_result_weekly_item_prescription_invalid", path)


def _validate_weekly_shape(
    payload: Mapping[str, Any], expected: ResultValidationExpectation
) -> None:
    ...
    if len(artifacts) != 2 or set(by_kind) != {
        "weekly_summary",
    ...
    if (
        len(review_days) != 7
        or len(plan_days) != 7
        or date.fromisoformat(plan_days[0])
        != date.fromisoformat(review_days[-1]) + timedelta(days=1)
    ):
    ...
    if dict(by_kind["weekly_training_plan"]["period"]) != dict(plan_period):
        _reject(
            "analysis_result_date_mismatch", "artifacts.weekly_training_plan.period"
        )
    training_plan_value = payload["training_plan"]
    if not isinstance(training_plan_value, Mapping):
        _reject("analysis_result_weekly_plan_required", "training_plan")
    training_plan = cast(dict[str, Any], training_plan_value)
    ...
    summary_structured = cast(dict[str, Any], summary_structured)
    ...
    if not isinstance(items, list) or len(items) != 7:
        _reject(
            "analysis_result_weekly_item_cardinality_invalid", "training_plan.items"
        )
    for index, item in enumerate(items):
        if not isinstance(item, Mapping):
            _reject(
                "analysis_result_weekly_item_invalid", f"training_plan.items.{index}"
            )
        if (
            item.get("item_index") != index
            or item.get("local_date") != plan_days[index]
        ):
            _reject(
                "analysis_result_weekly_item_sequence_invalid",
                f"training_plan.items.{index}",
            )
        if item.get("activity_kind") not in {"running", "climbing", "rest"}:
            _reject(
                "analysis_result_weekly_item_kind_invalid",
                f"training_plan.items.{index}.activity_kind",
            )
        if (
            expected.available_training_weekdays is not None
            and item.get("activity_kind") == "running"
        ):
            if (
                date.fromisoformat(str(item["local_date"])).isoweekday()
                not in expected.available_training_weekdays
            ):
                _reject(
                    "analysis_result_training_weekday_forbidden",
                    f"training_plan.items.{index}.activity_kind",
...
def _validate_regenerated_plan_shape(
    payload: Mapping[str, Any], expected: ResultValidationExpectation
) -> None:
    ...
    if dict(artifacts[0]["period"]) != dict(expected_period) or dict(
        plan.get("period", {})
    ) != dict(expected_period):
        _reject("analysis_result_date_mismatch", "training_plan.period")
    ...
    for index, item in enumerate(items):
        if (
            ...
            or item.get("local_date") != days[index]
        ):
            _reject(
                "analysis_result_weekly_item_sequence_invalid",
                f"training_plan.items.{index}",
            )
        if item.get("activity_kind") not in {
            "running",
            "climbing",
            "rest",
        } or not isinstance(item.get("prescription"), dict):
            _reject(
                "analysis_result_weekly_item_invalid", f"training_plan.items.{index}"
            )
        if (
...
def _weekly_bases(
    ...
    if not isinstance(bases, Mapping) or len(bases) != 7:
        _reject(
            "analysis_result_weekly_safety_base_invalid", "weekly_safety_request_bases"
        )
    for day, base in bases.items():
        if (
            not isinstance(day, str)
            or not isinstance(base, Mapping)
            or "primary_items" in base
        ):
            _reject(
                "analysis_result_weekly_safety_base_invalid",
...
def _validate_weekly_safety(
    ...
    for index, item in enumerate(items):
        ...
        if (
            ...
            or not isinstance(primary_items, list)
            or len(primary_items) != 1
            or not isinstance(primary_items[0], Mapping)
        ):
            _reject(
        ...
        normalized_contract = _course_contract_for_item(
            safe, local_date=str(local_date)
        )
        if normalized_contract is None:
            _reject(
                "analysis_result_course_contract_invalid",
                f"training_plan.items.{index}.course_contract",
            )
        ...
        item["course_contract"] = normalized_contract
    ...
    if suspended and any(
        item["primary_item"].get("activity_kind") != "rest" for item in normalized
    ):
        _reject(
            "analysis_result_weekly_red_flag_plan_not_suspended", "training_plan.items"
        )
    high_days = [
        date.fromisoformat(str(item["local_date"]))
        for item in normalized
    ...
    if any(
        (right - left) < timedelta(hours=48)
        for left, right in zip(high_days, high_days[1:])
    ):
        _reject(
    ...
    hard_loads: list[tuple[date, Any]] = []
    for item in normalized:
        primary = item["primary_item"]
        kind = primary.get("activity_kind")
        hard_kind = (
            "running_sos"
            if kind == "running"
            and primary.get("hansons_session_role")
            in {"long", "tempo", "speed", "running_strength"}
            else "hard_climbing"
            if kind == "climbing" and primary.get("hard_session") is True
            else "other"
        )
        hard_loads.append((date.fromisoformat(str(item["local_date"])), hard_kind))
    profile = expected.coaching_profile
    max_hard_loads = 3
    min_gap_days = 2
    if isinstance(profile, Mapping):
        configured_max = profile.get("hard_load_max")
        configured_gap = profile.get("hard_load_min_gap_days")
        if isinstance(configured_max, int) and not isinstance(configured_max, bool):
            max_hard_loads = configured_max
        if isinstance(configured_gap, int) and not isinstance(configured_gap, bool):
            min_gap_days = configured_gap
    for candidate_date, candidate_kind in hard_loads:
        allowed, reason = hard_load_decision(
            hard_loads,
            candidate_date,
            candidate_kind,
            max_hard_loads=max_hard_loads,
            min_gap_days=min_gap_days,
        )
        if not allowed:
            _reject(f"analysis_result_{reason}", "training_plan.items")
    constraints = training_plan.get("constraints")
    if isinstance(constraints, dict):
        host_inputs = expected.progression_inputs
        if not isinstance(host_inputs, Mapping):
            host_inputs = {
                "recovery_ready": True,
                "adherence_stable": True,
                "capacity_change": "none",
            }
        recovery_ready = host_inputs.get("recovery_ready", False)
        adherence_stable = host_inputs.get("adherence_stable", False)
        capacity_change = host_inputs.get("capacity_change", "none")
        if not isinstance(recovery_ready, bool):
            recovery_ready = False
        if not isinstance(adherence_stable, bool):
            adherence_stable = False
        if capacity_change not in {"volume", "intensity", "none"}:
            capacity_change = "none"
        decision, decision_reason = progression_decision(
            recovery_ready=recovery_ready,
            adherence_stable=adherence_stable,
            capacity_change=capacity_change,
        )
        constraints["progression_decision"] = {
            "decision": decision,
            "reason": decision_reason,
            "single_change_dimension": (
    ...
    for item, safety_item in zip(items, normalized):
        if (
            item["item_index"] != safety_item["item_index"]
            or item["local_date"] != safety_item["local_date"]
            or item["prescription"] != safety_item["primary_item"]
        ):
    ...
    for index, (safety_item, exact_bpm_allowed) in enumerate(
        ...
        if not exact_bpm_allowed:
            for path, value in _strings(
                {"plan_item": items[index], "safety": safety_item}
            ):
        ...
        for row in expected.input_manifest:
            ...
            if kind == "summary":
                matches = (
                    ...
                    and row.get("source_artifact_kind") == "weekly_summary"
                    and dict(window) == prior_summary_period
                )
            else:
                try:
                    window_start = date.fromisoformat(str(window["start_local_date"]))
                    ...
                    matches = (
                        ...
                        and row.get("source_artifact_kind") == "weekly_training_plan"
...
def _revision_metadata(
    ...
    plan_id, artifact_id, period = (
        plan.get("plan_id"),
        plan.get("artifact_id"),
        plan.get("period"),
    )
    reason_id, effective = (
        reason.get("reason_event_id"),
        reason.get("effective_local_date"),
    )
    if not all(
        isinstance(value, str) and value
        for value in (plan_id, artifact_id, reason_id, effective)
    ) or not isinstance(period, Mapping):
        _reject("analysis_result_revision_expectation_invalid", "revision_metadata")
    assert (
        isinstance(plan_id, str)
        and isinstance(artifact_id, str)
        and isinstance(reason_id, str)
...
def _validate_revision_shape(
    ...
    if payload["mode"] != "revise_plan":
        _reject("analysis_result_route_not_implemented", "mode")
    plan_id, artifact_id, original_period, reason_id, effective = _revision_metadata(
        expected
    )
    original_days = _period_days(original_period, path="original_plan.period")
    if len(original_days) != 7:
        _reject(
            "analysis_result_revision_original_window_invalid", "original_plan.period"
        )
    if effective not in original_days:
        _reject(
            "analysis_result_revision_effective_date_invalid",
            "reason_event.effective_local_date",
        )
    artifacts = payload["artifacts"]
    if len(artifacts) != 1 or artifacts[0]["artifact_kind"] != "weekly_training_plan":
    ...
    if dict(artifact["period"]) != dict(original_period) or dict(
        revision.get("period", {})
    ) != dict(original_period):
        _reject("analysis_result_revision_period_invalid", "training_plan.period")
    artifact["structured_content"] = revision
    if (
        revision.get("original_plan_id"),
        revision.get("original_artifact_id"),
        revision.get("reason_event_id"),
        revision.get("effective_local_date"),
    ) != (plan_id, artifact_id, reason_id, effective):
    ...
    if not isinstance(items, list) or len(items) != len(expected_days):
        _reject(
            "analysis_result_revision_item_cardinality_invalid", "training_plan.items"
        )
    for index, item in enumerate(items):
        if (
            not isinstance(item, Mapping)
            or item.get("item_index") != index
            or item.get("local_date") != expected_days[index]
        ):
            _reject(
                "analysis_result_revision_item_sequence_invalid",
                f"training_plan.items.{index}",
            )
        if item.get("activity_kind") not in {
            "running",
            ...
            _reject(
                "analysis_result_revision_item_invalid", f"training_plan.items.{index}"
            )
        if (
            expected.available_training_weekdays is not None
            and item.get("activity_kind") == "running"
    ...
    if (
        not isinstance(originals, Sequence)
        or isinstance(originals, (str, bytes))
        or len(originals) != 7
    ):
        _reject(
            "analysis_result_revision_original_items_invalid", "original_plan_items"
        )
    for index, item in enumerate(originals):
        if (
            ...
            _reject(
                "analysis_result_revision_original_items_invalid",
                f"original_plan_items.{index}",
            )
...
def _validate_revision_source_usage(
    payload: Mapping[str, Any], expected: ResultValidationExpectation
) -> None:
    ...
    for entity_type, entity_id in required:
        rows = [
            ...
            for row in expected.input_manifest
            if isinstance(row, Mapping)
            and row.get("source_entity_type") == entity_type
            and row.get("source_entity_id") == entity_id
        ]
        if len(rows) != 1 or rows[0].get("ordinal") not in used:
            _reject(
                "analysis_result_revision_source_usage_required",
                f"source_usage.{entity_type}",
            )
...
def _validate_revision_safety(
    ...
    for index, item in enumerate(items):
        ...
        if (
            ...
            or "primary_items" in base
            or base.get("advice_local_date") != item["local_date"]
        ):
            _reject(
                "analysis_result_revision_safety_base_invalid",
        ...
        if (
            ...
            or len(primary) != 1
            or not isinstance(primary[0], Mapping)
        ):
            _reject(
                "analysis_result_safety_candidate_rejected",
                f"training_plan.items.{index}.prescription",
    ...
    if suspended and any(
        row["primary_item"].get("activity_kind") != "rest" for row in normalized
    ):
        _reject(
            "analysis_result_weekly_red_flag_plan_not_suspended", "training_plan.items"
        )
    ...
    hard_loads: list[tuple[date, Any]] = []
    for row in originals:
        if not isinstance(row, Mapping) or str(row.get("local_date")) >= effective:
            continue
        prescription = _original_plan_prescription(
            row, path="original_plan_items.prescription"
        )
        kind = prescription.get("activity_kind")
        hard_kind = (
            "running_sos"
            if kind == "running"
            and prescription.get("hansons_session_role")
            in {"long", "tempo", "speed", "running_strength"}
            else "hard_climbing"
            if kind == "climbing" and prescription.get("hard_session") is True
            else "other"
        )
        hard_loads.append((date.fromisoformat(str(row["local_date"])), hard_kind))
    for row in normalized:
        primary = row["primary_item"]
        kind = primary.get("activity_kind")
        hard_kind = (
            "running_sos"
            if kind == "running"
        ...
        hard_loads.append((date.fromisoformat(str(row["local_date"])), hard_kind))
    profile = expected.coaching_profile
    max_hard_loads, min_gap_days = 3, 2
    if isinstance(profile, Mapping):
        configured_max = profile.get("hard_load_max")
        configured_gap = profile.get("hard_load_min_gap_days")
        if isinstance(configured_max, int) and not isinstance(configured_max, bool):
    ...
    for candidate_date, candidate_kind in hard_loads:
        ...
        if not allowed:
            _reject(f"analysis_result_{reason}", "training_plan.items")
    constraints = revision.get("constraints")
    if not isinstance(constraints, dict):
        constraints = {}
        revision["constraints"] = constraints
    host_inputs = expected.progression_inputs
    if not isinstance(host_inputs, Mapping):
        host_inputs = {
            "recovery_ready": True,
    ...
    overall = (
        ...
        if any(row["safety_state"] == "warning" for row in normalized)
        else "normal"
    )
    payload["safety"] = {"safety_state": overall, "plan_items": normalized}
...
def _validate_safety(
    ...
    advice = next(
        item
        for item in payload["artifacts"]
        if item["artifact_kind"] == "daily_training_advice"
    )
    structured = advice["structured_content"]
    ...
    if "primary_items" in base:
        _reject(
            "analysis_result_safety_request_invalid",
            "safety_request_base.primary_items",
        )
    request = dict(base)
    request["primary_items"] = [candidate_for_safety]
    try:
    ...
    if not isinstance(expected_primary, Mapping) or expected_primary.get(
        "activity_kind"
    ) != candidate_for_safety.get("activity_kind"):
    ...
    assert isinstance(expected_primary, Mapping)
    ...
    if normalized_contract is None:
        _reject(
            "analysis_result_course_contract_invalid",
            "artifacts.daily_training_advice.structured_content.primary_item.course_contract",
        )
    assert normalized_contract is not None
    _validate_course_contract(
        normalized_contract,
    ...
    normalized_primary["course_contract"] = normalized_contract
    structured["primary_item"] = normalized_primary
    advice["structured_content"] = structured
    payload["safety"] = {
        "safety_state": evidence.get("safety_state"),
        "primary_item": normalized_primary,
    }
...
def _validate_daily_plan_binding(
    payload: dict[str, Any], expected: ResultValidationExpectation
) -> None:
    """Bind daily advice to the host-selected plan item, or a safe fallback.

    The model may evaluate a plan item, but it may not silently replace the
    formal weekly plan.  When no unique item exists, only a conservative
    running/rest fallback is accepted and it is marked for review.
    """

    state = expected.today_plan_state
    if state is None:
        return
    if state not in {"available", "unavailable", "ambiguous"}:
        _reject("analysis_result_today_plan_state_invalid", "today_plan_state")
    advice = next(
        item
        for item in payload["artifacts"]
        if item["artifact_kind"] == "daily_training_advice"
    )
    structured = advice.get("structured_content")
    if not isinstance(structured, dict):
        _reject("analysis_result_daily_advice_invalid", "daily_training_advice")
    primary = structured.get("primary_item")
    if not isinstance(primary, Mapping):
        _reject("analysis_result_primary_item_missing", "daily.primary_item")
    decision = structured.get("plan_decision")
    if decision is not None and decision not in {
        ...
        _reject("analysis_result_today_plan_decision_invalid", "daily.plan_decision")

    if state == "ambiguous":
        _reject("analysis_result_today_plan_ambiguous", "daily.primary_item")
    if state == "available":
        expected_item = expected.today_plan_item
        if not isinstance(expected_item, Mapping):
            _reject("analysis_result_today_plan_missing", "today_plan_item")
        expected_kind = expected_item.get("activity_kind")
        expected_prescription = expected_item.get("prescription")
        candidate = dict(primary)
        candidate.pop("course_contract", None)
        exact = (
            candidate.get("activity_kind") == expected_kind
            and isinstance(expected_prescription, Mapping)
            and candidate == dict(expected_prescription)
        )
        if exact:
            structured["plan_decision"] = "as_planned"
            return
        if decision not in {"modify_today", "postpone", "cancel"}:
            _reject("analysis_result_today_plan_replaced", "daily.primary_item")
        return

    # No unique formal item: permit only an easy run or rest, never an SOS,
    # race-pace, hard climbing, or an implicit Garmin-ready workout.
    kind = primary.get("activity_kind")
    if kind not in {"running", "rest"}:
        _reject("analysis_result_today_fallback_not_conservative", "daily.primary_item")
    if kind == "running" and (
        primary.get("hansons_session_role") not in {None, "easy"}
        or primary.get("hard_session") is True
        or primary.get("target_zone") in {4, 5}
    ):
        _reject("analysis_result_today_fallback_not_conservative", "daily.primary_item")
    contract = primary.get("course_contract")
    if (
        not isinstance(contract, Mapping)
        or contract.get("garmin_mapping") != "requires_review"
    ):
        if kind != "rest":
            _reject(
                "analysis_result_today_fallback_requires_review", "daily.primary_item"
            )
    structured["plan_decision"] = decision or "modify_today"
...
class AnalysisResultValidator:
    ...
    def validate(
        ...
        _ensure_course_contracts(payload, expected)
        ...
        if expected.mode == "daily" or (
            ...
            _validate_daily_plan_binding(payload, expected)
        elif expected.mode == "weekly" or (
            expected.mode == "regenerate"
            and expected.regeneration_source_shape in {"weekly", "plan_revision"}
        ):
