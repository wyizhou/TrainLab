...
def _rhr_rows(rows: Iterable[Any], start: date, end: date) -> tuple[_RhrEvidence, ...]:
    ...
    for row in rows:
        if isinstance(row, Mapping):
            ...
            bpm = _number(
                _mapping_value(row, "bpm", "heart_rate_bpm", "value"), low=30, high=150
            )
    ...
    return tuple(
        _RhrEvidence(day, median(values)) for day, values in sorted(grouped.items())
    )
...
def _run_rows(rows: Iterable[Any], start: date, end: date) -> tuple[_RunEvidence, ...]:
    ...
    for index, row in enumerate(rows):
        ...
        if day is None or not start <= day <= end:
            continue
        sport = str(
            _mapping_value(row, "sport", "activity_type", "type") or "running"
        ).casefold()
        if sport not in {"running", "run", "treadmill_running", "trail_running"}:
            continue
        values = _sample_values(
            _mapping_value(row, "heart_rate_bpm", "heart_rates", "samples")
        )
        duration = (
            _number(
                _mapping_value(row, "duration_seconds", "elapsed_seconds", "duration"),
                low=0,
                high=172800,
            )
            or 0.0
        ...
        interval = (
            _number(
                _mapping_value(row, "sample_interval_seconds", "interval_seconds"),
                low=0.1,
                high=120,
            )
            or 1.0
        ...
        quality = str(
            _mapping_value(row, "source_quality", "heart_rate_source") or "unknown"
        ).casefold()
        session = str(
            _mapping_value(row, "session_type", "workout_type", "purpose") or ""
        ).casefold()
        is_threshold = bool(row.get("is_threshold", False)) or any(
            token in session
            for token in ("tempo", "threshold", "race", "competition", "interval")
        )
        if values:
            result.append(
                _RunEvidence(
                    str(_mapping_value(row, "activity_id", "id") or f"run-{index}"),
                    day,
                    values,
                    duration,
                    interval,
...
def _zones(resting_bpm: float, max_bpm: float) -> tuple[dict[str, Any], ...] | None:
    ...
    for zone, low, high in ZONE_BANDS:
        ...
        result.append(
            {
                "zone": zone,
                "percent_hrr": [low, high],
                "bpm": [lower, max(lower, upper)],
            }
        )
    return tuple(result)
...
def _threshold_candidate(
    runs: Iterable[_RunEvidence],
) -> tuple[float | None, tuple[str, ...], str]:
    candidates: list[tuple[str, float]] = []
    for run in runs:
        if (
            not run.is_threshold
            or run.duration_seconds < MIN_THRESHOLD_DURATION_SECONDS
        ):
            continue
    ...
    if len(candidates) < 2:
        return (
            None,
            tuple(activity_id for activity_id, _ in candidates),
...
def calculate_zone_candidates(
    ...
    *,
    ...
    runs = _run_rows(
        running_evidence, end - timedelta(days=MAX_HEART_RATE_WINDOW_DAYS - 1), end
    )
    baseline = median(row.bpm for row in rhr) if len(rhr) >= 5 else None
    high_candidates = tuple(
        (run.activity_id, _high_candidate(run))
        for run in runs
        if _high_candidate(run) is not None
        and run.source_quality not in {"poor", "unknown", "invalid"}
    )
    max_hr: float | None = None
    max_evidence: tuple[str, ...] = ()
    if len(high_candidates) >= MIN_MAX_HR_RUNS:
        values = [float(value) for _, value in high_candidates if value is not None]
    ...
    hrr_zones = (
        _zones(float(baseline), max_hr)
        if baseline is not None and max_hr is not None
        else None
    )
    threshold_value, threshold_evidence, threshold_status = _threshold_candidate(
        run
        for run in runs
        if run.local_date >= end - timedelta(days=THRESHOLD_WINDOW_DAYS - 1)
    )
    tanaka_max = None
    tanaka_zones = None
    if (
        isinstance(age_years, int)
        and not isinstance(age_years, bool)
        and 18 <= age_years <= 100
    ):
        tanaka_max = round(208 - 0.7 * age_years)
        tanaka_zones = (
            _zones(float(baseline), tanaka_max) if baseline is not None else None
        )
    return {
        ...
        "hrr": {
            ...
            "resting_heart_rate_bpm": round(float(baseline))
        ...
        "historical_threshold_proxy": {
            ...
            "heart_rate_bpm": round(float(threshold_value))
        ...
        "tanaka_low_confidence": {
            "status": "available"
            if tanaka_zones is not None
            else ("max_only" if tanaka_max is not None else "unavailable"),
            "age_years": age_years,
