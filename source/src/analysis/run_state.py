...
def build_analysis_run_key(request: AnalysisRequest) -> RunKey:
    ...
    write_mode = cast(
        Literal["daily", "weekly", "revise_plan", "regenerate"], request.mode
    )
    return RunKey(
        value=value,
        subject_key=subject,
        mode=write_mode,
        invocation_id=_decode(invocation, "invocation"),
    )
...
def parse_analysis_run_key(value: str) -> RunKey:
    ...
    if mode == "daily" and len(pieces) == 5:
        _date(pieces[3], "summary_date")
        invocation = _decode(pieces[4], "invocation")
    elif mode == "weekly" and len(pieces) == 5:
        _date(pieces[3], "as_of_date")
        invocation = _decode(pieces[4], "invocation")
    elif mode == "revise_plan" and len(pieces) == 6:
        _decode(pieces[3], "plan_id")
        _decode(pieces[4], "reason_event_id")
        invocation = _decode(pieces[5], "invocation")
    elif mode == "regenerate" and len(pieces) == 5:
        _decode(pieces[3], "artifact_id")
        invocation = _decode(pieces[4], "invocation")
    else:
        raise AnalysisRunStateError("analysis_run_key_invalid_format")
...
class SubjectLock:
    ...
    def claim_for_finish(self) -> str:
        ...
        try:
            ...
            if payload != self._payload() or (info.st_dev, info.st_ino) != (
                self.device,
                self.inode,
            ):
    ...
    def release(self) -> None:
        ...
        try:
            ...
            if (info.st_dev, info.st_ino) != (self.device, self.inode):
                raise AnalysisRunStateError("analysis_lock_owner_record_replaced")
            _claim_private_lock_at(
                self.parent_fd, self.filename, info, payload, "release"
            )
        except (FileNotFoundError, AnalysisRunStateError, OSError) as error:
...
def _read_private_lock_at(
    ...
    if (
        ...
        or set(payload) != _LOCK_KEYS
        or not isinstance(payload.get("pid"), int)
        or payload["pid"] <= 0
        or not isinstance(payload.get("run_key"), str)
        or not _valid_utc(payload.get("started_at_utc"))
    ):
        raise AnalysisRunStateError("analysis_lock_busy")
    try:
        parse_analysis_run_key(payload["run_key"])
...
def _claim_private_lock_at(
    ...
    *,
    ...
    if (
        not canonical
        or "/" in canonical
        or not re.fullmatch(r"[A-Za-z0-9_-]+", purpose)
    ):
        raise AnalysisRunStateError("analysis_lock_busy")
    ...
    try:
        ...
        if allow_incomplete_owned_record:
            ...
            try:
                ...
                fd = os.open(
                    claim, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0), dir_fd=parent_fd
                )
                try:
                    claimed = os.fstat(fd)
                finally:
            ...
            if (
                ...
                and (claimed.st_dev, claimed.st_ino)
                == (expected_info.st_dev, expected_info.st_ino)
            ):
    ...
    if (
        payload != expected_payload
        or (claimed.st_dev, claimed.st_ino)
        != (expected_info.st_dev, expected_info.st_ino)
...
def _has_claim_blocker_at(parent_fd: int, canonical: str) -> bool:
    ...
    try:
        return any(
            name.startswith(prefix) and name.endswith(".claim")
...
class SubjectLockManager:
    ...
    def __init__(
        self, base_lock_path: Path, *, trusted_root: Path | None = None
    ) -> None:
    ...
    def _open_lock_parent(self, subject_key: str) -> tuple[int, os.stat_result, str]:
        ...
        try:
            parts = base.parent.relative_to(root).parts
        except ValueError as error:
            raise AnalysisRunStateError("analysis_lock_path_escape") from error
        flags = (
            os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0)
        ...
        try:
            ...
            raise AnalysisRunStateError("analysis_lock_parent_invalid") from error
        try:
            ...
            for part in parts:
                ...
                if (
                    ...
                    raise AnalysisRunStateError(
                        "analysis_lock_parent_unsafe_permissions"
                    )
                os.close(fd)
                fd = nxt
    ...
    def _existing_is_live_or_invalid(path: Path, subject_key: str) -> bool:
        ...
        try:
            ...
            if (
                set(payload) != _LOCK_KEYS
                or not isinstance(payload["pid"], int)
                or payload["pid"] <= 0
                or not isinstance(payload["run_key"], str)
                or not _valid_utc(payload["started_at_utc"])
            ):
                return True
            run_key_value = payload.get("run_key")
            if not isinstance(run_key_value, str):
                return True
            key = parse_analysis_run_key(run_key_value)
    ...
    @staticmethod
    def _existing_is_live_or_invalid_at(
        parent_fd: int, filename: str, subject_key: str
    ) -> bool:
        try:
            payload, _ = _read_private_lock_at(parent_fd, filename)
        except FileNotFoundError:
            return False
        except (OSError, AnalysisRunStateError):
            return True
        try:
    ...
    def acquire(self, subject_key: str, run_key: str) -> SubjectLock:
        ...
        try:
            if _has_claim_blocker_at(parent_fd, filename):
                raise AnalysisRunStateError("analysis_lock_busy")
            try:
                existing_payload, existing_info = _read_private_lock_at(
                    parent_fd, filename
                )
            except FileNotFoundError:
                pass
                ...
                try:
                    existing_run_key = existing_payload.get("run_key")
                    existing_pid = existing_payload.get("pid")
                    if not isinstance(existing_run_key, str) or not isinstance(
                        existing_pid, int
                    ):
                        raise AnalysisRunStateError("analysis_lock_busy")
                    existing_key = parse_analysis_run_key(existing_run_key)
                    if existing_key.subject_key != subject_key:
                        raise AnalysisRunStateError("analysis_lock_busy")
                    os.kill(existing_pid, 0)
                except ProcessLookupError:
                    _claim_private_lock_at(
                        parent_fd, filename, existing_info, existing_payload, "stale"
                    )
                except (
                    PermissionError,
                    OSError,
            ...
            if created_info is not None:
                try:
                    _claim_private_lock_at(
                        ...
                        payload,
                        "failed-create",
                        allow_incomplete_owned_record=True,
                    )
                    ...
                    try:
                        replacement_payload, replacement_info = _read_private_lock_at(
                            parent_fd, filename
                        )
                        claim = _claim_private_lock_at(
                            parent_fd,
...
class AnalysisRunRepository:
    ...
    _REQUIRED_COLUMNS = frozenset(
        {
            ...
            "subject_id",
            "analysis_kind",
            "target_start_local_date",
            "target_end_local_date",
            "status",
            "started_at_utc",
            "completed_at_utc",
        }
    ...
    def _verify_schema(self) -> None:
        ...
        if not any(
            [
                part[2]
                for part in self._conn.execute(
            ...
            raise AnalysisRunStateError("analysis_runs_schema_incompatible")
        subject_columns = {
            row[1] for row in self._conn.execute("PRAGMA table_info(data_subjects)")
        }
    ...
    def _existing_invocation_key(self, invocation_id: str) -> str | None:
        ...
        for row in self._conn.execute("SELECT run_key FROM analysis_runs ORDER BY id"):
            ...
            try:
                ...
                raise AnalysisRunStateError(
                    "analysis_runs_data_incompatible"
                ) from error
        if len(matches) > 1:
    ...
    def _validate_canonical_row(
        self, row: sqlite3.Row, *, request: AnalysisRequest | None = None
    ) -> RunKey:
        ...
        try:
            ...
            subject = self._conn.execute(
                "SELECT subject_key FROM data_subjects WHERE id=? AND is_active=1",
                (row["subject_id"],),
            ).fetchone()
            if (
                subject is None
                or str(subject[0]) != key.subject_key
                or row["analysis_kind"] != _KIND_BY_MODE[key.mode]
            ):
                raise ValueError
            pieces = key.value.split(":")
            expected: tuple[str | None, str | None]
            if key.mode == "daily":
                start_date = datetime.strptime(pieces[3], "%Y-%m-%d").date()
                expected = (
                    pieces[3],
                    start_date.fromordinal(start_date.toordinal() + 1).isoformat(),
                )
            elif key.mode == "weekly":
                expected = (pieces[3], pieces[3])
            elif key.mode == "revise_plan":
                expected = (
                    (request.effective_local_date, request.effective_local_date)
                    if request
                    else (
                        row["target_start_local_date"],
                        row["target_start_local_date"],
                    )
                )
                if not _DATE.fullmatch(str(row["target_start_local_date"])):
                    raise ValueError
            else:
                expected = (None, None)
            if (
                row["target_start_local_date"],
                row["target_end_local_date"],
            ) != expected:
                raise ValueError
            if row["status"] not in {
                ...
                "failed",
                "rejected",
            } or not _valid_utc(row["started_at_utc"]):
                raise ValueError
            completed = row["completed_at_utc"]
            if row["status"] == "started" and completed is not None:
                raise ValueError
            if row["status"] != "started" and (
                not _valid_utc(completed) or str(completed) < str(row["started_at_utc"])
            ):
                raise ValueError
            if (
                request is not None
                and key.value != build_analysis_run_key(request).value
            ):
                raise ValueError
            return key
    ...
    def preflight_succeeded(self, request: AnalysisRequest) -> RunDecision | None:
        ...
        if prior is not None and prior != key.value:
            raise AnalysisRunStateError("analysis_invocation_id_reused")
        row = self._conn.execute(
            "SELECT * FROM analysis_runs WHERE run_key=?", (key.value,)
        ).fetchone()
        if row is None or row["status"] != "succeeded":
            return None
        self._validate_canonical_row(row, request=request)
        return RunDecision(
            "unchanged", int(row["id"]), key.value, "succeeded", ("existing_succeeded",)
        )

    @staticmethod
    def _targets(request: AnalysisRequest) -> tuple[str | None, str | None]:
        if request.mode == "daily":
            return request.summary_local_date, request.advice_local_date
        if request.mode == "weekly":
            return request.as_of_local_date, request.as_of_local_date
        if request.mode == "revise_plan":
            return request.effective_local_date, request.effective_local_date
        return None, None
    ...
    def prepare_write(self, request: AnalysisRequest) -> RunDecision:
        ...
        try:
            ...
            if row is None:
                ...
                self._conn.execute(
                    "INSERT INTO analysis_runs(run_key,subject_id,analysis_kind,target_start_local_date,target_end_local_date,status,started_at_utc) VALUES(?,?,?,?,?,?,?)",
                    (
                        key.value,
                        subject_id,
                        _KIND_BY_MODE[key.mode],
                        start,
                        end,
                        "started",
                        _utc_now(),
                ...
                decision = RunDecision(
                    "started", run_id, key.value, "started", ("run_created",)
                )
                ...
                artifacts = self._conn.execute(
                    "SELECT 1 FROM analysis_artifacts WHERE generated_by_run_id=? LIMIT 1",
                    (row["id"],),
                ).fetchone()
                trace = (
                    "existing_started_with_artifact"
                    if artifacts
                    else "existing_started_recovered"
                )
                decision = RunDecision(
                    "unchanged" if artifacts else "recovered",
                    int(row["id"]),
                    key.value,
                    "started",
                    (trace,),
                ...
                raise AnalysisRunStateError(
                    "analysis_terminal_run_requires_new_invocation"
                )
    ...
    def _finish_authorized(
        ...
        try:
            row = self._conn.execute(
                "SELECT * FROM analysis_runs WHERE id=? AND run_key=?",
                (run_id, expected_run_key),
            ).fetchone()
            if row is None:
            ...
            updated = self._conn.execute(
                "UPDATE analysis_runs SET status=?,completed_at_utc=? WHERE id=? AND run_key=? AND status='started'",
                (status, _utc_now(), run_id, expected_run_key),
            ).rowcount
            if updated != 1:
                raise AnalysisRunStateError("analysis_run_transition_invalid")
            self._conn.execute("COMMIT")
    ...
    def status(self, request: AnalysisRequest) -> RunDecision | None:
        ...
        if request.run_key is None:
            rows = self._conn.execute(
                "SELECT r.* FROM analysis_runs r JOIN data_subjects s ON s.id=r.subject_id WHERE s.subject_key=? AND s.is_active=1 ORDER BY r.started_at_utc DESC,r.id DESC",
                (request.subject_id,),
            )
            row = next(
                (
                    ...
                    if str(candidate["run_key"]).startswith("analysis:")
                ),
                None,
            )
            if row is None:
        ...
        self._validate_canonical_row(row)
        action: Literal["started", "recovered", "unchanged"] = (
            "unchanged" if row["status"] == "succeeded" else "recovered"
        )
        return RunDecision(
            action, int(row["id"]), key.value, str(row["status"]), ("status_read",)
        )
...
class AnalysisRunCoordinator:
    ...
    def __init__(
        self, repository: AnalysisRunRepository, locks: SubjectLockManager
    ) -> None:
        ...
        self._prepared: dict[
            int, tuple[PreparedAnalysisRun, int, str, str, str, SubjectLock]
        ] = {}
    ...
    def finish(
        ...
        key = parse_analysis_run_key(run_key)
        if prepared.lock.run_key != key.value or key.subject_key != _atom(
        ...
        try:
            self._repository._finish_authorized(
                run_id, key.value, status, self._finish_capability
            )
        except Exception:
            try:
