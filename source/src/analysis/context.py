...
from typing import Any, Literal, Mapping, Never, Protocol, Sequence
...
from .sleep_period import attribute_samples, select_main_sleep
...
_SCHEMA_BYTES = resource_bytes("harness/schemas/analysis_input.schema.json")
...
_SLEEP_PERIOD_METRIC_TOKENS = frozenset(
    {
        "heart_rate",
        "heartrate",
        "hrv",
...
def _sleep_period_health_observations(
    rows: Sequence[Mapping[str, Any]],
    episode: Any,
    report_date: str,
) -> list[dict[str, Any]]:
    """Expose only bounded, non-raw physiology facts inside main sleep."""
    ...
    for row in rows:
        ...
        if not any(token in normalized for token in _SLEEP_PERIOD_METRIC_TOKENS):
            continue
        ...
        if value is None:
            continue
        source_id = _identifier(row.get("id"), "analysis_context_sleep_sample_invalid")
        revision = row.get("source_revision_id")
        output.append(
            {
                "metric_key": f"sleep_period.{metric}",
                "unit": row.get("canonical_unit") or row.get("raw_unit"),
                "value": value,
                "local_date": report_date,
                "source_id": f"health_sample:{source_id}",
                "source_revision_id": _identifier(
                    revision if revision is not None else f"health_sample:{source_id}",
                    "analysis_context_sleep_sample_revision_invalid",
                ),
            }
        )
    return output
...
def _base_candidates(
    ...
    if source_route == "daily":
        advice_day = periods["advice"]["end_local_date"]
        morning_window = (advice_day, advice_day)
        selected_sleep, sleep_state = select_main_sleep(
            trend_sleep_rows,
            date.fromisoformat(advice_day),
            as_of_utc=_utc(request.as_of_utc),
        )
        morning_sleep_rows = []
        if selected_sleep is not None:
            morning_sleep_rows = [
                row
                for row in trend_sleep_rows
                if str(row.get("id")) == str(selected_sleep.session_id)
            ]
        sleep_observations = _sleep_observations(
            morning_sleep_rows,
            morning_window,
        ...
        sleep_observations.append(
            {
                "metric_key": "sleep.selection_state",
                "unit": None,
                "value": sleep_state,
                "local_date": advice_day,
                "source_id": f"sleep-selection:{advice_day}",
                "source_revision_id": f"sleep-selection:{advice_day}",
            }
        )
        if selected_sleep is not None:
            health_rows_for_sleep = snapshot.views.get("v_current_health_samples", ())
            inside, _ = attribute_samples(health_rows_for_sleep, selected_sleep)
            sleep_observations.extend(
                _sleep_period_health_observations(inside, selected_sleep, advice_day)
            )
        _add_compact_observations(
            candidates,
            section="sleep",
            ...
            observations=sleep_observations,
            window=morning_window,
        )
    _add_compact_observations(
        candidates,
    ...
    for row in snapshot.views["v_current_activities"]:
        ...
        _add_row(
            ...
            entity_id=identity,
            revision_id=revision,
            display_fields=tuple(_DISPLAY_FIELDS),
        )
        fit_values = {
    ...
    row_groups: tuple[tuple[str, str, str, Sequence[Mapping[str, Any]]], ...] = (
        (
            "deterministic_features",
    ...
    for section, role, entity_type, section_rows in row_groups:
        if len(section_rows) > _MAX_ITEMS:
        ...
        for section_row in section_rows:
            ...
            lineage = feature_row.get("input_revision_ids")
            ...
            if (
                ...
                _fail("analysis_context_feature_lineage_invalid")
            if feature_row.get("subject_id") not in (None, request.subject_id):
                _fail("analysis_context_cross_subject")
            identity = _identifier(
                feature_row.get(
                    "key", feature_row.get("id", _sha(_sanitize_row(feature_row))[:32])
                )
            )
            algorithm = _identifier(
                feature_row.get("algorithm_version", FEATURE_LIBRARY_VERSION)
            )
            _add_row(
                candidates,
                section=section,
                ...
                origin=str(origin),
                entity_id=identity,
                revision_id=f"{entity_type}:{algorithm}:{_sha(_sanitize_row(feature_row))}",
            )
    ...
    plan_window_end = (
        periods["advice"]["end_local_date"]
        if source_route == "daily"
        else periods["plan"]["end_local_date"]
        if periods["plan"]
        else complete_end
    )
    plan_candidates = [
        ...
        if not (
            request.source_route() == "revise_plan" and row.get("id") != request.plan_id
        )
        and _within(row, plan_window_start, plan_window_end)
    ]
    if source_route == "daily" and len(plan_candidates) > 1:
        _fail("analysis_context_daily_plan_ambiguous")
    ...
    for row in snapshot.views["v_training_plan_items"]:
        ...
        if (
            source_route == "daily"
            and row.get("local_date") != periods["advice"]["end_local_date"]
        ):
            continue
        _add_row(
            candidates,
    ...
    for weekly_row in sorted(
        ...
        latest_weekly[str(weekly_row.get("artifact_kind"))] = dict(weekly_row)
    protected_weekly = {
        (_identifier(row.get("id")), _identifier(row.get("revision_no")))
    ...
    for prior_row in prior_rows:
        identity_key = (
            _identifier(prior_row.get("id")),
            _identifier(prior_row.get("revision_no")),
        )
        ...
        if identity_key not in daily_ids | protected_weekly:
            continue
        if prior_row.get("trust_class") not in (None, "prior_model_output"):
...
def _validate_manifest_bindings(context: Mapping[str, Any]) -> None:
    ...
    for expected_ordinal, manifest in enumerate(manifests):
        if manifest.get("ordinal") != expected_ordinal:
            _fail("analysis_context_manifest_binding_invalid")
        bound_item = item_by_ordinal.get(expected_ordinal)
        if bound_item is None or manifest.get("input_sha256") != _sha(
            bound_item["content"]
        ):
