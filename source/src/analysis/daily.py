...
def _default_dates(request: AnalysisRequest) -> AnalysisRequest:
    ...
    return replace(
        request,
        summary_local_date=(advice - timedelta(days=1)).isoformat(),
        advice_local_date=advice.isoformat(),
    )
...
def daily_primary_item_contract() -> Mapping[str, Any]:
    ...
    return {
        ...
        "input_revision_ids": [],
        "contract_version": "1",
        "allowed_activity_kinds": ["running", "climbing", "rest"],
        "running_template": {
...
def _canonical_utc_or_none(value: object) -> str | None:
    ...
    return (
        value
        if value == rendered or value == rendered.replace(".000000Z", "Z")
        else None
    )


def _accepted_user_safety_signals(
    snapshot: object, *, subject_id: int, as_of_utc: str
    ...
    for row in rows:
        ...
        if (
            key not in _USER_SAFETY_FACT_KINDS
            or isinstance(fact_id, bool)
            or not isinstance(fact_id, (str, int))
        ):
            continue
        ...
        signals.append(
            {
                "signal_id": f"user-fact:{fact_token}:{key}",
                "kind": key,
                "origin": "user_asserted",
                "source_revision_id": f"user-fact:{fact_token}",
                "current": True,
                "active": True,
...
def _safety_base(
    subject_id: int, advice_date: str, as_of_utc: str, snapshot: object
) -> Mapping[str, Any]:
    ...
    return {
        ...
        "as_of_utc": as_of_utc,
        "zone_evidence": zone_evidence_from_snapshot(snapshot),
        "safety_signals": _accepted_user_safety_signals(
            snapshot, subject_id=subject_id, as_of_utc=as_of_utc
        ),
        "quality_sessions": [],
        "substitution": None,
    }
...
def _today_plan_binding(
    snapshot: object, advice_date: str
) -> tuple[str, Mapping[str, Any] | None]:
    """Resolve exactly one current plan item for the report date.

    The context builder already exposes this item to the model, but the host
    keeps a separate typed copy for the post-generation binding gate.  A
    malformed prescription or duplicate plan is an explicit ambiguity, never
    a reason to silently invent a replacement course.
    """
    ...
    plans = tuple(
        row
        for row in views.get("v_current_training_plans", ())
        if isinstance(row, Mapping)
        and str(row.get("plan_start_local_date", "")) <= advice_date
        and str(row.get("plan_end_local_date", "")) >= advice_date
    )
    if len(plans) != 1:
        return ("unavailable", None) if not plans else ("ambiguous", None)
    plan_id = plans[0].get("id")
    items = tuple(
        row
        for row in views.get("v_training_plan_items", ())
        if isinstance(row, Mapping)
        and row.get("training_plan_id") == plan_id
        and row.get("local_date") == advice_date
    )
    if len(items) != 1:
        return ("unavailable", None) if not items else ("ambiguous", None)
    row = items[0]
    raw = row.get("prescription_json")
    try:
        prescription = json.loads(raw) if isinstance(raw, str) else raw
...
class DailyRoute:
    ...
    def execute(self, request: AnalysisRequest) -> AnalysisReceipt:
        ...
        try:
            ...
            code = (
                candidate
                if candidate.startswith("analysis_") and len(candidate) <= 160
                else "analysis_daily_begin_failed"
            )
            ...
            return self._receipt(
                ...
                errors=(
                    self._error(
                        "request", _safe_code(error, "analysis_daily_request_invalid")
                    ),
                ),
            )
        ...
        try:
            ...
            return self._receipt(
                ...
                analysis_run_id=str(prepared.decision.run_id),
                errors=(
                    self._error("service", _safe_code(error, "analysis_daily_failed")),
                ),
            )
    ...
    def _run_started(
        ...
        context_request = ContextBuildRequest(
            ...
            summary_local_date=request.summary_local_date,
            advice_local_date=request.advice_local_date,
        ...
        today_plan_state, today_plan_item = _today_plan_binding(
            snapshot, request.advice_local_date
        )
        expectation = ResultValidationExpectation(
            ...
            {
                "summary": {
                    "start_local_date": request.summary_local_date,
                    "end_local_date": request.summary_local_date,
                },
                "advice": {
                    "start_local_date": request.advice_local_date,
                    "end_local_date": request.advice_local_date,
                },
            },
            ...
            today_plan_state=today_plan_state,
            today_plan_item=today_plan_item,
        )
        ...
        ids = tuple(
            str(published.artifact_ids[k])
            for k in ("daily_summary", "daily_training_advice")
        )
        try:
            ...
            return self._receipt(
                ...
                errors=(
                    self._error(
                        "service",
                        _safe_code(error, "analysis_daily_delivery_pending_failed"),
                    ),
                ),
            )
        return self._receipt(
    ...
    def _create_pending_delivery(
        ...
        try:
            value = self.delivery.create_pending(
                publish_receipt=published, delivery_kind="daily_report"
            )
        except TypeError:
    ...
    def _receipt(
        ...
        *,
        ...
        return AnalysisReceipt(
            run_key or "analysis:invalid:daily:invalid:invalid",
