...
def _load_json(path: Path) -> tuple[dict[str, Any], bytes]:
    ...
    try:
        ...
        raise RuntimeError(
            f"training_safety_static_json_invalid:{path.name}"
        ) from error
...
_RELIABLE_BASELINE_METHODS = frozenset(
    {"provider_profile", "user_verified", "multiple_observations"}
)
...
def _validate_static_policy() -> None:
    errors = sorted(
        _POLICY_VALIDATOR.iter_errors(_POLICY),
...
def _reason(
    code: str, severity: str, revisions: tuple[str, ...] = ()
) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if kind == "garmin_sport_zones":
            ...
            if (
                zones is not None
                or heart_rate is not None
                or method != "activity_time_in_zone"
            ):
                raise SafetyRuleError(
                    "training_safety_prohibited_evidence_shape_invalid"
                )
...
def _hrr_zones(max_hr: int, resting_hr: int) -> tuple[dict[str, int], ...]:
    if (
        not 100 <= max_hr <= 240
        or not 30 <= resting_hr <= 120
        or max_hr - resting_hr < 30
    ):
...
def _zone_selection(
    ...
    for row in evidence:
        ...
        if not _effective(row, as_of, reasons):
            continue
        if (
            row["source_kind"] in {"max_hr_baseline", "resting_hr_baseline"}
            and row["measurement_method"] not in _RELIABLE_BASELINE_METHODS
        ):
            _add_reason(reasons, "prohibited_zone_evidence", "info", revision)
            continue
    ...
    if "hr_affecting_medication" in blocker_kinds:
        blocker_revisions = tuple(
            _revision(row["source_revision_id"])
            for row in hr_blockers
            if row["kind"] == "hr_affecting_medication"
        )
        _add_reason(
            reasons, "hr_medication_exact_bpm_blocked", "warning", blocker_revisions
        )
    ...
    if "hr_baseline_unreliable" in blocker_kinds:
        blocker_revisions = tuple(
            _revision(row["source_revision_id"])
            for row in hr_blockers
            if row["kind"] == "hr_baseline_unreliable"
        )
    ...
    for source_kind, reason_code in (
        ...
        if candidates:
            row = candidates[0]
            source_revision = _revision(row["source_revision_id"])
            _add_reason(reasons, reason_code, "info", (source_revision,))
            return {
    ...
    resting_rows = [
        row for row in eligible if row["source_kind"] == "resting_hr_baseline"
    ]
    usable_max = [
        row
        for row in max_rows
        if row["measurement_method"] in _RELIABLE_BASELINE_METHODS
    ]
    usable_rest = [
        row
...
def _validate_primary_semantics(item: Mapping[str, Any]) -> None:
    ...
    if kind == "running":
        ...
        if any(
            item[field] not in _TOKENS[token_group]
            for field, token_group in token_fields.items()
        ):
            raise SafetyRuleError("training_safety_controlled_token_invalid")
        if (
            item.get("planned_duration_seconds") is not None
            and item["planned_duration_seconds"]
        ...
        if any(
            ...
            or _TOKENS["strength_exercises"][row["exercise_key"]]
            != row["movement_kind"]
        ...
        if (
            not movement_kinds & {"squat", "hinge"}
            or not movement_kinds & {"push", "pull"}
            or not movement_kinds & {"carry", "core"}
        ):
        ...
        if (
            ...
            or any(
                value not in _TOKENS["rest_recovery_signals"]
                for value in item["recovery_signals"]
            )
            or any(
                value not in _TOKENS["rest_seek_help"]
                for value in item["seek_professional_help_if"]
            )
        ):
            raise SafetyRuleError("training_safety_controlled_token_invalid")
...
def _assess_substitution(row: Mapping[str, Any] | None) -> dict[str, Any] | None:
    ...
    for prefix in ("planned", "actual"):
        ...
        if seconds is not None and not math.isclose(
            float(seconds), minutes * 60, rel_tol=0, abs_tol=1e-9
        ):
            raise SafetyRuleError("training_safety_duration_units_ambiguous")
    ratio = float(row["actual_duration_minutes"]) / float(
    ...
    return {
        "status": "conflicts_with_exact_activity_evidence"
        if exact
        else "consistent_with_substitution",
...
def _high_intensity_rejection(
    ...
    maximum_total = _POLICY["quality_session"][f"zone{zone}_maximum_total_work_seconds"]
    total_work = 0
    total_session = 0
    ...
    if total_work > maximum_total or total_session > available_interval_seconds:
        ...
        return True
    window_start = advice_date - timedelta(
        days=_POLICY["quality_session"]["lookback_days"]
    )
    prior = [
        row
...
def _running_structure_rejection(
    ...
    quality_rpe = (
        item["prescribed_rpe"] >= _POLICY["quality_session"]["quality_rpe_threshold"]
    )
    if quality_rpe and (
        target_zone not in {4, 5} or item["course_type"] != "intervals" or not intervals
    ):
        _add_reason(reasons, "zone45_requires_intervals", "block")
        return True
    if intervals and (
...
class TrainingSafetyRuleEngine:
    ...
    def validate_request(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if any(
            _utc(row["end_time_utc"]) > as_of for row in payload["quality_sessions"]
        ):
        ...
        for signal in payload["safety_signals"]:
            ...
            if (
                signal["kind"] in _POLICY["red_flag_signal_kinds"]
                and signal["origin"] == "device_fact"
            ):
    ...
    def validate_result(self, result: Mapping[str, Any]) -> None:
        ...
        if (
            payload["professional_evaluation_recommended"]
            and payload["safety_state"] != "suspended"
        ):
            raise SafetyRuleError(
                "training_safety_result_professional_evaluation_invalid"
            )
        if (
            payload["substitution_assessment"] is not None
            and payload["substitution_assessment"]["source_revision_id"] not in lineage
        ):
            raise SafetyRuleError("training_safety_result_lineage_invalid")
        if payload["primary_items"]:
            _validate_primary_semantics(payload["primary_items"][0])
        if (
            payload["primary_items"]
            and payload["primary_items"][0]["activity_kind"] == "running"
        ):
            item = payload["primary_items"][0]
            result_rule_reasons: list[dict[str, Any]] = []
            if _running_structure_rejection(
                item, result_rule_reasons
            ) or _high_intensity_rejection(
                item,
                date(2000, 1, 1),
                [],
                result_rule_reasons,
            ...
            if item["target_zone"] is None:
                ...
                if not zone["exact_bpm_allowed"] or item["target_bpm_range"] is None:
                    raise SafetyRuleError("training_safety_result_zone_invalid")
                if not isinstance(zone["zones"], list):
                    raise SafetyRuleError("training_safety_result_zone_invalid")
                expected = next(
                    row for row in zone["zones"] if row["zone"] == item["target_zone"]
                )
    ...
    def evaluate(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        all_revisions = sorted(
            {
                *(
                    _revision(row["source_revision_id"])
                    for row in payload["zone_evidence"]
                ),
                *(
                    _revision(row["source_revision_id"])
                    for row in payload["safety_signals"]
                ),
                *(
                    _revision(row["source_revision_id"])
                    for row in payload["quality_sessions"]
                ),
                *(
        ...
        if substitution is not None:
            ...
            _add_reason(
                reasons,
                code,
                "warning" if "conflicts" in code else "info",
                (substitution["source_revision_id"],),
            )
        ...
        if red_flags:
            ...
            _add_reason(
                reasons, "professional_evaluation_recommended", "warning", revisions
            )
            result = {
                **base,
        ...
        if item["activity_kind"] == "running":
            ...
            if not required_stop <= set(item["stop_conditions"]):
                ...
                result = _rejected_result(
                    base, zone_selection, reasons, warnings, substitution
                )
                self.validate_result(result)
                return result
            ...
            if fallback:
                ...
                if target_zone is None:
                    ...
                    expected = next(
                        row
                        for row in zone_selection["zones"]
                        if row["zone"] == target_zone
                    )
                    expected_range = {
                        "minimum_bpm": expected["minimum_bpm"],
                        "maximum_bpm": expected["maximum_bpm"],
                    ...
                    if target_range is not None and target_range != expected_range:
                        _add_reason(
                            ...
                            "target_bpm_mismatch",
                            "block",
                            tuple(zone_selection["source_revision_ids"]),
                        )
                        result = _rejected_result(
                            base, zone_selection, reasons, warnings, substitution
                        )
            ...
            result = {
                **base,
                ...
                if warnings
                or status == "modified_for_safety"
                or any(row["severity"] == "warning" for row in reasons)
                else "normal",
                "professional_evaluation_recommended": False,
            ...
            if blockers:
                ...
                revisions = tuple(
                    _revision(row["source_revision_id"]) for row in blockers
                )
                ...
                result = {
                    **base,
                    "status": "accepted",
                    "safety_state": "warning"
                    if warnings or any(row["severity"] == "warning" for row in reasons)
                    else "normal",
                    "professional_evaluation_recommended": False,
