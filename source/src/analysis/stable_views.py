...
_SPECS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    (
        "v_current_daily_health",
        "SELECT id,subject_id,local_date,values_json,source_revision_id FROM v_current_daily_health WHERE subject_id=? AND local_date BETWEEN ? AND ? ORDER BY local_date,id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_physiology_records",
        "SELECT id,subject_id,domain,record_type,effective_at_utc,period_start_utc,period_end_utc,local_date,value_origin,status_key,status_text,source_revision_id FROM v_current_physiology_records WHERE subject_id=? AND COALESCE(local_date,substr(effective_at_utc,1,10)) BETWEEN ? AND ? ORDER BY COALESCE(local_date,substr(effective_at_utc,1,10)),id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_physiology_metrics",
        "SELECT m.id,r.subject_id,r.domain,r.record_type,r.effective_at_utc,m.physiology_record_id,m.metric_key,m.value_number,m.value_text,m.value_boolean,m.value_json,m.raw_unit,m.canonical_unit,m.value_origin FROM v_current_physiology_metrics m JOIN physiology_records r ON r.id=m.physiology_record_id WHERE r.subject_id=? AND COALESCE(r.local_date,substr(r.effective_at_utc,1,10)) BETWEEN ? AND ? ORDER BY COALESCE(r.local_date,substr(r.effective_at_utc,1,10)),m.metric_key,m.id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_sleep_sessions",
        "SELECT id,subject_id,session_type,start_time_utc,end_time_utc,values_json,source_revision_id FROM v_current_sleep_sessions WHERE subject_id=? AND start_time_utc<? AND end_time_utc>? ORDER BY start_time_utc,id",
        ("subject", "end_utc", "start_utc"),
    ),
    (
        "v_current_health_samples",
        "SELECT id,subject_id,observed_at_utc,local_date,metric_key,value_number,value_text,raw_unit,canonical_unit,source_revision_id FROM health_samples WHERE subject_id=? AND local_date BETWEEN ? AND ? ORDER BY observed_at_utc,id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_activities",
        """SELECT a.id,a.subject_id,a.provider_activity_id,a.name,a.sport,a.sub_sport,a.start_time_utc,a.end_time_utc,a.local_date,a.elapsed_seconds,a.timer_seconds,a.distance_m,a.primary_revision_id,a.provider_state,
      ...
      FROM v_current_activities a WHERE a.subject_id=? AND a.local_date BETWEEN ? AND ? ORDER BY a.local_date,a.start_time_utc,a.id""",
        ("subject", "date", "date"),
    ),
    (
        "v_activity_segments",
        "SELECT s.id,a.subject_id,s.activity_id,s.segment_type,s.segment_index,s.start_time_utc,s.end_time_utc,s.duration_seconds,s.distance_m,s.source_revision_id FROM v_activity_segments s JOIN activities a ON a.id=s.activity_id WHERE a.subject_id=? AND a.local_date BETWEEN ? AND ? ORDER BY a.local_date,s.activity_id,s.segment_index,s.id",
        ("subject", "date", "date"),
    ),
    (
        "v_activity_metric_sources",
        "SELECT m.id,a.subject_id,m.activity_id,m.metric_key,m.valid_from_utc,m.valid_to_utc,m.source_kind,m.device_id,m.developer_data_index,m.attribution_method,m.confidence FROM v_activity_metric_sources m JOIN activities a ON a.id=m.activity_id WHERE a.subject_id=? AND a.local_date BETWEEN ? AND ? ORDER BY a.local_date,m.activity_id,m.metric_key,m.id",
        ("subject", "date", "date"),
    ),
    (
        "v_active_user_facts",
        "SELECT id,subject_id,fact_key,fact_value_json,scope,effective_from_utc,expires_at_utc,confidence FROM v_active_user_facts WHERE subject_id=? ORDER BY fact_key,id",
        ("subject",),
    ),
    (
        "v_current_analysis_artifacts",
        "SELECT id,subject_id,artifact_kind,period_start_local_date,period_end_local_date,revision_no,schema_version,content_sha256,created_at_utc FROM v_current_analysis_artifacts WHERE subject_id=? AND period_end_local_date>=? AND period_start_local_date<=? ORDER BY period_start_local_date,artifact_kind,revision_no,id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_weekly_summaries",
        "SELECT id,subject_id,artifact_kind,period_start_local_date,period_end_local_date,revision_no,schema_version,content_sha256,created_at_utc FROM v_current_weekly_summaries WHERE subject_id=? AND period_end_local_date>=? AND period_start_local_date<=? ORDER BY period_start_local_date,revision_no,id",
        ("subject", "date", "date"),
    ),
    (
        "v_current_training_plans",
        "SELECT id,subject_id,analysis_artifact_id,plan_start_local_date,plan_end_local_date,timezone,status,objective_json,constraints_json,created_at_utc FROM v_current_training_plans WHERE subject_id=? AND plan_end_local_date>=? AND plan_start_local_date<=? ORDER BY plan_start_local_date,id",
        ("subject", "date", "date"),
    ),
    (
        "v_training_plan_items",
        "SELECT i.id,p.subject_id,i.training_plan_id,i.item_index,i.local_date,i.activity_kind,i.prescription_json,i.rationale_text FROM v_training_plan_items i JOIN v_current_training_plans p ON p.id=i.training_plan_id WHERE p.subject_id=? AND i.local_date BETWEEN ? AND ? ORDER BY i.local_date,i.item_index,i.id",
        ("subject", "date", "date"),
    ),
    (
        "v_analysis_history_context",
...
def _date(value: str) -> str:
    try:
        ...
        raise StableViewError("analysis_snapshot_date_invalid") from error
...
def _utc(value: str) -> tuple[str, datetime]:
    ...
    try:
        ...
        raise StableViewError("analysis_snapshot_utc_invalid") from error
    return value, parsed
...
_REASON_CHANGE_KINDS = frozenset(
    {"move", "cancel", "replace", "availability", "injury", "preference"}
)
...
def _reason_payload(row: dict[str, Any], subject_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "source_mail_message_id": row["mail_message_id"],
        "source_mail_thread_id": row["mail_thread_id"],
        "source_revision_id": row["source_revision_id"],
        "change_kind": payload["change_kind"],
        "affected_local_dates": payload["affected_local_dates"],
        "constraints": payload["constraints"],
        "effective_local_date": payload["effective_local_date"],
...
class StableViewRepository:
        ...
        try:
            ...
            try:
                self._restore_snapshot_state(
                    prior_factory, prior_query_only, rollback=True
                )
        ...
        *,
        ...
        if self._snapshot_read_active:
            try:
                ...
                if isinstance(error, StableViewError):
                    raise
                raise StableViewError("analysis_snapshot_read_failed") from error
        ...
        try:
            self._restore_snapshot_state(
                prior_factory, prior_query_only, rollback=not committed
            )
    ...
    def view(
        ...
        end_local_date: str,
        *,
        limit: int = _MAX_PUBLIC_VIEW_ROWS,
    ) -> tuple[dict[str, Any], ...]:
        start_local_date, end_local_date = (
            _date(start_local_date),
            _date(end_local_date),
        )
        maximum = (
            _MAX_VIEW_ROWS if self._snapshot_read_active else _MAX_PUBLIC_VIEW_ROWS
        )
        if (
            start_local_date > end_local_date
        ...
        _, sql, kinds = spec
        start_utc, end_utc = (
            f"{start_local_date}T00:00:00Z",
            f"{end_local_date}T23:59:59.999999Z",
        )
        params = tuple(
            {
                "subject": subject_id,
                "date": start_local_date if index == 1 else end_local_date,
                "start_utc": start_utc,
                "end_utc": end_utc,
            }[kind]
    ...
    def snapshot(
        ...
        if start_local_date > end_local_date:
            raise StableViewError("analysis_snapshot_date_invalid")
        if (
            isinstance(subject_id, bool)
            or not isinstance(subject_id, int)
            or subject_id <= 0
        ...
        try:
            ...
            subject_rows = tuple(
                self._execute(
                    "SELECT id,timezone,is_active FROM data_subjects WHERE id=?",
                    (subject_id,),
                )
            )
            identity_rows = tuple(
                self._execute(
                    "SELECT subject_id,provider,identity_kind,is_verified FROM subject_identities WHERE subject_id=? AND provider='garmin' AND identity_kind='account' AND is_verified=1",
                    (subject_id,),
                )
            )
            ...
            if (
                ...
                or subject["timezone"] != "Asia/Hong_Kong"
                or identity["subject_id"] != subject_id
                or identity["provider"] != "garmin"
                or identity["identity_kind"] != "account"
            ...
            def aux(
                op: str, sql: str, params: tuple[object, ...]
            ) -> tuple[dict[str, Any], ...]:
                return self._read(
                    op,
                    subject_id,
                    sql + " LIMIT ?",
                    params + (_MAX_AUX_ROWS + 1,),
                    start_local_date,
                    end_local_date,
                    limit=_MAX_AUX_ROWS,
                )
            ...
            facts = aux(
                "facts",
                "SELECT id,fact_key,scope,effective_from_utc,expires_at_utc,confidence FROM v_active_user_facts WHERE subject_id=? ORDER BY fact_key,id",
                (subject_id,),
            )
            capabilities = aux(
                "capabilities",
                "SELECT environment_key,resource_kind,capability_state,last_checked_at_utc,next_probe_at_utc FROM garmin_resource_capabilities WHERE subject_id=? ORDER BY environment_key,resource_kind,id",
                (subject_id,),
            )
            raw_plan_reasons = aux(
                "plan_reasons",
                "SELECT e.id,e.subject_id,e.event_type,e.actor_role,e.occurred_at_utc,e.trust_level,e.created_by,e.structured_payload_json AS reason_payload,m.id AS mail_message_id,m.mail_thread_id,m.actor_role AS mail_actor_role,m.direction,m.processing_state,m.source_revision_id,t.subject_id AS thread_subject_id,t.is_current AS thread_is_current,CASE WHEN sr.id IS NULL THEN 0 ELSE 1 END AS source_is_current FROM v_plan_revision_reason_events e JOIN mail_messages m ON m.id=e.mail_message_id JOIN mail_threads t ON t.id=m.mail_thread_id LEFT JOIN source_revisions sr ON sr.id=m.source_revision_id AND sr.is_current=1 AND sr.provider='gmail' AND sr.resource_kind='message_json' AND sr.provider_object_id=m.provider_message_id WHERE e.subject_id=? ORDER BY e.occurred_at_utc,e.id",
                (subject_id,),
            )
            plan_reasons = tuple(
                _reason_payload(dict(row), subject_id) for row in raw_plan_reasons
            )
            if (
                sum(
                    len(x)
    ...
    def technical_samples(
        ...
        *,
        ...
        if (
            ...
            or isinstance(limit, bool)
            or not isinstance(limit, int)
            or not 1 <= limit <= _MAX_SAMPLE_LIMIT
        ):
            raise StableViewError("analysis_sample_request_invalid")
        start_utc, start_at = _utc(start_utc)
        end_utc, end_at = _utc(end_utc)
        if start_at >= end_at:
            raise StableViewError("analysis_sample_window_invalid")
        column = {
            "heart_rate_bpm",
            "cadence_rpm",
            "power_w",
        ...
        try:
            rows = self._read(
                ...
                subject_id,
                sql,
                (activity_id, subject_id, start_key, end_key, limit + 1),
                start_utc,
                end_utc,
                limit=limit + 1,
            )
