...
def regeneration_contract(
    source: Mapping[str, Any], reason_code: str
) -> Mapping[str, Any]:
    return {
...
class RegenerateRoute:
    def __init__(
        ...
        *,
        ...
        self.config, self.coordinator, self.stable_views, self.runner = (
            config,
            coordinator,
            stable_views,
            runner,
        ...
        self.context_builder, self.quality_gate, self.validator = (
            context_builder or AnalysisContextBuilder(),
            quality_gate or QualityGate(),
            validator,
        )
    ...
    def execute(self, request: AnalysisRequest) -> AnalysisReceipt:
        ...
        try:
            ...
            return self._receipt(
                ...
                errors=(
                    self._error(
                        "request",
                        _safe_code(error, "analysis_regeneration_request_invalid"),
                    ),
                ),
            )
        ...
        try:
            ...
            return self._receipt(
                ...
                errors=(
                    self._error(
                        "service", _safe_code(error, "analysis_regeneration_failed")
                    ),
                ),
            )
    ...
    def _persisted_receipt(
        ...
        if superseded_artifact_id is not None:
            ...
            superseded_plan_id = str(old_plan["id"]) if old_plan is not None else None
    ...
    def _source(self, request: AnalysisRequest) -> dict[str, Any]:
        ...
        row = self.connection.execute(
            "SELECT a.id,a.subject_id,a.artifact_kind,a.period_start_local_date,a.period_end_local_date,a.generated_by_run_id,a.is_current,r.analysis_kind,s.subject_key FROM analysis_artifacts a JOIN analysis_runs r ON r.id=a.generated_by_run_id JOIN data_subjects s ON s.id=a.subject_id WHERE a.id=?",
            (int(request.artifact_id),),
        ).fetchone()
        if (
            row is None
            or row["subject_key"] != request.subject_id
            or row["is_current"] != 1
        ):
            raise ValueError("analysis_regeneration_target_unavailable")
        kind, run_kind = row["artifact_kind"], row["analysis_kind"]
        if kind in {"daily_summary", "daily_training_advice"}:
            shape = "daily"
        elif (
            kind in {"weekly_summary", "weekly_training_plan"} and run_kind == "weekly"
        ):
            shape = "weekly"
        elif kind == "weekly_training_plan" and run_kind == "plan_revision":
            shape = "plan_revision"
        else:
            raise ValueError("analysis_regeneration_target_unsupported")
        return {
            "artifact_id": int(row["id"]),
            "subject_id": int(row["subject_id"]),
            "shape": shape,
            "kind": kind,
            "start": row["period_start_local_date"],
            "end": row["period_end_local_date"],
        }
    ...
    def _run(
        ...
        if shape == "daily":
            ...
            context_request = ContextBuildRequest(
                route="regenerate",
                regenerate_source_route="daily",
                run_key=prepared.decision.run_key,
                subject_id=subject_id,
                as_of_utc=request.requested_at_utc,
                summary_local_date=summary,
                advice_local_date=advice,
                artifact_id=int(source["artifact_id"]),
                max_context_bytes=self.config.max_context_bytes,
            )
            periods = {
                "summary": {"start_local_date": summary, "end_local_date": summary},
                "advice": {"start_local_date": advice, "end_local_date": advice},
            }
            ...
            context_request = ContextBuildRequest(
                ...
                subject_id=subject_id,
                as_of_utc=request.requested_at_utc,
                review_end_local_date=review_end,
                artifact_id=int(source["artifact_id"]),
                max_context_bytes=self.config.max_context_bytes,
            )
            periods = {"review": review, "plan": plan}
            gate_kind, feature = "weekly_summary", None
        start, end = context_request.repository_window()
        snapshot = self.stable_views.snapshot(subject_id, start, end)
        gate = self.quality_gate.evaluate(
            QualityGateRequest(
                cast(
                    Literal[
                        "daily_summary", "weekly_summary", "weekly_plan", "revise_plan"
                    ],
                    gate_kind,
                ),
                subject_id,
                (periods.get("summary") or periods["review"])["start_local_date"],
                (periods.get("summary") or periods["review"])["end_local_date"],
        ...
        if gate.state == "blocked":
            ...
            return self._receipt(
                ...
                run_key=prepared.decision.run_key,
                analysis_run_id=str(prepared.decision.run_id),
                target_periods=self._complete_target_periods(periods),
                quality=gate.state,
                warnings=tuple(
                    AnalysisWarning(x.code, x.entity, x.summary) for x in gate.blockers
                ),
        ...
        plan_adherence = (
            () if shape == "daily" else _plan_adherence(snapshot, periods["review"])
        )
        ...
        safety = (
            ...
            if shape == "daily"
                ...
                (
                    date.fromisoformat(periods["plan"]["start_local_date"])
                    + timedelta(days=i)
        ...
        expected = ResultValidationExpectation(
            ...
            _safety_base(
                subject_id,
                periods["advice"]["start_local_date"],
                request.requested_at_utc,
                snapshot,
            )
            if shape == "daily"
            else {},
            weekly_safety_request_bases=safety or None,
            prior_artifact_state=None
            if shape != "weekly"
            else _prior_artifact_state(snapshot, periods["review"]),
            regeneration_source_shape=shape,
            regeneration_source_artifact_id=str(source["artifact_id"]),
            training_difficulty_level=self.config.training_difficulty_level,
            coaching_profile=self.config.coaching_profile_contract,
            progression_inputs={
                "recovery_ready": gate.state == "ready",
                "adherence_stable": gate.state == "ready",
        ...
        evidence = RunEvidence(
            ...
            dict(getattr(result, "audit", {})),
        )
        delivery_kind = (
            "daily_report"
            if shape == "daily"
            else ("weekly_report" if shape == "weekly" else "plan_revision")
        )
        ...
        ids = tuple(
            str(getattr(x, "artifact_id", ""))
            for x in getattr(pending, "artifacts", ())
        )
        delivery = AnalysisDelivery(
            str(getattr(pending, "delivery_id")), "pending", ids
        )
        ...
        return self._receipt(
            ...
            target_periods=self._complete_target_periods(periods),
            quality=gate.state,
            artifact_ids=tuple(str(v) for v in published.artifact_ids.values()),
            training_plan_id=str(published.training_plan_id)
            if published.training_plan_id
            else None,
            ...
            content_same=published.content_same,
    ...
    def _target_periods(source: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if source["shape"] == "daily":
            ...
            return {
                "summary": {
                    "start_local_date": summary.isoformat(),
                    "end_local_date": summary.isoformat(),
                },
                "advice": {
                    "start_local_date": advice.isoformat(),
                    "end_local_date": advice.isoformat(),
                },
                "review": None,
                "plan": None,
            }
        anchor = (
            date.fromisoformat(str(source["end"])) + timedelta(days=1)
            if source["shape"] == "weekly" and source["kind"] == "weekly_summary"
            else start
        )
        review, plan = _periods(anchor.isoformat())
    ...
    def _receipt(
        ...
        *,
        ...
        target_periods: dict[str, Any] | None = None,
        quality: str = "blocked",
        ...
        return AnalysisReceipt(
            run_key or "analysis:invalid:regenerate:invalid:invalid",
            ...
            or {"summary": None, "advice": None, "review": None, "plan": None},
