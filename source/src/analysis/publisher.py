...
def _content_hash(structured: Any, text: str) -> str:
    return sha256(
        _canonical(
            {"structured_content": structured, "user_visible_text": text}
        ).encode("utf-8")
    ).hexdigest()
...
def _utc_bounds(window: Mapping[str, Any]) -> tuple[str, str]:
    ...
    end_utc = datetime.combine(end + timedelta(days=1), time.min, _SG).astimezone(
        timezone.utc
    )
    return (
...
class AnalysisPublisher:
    ...
    def publish_with_pending_delivery(
        ...
        *,
        ...
        try:
            ...
            create = getattr(delivery_factory, "create_pending_in_transaction", None)
            if not callable(create):
                raise AnalysisPublishError("analysis_publish_delivery_adapter_invalid")
            pending = create(
    ...
    def _publish_in_transaction(
        ...
        if (
            result.get("run_key") != run["run_key"]
            or result.get("subject_id") != run["subject_id"]
        ):
            raise AnalysisPublishError("analysis_publish_run_identity_mismatch")
        if result.get("mode") == "daily" and run["analysis_kind"] == "daily":
            return self._publish_daily(
                run, result, input_manifest, run_evidence, failpoint
            )
        if result.get("mode") == "weekly" and run["analysis_kind"] == "weekly":
            return self._publish_weekly(
                run, result, input_manifest, run_evidence, failpoint
            )
        if (
            result.get("mode") == "revise_plan"
            and run["analysis_kind"] == "plan_revision"
        ):
            return self._publish_plan_revision(
                run, result, input_manifest, run_evidence, failpoint
            )
        if (
            result.get("mode") == "regenerate"
            and run["analysis_kind"] == "regeneration"
        ):
            return self._publish_regeneration(
                run,
                result,
                input_manifest,
                run_evidence,
    ...
    def _publish_regeneration(
        ...
        if (
            ...
            raise AnalysisPublishError("analysis_publish_regeneration_source_invalid")
        if shape == "daily":
            receipt = self._publish_daily(
                run, result, input_manifest, run_evidence, failpoint
            )
        elif shape == "weekly":
            receipt = self._publish_weekly(
                run, result, input_manifest, run_evidence, failpoint
            )
        else:
            receipt = self._publish_regenerated_plan(
                run, result, input_manifest, run_evidence, failpoint
            )
    ...
    def _publish_regenerated_plan(
        ...
        if (
            ...
            or artifacts[0].get("artifact_kind") != "weekly_training_plan"
        ):
            raise AnalysisPublishError(
                "analysis_publish_regeneration_plan_cardinality_invalid"
            )
        artifact = artifacts[0]
        plan = self._validate_weekly_plan(result.get("training_plan"), artifact)
        ...
        artifact_id, revision = self._insert_revision(
            int(run["subject_id"]), int(run["id"]), "weekly_training_plan", artifact
        )
        self._fire(failpoint, "after_artifact:weekly_training_plan")
        superseded = self._supersede_overlapping_plans(
            int(run["subject_id"]), plan["start"], plan["end"]
        )
        ...
        cursor = self._connection.execute(
            ...
            (
                ...
                plan["end"].isoformat(),
                plan["timezone"],
                _canonical(plan["objective"]),
                _canonical(plan["constraints"]),
                now,
            ),
        )
        plan_id = require_lastrowid(cursor)
        ...
        for item in plan["items"]:
            self._connection.execute(
                ...
                (
                    ...
                    item["local_date"],
                    item["activity_kind"],
                    _canonical(item["prescription"]),
                    item["rationale_text"],
                    _canonical(item["stop_conditions"]),
                ),
            )
        ...
        return PublishReceipt(
            int(run["id"]),
            {"weekly_training_plan": artifact_id},
            {"weekly_training_plan": revision},
            len(input_manifest),
            plan_id,
            superseded,
        )
    ...
    def _publish_daily(
        ...
        for left, right in (
            ...
            ("daily_training_advice", "daily_summary"),
        ):
            self._connection.execute(
        ...
        self._record_prior_artifact_relations(
            int(run["subject_id"]), ids, input_manifest, now
        )
        self._fire(failpoint, "after_relations")
        return PublishReceipt(
            int(run["id"]), dict(ids), dict(revisions), len(input_manifest)
        )
    ...
    def _publish_weekly(
        ...
        plan = self._validate_weekly_plan(
        ...
        for left, right in (
            ...
            ("weekly_training_plan", "weekly_summary"),
        ):
            self._connection.execute(
                "INSERT INTO analysis_artifact_relations(from_artifact_id,to_artifact_id,relation_type,created_at_utc) VALUES(?,?,?,?)",
                (ids[left], ids[right], "paired_with", now),
    ...
    def _publish_plan_revision(
        ...
        if len(by_kind) != 1 or set(by_kind) != {"weekly_training_plan"}:
            raise AnalysisPublishError(
                "analysis_publish_plan_revision_cardinality_invalid"
            )
        ...
        old_items = self._load_plan_items(
            int(old["plan_id"]), plan["start"], plan["end"]
        )
        ...
        new_plan_id = require_lastrowid(cursor)
        self._fire(failpoint, "after_plan")
        prefix_count = (
        ...
        for item, copied in published_items:
            self._connection.execute(
                ...
                (
                    ...
                    if copied
                    else _canonical(item["prescription"]),
                    item["rationale_text"],
                    item["stop_conditions_json"]
                    if copied
                    else _canonical(item["stop_conditions"]),
                ),
            )
            self._fire(failpoint, f"after_plan_item:{item['item_index']}")
        self._fire(failpoint, "after_plan_items")
        return PublishReceipt(
            int(run["id"]),
            {"weekly_training_plan": artifact_id},
            {"weekly_training_plan": artifact_revision},
            len(input_manifest),
            new_plan_id,
            (int(old["plan_id"]),),
        )
    ...
    def _validate_plan_revision(
        value: Any, artifact: Mapping[str, Any]
        ...
        try:
            ...
            raise AnalysisPublishError(
                "analysis_publish_plan_revision_invalid"
            ) from error
        if (
        ...
        for ordinal, row in enumerate(rows):
            ...
            if (
                ...
                != (effective + timedelta(days=ordinal)).isoformat()
    ...
    def _load_revision_source(
        self, subject_id: int, revision: Mapping[str, Any]
    ) -> sqlite3.Row:
        row = self._connection.execute(
            ...
            (
                revision["original_plan_id"],
                subject_id,
                revision["original_artifact_id"],
            ),
        ).fetchone()
        if (
            row is None
            or row["artifact_subject_id"] != subject_id
    ...
    def _validate_revision_manifest(
        ...
        actual = [
            (
                ...
                _integer_reference(item.get("source_entity_id")),
            )
        ...
        if (
            reason is None
            or reason["subject_id"] != old["subject_id"]
            or reason["event_type"] != "plan_revision_reason_recorded"
            or reason["actor_role"] != "trainlab"
            or reason["trust_level"] != "system_generated"
        ):
    ...
    def _assert_reason_unused(self, reason_event_id: int) -> None:
        ...
        if row is not None:
            raise AnalysisPublishError(
                "analysis_publish_revision_reason_already_consumed"
            )

    def _load_plan_items(
        self, plan_id: int, start: date, end: date
    ) -> list[dict[str, Any]]:
        ...
        for index, row in enumerate(rows):
            if (
                ...
                or row["local_date"] != (start + timedelta(days=index)).isoformat()
                or row["activity_kind"] not in _PLAN_ITEM_KINDS
                or not isinstance(row["rationale_text"], str)
            ):
                raise AnalysisPublishError(
                    "analysis_publish_revision_source_items_invalid"
                )
            try:
                prescription, stops = (
                    json.loads(row["prescription_json"]),
                    json.loads(row["stop_conditions_json"]),
                )
    ...
    def _validate_weekly_plan(
        value: Any, plan_artifact: Mapping[str, Any]
    ) -> dict[str, Any]:
        if not isinstance(value, Mapping) or set(value) != {
            "period",
            "timezone",
        ...
        try:
            ...
            raise AnalysisPublishError(
                "analysis_publish_weekly_plan_period_invalid"
            ) from error
        if (
            end - start != timedelta(days=6)
            or value.get("timezone") != "Asia/Hong_Kong"
            ...
            raise AnalysisPublishError("analysis_publish_weekly_plan_period_invalid")
        if not isinstance(value.get("objective"), Mapping) or not isinstance(
            value.get("constraints"), Mapping
        ):
            raise AnalysisPublishError("analysis_publish_weekly_plan_invalid")
        ...
        if (
            ...
            raise AnalysisPublishError(
                "analysis_publish_weekly_plan_prior_artifact_state_invalid"
            )
        rows = value.get("items")
        if (
            not isinstance(rows, Sequence)
        ...
        for ordinal, row in enumerate(rows):
            if not isinstance(row, Mapping) or set(row) != {
                ...
                raise AnalysisPublishError("analysis_publish_weekly_plan_items_invalid")
            if row.get("item_index") != ordinal or isinstance(
                row.get("item_index"), bool
            ):
                raise AnalysisPublishError("analysis_publish_weekly_plan_items_invalid")
            try:
                ...
                raise AnalysisPublishError(
                    "analysis_publish_weekly_plan_items_invalid"
                ) from error
            if (
                local_day != start + timedelta(days=ordinal)
                or row.get("activity_kind") not in _PLAN_ITEM_KINDS
            ):
                raise AnalysisPublishError("analysis_publish_weekly_plan_items_invalid")
            if not isinstance(row.get("prescription"), Mapping) or not isinstance(
                row.get("rationale_text"), str
            ):
                raise AnalysisPublishError("analysis_publish_weekly_plan_items_invalid")
            stops = row.get("stop_conditions")
            if (
                not isinstance(stops, Sequence)
                or isinstance(stops, (str, bytes))
                or any(not isinstance(x, str) or not x for x in stops)
            ):
                raise AnalysisPublishError("analysis_publish_weekly_plan_items_invalid")
            normalized.append(
                {
        ...
        return {
            ...
            "objective": dict(value["objective"]),
            "constraints": dict(value["constraints"]),
            "prior_artifact_state": dict(prior_state),
            "items": normalized,
        }

    def _supersede_overlapping_plans(
        self, subject_id: int, start: date, end: date
    ) -> tuple[int, ...]:
        rows = self._connection.execute(
        ...
        if ids:
            self._connection.executemany(
                "UPDATE training_plans SET status='superseded' WHERE id=? AND status IN ('active','proposed')",
                ((plan_id,) for plan_id in ids),
            )
        return ids
    ...
    def _persist_run_evidence(
        self,
        run: sqlite3.Row,
        value: RunEvidence | Mapping[str, Any],
        manifest: Sequence[Mapping[str, Any]],
    ) -> None:
        ...
        if snapshot.get("input_manifest") != list(manifest):
            raise AnalysisPublishError(
                "analysis_publish_run_evidence_manifest_mismatch"
            )
        ...
        try:
            ...
            raise AnalysisPublishError(
                "analysis_publish_run_evidence_invalid"
            ) from error
        if (
    ...
    def _record_prior_artifact_relations(
        ...
        for item in manifest:
            if (
                ...
                or item.get("source_entity_type") != "analysis_artifact"
            ):
                continue
            ...
            source = self._connection.execute(
                "SELECT id,subject_id,artifact_kind FROM analysis_artifacts WHERE id=?",
                (source_id,),
            ).fetchone()
    ...
    def _insert_inputs(
        self, run_id: int, manifest: Sequence[Mapping[str, Any]]
    ) -> None:
        if not isinstance(manifest, Sequence) or isinstance(manifest, (str, bytes)):
            raise AnalysisPublishError("analysis_publish_input_manifest_invalid")
        rows = list(manifest)
        for ordinal, item in enumerate(rows):
            ...
            if any(
                ...
                raise AnalysisPublishError("analysis_publish_input_manifest_invalid")
            if (
                item["trust_class"] not in _TRUST
                or len(item["input_sha256"]) != 64
                or any(c not in "0123456789abcdef" for c in item["input_sha256"])
            ):
                raise AnalysisPublishError("analysis_publish_input_manifest_invalid")
            ...
            self._connection.execute(
                ...
                (
                    ...
                    _integer_reference(item["source_entity_id"]),
                    _integer_reference(item["source_revision_id"]),
                    start_utc,
                    end_utc,
                    item["input_sha256"],
                    item["trust_class"],
    ...
    def _insert_revision(
        self, subject_id: int, run_id: int, kind: str, artifact: Mapping[str, Any]
    ) -> tuple[int, int]:
        period, structured, text = (
            artifact.get("period"),
            artifact.get("structured_content"),
            artifact.get("user_visible_text"),
        )
        ...
        try:
            start, end = (
                date.fromisoformat(str(period["start_local_date"])),
                date.fromisoformat(str(period["end_local_date"])),
            )
        except (KeyError, TypeError, ValueError) as error:
            raise AnalysisPublishError(
                "analysis_publish_artifact_period_invalid"
            ) from error
        if end < start:
        ...
        cursor = self._connection.execute(
            ...
            (
                ...
                text,
                _content_hash(structured, text),
                0,
                int(prior["id"]) if prior else None,
                now,
            ),
        )
        artifact_id = require_lastrowid(cursor)
        if prior:
            self._connection.execute(
                "UPDATE analysis_artifacts SET is_current=0 WHERE id=? AND is_current=1",
                (prior["id"],),
            )
            self._connection.execute(
                "INSERT INTO analysis_artifact_relations(from_artifact_id,to_artifact_id,relation_type,created_at_utc) VALUES(?,?,?,?)",
        ...
        self._connection.execute(
            "UPDATE analysis_artifacts SET is_current=1 WHERE id=?", (artifact_id,)
        )
