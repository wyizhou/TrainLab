...
`fit_activity_v2`。结果经[严格 Schema](../skills/_shared/schemas/fit_activity_v2.schema.json)
校验，按 FIT SHA 与 `fit-summary-2` 解析版本写入不可变 `parses`；原始秒级点不落库。
旧 `fit-summary-1` 保留明确重算入口，旧快照自行选择保存的版本；新版本追加，不覆盖旧结果。
...
- 两个解析版本的运动指标采样上限均为相邻 30 秒；更长间隔不填充，记为缺口。
...
解析逐字段投影，不保存原始 record 字典。按 A-022，v2 保留真实成对 GPS，经声明编码转换为度，
检查有限值和范围，并绑定实际时间及 FIT SHA；不平均或插值坐标。位置异常不丢弃运动指标。
活动、圈段和分段的 location 保存首尾实际定位点、点数及缺失/异常数。定位区间左闭右开，
最后窗口包含终点，定位点数可以核对去重；这与上文运动统计采样边界的语义不同。
相接 session 的共享交界点只归下一 session；有时间间隔时保留前一 session 的真实终点，
间隔内样本不归入两侧。活动、session、圈段、分段和细读共用该定位归属，不改变运动指标加权。
位置不足时字段保持 null，不冒称完整路线或以经纬度单独证明坡度、天气。
个人/设备序列号和凭据不进入解析结果。活动名由周证据从已保存 inventory capture 提取，
不会把 FIT workout_name 当活动名。原始 FIT 继续只留在私有实例。
