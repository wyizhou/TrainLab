...
变量，应在该文件中增加顶级字段；请从
[`config/examples/README.md`](../../config/examples/README.md) 仅提供安全字段说明。
...
| 当前用户设置模板 | `config/examples/README.md` | 跟踪 | 仅说明安全字段，不自动覆盖实例 |
| 当前数据基础与 Garmin 设置 | `config/foundation.yaml`、`config/garmin.yaml`（本地） | 忽略 | Foundation 存储根、Garmin 区域和认证运行参数 |
| 默认分析策略 | `src/resources/defaults/analysis.yaml` | 跟踪 | 分析层的版本化质量与训练策略 |
| Gmail MCP 模板 | `config/examples/README.md` | 跟踪 | 不保存 token 或账号 |
