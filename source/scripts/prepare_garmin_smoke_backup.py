...
from src.garmin_smoke_backup import prepare_backup
...
def main() -> int:
    ...
    try:
        receipt = prepare_backup(
            project_root=ROOT,
            output_root=args.output_root,
            key_file=args.key_file,
            authorization_id=args.authorization_id,
            operator_registered=args.operator_registered,
        )
    except Exception:
        print(
            json.dumps(
                {"status": "failed", "code": "backup_preflight_failed"}, sort_keys=True
            )
        )
        return 2
