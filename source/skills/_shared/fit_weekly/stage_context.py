...
def project(
    ...
    body = {
        "schema_version": "fit_weekly_stage_input_v3"
        if materials["schema_version"] == "fit_weekly_context_v4"
        else "fit_weekly_stage_input_v2"
        if week["parser_version"] == fit_parse.TIME_VERSION
        else "fit_weekly_stage_input_v1",
        "stage": stage,
    ...
    weekly_context.check_text(
        body,
        allow_sports_location=week["parser_version"] in fit_parse.LOCATION_VERSIONS,
    )
    if validate_payload(body, body["schema_version"]):
