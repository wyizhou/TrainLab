...
def answer(
    host: fit_detail.DetailHost, name: str, arguments: dict[str, Any]
    ...
    try:
        ...
        return response(body, failed=body["status"] != "available")
    except Exception as exc:
        code = str(exc)
        return failure(code if code in SAFE_ERRORS else "fit_detail_unavailable")


def create_server(host: fit_detail.DetailHost) -> Server:
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        return answer(host, name, arguments)

    return server


async def serve(host: fit_detail.DetailHost) -> None:
