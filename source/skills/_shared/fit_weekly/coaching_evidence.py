...
def resolve(ref: dict[str, Any], payload: dict[str, Any], *, root: Path) -> Any:
    ...
    if source in ("current", "detail"):
        ...
        with storage.open_store(root) as db:
            ...
            req = fit_detail.request_value(
                intent[1]["request"], bound_scope["parser_version"]
            )
            ...
            session_start = (
                fit_time.utc_time(session["start_utc"]) - origin
            ).total_seconds()
            session_end = (
                fit_time.utc_time(session["end_utc"]) - origin
            ).total_seconds()
            if (
                not session_start
                <= block["start_offset_seconds"]
    ...
    if source == "goal":
        ...
        if goal.get("schema_version") == "training_goal_text_v1" and ref["path"] != [
            "text"
        ]:
            raise ValueError("coaching_evidence_binding_invalid")
        return at(goal, ref["path"])
    if source == "history":
        histories = (
...
def validate_claim(
    ...
    *,
    ...
    if not claim["evidence"] and re.search(
        r"已(?:经)?(?:执行|完成)(?:降级|调整|减量)|(?:降级|调整|减量).{0,4}已(?:经)?执行",
        claim["text"],
    ):
        raise ValueError("coaching_completion_evidence_missing")
...
def _guard_heart_references(text: str) -> None:
    number = r"\d+(?:\.\d+)?"
    quantity = (
        rf"(?P<heart>(?:心率|heart[ _-]?rate)\s*(?:为|是|约|was|is|of)?\s*{number}"
        rf"|{number}\s*bpm)|rpe\s*{number}"
        rf"|{number}\s*(?:分钟|小时|公里|千米|秒|米|minutes?|seconds?|km|meters?)"
    )
    reference_target = (
        r"(?:作为|这是)[^，,；;]{0,16}(?:训练|本课)[^，,；;]{0,12}(?:目标|数值)"
        r"|(?:达到|保持|维持|控制)\s*(?:在)?\s*(?:这(?:个|一)?|该|此)(?:数值|水平)"
    )
    boundary = r"[，,；;]|但是|不过|然后|随后|而是|但|却|\bbut\b|\bhowever\b"
    for sentence in re.split(r"[。！？!?\n]|\.(?!\d)", text):
        quantities = list(re.finditer(quantity, sentence, re.I))
        for target in re.finditer(reference_target, sentence):
            preceding = [q for q in quantities if q.end() <= target.start()]
            if not preceding or preceding[-1].group("heart") is None:
                continue
            prefix = re.split(boundary, sentence[: target.start()], flags=re.I)[-1]
            suffix = re.split(boundary, sentence[target.end() :], flags=re.I)[0]
            denied = re.search(
                r"(?:不(?:要|应|得|能|会|必|需要|要求)?|无需|禁止|避免).{0,8}$",
                prefix,
            )
            stop = r"(?:时|即|就|则|便)(?:应|需|要)?(?:立即)?(?:停止|停下|终止)"
            conditional_stop = re.match(stop, suffix) or (
                re.search(r"(?:若|如果|当|一旦)[^，,；;]{0,8}$", prefix)
                and re.match(r".{0,20}?" + stop, suffix)
            )
            if not denied and not conditional_stop:
...
def guard_text(value: Any, *, future: bool) -> None:
    ...
    from skills._shared.fit_weekly import input_privacy

    for text in strings(value):
        input_privacy.check(text, allow_sports_location=True)
        _guard_heart_references(text)
        for clause in re.split(
            ...
            heart_notation = re.search(
                r"(?i)bpm|(?<![a-z])z[ -]?[1-9]|zone(?:\s*[1-9]|训练)|"
                r"hr[ _-]?max|max(?:imum)?[ _-]?heart[ _-]?rate|"
                r"heart[ _-]?rate[ _-]?zone|target[ _-]?hr|"
                r"心率.{0,8}(?:\d+|分区|区带|范围|上下限|最大|阈值)",
                clause,
            )
            prescribed_notation = heart_notation and (
                future
                or re.search(
                    r"建议|使用|采用|依据|按|目标|保持|维持|控制|把|target|keep|maintain",
                    clause,
                    re.I,
                )
            )
            if (heart_target or prescribed_notation) and not denied_heart:
                raise ValueError("coaching_heart_prescription")
            if re.search(
