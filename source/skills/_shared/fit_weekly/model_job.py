    ...
    try:
        with storage.open_store(root) as db:
            ...
            if (
                ...
                and stage_policy.legacy_goal(payload)
            ):
                raise ValueError("model_legacy_goal_readonly")
            ...
            if previous is None:
                preflight = getattr(adapter, "validate_new_attempt", None)
                if preflight is not None:
                    preflight(clone(request))
