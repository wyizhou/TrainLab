...
- 跑步独立规划分支R1补齐FIT同步总采集时间：首次耐久冻结截止时刻，启动/分页/下载共享余额，重启搬迁不返还；旧成功请求只读兼容，采集资源显式闭合。空实例同步、全运动解析、细读和搬迁重放已验证；本机3070项独立回归及Main实际验收通过，分支交付和双平台CI尚待完成。修正新增测试清单漏登记，保留首次全量失败证据。
