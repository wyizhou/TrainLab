...
- 跑步独立规划分支R2接通两个独立阶段：先投影跑步数据和可验证跑步历史制定计划，再以全部运动和不可变计划生成总结；阶段身份贯通同一模型账本、Codex适配和恢复，两阶段共享原周细读缓存与预算。旧单阶段保留必要只读兼容，安全与运动输入测试按新接口迁移。3089项本机独立回归及Main实际验收通过，分支交付/双平台CI尚待；真实CLI本机探针能力未确认，真实课程业务、PDF及发布尚未交付。
