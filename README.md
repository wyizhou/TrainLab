...
TrainLab 是一个运动数据分析项目。当前正在准备产品补丁版本 `v0.1.1`：前端覆盖 AI 分析、运动记录、通用运动详情、健康记录、佳明连接器、文件上传和设置页面；后端提供真实账号登录、用户归属的本地 FIT 上传与持久化、重命名、导入记录、可恢复删除、存储配额、数据库迁移和前后端一体化运行。FIT 详情使用稳定的训练组/攀岩段语义投影，未知 profile 字段会被保留而不会被猜测。佳明在线同步和 AI 仍保持模拟或未接入。
...
- 当前产品发布信息、范围和验证结果：`docs/releases/v0.1.1.md`。在 tag 和 GitHub Release 创建前，该文件只代表发布候选，不等于已经发布；上一正式版本保留在 `docs/releases/v0.1.0.md`。
