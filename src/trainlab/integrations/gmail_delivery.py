...
class GmailDeliveryError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        self.provider_message_id = _provider_id(provider_message_id) if provider_message_id is not None else None
        self.provider_thread_id = _provider_id(provider_thread_id) if provider_thread_id is not None else None
        super().__init__(code)
...
def _safe_text(value: object, *, code: str, allow_empty: bool = False, maximum: int = 1_000_000) -> str:
...
def _search_match(row: object, *, recipient: str, subject: str) -> _SearchMatch:
    ...
    candidate = _provider_id(row.get("id")) or _provider_id(row.get("messageId")) or _provider_id(row.get("message_id"))
    if candidate is None:
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    if row.get("subject") != subject or not _sender_matches(row.get("from", row.get("fromEmail", row.get("sender"))), recipient):
        raise GmailDeliveryError("gmail_delivery_search_identity_unverified")
    return _SearchMatch(candidate, _provider_id(row.get("threadId")) or _provider_id(row.get("thread_id")), _rfc_date_utc(row.get("date", row.get("Date"))))


def _search_matches(value: Any, *, recipient: str, subject: str) -> tuple[_SearchMatch, ...]:
    ...
    if isinstance(value, str):
        ...
        for block in blocks:
            item = re.fullmatch(r"ID: ([A-Za-z0-9_-]{1,256})\nSubject: ([^\r\n]*)\nFrom: ([^\r\n]*)\nDate: ([^\r\n]+)", block)
            if item is None:
                raise GmailDeliveryError("gmail_delivery_protocol_invalid")
            rows.append({"id": item.group(1), "subject": item.group(2), "from": item.group(3), "date": item.group(4)})
        value = {"emails": rows}
    ...
    for row in rows:
        item = _search_match(row, recipient=recipient, subject=subject)
        if item.provider_message_id not in {match.provider_message_id for match in result}:
            result.append(item)
    return tuple(result)
...
def _sent_message(value: Any) -> tuple[str, str | None]:
    if isinstance(value, str):
        match = re.fullmatch(r"Email sent successfully with ID: ([A-Za-z0-9_-]{1,256})", value)
        if match is None:
            raise GmailDeliveryError("gmail_delivery_protocol_invalid")
        return match.group(1), None
    if not isinstance(value, dict):
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    message_id = _provider_id(value.get("id")) or _provider_id(value.get("messageId")) or _provider_id(value.get("message_id"))
    if message_id is None:
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    thread_id = _provider_id(value.get("threadId")) or _provider_id(value.get("thread_id"))
...
def _label_id(value: Any) -> str:
    ...
    if not isinstance(value, dict):
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    label_id = _provider_id(value.get("id")) or _provider_id(value.get("labelId")) or _provider_id(value.get("label_id"))
    if label_id is None:
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    return label_id
...
def _modified_message(value: Any, *, expected_message_id: str) -> None:
    ...
    if not isinstance(value, dict):
        raise GmailDeliveryError("gmail_delivery_protocol_invalid")
    actual = _provider_id(value.get("id")) or _provider_id(value.get("messageId")) or _provider_id(value.get("message_id"))
...
class GmailDeliveryGateway:
    ...
    def deliver(
        ...
        *,
        ...
        if not isinstance(status, GmailEnvironmentStatus) or not status.available:
            code = status.code if isinstance(status, GmailEnvironmentStatus) and status.code in {
                "gmail_mcp_not_configured", "gmail_mcp_binding_invalid"
        ...
        try:
            ...
            existing = _search_matches(
                _result_payload(client.call_tool("search_emails", {"query": query, "maxResults": 2})),
            ...
            if len(existing) == 1:
                ...
                try:
                    ...
                    raise GmailDeliveryError(exc.code, may_have_sent=True, provider_message_id=item.provider_message_id, provider_thread_id=item.provider_thread_id) from None
                except Exception as exc:
                    raise self._translate(exc, may_have_sent=True, provider_message_id=item.provider_message_id, provider_thread_id=item.provider_thread_id) from None
                return GmailDeliveryReceipt("already_sent", item.provider_message_id, item.provider_thread_id, label_id, item.provider_sent_at_utc)
            if len(existing) > 1:
                raise GmailDeliveryError("gmail_delivery_idempotency_ambiguous")

            send_started = True
            sent_id, sent_thread_id = _sent_message(_result_payload(client.call_tool("send_email", {
                "to": [self._recipient],
    ...
    def reconcile(self, *, subject: str, idempotency_key: str) -> GmailDeliveryReceipt:
        ...
        status = self._inspector()
        if not isinstance(status, GmailEnvironmentStatus) or not status.available:
            code = status.code if isinstance(status, GmailEnvironmentStatus) and status.code in {"gmail_mcp_not_configured", "gmail_mcp_binding_invalid"} else "gmail_delivery_binding_invalid"
            raise GmailDeliveryError(code)
        client: GmailDeliveryClient | None = None
        try:
            client = self._client_factory(
            ...
            matches = _search_matches(
                _result_payload(client.call_tool("search_emails", {"query": _query_for_key(key), "maxResults": 2})),
                recipient=self._recipient,
                subject=subject,
            )
            if not matches:
    ...
    def _apply_label(client: GmailDeliveryClient, message_id: str) -> str:
        label_id = _label_id(_result_payload(client.call_tool("get_or_create_label", {"name": _LABEL_NAME})))
        ...
        _modified_message(_result_payload(client.call_tool("modify_email", {
    ...
    def _translate(
        ...
        *,
        ...
        details = {"may_have_sent": may_have_sent, "provider_message_id": provider_message_id, "provider_thread_id": provider_thread_id}
        if isinstance(exc, TimeoutError):
            return GmailDeliveryError("gmail_delivery_timeout", **details)
        if isinstance(exc, MCPResponseError):
            if exc.status_code == 401:
                return GmailDeliveryError("gmail_delivery_auth_required", **details)
            if exc.status_code == 403:
                return GmailDeliveryError("gmail_delivery_forbidden", **details)
            if exc.status_code == 429:
                return GmailDeliveryError("gmail_delivery_rate_limited", **details)
            if exc.status_code is not None and 500 <= exc.status_code <= 599:
                return GmailDeliveryError("gmail_delivery_provider_error", **details)
            return GmailDeliveryError("gmail_delivery_protocol_error" if exc.kind in {"protocol", "tool"} else "gmail_delivery_transport_error", **details)
        return GmailDeliveryError("gmail_delivery_transport_error", **details)
