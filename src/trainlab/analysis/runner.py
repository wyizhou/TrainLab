...
def _safe_bundle_files(config: AnalysisConfig, bundle: HarnessBundle) -> tuple[tuple[str, bytes], ...]:
    if not isinstance(bundle, HarnessBundle) or bundle.route == "delivery" or not bundle.files:
...
def _command(
    ...
    if Path(output_schema_path).is_absolute() or output_schema_path != "output.schema.json":
        raise AnalysisRunnerError("analysis_runner_command_invalid")
...
class AnalysisCodexRunner:
    ...
    def execute(self, bundle: HarnessBundle, canonical_context: bytes) -> AnalysisRunnerResult:
        ...
        try:
            with tempfile.TemporaryDirectory(dir=self._config.temp_root, prefix="codex-") as directory:
                ...
                try:
                    ...
                    raise AnalysisRunnerError("analysis_runner_process_start_failed") from error
                ...
                try:
                    ...
                    raise AnalysisRunnerError("analysis_runner_process_io_failed") from error

                output = self._collect_output(process, self._config.max_context_bytes)
                process_cleaned = _terminate_group(process) if process.poll() is None else True
                ...
                for key, _ in selector.select(min(remaining, 0.1)):
                    ...
                    if key.data == "stdout":
                        ...
                        if len(output) > maximum:
                            _terminate_group(process)
                            raise AnalysisRunnerError("analysis_runner_output_limit_exceeded")
