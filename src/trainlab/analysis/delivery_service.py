...
class AnalysisDeliveryService:
    ...
    def execute(self, delivery_id: int, mode: DeliveryMode) -> DeliveryExecution:
        ...
        if mode == "reconcile_delivery":
            return self._reconcile(pending.subject_id, state, pending.idempotency_key, rendered.subject)
        ...
        try:
            ...
            if error.may_have_sent or error.code == "gmail_delivery_idempotency_ambiguous":
    ...
    def _reconcile(
        self, subject_id: int, state: AnalysisDeliveryState, idempotency_key: str, subject: str
    ) -> DeliveryExecution:
        try:
