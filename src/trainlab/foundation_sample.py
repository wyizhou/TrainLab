...
from .foundation import FOUNDATION_SCHEMA_VERSION, FoundationConfig, FoundationRequest, FoundationTool, TABLES, VIEWS
...
def _write_owner_file(path: Path, content: bytes) -> None:
    """Create a data-root file without inheriting a permissive umask mode."""
...
def _secure_tree(root: Path) -> None:
    """Normalize all generated data-root objects after SQLite has closed."""
...
def scan_sample_privacy(root: Path) -> list[str]:
    ...
    patterns=(
        ...
        ("credential",r"(?:access|refresh)[_-]?token|api[_-]?key|password|client[_-]?secret|begin private key"),
        ("authorization_url",r"https?://accounts\.google|authorization\s*[:=]"),
        ("raw_html",r"<!doctype\s+html|</?(?:html|head|body|script|style|div|span|p|table|form|iframe)\b"),
        ("full_payload",r"full[_ -]?(?:provider|model)?[_ -]?payload|raw[_ -]?model[_ -]?response"),
        ("hidden_reasoning",r"chain[_ -]?of[_ -]?thought|hidden[_ -]?reasoning|<thinking>"),
    )
    values: list[tuple[str,str]] = []
    conn=sqlite3.connect(root / "data.db")
    try:
        for table in TABLES:
            columns=[row[1] for row in conn.execute(f'PRAGMA table_info("{table}")') if row[2].upper() in {"TEXT","BLOB"}]
            if not columns: continue
            selected=",".join(f'"{column}"' for column in columns)
            for row_index,row in enumerate(conn.execute(f'SELECT {selected} FROM "{table}"')):
                for column,value in zip(columns,row):
                    if value is not None:
                        values.append((f"database:{table}:{row_index}:{column}",value.decode("utf-8","replace") if isinstance(value,bytes) else str(value)))
    finally:
        conn.close()
    for path in sorted(root.rglob("*")):
        if path.is_file() and path.name != "data.db":
            values.append((f"file:{path.relative_to(root)}",path.read_bytes().decode("utf-8","replace")))
    for location,value in values:
        lowered=value.lower()
        for code,pattern in patterns:
            if re.search(pattern,lowered):
                findings.append(f"{code}:{location}")
...
def generate_sample_database(output_root: Path) -> Path:
    ...
    try:
        conn.execute("INSERT INTO data_subjects(subject_key,timezone,is_active,created_at_utc) VALUES('synthetic-subject','Asia/Singapore',1,?)", (UTC,)); subject = _id(conn, "data_subjects")
        ...
        conn.execute("INSERT INTO training_plans(subject_id,analysis_artifact_id,plan_start_local_date,plan_end_local_date,timezone,status,created_at_utc) VALUES(?,?,?,?,?,?,?)", (subject, artifacts['weekly_training_plan'], "2025-12-29", "2026-01-04", "Asia/Singapore", "active", UTC)); plan = _id(conn, "training_plans")
        ...
        conn.execute("INSERT INTO scheduler_jobs(job_key,workflow_kind,timezone,schedule_spec_json,is_enabled,misfire_policy,config_sha256,updated_at_utc) VALUES(?,?,?,?,?,?,?,?)", ("daily", "daily", "Asia/Singapore", '{}', 1, "skip", "8" * 64, UTC))
