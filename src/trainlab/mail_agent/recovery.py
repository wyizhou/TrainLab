...
RecoveryAction = Literal[
    "retry", "refresh_auth", "deferred", "auth_required", "rejected", "failed",
    "record_sent", "operator_review", "apply_label",
]
...
def decide_retry(
    *,
    error_kind: Literal["timeout", "network", "server", "rate_limited", "auth", "forbidden", "other"],
    attempt_count: int,
    now: datetime,
    deadline: datetime,
    ...
    """
    if error_kind not in {"timeout", "network", "server", "rate_limited", "auth", "forbidden", "other"}:
    ...
    if retry_after_seconds is not None:
        ...
        if retry_after_seconds > policy.inline_retry_after_max_seconds:
            return RetryDecision("deferred", "rate_limited", _stamp(current + timedelta(seconds=wait_seconds)))
    else:
...
def reconcile_unknown_delivery(
    *,
    ...
    run_id, thread_id = _id(expected_run_id, "expected_run_id"), _id(expected_thread_id, "expected_thread_id")
    if not isinstance(candidates, tuple) or any(not isinstance(item, DeliveryCandidate) for item in candidates):
        raise MailRecoveryError("mail_recovery_candidates_invalid")
    matches = tuple(item for item in candidates if item.run_id == run_id and item.provider_thread_id == thread_id)
    if len(matches) == 1:
        match = matches[0]
        return ReconcileDecision("record_sent", "delivery_reconciled", match.provider_message_id, match.provider_thread_id)
    if not matches:
...
def recover_label_only(*, delivery_status: str, provider_message_id: str | None) -> LabelRecoveryDecision:
    """Retry a missing label without resending an already-confirmed response."""
    if delivery_status not in {"sent", "already_sent", "delivery_unknown", "pending", "failed"}:
        raise MailRecoveryError("mail_recovery_delivery_status_invalid")
    if delivery_status in {"sent", "already_sent"} and isinstance(provider_message_id, str) and provider_message_id:
        return LabelRecoveryDecision("apply_label", "label_pending", provider_message_id)
    return LabelRecoveryDecision("operator_review", "label_recovery_evidence_missing", None)
