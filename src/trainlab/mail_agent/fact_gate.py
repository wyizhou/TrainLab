...
class MailFactGate:
        ...
        *,
        ...
        try:
            raw_policy = (
                ...
                else (root / _POLICY_PATH).read_bytes()
            )
            raw_schema = (
                policy_schema_bytes
                if policy_schema_bytes is not None
                else (root / _POLICY_SCHEMA_PATH).read_bytes()
            )
        ...
        self.provider_limitations = tuple(
            policy["provider_limitation_markers"]
        )
    ...
    def _identity(
        ...
        try:
            ...
            if (
                ...
                or (
                    plan is not None
                    and plan["subject_id"] != evidence.subject_id
                )
    ...
    def _content(
        ...
        if _has_phrase(combined, self.medical_phrases) or (
            ...
            and _has_phrase(combined, self.medical_conditions)
        ):
            raise MailFactGateError(
        ...
        if not provider_sentences:
            return
        manifest = {
            item["ordinal"]: item for item in context["input_manifest"]
        }
        used_provider_ids = {
            (
                item["input_role"],
        ...
        for role, source_id in used_provider_ids:
            ...
            provider_sources.extend(
                row for row in collection if row["id"] == source_id
            )
        ...
        for sentence in provider_sentences:
            ...
            if (
                ...
                and (
                    not metrics
                    or any(
                        _normalized(metric) not in source_text
    ...
    def _plan_request(
        ...
        trigger_day = _utc(
            trigger["timestamp_utc"], "mail_fact_gate_plan_request_invalid"
    ...
    def _dependency(
        ...
        if (
            ("analysis_artifact", dependency.artifact_id) not in used
            or ("current_plan", dependency.training_plan_id) not in used
        ):
    ...
    def evaluate(
        ...
        if action == "await_analysis":
            ...
            if action != "reply":
                raise MailFactGateError("mail_fact_gate_plan_action_invalid")
            ready, dependency_id, reason = self._dependency(
