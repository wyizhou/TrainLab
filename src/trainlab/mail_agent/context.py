...
_OAUTH_PARAMETER=re.compile(r"([?&](?:access_token|refresh_token|id_token|token|client_secret|code|state)=)[^&#\s]*",re.I)
_SECRET_ASSIGNMENT=re.compile(r"(?<![?&])\b(?:access_token|refresh_token|id_token|client_secret|password|credential)\s*[:=]\s*[^\s,;]+",re.I)

class MailContextError(RuntimeError): pass
@dataclass(frozen=True)
class ContextBuildResult:
    payload: dict[str,Any]; canonical_json: str; sha256: str; manifest: tuple[InputDraft,...]
def _j(x:Any)->str:return json.dumps(x,ensure_ascii=False,sort_keys=True,separators=(",",":"))
def _h(x:Any)->str:return hashlib.sha256(_j(x).encode()).hexdigest()
def _clip(s:str,n:int)->tuple[str,bool]:
    b=s.encode(); return (s,False) if len(b)<=n else (b[:n].decode("utf-8","ignore"),True)
def _redact_text(value:str)->str:
    ...
    for pattern in (_GOOGLE_ACCESS_TOKEN,_GOOGLE_CLIENT_SECRET,_JWT,_API_KEY): value=pattern.sub("<redacted-secret>",value)
    return _COORD.sub("<redacted-precise-location>",value)
def _safe_json_text(value:str)->str:
    try:return _j(_safe(json.loads(value)))
    except MailContextError: raise
    except Exception as e: raise MailContextError("context_embedded_json_invalid") from e
def _safe(x:Any)->Any:
    if isinstance(x,dict): return {str(k):(_safe_json_text(v) if str(k).endswith('_json') and isinstance(v,str) else _safe(v)) for k,v in sorted(x.items()) if not _BAD_KEY.search(str(k))}
    if isinstance(x,list): return [_safe(v) for v in x]
    if isinstance(x,str): return _redact_text(x)
    return x
...
class MailContextBuilder:
    def __init__(self,connection:sqlite3.Connection,*,schema_version:str="1",policy_version:str="mail-context-v1",shared_harness_version:str="unknown",mail_harness_version:str="unknown"):
        self.connection=connection; self.schema_version=schema_version; self.policy_version=policy_version; self.shared_harness_version=shared_harness_version; self.mail_harness_version=mail_harness_version
        schema=Path(__file__).resolve().parents[3]/"harness/schemas/mail_agent_input.schema.json"
        try:self.validator=Draft202012Validator(json.loads(schema.read_text()), format_checker=FormatChecker())
        except Exception as e: raise MailContextError("mail_input_schema_unavailable") from e
    def build(self,*,run_id:int,subject_id:int,trigger_message_id:int,as_of_local_date:str,requested_start_local_date:str|None=None,extension_reason:str|None=None)->ContextBuildResult:
        try:asof=date.fromisoformat(as_of_local_date); requested=date.fromisoformat(requested_start_local_date) if requested_start_local_date else None
        except Exception as e:raise MailContextError("context_date_invalid") from e
        if requested and requested>asof:raise MailContextError("context_date_future_invalid")
        if requested and (extension_reason not in _EXTENSION_REASONS or (asof-requested).days+1>MAX_EXTENSION_DAYS):raise MailContextError("context_date_extension_invalid")
        if self.connection.in_transaction:raise MailContextError("context_caller_transaction_active")
        old=self.connection.row_factory
        try:
            self.connection.row_factory=sqlite3.Row; self.connection.execute("PRAGMA query_only=ON"); self.connection.execute("BEGIN")
            out=self._build(run_id,subject_id,trigger_message_id,asof,requested,extension_reason)
            self.connection.execute("COMMIT"); return out
            ...
            self.connection.row_factory=old; self.connection.execute("PRAGMA query_only=OFF")
    def _build(self,run_id:int,sid:int,trigger_id:int,asof:date,requested:date|None,reason:str|None)->ContextBuildResult:
        run=self.connection.execute("SELECT id,run_key,invocation_id,request_kind,status FROM mail_agent_runs WHERE id=? AND subject_id=? AND request_kind='process' AND status IN ('started','deferred')",(run_id,sid)).fetchone()
        if not run:raise MailContextError("context_run_not_buildable")
        trigger=self.connection.execute("SELECT m.*,t.provider_thread_id FROM v_current_mail_messages m JOIN v_current_mail_threads t ON t.id=m.mail_thread_id JOIN source_revisions r ON r.id=m.source_revision_id AND r.is_current=1 AND r.provider='gmail' AND r.resource_kind='message_json' AND r.provider_object_id=m.provider_message_id WHERE m.id=? AND t.subject_id=? AND m.actor_role='user' AND m.direction='inbound' AND m.processing_state='queued'",(trigger_id,sid)).fetchone()
        if not trigger or not isinstance(trigger['body_text'],str) or not trigger['body_text'].strip():raise MailContextError("context_trigger_not_canonical_or_eligible")
        text,cut=_clip(trigger['body_text'],MAX_TRIGGER_BYTES)
        if cut:raise MailContextError("context_trigger_limit_exceeded")
        tid=trigger['mail_thread_id']; omissions:list[dict[str,Any]]=[]
        # Inner DESC gets newest; outer ASC restores chronological context, with id as a stable tie breaker.
        rows=list(self.connection.execute("SELECT * FROM (SELECT m.* FROM v_current_mail_messages m JOIN source_revisions r ON r.id=m.source_revision_id AND r.is_current=1 AND r.provider='gmail' AND r.resource_kind='message_json' AND r.provider_object_id=m.provider_message_id WHERE m.mail_thread_id=? ORDER BY COALESCE(m.received_at_utc,m.sent_at_utc) DESC,m.id DESC LIMIT ?) ORDER BY COALESCE(received_at_utc,sent_at_utc),id",(tid,MAX_THREAD_MESSAGES)))
        total=self.connection.execute("SELECT count(*) FROM v_current_mail_messages WHERE mail_thread_id=?",(tid,)).fetchone()[0]
        if total>len(rows):omissions.append({"kind":"thread_messages_omitted","count":total-len(rows),"selection":"oldest_first_removed"})
        thread=[]; used=0
        # Allocate bytes from newest to oldest, then restore chronological order.
        for r in reversed(rows):
            raw=r['body_text'] or ''; avail=max(0,MAX_THREAD_BYTES-used); body,c=_clip(raw,avail); used+=len(body.encode())
            if c:omissions.append({"kind":"thread_body_truncated","entity_id":r['id'],"omitted_bytes":len(raw.encode())-len(body.encode()),"selection":"oldest_body_first"})
            if r['actor_role']=='user' and r['direction']=='inbound': origin='user_asserted'
            elif r['actor_role']=='trainlab' or r['direction']=='outbound': origin='prior_model_output'
            else:
        ...
        if len(facts)>MAX_FACTS:raise MailContextError("context_active_facts_limit")
        plans=[_safe(dict(r)) for r in self.connection.execute("SELECT p.id,p.subject_id,p.analysis_artifact_id,p.plan_start_local_date,p.plan_end_local_date,p.timezone,p.status,p.objective_json,p.constraints_json,p.created_at_utc,a.revision_no AS artifact_revision_no FROM v_current_training_plans p JOIN v_current_analysis_artifacts a ON a.id=p.analysis_artifact_id JOIN analysis_runs ar ON ar.id=a.generated_by_run_id AND ar.subject_id=p.subject_id AND ar.status='succeeded' WHERE p.subject_id=? AND p.status='active' AND p.plan_start_local_date<=? AND p.plan_end_local_date>=? ORDER BY p.plan_start_local_date DESC,p.id DESC",(sid,str(asof),str(asof)))]
        if len(plans)>1:raise MailContextError("context_current_plan_ambiguous")
        if plans:
            plans[0]['items']=[_safe(dict(x)) for x in self.connection.execute("SELECT id,training_plan_id,item_index,local_date,activity_kind,prescription_json,rationale_text,stop_conditions_json FROM v_training_plan_items WHERE training_plan_id=? ORDER BY item_index,id",(plans[0]['id'],))]
        artifacts=[_safe(dict(r)) for r in self.connection.execute("SELECT a.* FROM v_current_analysis_artifacts a JOIN analysis_runs ar ON ar.id=a.generated_by_run_id AND ar.subject_id=a.subject_id AND ar.status='succeeded' WHERE a.subject_id=? AND a.is_current=1 AND a.period_end_local_date>=? AND a.period_start_local_date<=? AND a.artifact_kind IN ('daily_summary','daily_training_advice','weekly_summary','weekly_training_plan') ORDER BY a.period_end_local_date DESC,a.created_at_utc DESC,a.id DESC LIMIT ?",(sid,str(start),str(asof),MAX_ARTIFACTS))][::-1]
        excluded_artifacts=self.connection.execute("SELECT count(*) FROM v_current_analysis_artifacts a WHERE a.subject_id=? AND (a.period_end_local_date<? OR a.period_start_local_date>?)",(sid,str(start),str(asof))).fetchone()[0]
        if excluded_artifacts: omissions.append({"kind":"unrelated_artifacts_excluded","count":excluded_artifacts,"window_start":str(start),"window_end":str(asof)})
        prior=[_safe(dict(r)) for r in self.connection.execute("SELECT h.* FROM v_mail_response_history_context h WHERE h.subject_id=? AND h.mail_thread_id=? AND EXISTS(SELECT 1 FROM mail_agent_runs r WHERE r.id=h.generated_by_mail_agent_run_id AND r.subject_id=h.subject_id) ORDER BY h.created_at_utc DESC,h.id DESC LIMIT ?",(sid,tid,MAX_PRIOR_RESPONSES))][::-1]
        events=[_safe(dict(r)) for r in self.connection.execute("SELECT e.* FROM v_conversation_context e WHERE e.subject_id=? AND (e.mail_message_id IN (SELECT id FROM mail_messages WHERE mail_thread_id=?) OR e.analysis_artifact_id IN (SELECT id FROM v_current_analysis_artifacts WHERE subject_id=?)) ORDER BY e.occurred_at_utc,e.id LIMIT ?",(sid,tid,sid,MAX_EVENTS+1))]
        if len(events)>MAX_EVENTS:
            dropped=events[:-MAX_EVENTS]; events=events[-MAX_EVENTS:]
            omissions.append({"kind":"conversation_events_omitted","count":len(dropped),"selection":"oldest_first"})
        ...
        quality=[_safe(dict(r)) for r in self.connection.execute(quality_sql,(sid,tid,sid,str(start),str(asof),sid,str(start),str(asof),sid,sid,sid,MAX_QUALITY+1))]
        if len(quality)>MAX_QUALITY:
            dropped=quality[MAX_QUALITY:]; quality=quality[:MAX_QUALITY]
            omissions.append({"kind":"quality_issues_omitted","count":len(dropped),"selection":"lowest_priority_last"})
        entries=[]; dbmanifest=[]
        def add(role:str,typ:str,item:dict[str,Any],trust:str,revision:int|None,*,entity_revision:Any=None,instruction:str|None=None):
            entity_revision=item.get('revision_no', revision if entity_revision is None else entity_revision)
            if entity_revision is None:raise MailContextError("context_source_revision_required")
            if trust not in {'provider_fact','user_asserted','derived_statistic','prior_model_output'}: raise MailContextError("context_unpersistable_trust")
            digest=_h(item); dbmanifest.append(InputDraft(role,typ,digest,trust,item.get('id'),revision)); entries.append({"ordinal":len(entries),"input_role":role,"source_entity_type":typ,"source_entity_id":item.get('id'),"source_revision_id":revision,"entity_revision":entity_revision,"value_origin":trust,"trust_class":trust,"content_instruction_trust":instruction or 'untrusted_content',"input_sha256":digest,"context_schema_version":self.schema_version,"policy_version":self.policy_version,"shared_harness_version":self.shared_harness_version,"mail_harness_version":self.mail_harness_version})
        trig={"id":trigger_id,"provider_message_id":trigger['provider_message_id'],"thread_id":tid,"provider_thread_id":trigger['provider_thread_id'],"timestamp_utc":trigger['received_at_utc'] or trigger['sent_at_utc'],"subject":trigger['subject'],"latest_authored_text":_safe(text),"body_sha256":trigger['body_sha256'],"source_revision_id":trigger['source_revision_id'],"value_origin":"user_asserted","content_instruction_trust":"untrusted_content"}; add('trigger_message','mail_message',trig,'user_asserted',trigger['source_revision_id'],instruction='untrusted_content')
        for x in thread:add('thread_message','mail_message',x,x['value_origin'],x['source_revision_id'],instruction=x['content_instruction_trust'])
        ...
        for x in prior:add('prior_mail_response','mail_response_artifact',x,'prior_model_output',None,entity_revision=x['revision_no'])
        ...
        def refresh_manifest(role:str, entity:int, value:dict[str,Any])->None:
            ...
            for entry in entries:
                if entry['input_role']==role and entry['source_entity_id']==entity: entry['input_sha256']=digest
            for index,entry in enumerate(dbmanifest):
                if entry.input_role==role and entry.source_entity_id==entity:
                    dbmanifest[index]=InputDraft(entry.input_role,entry.source_entity_type,digest,entry.trust_class,entry.source_entity_id,entry.source_revision_id)
        ...
        while len(_j(payload).encode())>MAX_CONTEXT_BYTES:
            candidate=next((x for x in payload['thread_context'] if x['id']!=trigger_id),None)
            if candidate is None: break
            payload['thread_context'].remove(candidate); remove_manifest('thread_message',candidate['id'])
            omissions.append({"kind":"thread_message_omitted_total_limit","entity_id":candidate['id'],"selection":"oldest_nontrigger_first"})
        for index,x in enumerate(payload['current_activity_context']):
            if len(_j(payload).encode())<=MAX_CONTEXT_BYTES: break
            compact={key:x.get(key) for key in ('id','provider_activity_id','sport','local_date','start_time_utc','elapsed_seconds','distance_m','primary_revision_id')}
            if x != compact:
                payload['current_activity_context'][index]=compact; refresh_manifest('activity_fact',x['id'],compact)
                omissions.append({"kind":"activity_detail_compacted_total_limit","entity_id":x['id'],"omitted_fields":sorted(set(x)-set(compact))})
        while len(_j(payload).encode())>MAX_CONTEXT_BYTES and payload['relevant_analysis_artifacts']:
            x=payload['relevant_analysis_artifacts'].pop(0); omissions.append({"kind":"artifact_omitted_total_limit","entity_id":x['id']}); remove_manifest('analysis_artifact',x['id'])
        while len(_j(payload).encode())>MAX_CONTEXT_BYTES and payload['prior_mail_responses']:
            x=payload['prior_mail_responses'].pop(0); omissions.append({"kind":"prior_response_omitted_total_limit","entity_id":x['id']}); remove_manifest('prior_mail_response',x['id'])
        for ordinal,item in enumerate(entries): item['ordinal']=ordinal
        payload['context_limits']['thread_messages']=len(payload['thread_context'])
        payload['context_limits']['thread_body_bytes']=sum(len(x['body_text'].encode()) for x in payload['thread_context'])
        payload['context_limits']['prior_responses']=len(payload['prior_mail_responses'])
        payload['context_limits']['related_artifacts']=len(payload['relevant_analysis_artifacts'])
        canonical=_j(payload)
        if len(canonical.encode())>MAX_CONTEXT_BYTES:raise MailContextError("context_uncuttable_core_limit_exceeded")
        self._validate_nested(payload)
        errors=sorted(self.validator.iter_errors(payload),key=lambda e:list(e.path))
        if errors:raise MailContextError("mail_input_schema_invalid")
        ...
        return ContextBuildResult(payload,canonical,hashlib.sha256(canonical.encode()).hexdigest(),tuple(dbmanifest))
    ...
    def _validate_manifest(self,payload:dict[str,Any])->None:
        ...
        specs=(
            ...
            *([] if payload['current_training_plan'] is None else [('current_plan','training_plan',payload['current_training_plan'],'prior_model_output',lambda x:None,lambda x:x['artifact_revision_no'])]),
            *[('conversation_event','conversation_event',x,'derived_statistic',lambda x:None,lambda x:x['id']) for x in payload['conversation_events']],
        ...
        for key,(typ,item,trust,source_revision,entity_revision) in expected.items():
            entry=actual[key]
            if (entry['source_entity_type'],entry['value_origin'],entry['trust_class'],entry['source_revision_id'],entry['entity_revision'],entry['input_sha256']) != (typ,trust,trust,source_revision,entity_revision,_h(item)):
                raise MailContextError("mail_input_manifest_fragment_invalid")
            if (entry['context_schema_version'],entry['policy_version'],entry['shared_harness_version'],entry['mail_harness_version']) != (self.schema_version,payload['policies']['version'],self.shared_harness_version,self.mail_harness_version):
    ...
    def _validate_context_limits(payload:dict[str,Any])->None:
        ...
        collections={'thread_context':{item['id'] for item in payload['thread_context']},'current_activity_context':{item['id'] for item in payload['current_activity_context']},'relevant_analysis_artifacts':{item['id'] for item in payload['relevant_analysis_artifacts']},'prior_mail_responses':{item['id'] for item in payload['prior_mail_responses']}}
        for omission in limits['omissions']:
            kind=omission['kind']
            if 'count' in omission and omission['count'] < 1: raise MailContextError("mail_input_omission_invalid")
            if kind=='thread_body_truncated' and (omission['entity_id'] not in collections['thread_context'] or omission['omitted_bytes'] < 1): raise MailContextError("mail_input_omission_invalid")
            if kind=='thread_message_untrusted_excluded' and omission['entity_id'] in collections['thread_context']: raise MailContextError("mail_input_omission_invalid")
            if kind=='thread_message_omitted_total_limit' and omission['entity_id'] in collections['thread_context']: raise MailContextError("mail_input_omission_invalid")
            if kind=='activity_detail_compacted_total_limit' and (omission['entity_id'] not in collections['current_activity_context'] or any(field in next(item for item in payload['current_activity_context'] if item['id']==omission['entity_id']) for field in omission['omitted_fields'])): raise MailContextError("mail_input_omission_invalid")
            if kind=='artifact_omitted_total_limit' and omission['entity_id'] in collections['relevant_analysis_artifacts']: raise MailContextError("mail_input_omission_invalid")
            if kind=='prior_response_omitted_total_limit' and omission['entity_id'] in collections['prior_mail_responses']: raise MailContextError("mail_input_omission_invalid")

    @staticmethod
    def _validate_training_plan(plan:dict[str,Any]|None)->None:
        if plan is None:return
        if plan['status']!='active': raise MailContextError("mail_input_plan_state_invalid")
        start=date.fromisoformat(plan['plan_start_local_date']); end=date.fromisoformat(plan['plan_end_local_date'])
        if (end-start).days!=6: raise MailContextError("mail_input_plan_window_invalid")
        items=plan['items']
        if len(items)!=7 or [item['item_index'] for item in items] != list(range(7)): raise MailContextError("mail_input_plan_item_ordinal_invalid")
        dates=[date.fromisoformat(item['local_date']) for item in items]
        if any(item['training_plan_id']!=plan['id'] for item in items) or set(dates)!={start+timedelta(days=offset) for offset in range(7)}: raise MailContextError("mail_input_plan_item_invalid")
    ...
    def _validate_nested(payload:dict[str,Any])->None:
        ...
        allowed={
          ...
          'data_quality':{'id','entity_type','entity_id','issue_code','severity','details_json','status','first_seen_at_utc','last_seen_at_utc','resolved_at_utc','source_revision_id'},
          'conversation_events':{'id','subject_id','event_type','actor_role','occurred_at_utc','mail_message_id','analysis_artifact_id','mail_response_artifact_id','analysis_delivery_id','mail_delivery_id','related_run_key','content_text','structured_payload_json','trust_level','created_by'},
        }
        for field,names in allowed.items():
            for record in payload[field]:
                if not isinstance(record,dict) or set(record)-names: raise MailContextError('mail_input_nested_schema_invalid')
        plan=payload['current_training_plan']
        if plan is not None and (not isinstance(plan,dict) or not {'id','analysis_artifact_id','artifact_revision_no','items'}<=set(plan)):
