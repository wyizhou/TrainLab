...
RunStatus = Literal["started", "succeeded", "partial", "failed", "deferred", "cancelled"]
...
AlertStatus = Literal["pending", "sending", "sent", "already_sent", "delivery_unknown", "failed"]
...
_STEP_TRANSITIONS = {
    "pending": {"running", "skipped"}, "running": {"succeeded", "failed", "deferred"},
    "deferred": {"running", "failed", "skipped"},
    "succeeded": set(), "failed": set(), "skipped": set(),
}
_INCIDENT_TRANSITIONS = {
    "open": {"acknowledged", "resolved", "suppressed"},
    "acknowledged": {"resolved", "suppressed"},
    "resolved": {"open"}, "suppressed": {"open"},
}
_ALERT_TRANSITIONS = {
    ...
    "failed": {"sending"}, "sent": set(), "already_sent": set(),
}
...
class OrchestrationRepository:
    ...
    def __init__(self, database_path: Path, *, manifest_path: Path | None = None) -> None:
        ...
        self._manifest_path = manifest_path or Path(__file__).resolve().parents[3] / "harness/schemas/foundation_schema_manifest.json"
    ...
    def create_workflow(
        self, *, workflow_key: str, workflow_kind: str, trigger_kind: Literal["scheduled", "manual", "recovery", "reconcile"],
        started_at_utc: datetime, logical_local_date: str | None = None, subject_id: int | None = None,
        deadline_at_utc: datetime | None = None, parent_workflow_run_id: int | None = None,
    ) -> WorkflowRunRecord:
        _identifier(workflow_key); _identifier(workflow_kind); start = _utc_text(started_at_utc)
        if logical_local_date is not None:
            try: date.fromisoformat(logical_local_date)
            except ValueError as exc: raise OrchestrationRepositoryError("orchestrator_logical_date_invalid") from exc
        if deadline_at_utc is not None and _parse_utc(_utc_text(deadline_at_utc)) < _parse_utc(start):
        ...
        with self._transaction() as conn:
            try:
                cursor = conn.execute(
                    ...
                    (workflow_key, workflow_kind, subject_id, logical_local_date, trigger_kind, _optional_utc(deadline_at_utc), parent_workflow_run_id, start),
                )
            except sqlite3.IntegrityError as exc:
                raise OrchestrationRepositoryError("orchestrator_workflow_key_conflict") from exc
    ...
    def load_workflow_definition(
        ...
        if not isinstance(workflow_key, str):
            raise OrchestrationRepositoryError(
                "workflow_definition_key_invalid"
            )
    ...
    def upsert_scheduler_job(self, projection: SchedulerJobProjection, *, is_enabled: bool, updated_at_utc: datetime) -> SchedulerJobRecord:
        """Persist a fixed S5-02 projection; it cannot redefine scheduling semantics."""
        _identifier(projection.job_key); _identifier(projection.workflow_kind); _hash(projection.config_sha256)
        schedule = json.dumps({
    ...
    def transition_workflow(self, workflow_key: str, status: RunStatus, *, at_utc: datetime) -> WorkflowRunRecord:
        _identifier(workflow_key); timestamp = _utc_text(at_utc)
        with self._transaction() as conn:
            ...
            if current.status == status:
                if (current.completed_at_utc or current.started_at_utc) == timestamp:
                    return current
                raise OrchestrationRepositoryError("orchestrator_run_same_status_conflict")
            ...
            conn.execute("UPDATE orchestrator_runs SET status=?,completed_at_utc=? WHERE id=?", (status, complete, current.id))
            return self._run_by_id(conn, current.id)

    def create_step(
        self, *, workflow_key: str, step_key: str, ordinal: int, layer_no: int, tool_mode: str,
        request_sha256: str, status: StepStatus = "pending", invocation_id: str | None = None,
        downstream_run_id: str | None = None,
    ) -> WorkflowStepRecord:
        _identifier(workflow_key); _identifier(step_key); _identifier(tool_mode); _hash(request_sha256)
        if ordinal < 0 or not 1 <= layer_no <= 5 or status != "pending":
            raise OrchestrationRepositoryError("orchestrator_step_invalid_initial_state")
        if invocation_id is not None: _identifier(invocation_id)
        if downstream_run_id is not None: _identifier(downstream_run_id)
        with self._transaction() as conn:
            ...
            if conn.execute("SELECT 1 FROM orchestrator_steps WHERE orchestrator_run_id=? AND ordinal=?", (run.id, ordinal)).fetchone():
            ...
            try:
                cursor = conn.execute(
                    ...
                    (run.id, step_key, ordinal, layer_no, tool_mode, request_sha256, invocation_id, downstream_run_id, status),
                )
            except sqlite3.IntegrityError as exc:
                raise OrchestrationRepositoryError("orchestrator_step_key_conflict") from exc
            summary = _validated_step_summary(self._run_summary(conn, run.id))
            summary["steps"][step_key] = {"events": []}
            conn.execute("UPDATE orchestrator_runs SET result_summary_json=? WHERE id=?", (json.dumps(summary, sort_keys=True, separators=(",", ":")), run.id))
            return self._step_by_id(conn, cursor.lastrowid)

    def transition_step(self, *, workflow_key: str, step_key: str, status: StepStatus, at_utc: datetime,
                        receipt_sha256: str | None = None, controlled_counts: Mapping[str, int] | None = None,
                        evidence_code: str | None = None,
                        next_retry_at_utc: datetime | None = None) -> WorkflowStepRecord:
        _identifier(workflow_key); _identifier(step_key); timestamp = _utc_text(at_utc)
        if receipt_sha256 is not None: _hash(receipt_sha256)
        _counts(controlled_counts or {})
        if evidence_code is not None: _code(evidence_code)
        _validate_transition_payload(status, receipt_sha256, evidence_code, next_retry_at_utc, at_utc)
        with self._transaction() as conn:
            ...
            previous_events = history["steps"].get(current.step_key, {}).get("events", [])
            if previous_events and _parse_utc(timestamp) < _parse_utc(previous_events[-1]["at_utc"]):
                raise OrchestrationRepositoryError("orchestrator_step_timestamp_invalid")
            if current.status == status:
                if _same_step_event(raw_summary, current.step_key, status, timestamp, receipt_sha256, controlled_counts or {}, evidence_code, retry):
                    return current
                raise OrchestrationRepositoryError("orchestrator_step_same_status_conflict")
            _transition(_STEP_TRANSITIONS, current.status, status, "orchestrator_step")
            if current.started_at_utc is not None and _parse_utc(timestamp) < _parse_utc(current.started_at_utc):
                raise OrchestrationRepositoryError("orchestrator_step_timestamp_invalid")
            if status in {"succeeded", "failed", "skipped"}:
                retry = None
            completed = timestamp if status in {"succeeded", "failed", "skipped"} else None
            started = timestamp if status == "running" and current.status == "pending" else None
            attempts = current.attempt_count + (1 if status == "running" and current.status in {"pending", "deferred"} else 0)
            summary = _append_step_event(current.step_key, raw_summary, attempts, status, timestamp, controlled_counts or {}, receipt_sha256, evidence_code, retry)
            ...
            conn.execute(
                ...
                (status, receipt_sha256, attempts, retry, started, completed, current.id),
            )
            conn.execute("UPDATE orchestrator_runs SET result_summary_json=? WHERE id=?", (summary, current.orchestrator_run_id))
    ...
    def record_health_check(self, *, check_kind: str, target_kind: str, status: str, checked_at_utc: datetime,
        ...
        if threshold_version is not None: _identifier(threshold_version)
    ...
    def record_incident(self, *, incident_key: str, category: str, severity: Literal["info", "warning", "error", "critical"],
                        ...
                        related_step_id: int | None = None) -> IncidentRecord:
        _identifier(incident_key); _code(category)
        if severity not in {"info", "warning", "error", "critical"}:
            raise OrchestrationRepositoryError("operational_incident_severity_invalid")
        for value in (error_code, error_summary, next_action): _safe_text(value)
        timestamp = _utc_text(seen_at_utc)
        with self._transaction() as conn:
            if related_workflow_run_id is not None and related_step_id is not None:
                owner = conn.execute("SELECT orchestrator_run_id FROM orchestrator_steps WHERE id=?", (related_step_id,)).fetchone()
                if owner is None or owner[0] != related_workflow_run_id:
                    raise OrchestrationRepositoryError("operational_incident_reference_invalid")
            row = conn.execute("SELECT id FROM operational_incidents WHERE incident_key=?", (incident_key,)).fetchone()
            if row:
                previous = self._incident_by_id(conn, row[0])
                if previous.category != category or previous.related_workflow_run_id != related_workflow_run_id or previous.related_step_id != related_step_id:
                    raise OrchestrationRepositoryError("operational_incident_identity_conflict")
                severity_value = _higher_severity(previous.severity, severity)
                reopened = previous.state in {"resolved", "suppressed"}
                seen = max(_parse_utc(previous.last_seen_at_utc), _parse_utc(timestamp)).isoformat().replace("+00:00", "Z")
                conn.execute("UPDATE operational_incidents SET severity=?,state=?,resolved_at_utc=?,last_seen_at_utc=?,occurrence_count=occurrence_count+1,error_code=COALESCE(?,error_code),error_summary=COALESCE(?,error_summary),next_action=COALESCE(?,next_action) WHERE id=?", (severity_value, "open" if reopened else previous.state, None if reopened else previous.resolved_at_utc, seen, error_code, error_summary, next_action, previous.id))
                return self._incident_by_id(conn, previous.id)
            try:
                cursor = conn.execute("INSERT INTO operational_incidents (incident_key,category,severity,state,related_workflow_run_id,related_step_id,first_seen_at_utc,last_seen_at_utc,occurrence_count,error_code,error_summary,next_action) VALUES (?,?,?,'open',?,?,?,?,1,?,?,?)", (incident_key, category, severity, related_workflow_run_id, related_step_id, timestamp, timestamp, error_code, error_summary, next_action))
            except sqlite3.IntegrityError as exc:
                raise OrchestrationRepositoryError("operational_incident_reference_invalid") from exc
            return self._incident_by_id(conn, cursor.lastrowid)

    def transition_incident(self, incident_key: str, state: IncidentState, *, at_utc: datetime) -> IncidentRecord:
        _identifier(incident_key); timestamp = _utc_text(at_utc)
        with self._transaction() as conn:
            current = self._incident_by_key(conn, incident_key)
            if current.state == state:
                return current
            _transition(_INCIDENT_TRANSITIONS, current.state, state, "operational_incident")
            if _parse_utc(timestamp) < _parse_utc(current.first_seen_at_utc) or _parse_utc(timestamp) < _parse_utc(current.last_seen_at_utc):
                raise OrchestrationRepositoryError("operational_incident_timestamp_invalid")
    ...
    def create_alert_delivery(self, *, incident_key: str, idempotency_key: str) -> AlertDeliveryRecord:
        ...
        with self._transaction() as conn:
            ...
            existing = conn.execute("SELECT id FROM operational_alert_deliveries WHERE idempotency_key=?", (idempotency_key,)).fetchone()
            if existing:
                alert = self._alert_by_id(conn, existing[0])
                if alert.operational_incident_id != incident.id:
                    raise OrchestrationRepositoryError("operational_alert_idempotency_conflict")
                return alert
            try:
                cursor = conn.execute("INSERT INTO operational_alert_deliveries (operational_incident_id,idempotency_key,status) VALUES (?,?,'pending')", (incident.id, idempotency_key))
            except sqlite3.IntegrityError as exc:
    ...
    def get_scheduler_job(self, job_key: str) -> SchedulerJobRecord | None:
        _identifier(job_key)
        return self._readonly(lambda conn: self._job_by_key(conn, job_key, missing_none=True))
    ...
    def scheduler_jobs(self, limit: int = 16) -> tuple[SchedulerJobRecord, ...]:
        ...
        return self._readonly(lambda conn: tuple(
            self._job_from_row(row) for row in conn.execute(
                ...
                "FROM scheduler_jobs ORDER BY job_key ASC LIMIT ?", (limit,)
            )
        ))
    ...
    def claim_scheduler_due(
        ...
        *,
        ...
        """
        _identifier(job_key); _identifier(owner_instance_id); _identifier(workflow_key); _identifier(workflow_kind)
        ...
        due, next_due, now = _utc_text(due_at_utc), _utc_text(next_due_at_utc), _utc_text(now_utc)
        ...
        with self._transaction() as conn:
            ...
            if (lease is None or lease.owner_instance_id != owner_instance_id
                    or lease.owner_pid != owner_pid or _parse_utc(lease.expires_at_utc) <= _parse_utc(now)):
                return False
            job = self._job_by_key(conn, job_key, missing_none=True)
            if job is None or not job.is_enabled or job.next_due_at_utc != due or job.last_due_at_utc == due:
                return False
            generation = _lease_generation(lease)
            handoff_summary = _handoff_summary(job_key, due, next_due, job.config_sha256, generation, workflow_key, workflow_kind, subject_id, logical_local_date, trigger_kind, _optional_utc(deadline_at_utc))
            existing = self._run_by_key(conn, workflow_key, missing_none=True)
            if existing is not None:
                handoff = _decode_handoff(existing.result_summary_json)
                if handoff is not None and not _handoff_identity_matches(existing, handoff, workflow_kind, subject_id, logical_local_date, trigger_kind, deadline_at_utc):
                    raise OrchestrationRepositoryError("scheduler_handoff_identity_conflict")
            else:
                cursor = conn.execute(
                    ...
                    (workflow_key, workflow_kind, subject_id, logical_local_date, trigger_kind, _optional_utc(deadline_at_utc), now, handoff_summary),
                )
                if cursor.lastrowid is None:
                    raise OrchestrationRepositoryError("scheduler_handoff_persistence_failed")
            changed = conn.execute(
    ...
    def claim_pending_scheduler_handoff(
        self, *, workflow_key: str, owner_instance_id: str, owner_pid: int, now_utc: datetime,
    ) -> SchedulerHandoffRecord | None:
        """Re-dispatch one abandoned handoff only to a new lease generation."""
        _identifier(workflow_key); _identifier(owner_instance_id)
        if type(owner_pid) is not int or isinstance(owner_pid, bool) or owner_pid <= 0:
            raise OrchestrationRepositoryError("scheduler_claim_owner_invalid")
        now = _utc_text(now_utc)
        with self._transaction() as conn:
            lease = self._lease_by_key(conn, "supervisor", missing_none=True)
            if (lease is None or lease.owner_instance_id != owner_instance_id or lease.owner_pid != owner_pid
            ...
            for row in rows:
                ...
                if handoff is None or handoff["dispatch_generation"] == generation:
                    continue
                updated = _handoff_summary(handoff["job_key"], handoff["due_at_utc"], handoff["next_due_at_utc"], handoff["config_sha256"], generation, run.workflow_key, run.workflow_kind, run.subject_id, run.logical_local_date, run.trigger_kind, run.deadline_at_utc)
                changed = conn.execute(
    ...
    def pending_scheduler_handoffs(
        self, *, owner_instance_id: str, owner_pid: int, now_utc: datetime, limit: int = 200,
        ...
        with self._transaction() as conn:
            ...
            for row in rows:
                run = self._run_from_row(row); handoff = _decode_handoff(run.result_summary_json)
    ...
    def skip_scheduler_due(
        self, *, job_key: str, due_at_utc: datetime, next_due_at_utc: datetime,
        now_utc: datetime, owner_instance_id: str, owner_pid: int,
    ) -> bool:
        """Atomically consume an expired misfire without creating a workflow."""
        _identifier(job_key); _identifier(owner_instance_id)
        if type(owner_pid) is not int or isinstance(owner_pid, bool) or owner_pid <= 0:
            raise OrchestrationRepositoryError("scheduler_claim_owner_invalid")
        due, next_due, now = _utc_text(due_at_utc), _utc_text(next_due_at_utc), _utc_text(now_utc)
        if _parse_utc(next_due) <= _parse_utc(due):
    ...
    def get_scheduler_lease(self, lease_key: str) -> SchedulerLeaseRecord | None:
        ...
        return self._readonly(lambda conn: self._lease_by_key(conn, lease_key, missing_none=True))
    ...
    def get_step(self, workflow_key: str, step_key: str) -> WorkflowStepRecord | None:
        ...
        return self._readonly(lambda conn: self._validated_step_by_key(conn, workflow_key, step_key, missing_none=True))

    def get_incident(self, incident_key: str) -> IncidentRecord | None:
        _identifier(incident_key)
        return self._readonly(lambda conn: self._incident_by_key(conn, incident_key, missing_none=True))

    def get_alert_delivery(self, idempotency_key: str) -> AlertDeliveryRecord | None:
        _identifier(idempotency_key)
        return self._readonly(lambda conn: self._alert_by_key(conn, idempotency_key, missing_none=True))

    def recent_workflows(self, limit: int = 50) -> tuple[WorkflowRunRecord, ...]:
        if not 1 <= limit <= 200: raise OrchestrationRepositoryError("orchestrator_query_limit_invalid")
        return self._readonly(lambda conn: tuple(self._validated_run_by_key(conn, row[1]) for row in conn.execute("SELECT id,workflow_key FROM v_recent_orchestrator_runs ORDER BY started_at_utc DESC,id DESC LIMIT ?", (limit,))))

    def active_workflow_definitions(self, limit: int = 200, offset: int = 0) -> tuple["WorkflowDefinitionAggregate", ...]:
        ...
        try:
            rows = connection.execute(
                ...
                "ORDER BY COALESCE(deadline_at_utc,started_at_utc) ASC,workflow_key ASC LIMIT ? OFFSET ?", (limit, offset)
            ).fetchall()
            from .workflow_definition_store import load_persisted_workflow_definition
            values = tuple(
                load_persisted_workflow_definition(self, connection, row[0], missing_none=False)
                for row in rows
            )
    ...
    def open_incidents(self, limit: int = 50) -> tuple[IncidentRecord, ...]:
        if not 1 <= limit <= 200: raise OrchestrationRepositoryError("orchestrator_query_limit_invalid")
        return self._readonly(lambda conn: tuple(self._incident_from_row(row) for row in conn.execute("SELECT id,incident_key,category,severity,state,related_workflow_run_id,related_step_id,first_seen_at_utc,last_seen_at_utc,occurrence_count,resolved_at_utc,error_code,error_summary,next_action FROM v_open_operational_incidents ORDER BY last_seen_at_utc DESC,id DESC LIMIT ?", (limit,))))

    def _connect(self) -> sqlite3.Connection:
        fd, identity = _open_database_identity(self._database_path)
        try:
            conn = sqlite3.connect(f"file:{self._database_path}?mode=rw", uri=True, timeout=0)
        except BaseException as exc:
            os.close(fd)
            raise OrchestrationSchemaIncompatible("orchestrator_database_unavailable") from exc
        try:
            _assert_database_identity(self._database_path, fd, identity)
            listing = conn.execute("PRAGMA database_list").fetchall()
            if (len(listing) != 1 or listing[0][1] != "main"
                    or os.path.realpath(listing[0][2]) != os.path.realpath(self._database_path)):
            ...
            conn.close(); os.close(fd); raise OrchestrationSchemaIncompatible("orchestrator_schema_unreadable") from exc
        if issues or not _ALLOWED_TABLES <= set(self._manifest.get("tables", {})):
            conn.close(); os.close(fd); raise OrchestrationSchemaIncompatible("orchestrator_schema_incompatible")
    ...
    def _verify_connected_identity(self, conn: sqlite3.Connection) -> None:
        ...
        if record is None: raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe")
    ...
    def _close_connected(self, conn: sqlite3.Connection) -> None:
        ...
        try: conn.close()
            ...
            if record is not None: os.close(record[0])
    ...
    def _readonly(self, query):
        conn = self._connect()
        try: return query(conn)
        finally: self._close_connected(conn)

    @staticmethod
    def _run_summary(conn, run_id: int) -> str:
        row = conn.execute("SELECT result_summary_json FROM orchestrator_runs WHERE id=?", (run_id,)).fetchone()
        if row is None: raise OrchestrationRepositoryError("orchestrator_workflow_not_found")
        return row[0]
    ...
    def _run_by_id(self, conn, record_id: int) -> WorkflowRunRecord:
        row = conn.execute("SELECT id,workflow_key,workflow_kind,subject_id,logical_local_date,trigger_kind,status,deadline_at_utc,parent_workflow_run_id,started_at_utc,completed_at_utc,result_summary_json FROM orchestrator_runs WHERE id=?", (record_id,)).fetchone()
        if row is None: raise OrchestrationRepositoryError("orchestrator_workflow_not_found")
        return self._run_from_row(row)
    def _run_by_key(self, conn, key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT id,workflow_key,workflow_kind,subject_id,logical_local_date,trigger_kind,status,deadline_at_utc,parent_workflow_run_id,started_at_utc,completed_at_utc,result_summary_json FROM orchestrator_runs WHERE workflow_key=?", (key,)).fetchone()
        if row is None:
            if missing_none: return None
    ...
    def _validated_run_by_key(self, conn, key: str, *, missing_none: bool = False):
        ...
        if handoff is not None:
            if run.status != "started" or run.completed_at_utc is not None or run.parent_workflow_run_id is not None:
                raise OrchestrationRepositoryError("scheduler_handoff_row_mismatch")
            if conn.execute("SELECT 1 FROM orchestrator_steps WHERE orchestrator_run_id=? LIMIT 1", (run.id,)).fetchone() is not None:
                raise OrchestrationRepositoryError("scheduler_handoff_row_mismatch")
        ...
        if (run.status == "started" and run.completed_at_utc is not None) or (run.status in terminal and run.completed_at_utc is None):
            raise OrchestrationRepositoryError("orchestrator_run_summary_row_mismatch")
        if run.completed_at_utc is not None and _parse_utc(run.completed_at_utc) < started: raise OrchestrationRepositoryError("orchestrator_run_summary_row_mismatch")
        rows = [self._step_from_row(row) for row in conn.execute("SELECT id,orchestrator_run_id,step_key,ordinal,layer_no,tool_mode,request_sha256,invocation_id,downstream_run_id,receipt_sha256,status,attempt_count,next_retry_at_utc,started_at_utc,completed_at_utc FROM orchestrator_steps WHERE orchestrator_run_id=?", (run.id,))]
        if set(summary["steps"]) != {row.step_key for row in rows}: raise OrchestrationRepositoryError("orchestrator_run_summary_row_mismatch")
        for row in rows: _validate_step_row(row, summary)
        ...
        return run
    @staticmethod
    def _step_from_row(row): return WorkflowStepRecord(*row)
    def _step_by_id(self, conn, record_id: int):
        row = conn.execute("SELECT id,orchestrator_run_id,step_key,ordinal,layer_no,tool_mode,request_sha256,invocation_id,downstream_run_id,receipt_sha256,status,attempt_count,next_retry_at_utc,started_at_utc,completed_at_utc FROM orchestrator_steps WHERE id=?", (record_id,)).fetchone()
        if row is None: raise OrchestrationRepositoryError("orchestrator_step_not_found")
        return self._step_from_row(row)
    def _step_by_key(self, conn, workflow_key: str, step_key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT s.id,s.orchestrator_run_id,s.step_key,s.ordinal,s.layer_no,s.tool_mode,s.request_sha256,s.invocation_id,s.downstream_run_id,s.receipt_sha256,s.status,s.attempt_count,s.next_retry_at_utc,s.started_at_utc,s.completed_at_utc FROM orchestrator_steps s JOIN orchestrator_runs r ON r.id=s.orchestrator_run_id WHERE r.workflow_key=? AND s.step_key=?", (workflow_key, step_key)).fetchone()
        if row is None:
            if missing_none: return None
            raise OrchestrationRepositoryError("orchestrator_step_not_found")
        return self._step_from_row(row)
    def _validated_step_by_key(self, conn, workflow_key: str, step_key: str, *, missing_none: bool = False):
        step = self._step_by_key(conn, workflow_key, step_key, missing_none=missing_none)
        if step is None: return None
        self._validated_run_by_key(conn, workflow_key)
        return step
    @staticmethod
    def _job_from_row(row): return SchedulerJobRecord(*row)
    def _job_by_key(self, conn, key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT id,job_key,workflow_kind,timezone,schedule_spec_json,is_enabled,misfire_policy,last_due_at_utc,next_due_at_utc,config_sha256,updated_at_utc FROM scheduler_jobs WHERE job_key=?", (key,)).fetchone()
        if row is None:
            if missing_none: return None
            raise OrchestrationRepositoryError("scheduler_job_not_found")
        return self._job_from_row(row)
    @staticmethod
    def _lease_from_row(row): return SchedulerLeaseRecord(*row)
    def _lease_by_key(self, conn, key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT id,lease_key,owner_instance_id,owner_pid,acquired_at_utc,heartbeat_at_utc,expires_at_utc FROM scheduler_leases WHERE lease_key=?", (key,)).fetchone()
        if row is None:
            if missing_none: return None
            raise OrchestrationRepositoryError("scheduler_lease_not_found")
        return self._lease_from_row(row)
    @staticmethod
    def _incident_from_row(row): return IncidentRecord(*row)
    def _incident_by_id(self, conn, record_id: int):
        row = conn.execute("SELECT id,incident_key,category,severity,state,related_workflow_run_id,related_step_id,first_seen_at_utc,last_seen_at_utc,occurrence_count,resolved_at_utc,error_code,error_summary,next_action FROM operational_incidents WHERE id=?", (record_id,)).fetchone()
        if row is None: raise OrchestrationRepositoryError("operational_incident_not_found")
        return self._incident_from_row(row)
    def _incident_by_key(self, conn, key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT id,incident_key,category,severity,state,related_workflow_run_id,related_step_id,first_seen_at_utc,last_seen_at_utc,occurrence_count,resolved_at_utc,error_code,error_summary,next_action FROM operational_incidents WHERE incident_key=?", (key,)).fetchone()
        if row is None:
            if missing_none: return None
            raise OrchestrationRepositoryError("operational_incident_not_found")
        return self._incident_from_row(row)
    @staticmethod
    def _alert_from_row(row): return AlertDeliveryRecord(*row)
    def _alert_by_id(self, conn, record_id: int):
        row = conn.execute("SELECT id,operational_incident_id,idempotency_key,status,provider_message_id,provider_thread_id,sent_at_utc,last_verified_at_utc,error_code,error_summary FROM operational_alert_deliveries WHERE id=?", (record_id,)).fetchone()
        if row is None: raise OrchestrationRepositoryError("operational_alert_not_found")
        return self._alert_from_row(row)
    def _alert_by_key(self, conn, key: str, *, missing_none: bool = False):
        row = conn.execute("SELECT id,operational_incident_id,idempotency_key,status,provider_message_id,provider_thread_id,sent_at_utc,last_verified_at_utc,error_code,error_summary FROM operational_alert_deliveries WHERE idempotency_key=?", (key,)).fetchone()
        if row is None:
            if missing_none: return None
...
def _utc_text(value: datetime) -> str:
    if not isinstance(value, datetime) or value.tzinfo is None or value.utcoffset() is None: raise OrchestrationRepositoryError("orchestrator_timestamp_not_utc")
...
def _parse_utc(value: str) -> datetime:
    ...
    if result.tzinfo is None or result.utcoffset() is None: raise OrchestrationRepositoryError("orchestrator_timestamp_invalid")
    return result.astimezone(UTC)
def _optional_utc(value: datetime | None) -> str | None: return None if value is None else _utc_text(value)
...
def _handoff_summary(
    job_key: str, due: str, next_due: str, config_sha256: str, generation: str,
    workflow_key: str, workflow_kind: str, subject_id: int | None,
    logical_local_date: str | None, trigger_kind: str, deadline_at_utc: str | None,
) -> str:
    _identifier(job_key); _hash(config_sha256); _hash(generation)
    _identifier(workflow_key); _identifier(workflow_kind); _identifier(trigger_kind)
    _parse_utc(due); _parse_utc(next_due)
    ...
    if logical_local_date is not None:
        try: date.fromisoformat(logical_local_date)
        except (TypeError, ValueError) as exc: raise OrchestrationRepositoryError("scheduler_handoff_invalid") from exc
    if deadline_at_utc is not None: _parse_utc(deadline_at_utc)
    value = {
    ...
    evidence = hashlib.sha256(json.dumps(value, sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")).hexdigest()
...
def _decode_handoff(raw: object) -> dict[str, Any] | None:
    ...
    if type(value) is not dict or value.get("schema_version") != "scheduler_handoff_v1":
        return None
    required = {"schema_version", "job_key", "due_at_utc", "next_due_at_utc", "config_sha256",
                "dispatch_generation", "workflow_key", "workflow_kind", "subject_id",
                "logical_local_date", "trigger_kind", "deadline_at_utc",
                "materialization_command_sha256", "materialization_evidence_sha256"}
    if set(value) != required:
        raise OrchestrationRepositoryError("scheduler_handoff_invalid")
    if not all(type(value[name]) is str for name in ("job_key", "due_at_utc", "next_due_at_utc", "config_sha256", "dispatch_generation", "workflow_key", "workflow_kind", "trigger_kind", "materialization_command_sha256", "materialization_evidence_sha256")):
        raise OrchestrationRepositoryError("scheduler_handoff_invalid")
    _identifier(value["job_key"]); _parse_utc(value["due_at_utc"]); _parse_utc(value["next_due_at_utc"])
    _identifier(value["workflow_key"]); _identifier(value["workflow_kind"]); _identifier(value["trigger_kind"])
    for name in ("config_sha256", "dispatch_generation", "materialization_command_sha256", "materialization_evidence_sha256"): _hash(value[name])
    if value["materialization_command_sha256"] != value["config_sha256"]:
        raise OrchestrationRepositoryError("scheduler_handoff_invalid")
    if value["subject_id"] is not None and (type(value["subject_id"]) is not int or value["subject_id"] <= 0):
        raise OrchestrationRepositoryError("scheduler_handoff_invalid")
    if value["logical_local_date"] is not None:
        try: date.fromisoformat(value["logical_local_date"])
        except (TypeError, ValueError) as exc: raise OrchestrationRepositoryError("scheduler_handoff_invalid") from exc
    if value["deadline_at_utc"] is not None: _parse_utc(value["deadline_at_utc"])
    evidence_source = dict(value); evidence = evidence_source.pop("materialization_evidence_sha256")
    expected = hashlib.sha256(json.dumps(evidence_source, sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")).hexdigest()
...
def _handoff_identity_matches(run: WorkflowRunRecord, handoff: Mapping[str, Any], workflow_kind: str, subject_id: int | None, logical_local_date: str | None, trigger_kind: str, deadline_at_utc: datetime | None) -> bool:
    return (run.workflow_kind == workflow_kind and run.subject_id == subject_id
            and run.logical_local_date == logical_local_date and run.trigger_kind == trigger_kind
            and run.deadline_at_utc == _optional_utc(deadline_at_utc)
            and handoff["workflow_key"] == run.workflow_key and handoff["workflow_kind"] == run.workflow_kind
            and handoff["subject_id"] == run.subject_id and handoff["logical_local_date"] == run.logical_local_date
            and handoff["trigger_kind"] == run.trigger_kind and handoff["deadline_at_utc"] == run.deadline_at_utc)


def _handoff_record(run: WorkflowRunRecord, value: Mapping[str, Any]) -> SchedulerHandoffRecord:
    if not _handoff_identity_matches(run, value, run.workflow_kind, run.subject_id, run.logical_local_date, run.trigger_kind, None if run.deadline_at_utc is None else _parse_utc(run.deadline_at_utc)):
        raise OrchestrationRepositoryError("scheduler_handoff_row_mismatch")
    return SchedulerHandoffRecord(
        run, value["job_key"], value["due_at_utc"], value["next_due_at_utc"],
        value["config_sha256"], value["dispatch_generation"],
        value["materialization_command_sha256"], value["materialization_evidence_sha256"],
    )
def _open_database_identity(path: Path) -> tuple[int, tuple[int, int, int, int]]:
    ...
    try:
        ...
        raise OrchestrationSchemaIncompatible("orchestrator_database_unavailable") from exc
    if (not stat.S_ISREG(before.st_mode) or stat.S_ISLNK(before.st_mode) or before.st_uid != os.getuid()
            or stat.S_IMODE(before.st_mode) != 0o600):
        raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe")
    try:
        parent_fd = os.open(path.parent, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW)
        try: fd = os.open(path.name, os.O_RDONLY | os.O_NOFOLLOW, dir_fd=parent_fd)
        finally: os.close(parent_fd)
    except OSError as exc:
        raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe") from exc
    opened = os.fstat(fd)
    identity = (opened.st_dev, opened.st_ino, opened.st_uid, stat.S_IMODE(opened.st_mode))
    if identity != (before.st_dev, before.st_ino, before.st_uid, stat.S_IMODE(before.st_mode)):
        os.close(fd); raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe")
    return fd, identity


def _assert_database_identity(path: Path, fd: int, expected: tuple[int, int, int, int]) -> None:
    opened = os.fstat(fd)
    ...
    for info in (opened, current):
        if (not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode) or info.st_uid != os.getuid()
            ...
            raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe")
    if (opened.st_dev, opened.st_ino, opened.st_uid, stat.S_IMODE(opened.st_mode)) != expected or (current.st_dev, current.st_ino, current.st_uid, stat.S_IMODE(current.st_mode)) != expected:
        raise OrchestrationSchemaIncompatible("orchestrator_database_path_unsafe")


def _identifier(value: str) -> None:
    if not _IDENTIFIER.fullmatch(value): raise OrchestrationRepositoryError("orchestrator_identifier_invalid")
def _hash(value: str) -> None:
    if not _HASH.fullmatch(value): raise OrchestrationRepositoryError("orchestrator_sha256_invalid")
def _safe_text(value: str | None) -> None:
    if value is not None: _code(value)
def _code(value: str) -> None:
    if not _CODE.fullmatch(value): raise OrchestrationRepositoryError("orchestrator_controlled_code_invalid")
...
def _validate_transition_payload(status: str, receipt: str | None, evidence: str | None, retry_value: datetime | None, at_value: datetime) -> None:
    ...
    if status == "running" and (receipt is not None or evidence is not None or retry is not None): raise OrchestrationRepositoryError("orchestrator_step_payload_invalid")
    if status == "succeeded" and (receipt is None or evidence not in {None, "receipt_received"} or retry is not None): raise OrchestrationRepositoryError("orchestrator_step_payload_invalid")
    if status in {"failed", "skipped"} and ((receipt is None and evidence not in _NO_RECEIPT_EVIDENCE) or (receipt is not None and evidence not in {None, "receipt_received"}) or retry is not None): raise OrchestrationRepositoryError("orchestrator_step_payload_invalid")
    if status == "deferred" and ((receipt is not None and (evidence not in {None, "receipt_received"} or retry is None)) or (receipt is None and retry is None and evidence not in _NO_RECEIPT_EVIDENCE) or (retry is not None and _parse_utc(retry) <= _parse_utc(at))): raise OrchestrationRepositoryError("orchestrator_step_payload_invalid")
...
def _has_workflow_definition(raw: str) -> bool:
    try:
        ...
        raise OrchestrationRepositoryError("orchestrator_result_summary_invalid") from exc
...
def _validated_step_summary(raw: str) -> dict[str, Any]:
    ...
    if set(parsed) != allowed_keys or parsed.get("schema_version") != "step_summary_v2" or not isinstance(parsed.get("steps"), dict):
        raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
    ...
    if "workflow_definition" in parsed:
        ...
        if (
            ...
            raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
        if "scheduler_handoff" in envelope:
            if not isinstance(envelope["scheduler_handoff"], dict) or _decode_handoff(
                json.dumps(envelope["scheduler_handoff"], sort_keys=True, separators=(",", ":"), allow_nan=False)
            ) is None:
                raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
    ...
    for key, value in parsed["steps"].items():
        _identifier(key)
        if not isinstance(value, dict) or set(value) != {"events"} or not isinstance(value["events"], list) or not 0 <= len(value["events"]) <= 32:
            raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
        prior_status = "pending"; prior_attempt = 0; prior_time: datetime | None = None
        for event in value["events"]:
            if not isinstance(event, dict) or set(event) != {"attempt", "status", "at_utc", "counts", "receipt_sha256", "evidence_code", "next_retry_at_utc"}:
                raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if type(event["attempt"]) is not int or not 0 <= event["attempt"] <= 16 or event["status"] not in _STEP_TRANSITIONS or not isinstance(event["counts"], dict): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            _parse_utc(event["at_utc"]); _counts(event["counts"])
            if event["receipt_sha256"] is not None: _hash(event["receipt_sha256"])
            if event["evidence_code"] is not None: _code(event["evidence_code"])
            if event["next_retry_at_utc"] is not None: _parse_utc(event["next_retry_at_utc"])
            at = _parse_utc(event["at_utc"])
            if prior_time is not None and (at <= prior_time if stateful_definition else at < prior_time): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if stateful_definition:
                prior_time = at
                continue
            if event["status"] not in _STEP_TRANSITIONS.get(prior_status, set()): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            expected_attempt = prior_attempt + (1 if event["status"] == "running" and prior_status in {"pending", "deferred"} else 0)
            if event["attempt"] != expected_attempt: raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] == "deferred" and event["next_retry_at_utc"] is not None and _parse_utc(event["next_retry_at_utc"]) <= at: raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] in {"succeeded", "failed", "skipped"} and event["next_retry_at_utc"] is not None: raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] == "succeeded" and event["receipt_sha256"] is None: raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            receipt, evidence, retry = event["receipt_sha256"], event["evidence_code"], event["next_retry_at_utc"]
            if event["status"] == "running" and (receipt is not None or evidence is not None or retry is not None): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] == "succeeded" and (evidence != "receipt_received" or retry is not None): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] in {"failed", "skipped"} and ((receipt is None and evidence not in _NO_RECEIPT_EVIDENCE) or (receipt is not None and evidence != "receipt_received")): raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            if event["status"] == "deferred" and ((receipt is not None and (evidence != "receipt_received" or retry is None)) or (receipt is None and retry is None and evidence not in _NO_RECEIPT_EVIDENCE)):
                raise OrchestrationRepositoryError("orchestrator_result_summary_invalid")
            prior_status, prior_attempt, prior_time = event["status"], event["attempt"], at
    return parsed
def _validate_step_row(row: WorkflowStepRecord, summary: dict[str, Any]) -> None:
    events = summary["steps"].get(row.step_key, {}).get("events", [])
    if not events:
        if row.status != "pending" or row.attempt_count != 0 or row.receipt_sha256 is not None or row.next_retry_at_utc is not None or row.started_at_utc is not None or row.completed_at_utc is not None:
            raise OrchestrationRepositoryError("orchestrator_step_summary_row_mismatch")
        return
    last = events[-1]
    if (row.status != last["status"] or row.attempt_count != last["attempt"] or row.receipt_sha256 != last["receipt_sha256"] or row.next_retry_at_utc != last["next_retry_at_utc"]):
        raise OrchestrationRepositoryError("orchestrator_step_summary_row_mismatch")
    first_running = next((event for event in events if event["status"] == "running"), None)
    if (first_running is None and row.started_at_utc is not None) or (first_running is not None and row.started_at_utc != first_running["at_utc"]): raise OrchestrationRepositoryError("orchestrator_step_summary_row_mismatch")
    if row.status in {"running", "deferred"} and row.completed_at_utc is not None: raise OrchestrationRepositoryError("orchestrator_step_summary_row_mismatch")
    if row.status in {"succeeded", "failed", "skipped"} and row.completed_at_utc != last["at_utc"]: raise OrchestrationRepositoryError("orchestrator_step_summary_row_mismatch")
def _append_step_event(step_key: str, raw: str, attempt: int, status: str, at_utc: str, counts: Mapping[str, int], receipt_sha256: str | None, evidence_code: str | None, retry: str | None) -> str:
    parsed = _validated_step_summary(raw)
    events = parsed["steps"].setdefault(step_key, {"events": []})["events"]
    if len(events) >= 32 or attempt > 16: raise OrchestrationRepositoryError("orchestrator_result_summary_limit")
    events.append({"attempt": attempt, "status": status, "at_utc": at_utc, "counts": dict(counts), "receipt_sha256": receipt_sha256, "evidence_code": evidence_code or ("receipt_received" if receipt_sha256 else None), "next_retry_at_utc": retry})
    return json.dumps(parsed, sort_keys=True, separators=(",", ":"))
def _same_step_event(raw: str, step_key: str, status: str, at_utc: str, receipt: str | None, counts: Mapping[str, int], evidence: str | None, retry: str | None) -> bool:
    parsed = _validated_step_summary(raw); events = parsed["steps"].get(step_key, {}).get("events", [])
    if not events: return receipt is None and not counts and evidence is None and retry is None
    event = events[-1]
    return event == {"attempt": event["attempt"], "status": status, "at_utc": at_utc, "counts": dict(counts), "receipt_sha256": receipt, "evidence_code": evidence or ("receipt_received" if receipt else None), "next_retry_at_utc": retry}
...
def _transition(table, current: str, requested: str, label: str) -> None:
    ...
    if requested not in table.get(current, set()): raise OrchestrationRepositoryError(f"{label}_transition_invalid")
