...
def test_controlled_acceptance_protocol_is_authorization_only_and_redacted() -> None:
    text = (ROOT / "docs/runbooks/analysis-controlled-acceptance.md").read_text(encoding="utf-8").lower()
    for required in (
    ...
    for prohibited in ("token=", "password=", "authorization:", "client_secret", "@gmail.com"):
        assert prohibited not in text


def test_shadow_protocol_requires_release_authorization_and_no_side_effects() -> None:
    text = (ROOT / "docs/runbooks/analysis-shadow-migration.md").read_text(encoding="utf-8").lower()
    for required in (
