...
def test_morning_audits_yesterday_and_snapshots_today_once() -> None:
    audit = garmin("audit")
    audit["coverage_state"] = None
    queue = Queue([garmin("incremental"), garmin("snapshot", coverage="partial"), audit, analysis("daily")])
    result = MorningWorkflowService(queue).execute(request())
    assert result.status == "succeeded"
    assert [(call.layer, call.mode) for call in queue.calls] == [("garmin", "incremental"), ("garmin", "snapshot"), ("garmin", "audit"), ("analysis", "daily")]
    assert queue.calls[0].through_local_date == "2026-07-23"
    assert queue.calls[1].snapshot_local_date == "2026-07-24"
    assert queue.calls[2].health_from_local_date == "2026-07-23"
    assert queue.calls[2].through_local_date == "2026-07-23"
...
def test_collection_coverage_remains_the_quality_authority() -> None:
    queue = Queue([
        ...
        garmin("snapshot", coverage="partial"),
    ...
    assert [item.mode for item in queue.calls] == ["incremental", "snapshot", "audit", "repair", "audit"]


def test_blocked_quality_is_repaired_once_then_does_not_call_daily_when_still_blocked() -> None:
    queue = Queue([garmin("incremental"), garmin("snapshot", coverage="partial"), garmin("audit", coverage="partial", gaps=1), garmin("repair"), garmin("audit", coverage="partial", gaps=1)])
    result = MorningWorkflowService(queue).execute(request())
    assert result.status == "deferred" and result.next_action == "repair_data"
    assert [item.mode for item in queue.calls] == ["incremental", "snapshot", "audit", "repair", "audit"]
...
def test_failed_current_snapshot_stops_before_quality_and_daily() -> None:
    queue = Queue([garmin("incremental"), garmin("snapshot", "auth_required", coverage="partial")])
    result = MorningWorkflowService(queue).execute(request())
...
def test_completed_reentry_returns_same_result_without_second_artifact() -> None:
    queue = Queue([garmin("incremental"), garmin("snapshot", coverage="partial"), garmin("audit"), analysis("daily")])
