...
from trainlab.analysis.shadow import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"artifact_ids": ("artifact-1",), "artifact_count": 2}, "shadow_artifact_count_invalid"),
    ],
)
def test_shadow_side_effect_and_manifest_invariants(changes: dict[str, object], code: str) -> None:
    shadow = evidence("shadow", **changes)
    if code == "shadow_email_forbidden":
        result = compare_shadow_evidence(evidence("legacy"), shadow, release_authorization_declared=True)
...
def test_shadow_module_has_no_io_or_runner_imports() -> None:
    module = ast.parse(inspect.getsource(__import__("trainlab.analysis.shadow", fromlist=["*"])))
    imported = {
        alias.name.split(".")[0]
    ...
    assert not ({"sqlite3", "pathlib", "subprocess", "socket", "requests", "urllib"} & imported)
    source = Path(__file__).parents[1] / "src/trainlab/analysis/shadow.py"
