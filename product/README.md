...
- 日报每天香港时间 09:00 发送：报告日 D 包含 D 白天、D→D+1 主睡眠和
  ...
  确认且时长自洽的主睡眠会在昨日回顾后显示深睡、浅睡、REM 与清醒阶段图表；
  邮件同时显示可验证的恢复指标卡、最近7日活动时长图，并在昨日存在运动时显示
  运动概览。运动概览以活动计时时长为训练量，不把暂停/休息计入；同时显示距离、
  平均配速、平均/最高心率、天气，并允许 AI 从已验证 FIT 中选择最多两项有解释
  价值的步频、功率或爬升指标。缺失指标会明确标注，不会按正常值处理。
...
- 心率区间每周随周分析自动追加一版候选；如果系统从未生成候选，日报路线会立即
  补算一次。确认邮件以 HRR 为主，并同列历史阈值代理和 Tanaka 低置信度对比；只有
...
TrainLab 每周自动计算一份追加式候选：HRR 为主方法，历史阈值心率代理仅作一致性
校验，Tanaka 年龄公式仅作低置信度备用。计算不会自动改写 Garmin 数据或自动生效，
结果会通过邮件请求二次确认；确认后新增 revision，AI 只使用最新确认 revision。
如果数据库尚无候选，下一次日报会先完成一次初始化计算。周计划还会读取经过压缩、
带来源 revision 的最近 28/90 天跑步次数、跑量、时长、最长跑和全部活动时长，避免
把每周计划当成彼此无关的单次安排；原始 FIT 和逐秒数据不会因此进入模型上下文。

Connect/FIT 时长采用三个独立语义：`elapsedDuration` 是包含暂停的总经过时间，
`duration`/FIT `total_timer_time` 是活动计时时间，`movingDuration` 是移动时间。
邮件训练量默认展示活动计时时间，对账不会再把移动时间错误地当成 FIT timer。
...
在 macOS 单独运行 `plutil`/LaunchAgent 静态验收；配置位于仓库根目录的
