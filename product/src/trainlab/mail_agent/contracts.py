...
_MODES = frozenset({"run", "poll", "process", "deliver_response", "reconcile", "status"})
_STATUSES = frozenset({"succeeded", "unchanged", "partial", "deferred", "lock_busy", "auth_required", "rejected", "failed"})
_EXIT_CODES = {
    "succeeded": 0,
    "unchanged": 0,
    "partial": 10,
...
class MailRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.subject_id, int) or isinstance(self.subject_id, bool) or self.subject_id <= 0:
    ...
    def _validate_mode_fields(self) -> None:
        allowed: dict[str, set[str]] = {
            ...
            "poll": {"max_threads"},
            "process": {"mail_message_ids", "dependency_analysis_artifact_ids", "regeneration_reason_code"},
        ...
        if self.mode == "deliver_response" and len(self.mail_response_artifact_ids) != 1:
            raise ValueError("deliver_response_requires_exactly_one_mail_response_artifact_id")
...
class BoundaryOnlyMailService:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        receipt_run_key = request.run_key if request.mode == "status" and request.run_key else request.stable_run_key
        completed = utc_now()
        return MailReceipt(
            ...
            mode=request.mode,
            status="failed",
            errors=({"stage": "boundary", "code": "mail_agent_not_implemented", "summary": "M4-01 boundary only"},),
...
class MailTool:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        ...
        expected_run_key = request.run_key if request.mode == "status" and request.run_key else request.stable_run_key
        if receipt.invocation_id != request.invocation_id or receipt.mode != request.mode or receipt.run_key != expected_run_key:
