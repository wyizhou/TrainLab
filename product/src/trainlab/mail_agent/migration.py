...
_STREAMS: dict[str, tuple[str, tuple[str, ...]]] = {
    ...
    "messages": ("provider_message_id", ("provider_thread_id", "processing_state", "source_revision_id")),
    "responses": ("idempotency_key", ("provider_thread_id", "response_revision_no", "content_sha256", "is_current")),
    "deliveries": ("idempotency_key", ("status", "provider_message_id", "provider_thread_id")),
...
def _index(
    ...
    for position, row in enumerate(rows):
        if not isinstance(row, Mapping) or not isinstance(row.get(key), str) or not row[key]:
            findings.append(MigrationFinding("snapshot_identity_invalid", f"{stream}[{position}]")); continue
        identity = row[key]
        if identity in result:
            findings.append(MigrationFinding("snapshot_identity_duplicate", f"{stream}:{identity}")); continue
        result[identity] = row
...
def compare_shadow_snapshots(
    ...
    for stream, (key, fields) in _STREAMS.items():
        ...
        findings.extend(old_errors); findings.extend(new_errors)
        ...
        for identity in sorted(set(new_index) - set(old_index)):
            findings.append(MigrationFinding("shadow_unexpected", f"{stream}:{identity}"))
        for identity in sorted(set(old_index) & set(new_index)):
            if any(old_index[identity].get(name) != new_index[identity].get(name) for name in fields):
                findings.append(MigrationFinding("shadow_state_mismatch", f"{stream}:{identity}"))
...
def evaluate_rollback(*, backup_id: str | None, new_writer_enabled: bool) -> CutoverDecision:
