    ...
    if not rows:
        ...
        for row in rows[:100]:
            ...
            if row.get("effective_from_utc") or row.get("expires_at_utc"):
                lines.append(
                    "  生效: " + str(row.get("effective_from_utc") or "立即")
                    + "；失效: " + str(row.get("expires_at_utc") or "未设置")
                )
    text = "\n".join(lines)
...
__all__ = ["FactBrowserCommand", "parse_fact_browser_command", "render_fact_browser_text"]
