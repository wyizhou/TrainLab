...
from .contracts import MailCounts, MailReceipt, MailRequest
from .eligibility import CanonicalMessage
from .gmail_adapter import GmailAdapterError, GmailIdentity, GmailMCPAdapter, GmailThreadEvidence
from .locks import MailLockBusyError, MailWriteLock
from .repository import MailRepository, MailRepositoryError, _PUBLIC_SUMMARIES, utc_now, validate_poll_receipt
...
class _SafeText(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        if tag.lower() in {"script", "style", "noscript", "template", "iframe", "object"}:
            self._hidden += 1
        elif self._hidden == 0 and tag.lower() in {"br", "p", "div", "li", "tr"}:
            self.parts.append(" ")

    def handle_endtag(self, tag: str) -> None:
        if tag.lower() in {"script", "style", "noscript", "template", "iframe", "object"} and self._hidden:
            self._hidden -= 1
...
class MailPollService:
    ...
    def __init__(self, repository: MailRepository, adapter: GmailMCPAdapter, config: MailPollConfig, *, clock: Callable[[], str] = utc_now) -> None:
    ...
    def execute(self, request: MailRequest) -> MailReceipt:
        ...
        try:
            ...
            return self._receipt(request, "lock_busy", MailCounts(), (), (), ({"stage": "lock", "code": "lock_busy", "summary": "mail writer is active"},), started)
        try:
            ...
            if run.status != "started":
                ...
                return self._receipt(
                    request, "failed", MailCounts(failed=1), (), (),
                    ({"stage": "replay", "code": "terminal_receipt_invalid", "summary": "terminal run requires operator review"},),
                    started, run.id,
                )
            try:
                verified_identity = self.adapter.prepare(self.repository.connection, request.subject_id)
            except GmailAdapterError as error:
                code = self._safe_adapter_code(error.code)
                return self._persist_terminal(
                    request, run.id, "auth_required" if code == "auth_required" else "failed", MailCounts(failed=1), (), (),
                    ({"stage": "prepare", "code": code, "summary": "gmail adapter unavailable"},), started,
                )
            identity_id = self.adapter.verified_identity_id
            if identity_id is None:
                return self._persist_terminal(
                    request, run.id, "failed", MailCounts(failed=1), (), (),
                    ({"stage": "prepare", "code": "verified_identity_required", "summary": "gmail identity unavailable"},), started,
                )
            now = _as_utc(self._clock())
            budget = _Budget(request.max_threads)
            tracked_at_start = {str(row[0]) for row in self.repository.connection.execute("SELECT provider_thread_id FROM mail_threads WHERE subject_id=? AND is_current=1", (request.subject_id,)).fetchall()}
            ...
            def poll_label() -> tuple[bool, dict[str, str]]:
                return self._poll_label(
                    run.id, request, identity_id, verified_identity,
                    label_progress, budget, read_results, counts, errors,
                )

            def poll_tracked() -> tuple[bool, dict[str, str]]:
                return self._poll_tracked(
                    run.id, request, identity_id, verified_identity,
                    tracked_progress, budget, read_results, tracked_at_start,
                    counts, errors,
                )
            ...
            tracked_first = (
                tracked_progress.is_continuation
                and (
            ...
            if status == "succeeded" and counts["archived"] == 0 and counts["discovered"] == 0:
                status = "unchanged"
            ...
            receipt = self._persist_terminal(request, run.id, status, MailCounts(**counts), tuple(states), (), tuple(errors), started)
            return receipt
            ...
            if (
                ...
                existing = self.repository.connection.execute(
                    "SELECT id,status FROM mail_agent_runs "
                    "WHERE invocation_id=?",
                        ...
                        (
                            {
                                ...
                                "summary": (
                                    "terminal run requires operator review"
                                ),
                            },
                        ),
                        started,
            ...
            code = self._safe_adapter_code(error.code) if isinstance(error, GmailAdapterError) else "poll_failed"
            if run_id is not None:
                try:
                    receipt = self._persist_terminal(
                        request, run_id, "failed", MailCounts(failed=1), (), (),
                        ({"stage": "poll", "code": code, "summary": "poll did not complete"},), started,
                    )
                    return receipt
                except Exception:
                    pass
            return self._receipt(
                request, "failed", MailCounts(failed=1), (), (),
                ({"stage": "poll", "code": "terminal_persistence_failed", "summary": "poll requires operator review"},),
                started, run_id,
            )
        finally:
    ...
    def _run_status_for_public(status: str) -> str:
        mapping = {
            "succeeded": "succeeded", "unchanged": "succeeded", "partial": "partial", "deferred": "deferred",
            "failed": "failed", "auth_required": "failed", "rejected": "rejected",
        }
        try:
    ...
    def _persist_terminal(
        self, request: MailRequest, run_id: int, status: str, counts: MailCounts,
        poll_state: tuple[dict[str, str], ...], processed: tuple[str, ...], errors: tuple[dict[str, str], ...], started: str,
        next_retry_at_utc: str | None = None,
    ) -> MailReceipt:
        receipt = self._receipt(request, status, counts, poll_state, processed, errors, started, run_id, next_retry_at_utc)
        self.repository.finish_poll_with_receipt(run_id, self._run_status_for_public(status), receipt.to_dict())
        return receipt
    ...
    def _terminal_replay(self, request: MailRequest, run: object) -> MailReceipt | None:
        ...
        try:
            ...
            for name in ("processed_message_ids", "mail_response_artifact_ids", "mail_delivery_ids", "pending_dependencies", "poll_state", "warnings", "errors"):
    ...
    def _window(self, subject_id: int, identity_id: int, stream: str, now: datetime) -> tuple[datetime, datetime]:
    ...
    def _advance_cursor(self, run_id: int, subject_id: int, identity_id: int, stream: str, observed: datetime, start: datetime) -> None:
    ...
    def _poll_label(
        self, run_id: int, request: MailRequest, identity_id: int,
        identity: GmailIdentity, progress: _StreamProgress, budget: _Budget,
        read_results: dict[str, bool], counts: dict[str, int],
        errors: list[dict[str, str]],
    ) -> tuple[bool, dict[str, str]]:
        start, end = progress.start, progress.end
        thread_ids = self._discover_label_window(
            start, end, counts, errors, depth=0,
        )
        ...
        complete = thread_ids is not None and self._process_thread_ids(
            run_id, request, identity, thread_ids, progress, budget,
            read_results, counts, errors, error_summary="label window incomplete",
        )
        if complete:
            self._advance_cursor(run_id, request.subject_id, identity_id, "trainlab_label", end, start)
        return complete, {"stream": "trainlab_label", "status": "succeeded" if complete else "partial", "window_start_utc": start.isoformat().replace("+00:00", "Z"), "window_end_utc": end.isoformat().replace("+00:00", "Z")}

    def _discover_label_window(
        self, start: datetime, end: datetime, counts: dict[str, int],
        errors: list[dict[str, str]], *, depth: int,
    ) -> set[str] | None:
        if depth >= _MAX_WINDOW_SPLIT_DEPTH:
            errors.append({"stage": "discover", "code": "provider_page_limit_unsplittable", "summary": "label window incomplete"})
            counts["deferred"] += 1
            return None
        if end - start > _MAX_PROVIDER_WINDOW:
            midpoint = start + (end - start) / 2
            left = self._discover_label_window(
                start, midpoint, counts, errors, depth=depth + 1,
            )
            if left is None:
                return None
            right = self._discover_label_window(
                midpoint, end, counts, errors, depth=depth + 1,
            )
            return None if right is None else left | right
        try:
            matches = self.adapter.search_trainlab_window(start_date=start, end_date=end, max_results=self.config.provider_page_limit)
        except GmailAdapterError as error:
            errors.append({"stage": "discover", "code": self._safe_adapter_code(error.code), "summary": "label discovery failed"})
            counts["failed"] += 1
            return None
        if not isinstance(matches, (list, tuple)):
            errors.append({"stage": "discover", "code": "provider_thread_id_invalid", "summary": "label result rejected"})
            counts["failed"] += 1
            return None
    ...
    def _process_thread_ids(
        self, run_id: int, request: MailRequest, identity: GmailIdentity,
        thread_ids: set[str], progress: _StreamProgress, budget: _Budget,
        read_results: dict[str, bool], counts: dict[str, int],
        errors: list[dict[str, str]], *, error_summary: str,
    ) -> bool:
        unseen = sorted(
        ...
        for thread_id in unseen + retries:
            if thread_id in read_results:
                ...
                continue
            if not budget.take():
                errors.append({"stage": "discover", "code": "max_threads_reached", "summary": error_summary})
                counts["deferred"] += 1
                return False
            result = self._read_archive(run_id, request, identity, thread_id, counts, errors)
            read_results[thread_id] = result
            if not result:
                return False
        return True

    def _record_candidates(self, run_id: int, stream: str, thread_ids: set[str]) -> None:
    ...
    def _provider_thread_id(match: object) -> str | None:
        if not isinstance(match, dict):
            return None
        candidates = [
            match[name] for name in ("thread_id", "threadId")
    ...
    def _poll_tracked(
        self, run_id: int, request: MailRequest, identity_id: int,
        identity: GmailIdentity, progress: _StreamProgress, budget: _Budget,
        read_results: dict[str, bool], tracked_at_start: set[str],
        counts: dict[str, int], errors: list[dict[str, str]],
    ) -> tuple[bool, dict[str, str]]:
        start, end = progress.start, progress.end
        ...
        if complete:
            ...
            complete = self._process_thread_ids(
                run_id, request, identity, thread_ids, progress, budget,
                read_results, counts, errors, error_summary="tracked stream incomplete",
            )
        if complete:
            self._advance_cursor(run_id, request.subject_id, identity_id, "tracked_threads", end, start)
        return complete, {"stream": "tracked_threads", "status": "succeeded" if complete else "partial", "window_start_utc": start.isoformat().replace("+00:00", "Z"), "window_end_utc": end.isoformat().replace("+00:00", "Z")}

    def _read_archive(self, run_id: int, request: MailRequest, identity: GmailIdentity, thread_id: str, counts: dict[str, int], errors: list[dict[str, str]]) -> bool:
        try:
            self.repository.record_item(run_id, logical_item_kind="thread", logical_item_id=thread_id, stage="discover", status="succeeded")
            ...
            outcome = self._archive_normalize(run_id, request.subject_id, identity, evidence)
            self.repository.record_item(run_id, logical_item_kind="thread", logical_item_id=thread_id, stage="normalize", status="unchanged" if outcome["changed"] == 0 and not outcome["thread_revised"] else "succeeded")
        except (GmailAdapterError, MailPollError, sqlite3.Error, RuntimeError) as error:
            code = self._safe_adapter_code(error.code) if isinstance(error, GmailAdapterError) else "archive_or_normalize_failed"
            errors.append({"stage": "archive", "code": code, "summary": "thread not completed"})
            try:
                self.repository.record_item(run_id, logical_item_kind="thread", logical_item_id=thread_id, stage="archive", status="failed", error_code=code, error_summary="thread not completed")
            except Exception:
                pass
            try:
                self.repository.record_item(run_id, logical_item_kind="thread", logical_item_id=thread_id, stage="normalize", status="failed", error_code=code, error_summary="thread not completed")
            except Exception:
                pass
            counts["failed"] += 1
            return False
        if outcome["changed"] == 0 and not outcome["thread_revised"]:
            ...
            counts["archived"] += 1 if outcome["thread_revised"] or outcome["changed"] else 0
        counts["queued"] += outcome.get("queued", 0)
        counts["ignored"] += outcome.get("ignored", 0)
        return True

    def _archive_normalize(self, run_id: int, subject_id: int, identity: GmailIdentity, evidence: GmailThreadEvidence) -> dict[str, int]:
        ...
        observed = max(_as_utc(message.received_at_utc or "") for message in evidence.thread.messages)
        ...
        for item in raw["messages"]:
            ...
            item_payload = json.dumps(item, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
            item_digest = hashlib.sha256(item_payload).hexdigest()
            item_relative, _ = self._archive_raw(item_payload, item_digest, _as_utc(item.get("internal_date_utc") or item.get("received_at_utc")))
        ...
        self.repository.record_item(run_id, logical_item_kind="thread", logical_item_id=evidence.thread.provider_thread_id, stage="archive", status="succeeded")
        try:
            with self.repository._write_transaction():
                conn = self.repository.connection
                current = conn.execute("SELECT id FROM source_revisions WHERE provider='gmail' AND resource_kind='thread_json' AND provider_object_id=? AND payload_hash=? AND is_current=1", (evidence.thread.provider_thread_id, digest)).fetchone()
                ...
                raw_row = conn.execute("SELECT id FROM raw_objects WHERE sha256=?", (digest,)).fetchone()
                if raw_row is None:
                    conn.execute("INSERT INTO raw_objects(sha256,relative_path,media_type,size_bytes,provider,resource_kind,original_name,fetched_at_utc) VALUES(?,?,?,?,?,?,?,?)", (digest, relative, "application/json", len(payload), "gmail", "thread_json", f"{evidence.thread.provider_thread_id}.json", self._clock()))
                    raw_id = conn.execute("SELECT id FROM raw_objects WHERE sha256=?", (digest,)).fetchone()[0]
                else:
                    raw_id = raw_row[0]
                if current is None:
                    revision = conn.execute("SELECT COALESCE(MAX(revision_no),0)+1 FROM source_revisions WHERE provider='gmail' AND resource_kind='thread_json' AND provider_object_id=?", (evidence.thread.provider_thread_id,)).fetchone()[0]
                    conn.execute("UPDATE source_revisions SET is_current=0 WHERE provider='gmail' AND resource_kind='thread_json' AND provider_object_id=? AND is_current=1", (evidence.thread.provider_thread_id,))
                    conn.execute("INSERT INTO source_revisions(provider,resource_kind,provider_object_id,revision_no,raw_object_id,payload_hash,parser_name,parser_version,is_current,parsed_at_utc) VALUES('gmail','thread_json',?,?,?,?,'mail_poll','1',1,?)", (evidence.thread.provider_thread_id, revision, raw_id, digest, self._clock()))
                    revision_id = conn.execute("SELECT id FROM source_revisions WHERE provider='gmail' AND resource_kind='thread_json' AND provider_object_id=? AND revision_no=?", (evidence.thread.provider_thread_id, revision)).fetchone()[0]
                else:
                    revision_id = current[0]
                message_raw_ids: dict[str, int] = {}
                for message_id, (item_digest, item_relative, item_size) in message_raw.items():
                    row = conn.execute("SELECT id FROM raw_objects WHERE sha256=?", (item_digest,)).fetchone()
                    if row is None:
                        conn.execute("INSERT INTO raw_objects(sha256,relative_path,media_type,size_bytes,provider,resource_kind,original_name,fetched_at_utc) VALUES(?,?,?,?,?,?,?,?)", (item_digest, item_relative, "application/json", item_size, "gmail", "message_json", f"{message_id}.json", self._clock()))
                        row = conn.execute("SELECT id FROM raw_objects WHERE sha256=?", (item_digest,)).fetchone()
                    message_raw_ids[message_id] = row[0]
                outcome = self._project(conn, run_id, subject_id, identity, evidence, revision_id, message_raw_ids)
    ...
    def _validate_evidence(self, evidence: GmailThreadEvidence, identity: GmailIdentity) -> None:
        if (
        ...
        if not isinstance(raw_messages, list) or len(raw_messages) != len(evidence.thread.messages):
            raise MailPollError("gmail_raw_typed_message_mismatch")
        typed_ids = {message.provider_message_id for message in evidence.thread.messages}
        ...
        for message in evidence.thread.messages:
            ...
            raw = next(item for item in raw_messages if (item.get("message_id") or item.get("id")) == message.provider_message_id)
            headers = raw["headers"]
            raw_to = tuple(str(v) for v in (raw.get("to") if isinstance(raw.get("to"), list) else [raw.get("to")] if raw.get("to") else []))
            if (_parse_utc(raw.get("internal_date_utc") or raw.get("received_at_utc")) != _parse_utc(message.received_at_utc)
                    or raw.get("from") != message.sender or raw_to != message.recipients or raw.get("subject") != message.subject
                    or tuple(str(v) for v in raw.get("label_ids", [])) != message.label_ids
                    or headers.get("message_id") != message.message_id_header or headers.get("in_reply_to") != message.in_reply_to
                    or tuple(str(v) for v in headers.get("references", [])) != message.references
                    or headers.get("x_trainlab_run_id") != message.trainlab_run_id
                    or bool(raw.get("has_html")) != message.has_html
                    or str(raw.get("plain_text") or raw.get("body_text") or "") != message.body_text
    ...
    def _archive_raw(self, payload: bytes, digest: str, observed: datetime) -> tuple[str, bool]:
        root_fd = self._open_raw_root()
        final_fd = root_fd
        try:
            for component in ("gmail", "json", f"{observed.year:04d}", f"{observed.month:02d}"):
            ...
            try:
                ...
                return f"raw/gmail/json/{observed.year:04d}/{observed.month:02d}/{final_name}", False
            except FileNotFoundError:
                pass
            temp_name = f".{digest}.{secrets.token_hex(12)}.tmp"; temp_created = False; winner = False
            try:
                descriptor = os.open(temp_name, os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o600, dir_fd=final_fd)
            ...
            try:
                ...
                while offset < len(payload):
                    try: written = os.write(descriptor, payload[offset:])
                    except OSError as exc: raise MailPollError("gmail_raw_write_failed") from exc
                    if written <= 0: raise MailPollError("gmail_raw_write_failed")
                    offset += written
                os.fsync(descriptor)
            finally:
                os.close(descriptor)
            ...
            try:
                os.link(temp_name, final_name, src_dir_fd=final_fd, dst_dir_fd=final_fd, follow_symlinks=False)
            ...
            if 'temp_created' in locals() and temp_created:
                try:
                    os.unlink(temp_name, dir_fd=final_fd); os.fsync(final_fd)
                except OSError: pass
            raise
        finally:
            if final_fd != root_fd: os.close(final_fd)
            os.close(root_fd)
        return f"raw/gmail/json/{observed.year:04d}/{observed.month:02d}/{digest}.json", winner

    def _open_raw_root(self) -> int:
        data_root = self.config.raw_root.parent
        if self.config.raw_root.name != "raw" or self.config.raw_root.parent != data_root:
            raise MailPollError("gmail_raw_path_invalid")
        try:
            ...
            if not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid() or info.st_mode & 0o077 or (info.st_dev,info.st_ino)!=(before.st_dev,before.st_ino) or (info.st_dev,info.st_ino)!=(after.st_dev,after.st_ino): raise MailPollError("gmail_raw_path_invalid")
            raw_name = self.config.raw_root.name
            raw_before = os.stat(raw_name, dir_fd=data_fd, follow_symlinks=False)
            fd = os.open(raw_name, os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0), dir_fd=data_fd)
            raw, raw_named = os.fstat(fd), os.stat(raw_name, dir_fd=data_fd, follow_symlinks=False)
            if not stat.S_ISDIR(raw.st_mode) or raw.st_uid != os.getuid() or raw.st_mode & 0o077 or (raw.st_dev,raw.st_ino)!=(raw_before.st_dev,raw_before.st_ino) or (raw.st_dev,raw.st_ino)!=(raw_named.st_dev,raw_named.st_ino): raise MailPollError("gmail_raw_path_invalid")
            ...
            if 'data_fd' in locals(): os.close(data_fd)
    ...
    def _mkdir_openat(parent_fd: int, name: str) -> int:
        ...
        except OSError as exc: raise MailPollError("gmail_raw_path_invalid") from exc
        fd: int | None = None
        try:
            fd = os.open(name, os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | getattr(os, "O_NOFOLLOW", 0), dir_fd=parent_fd)
            info, named = os.fstat(fd), os.stat(name, dir_fd=parent_fd, follow_symlinks=False)
            if not stat.S_ISDIR(info.st_mode) or info.st_uid != os.getuid() or info.st_mode & 0o077 or (info.st_dev, info.st_ino) != (before.st_dev, before.st_ino) or (info.st_dev, info.st_ino) != (named.st_dev, named.st_ino): raise MailPollError("gmail_raw_path_invalid")
            return fd
        except Exception as exc:
            if fd is not None: os.close(fd)
            if isinstance(exc, MailPollError): raise
            raise MailPollError("gmail_raw_path_invalid") from exc

    @staticmethod
    def _verify_raw_fd(directory_fd: int, name: str, payload: bytes, digest: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if (info.st_dev, info.st_ino) != (named.st_dev, named.st_ino): raise MailPollError("gmail_raw_path_invalid")
                if (not stat.S_ISREG(info.st_mode) or info.st_uid != os.getuid() or info.st_mode & 0o077 or info.st_size != len(payload)):
                    raise MailPollError("gmail_raw_path_invalid")
                ...
                if (info.st_dev, info.st_ino) != (named_after.st_dev, named_after.st_ino): raise MailPollError("gmail_raw_path_invalid")
            finally:
                os.close(descriptor)
    ...
    def _project(self, conn: sqlite3.Connection, run_id: int, subject_id: int, identity: GmailIdentity, evidence: GmailThreadEvidence, revision_id: int, message_raw_ids: dict[str, int]) -> dict[str, int]:
        messages = tuple(sorted(
            evidence.thread.messages,
            key=lambda item: (_as_utc(item.received_at_utc or ""), item.provider_message_id),
        ))
        ...
        raw_thread_labels = evidence.raw_payload.get("label_ids", evidence.raw_payload.get("labelIds", []))
        if not isinstance(raw_thread_labels, list) or any(not isinstance(label, str) for label in raw_thread_labels):
            raise MailPollError("gmail_raw_typed_message_mismatch")
        thread_label_present = "TrainLab" in raw_thread_labels
        prior_thread = conn.execute("SELECT id FROM mail_threads WHERE subject_id=? AND provider_thread_id=? AND is_current=1", (subject_id, evidence.thread.provider_thread_id)).fetchone()
        thread_was_tracked = prior_thread is not None
        conn.execute("INSERT INTO mail_threads(subject_id,provider_thread_id,normalized_subject,first_message_at_utc,last_message_at_utc,message_count,trainlab_label_state,is_current) VALUES(?,?,?,?,?,?,?,1) ON CONFLICT(subject_id,provider_thread_id) DO UPDATE SET normalized_subject=excluded.normalized_subject,first_message_at_utc=excluded.first_message_at_utc,last_message_at_utc=excluded.last_message_at_utc,message_count=excluded.message_count,trainlab_label_state=excluded.trainlab_label_state,is_current=1", (subject_id, evidence.thread.provider_thread_id, subject, first, last, len(messages), "present" if ("TrainLab" in labels or thread_label_present) else "absent"))
        thread_id = conn.execute("SELECT id FROM mail_threads WHERE subject_id=? AND provider_thread_id=?", (subject_id, evidence.thread.provider_thread_id)).fetchone()[0]
        raw_messages = evidence.raw_payload.get("messages") if isinstance(evidence.raw_payload.get("messages"), list) else []
        raw_by_id = {str(item.get("message_id") or item.get("id")): item for item in raw_messages if isinstance(item, dict)}
        if len(raw_by_id) != len(raw_messages) or set(raw_by_id) != {item.provider_message_id for item in messages}:
            raise MailPollError("gmail_raw_typed_message_mismatch")
        ...
        thread_label_candidate = messages[-1].provider_message_id if thread_label_present else None
        ...
        for message in messages:
            ...
            message_payload = json.dumps(raw_message, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
            message_hash = hashlib.sha256(message_payload).hexdigest()
            current_revision = conn.execute("SELECT id FROM source_revisions WHERE provider='gmail' AND resource_kind='message_json' AND provider_object_id=? AND payload_hash=? AND is_current=1", (message.provider_message_id, message_hash)).fetchone()
            message_revision_id: int | None = None
            if current_revision is not None:
                message_revision_id = current_revision[0]
                unchanged += 1
            else:
                number = conn.execute("SELECT COALESCE(MAX(revision_no),0)+1 FROM source_revisions WHERE provider='gmail' AND resource_kind='message_json' AND provider_object_id=?", (message.provider_message_id,)).fetchone()[0]
                conn.execute("UPDATE source_revisions SET is_current=0 WHERE provider='gmail' AND resource_kind='message_json' AND provider_object_id=? AND is_current=1", (message.provider_message_id,))
                conn.execute("INSERT INTO source_revisions(provider,resource_kind,provider_object_id,revision_no,raw_object_id,payload_hash,parser_name,parser_version,is_current,parsed_at_utc) VALUES('gmail','message_json',?,?,?,?,'mail_poll','1',1,?)", (message.provider_message_id, number, message_raw_ids[message.provider_message_id], message_hash, self._clock()))
                message_revision_id = conn.execute("SELECT id FROM source_revisions WHERE provider='gmail' AND resource_kind='message_json' AND provider_object_id=? AND revision_no=?", (message.provider_message_id, number)).fetchone()[0]
                changed += 1
            ...
            if existing is None:
                conn.execute("INSERT INTO mail_messages(mail_thread_id,provider_message_id,direction,actor_role,received_at_utc,subject,body_text,body_sha256,in_reply_to_provider_message_id,labels_json,source_revision_id,processing_state) VALUES(?,?, 'unknown','unknown',?,?,?,?,?,?,?,'normalized')", (thread_id, message.provider_message_id, timestamp, message.subject, body, body_hash, message.in_reply_to, json.dumps(list(message.label_ids), sort_keys=True), message_revision_id))
                message_id = conn.execute("SELECT id FROM mail_messages WHERE provider_message_id=?", (message.provider_message_id,)).fetchone()[0]
            elif current_revision is None:
                message_id = existing["id"]
                conn.execute("UPDATE mail_messages SET mail_thread_id=?,received_at_utc=?,subject=?,body_text=?,body_sha256=?,in_reply_to_provider_message_id=?,labels_json=?,source_revision_id=?,processing_state='normalized' WHERE id=?", (thread_id, timestamp, message.subject, body, body_hash, message.in_reply_to, json.dumps(list(message.label_ids), sort_keys=True), message_revision_id, message_id))
                conn.execute("DELETE FROM mail_attachments WHERE mail_message_id=?", (message_id,))
            else:
                message_id = existing["id"]
            if existing is None or current_revision is None:
                for attachment in message.attachments:
                    size = attachment.get("size_bytes") if "size_bytes" in attachment else attachment.get("size")
                    if (
                        size is not None
                        and (not isinstance(size, int) or isinstance(size, bool) or size < 0)
                    ):
                        raise MailPollError("gmail_attachment_metadata_invalid")
                    conn.execute("INSERT INTO mail_attachments(mail_message_id,provider_attachment_id,filename,media_type,size_bytes,raw_object_id,content_disposition) VALUES(?,?,?,?,?,?,?)", (message_id, attachment.get("id") or attachment.get("attachment_id"), attachment.get("filename"), attachment.get("media_type") or attachment.get("mime_type"), size, None, attachment.get("content_disposition")))
            current_row = conn.execute("SELECT processing_state FROM mail_messages WHERE id=?", (message_id,)).fetchone()
            assert current_row is not None
            if current_row["processing_state"] == "normalized":
                raw_headers = raw_message.get("headers", {})
                auto = raw_headers.get("auto_submitted") or raw_headers.get("Auto-Submitted")
                self_email = identity.email.strip().lower()
                ...
                decision = self.repository._classify_normalized_message_txn(
                    ...
                    tracked_for_later_messages, bool(message.trainlab_run_id),
                    thread_label_candidate == message.provider_message_id,
                )
                if decision.processing_state == "queued":
                    queued += 1
                elif decision.processing_state in {"ignored", "store_only", "quarantined"}:
                    ignored += 1
                ...
                if decision.actor_role == "trainlab" or (
                    decision.event_type == "new_request_received" and decision.actor_role == "user"
                ):
            ...
            for stage in ("discover", "archive", "normalize"):
                conn.execute(
                    ...
                    (run_id, "message", message.provider_message_id, message_id, stage, item_status, 1, self._clock(), self._clock()),
                )
        return {"changed": changed, "unchanged": unchanged, "queued": queued, "ignored": ignored}

    def _receipt(self, request: MailRequest, status: str, counts: MailCounts, poll_state: tuple[dict[str, str], ...], processed: tuple[str, ...], errors: tuple[dict[str, str], ...], started: str, run_id: int | None = None, next_retry_at_utc: str | None = None) -> MailReceipt:
        action = {
            "succeeded": "none", "unchanged": "none", "partial": "continue_poll", "deferred": "continue_poll",
            "auth_required": "reauthenticate", "rejected": "operator_review", "failed": "operator_review", "lock_busy": "continue_poll",
        ...
        for item in errors:
            ...
            public_errors.append({"stage": key[0], "code": key[1], "summary": _PUBLIC_SUMMARIES[key]})
        return MailReceipt(run_key=request.stable_run_key, mail_agent_run_id=None if run_id is None else str(run_id), invocation_id=request.invocation_id, mode="poll", status=status, counts=counts, poll_state=poll_state, processed_message_ids=processed, errors=tuple(public_errors), next_action=action, next_retry_at_utc=next_retry_at_utc, started_at_utc=started, completed_at_utc=self._clock())
