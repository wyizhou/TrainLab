...
def _utc(value: str) -> str:
    ...
    try:
        return datetime.fromisoformat(value[:-1] + "+00:00").astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    except ValueError as exc:
        raise LegacyReconciliationError("legacy_reconciliation_timestamp_invalid") from exc
...
def build_legacy_reconciliation_report(
    *,
    ...
    start, end = _iso_date(start_local_date, "legacy_reconciliation_date_invalid"), _iso_date(end_local_date, "legacy_reconciliation_date_invalid")
    if start > end:
        raise LegacyReconciliationError("legacy_reconciliation_date_range_invalid")
    normalized: list[dict[str, object]] = []
    for difference in differences:
        if type(difference) is not ReconciliationDifference or difference.code not in _DIFFERENCE_CODES:
    ...
    normalized.sort(key=lambda item: (str(item["code"]), str(item["entity_kind"]), int(item["legacy_count"]), int(item["garmin_count"])))
    legacy = {"activity_count": _count(legacy_counts.activity_count), "daily_coverage_count": _count(legacy_counts.daily_coverage_count)}
    garmin = {"activity_count": _count(garmin_counts.activity_count), "daily_coverage_count": _count(garmin_counts.daily_coverage_count)}
    report: dict[str, object] = {
        ...
        "legacy": legacy,
        "garmin": garmin,
        "snapshot_hashes": {"legacy_snapshot_sha256": _hash(legacy_snapshot_sha256), "garmin_snapshot_sha256": _hash(garmin_snapshot_sha256)},
