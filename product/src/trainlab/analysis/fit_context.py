...
_SECRET_KEYS = frozenset({
    "access_token", "authorization", "authorization_url", "client_id",
    "client_secret", "credential", "credentials", "identity_hmac",
    "password", "refresh_token", "secret", "token", "provider_account_id",
})
_DEVICE_KEYS = frozenset({
    "serial", "serial_number", "device_serial", "device_serial_number",
    "device_id", "device_uid", "device_uid_hash", "unit_id",
})
_GPS_KEYS = frozenset({
    "latitude", "longitude", "position_lat", "position_long", "gps",
    "coordinates", "route", "polyline", "course_points",
})
...
def _clean(value: Any, *, include_gps: bool, depth: int = 0) -> Any:
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        ...
        return [_clean(item, include_gps=include_gps, depth=depth + 1) for item in value]
...
def build_explicit_fit_context(
    ...
    *,
    ...
    if not requested or len(requested) != len(set(requested)) or len(requested) > 16:
        raise ExplicitFitContextError("fit_context_activity_ids_invalid")
    if not isinstance(include_gps, bool) or not isinstance(max_bytes, int) or isinstance(max_bytes, bool):
    ...
    for row in decoded_fit:
        ...
        if isinstance(revision, bool) or not isinstance(revision, (str, int)) or not str(revision):
...
__all__ = ["FIT_CONTEXT_MAX_BYTES", "ExplicitFitContextError", "build_explicit_fit_context"]
