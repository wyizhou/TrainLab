...
数据库备份包含账号、会话、活动名称覆盖、导入/删除状态、GPS 和健康相关运动指标，应视为敏感信息，不提交到 Git。
...
数据库 dump 和私有卷快照必须标记同一个停止写入时间点。在线 DELETE 只删除当前主数据库和主存储中的数据，不会追溯修改既有离线备份；备份可能继续包含用户已经在线删除的数据，必须按独立的保留期限、访问控制和安全销毁流程管理。
...
恢复私有卷时必须保持数据库 `storage_key` 对应的相对目录结构，并拒绝任何逃逸目标根目录的路径。恢复是破坏性操作，必须核对环境、数据库备份和私有卷快照属于同一时间点。恢复后检查 `/readyz`、登录、迁移版本、活动列表、导入记录、存储用量以及所有者原文件下载；抽查不得把内容写入日志。

恢复完成后先只读审计，不要直接删除不匹配文件：

```bash
backend/scripts/compose.sh exec backend trainlab reconcile-storage
```

该命令默认 dry-run，只报告超过宽限期且没有数据库引用的生成文件数量和字节数。确认恢复点完整、审计结果合理且已经保留恢复前快照后，才能运行 `reconcile-storage --apply`。它不是数据库/私有卷一致性修复器，也不能找回数据库引用但卷内缺失的原文件。
