...
本文件记录用户确认的目标需求，并按用户批准的顺序补充三个技术合同。当前唯一获准先行落地
的运行文件是训练报告发布 Skill 及其索引；其他产品实现、数据库和私人数据继续只做设计。

在用户明确批准实施前，不修改 `source/state/`、数据库、raw 文件、产品代码、配置或外部
系统，不调用 Garmin/Gmail，也不提交或推送。
...
## 技术合同 1：SQLite 六表 Schema v1（设计完成，未实施）

本合同只设计字段和数据库内约束，不创建数据库，不迁移或读取私人数据，也不提前设计备份
命令或六个 Skill 的完整输入输出 Schema。
...
## 技术合同 2：SQLite 备份、恢复与保留 v1（设计完成，未实施）
...
## 技术合同 3：六个运行 Skills 输入、输出、失败与幂等 v1（设计完成，部分未实施）

当前只有 `training-report-publisher` 的项目 Skill 骨架已经建立；其余五个 Skill、公共脚本、
Schema、SQLite 和模板都仍待后续实现。本合同不表示这些能力当前可运行。
...
- 目录变化关联哪些源码、数据库字段、Schema、测试和文档；
- `backup` 中现有数据的最终处理方式；
- 移除 `gmail` 对入站邮件工作流的影响；
- 文件移动、数据库同步、回滚和验证方案；
- health 文件名中的内容哈希长度、碰撞处理和完整 SHA-256 字段设计；
- activity hash 的历史数据绑定与迁移实现；
- 取消 activity summary 后各原始格式的可重建字段边界；
- 每日同步窗口变化关联哪些 cursor、coverage、重试、活动生命周期和质量门实现。
- 非 raw state 的精确清单、删除顺序、新数据库初始化和验证方式。
- 六表的实际 SQL DDL、升级/回退代码、公共 `_shared` 实现和合成测试；
- 六个本地 Skill 的具体脚本、Schema、自测与 MCP 适配实现；
- 旧运行实现向本地 Skills 迁移时的删除顺序、等价能力清单和最终验证方式。
- 两条 cron 的实际安装方式、环境加载、超时、并发和操作系统配置。

## 依赖与隔离

- 显式依赖：用户已确认本轮产品需求稳定；M6 继续保持 `blocked`。本文件的三个技术合同已经
  完成设计，后续实际重构仍需依据最新仓库事实拆分并批准实施任务。
- 当前允许写入：本需求文档、必要的协调状态记录、`source/skills/README.md` 和
  `source/skills/training-report-publisher/**`。
- 当前禁止写入：`source/state/`、现有产品源码、测试、配置、`data-backup/` 和外部系统。
...
- 当前 Loop：报告发布 Skill 骨架和三个技术合同均已写入并通过全新独立只读验证；数据库、
  模板、公共脚本与其余五个 Skill 仍未实施，等待用户批准下一阶段的实现分解。
- 最近完成：确认 Provider 分层、health/activities 双来源结构、inventory 数据库控制状态、
  取消 activity summary、每日 12:00 无历史回读、重置全部旧非 raw state，以及完全由 AI 与
  六个本地 Skills 驱动的最小 source 运行结构；确认每日与每周两个无状态 cron Prompt 以及
  Garmin My Workouts 中 GTS 模板的安全替换范围；确认 raw 不清洗入库、training-coach 脚本
  按需解析、正常历史上下文复用 14 份日报与 4 份周报，以及 SQLite 保存全部 Skill 输出和状态。
- 已完成的三个技术合同：
  1. 六张 SQLite 表的字段、关系、约束和状态枚举；
  2. SQLite 一致性备份、恢复和保留；
  3. 六个本地 Skill 的输入、输出、失败和幂等。
- 验证状态：项目 Skill 已通过 `quick_validate.py`，且一次无数据前向测试确认默认 open report、
  无 Sites/Gmail 授权时零外部动作；全新高风险 Validator 最终 `PASS`，独立确认六表、备份、
  六 Skill、模板路径和失败码合同一致。当前仅新增运行 Skill 文档/元数据与规划，没有修改
  state、数据库、现有产品代码或外部系统。
