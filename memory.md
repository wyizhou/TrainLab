...
- 当前完整合同RUN-VC-001/A-024：规划只看跑步/可验证跑步历史/目标限制时间，总结看全部运动/最多四完整周报/固定计划；两阶段共享20次细读，无自动模型重试。R2内部接线及R3周报课程校验已交付，完整运行尚未完成。
...
R0协作与继承基线已由独立验收、Main实际检查及macOS/Linux全量CI确认，d525a39已推送当前实验分支。R1总采集预算、旧记录只读和独立副本接线已通过3070项独立测试、Main实际验收及macOS/Linux全量CI，5c67ead已推送。R2两阶段内部接线已通过3089项本机独立回归、Main实际验收及macOS/Linux同SHA全量CI，671f7b2已推送当前分支。R3七部分周报、固定课程与来源校验已通过3158项完整独立回归、Main实际验收及macOS/Linux各3158项完整CI，7bc2175已推送当前分支。R4本地周报、显式修订与封存已通过3189项完整独立回归、PDF全页实视和Main实际验收，分支提交及双平台CI尚待；前置模块不重做，旧失败保留。额外真实CLI本机探针能力未确认，不能代替完整运行证明。真实业务调用尚未开始，旧失败不改判。准确步骤和保护证据只见活动计划。
