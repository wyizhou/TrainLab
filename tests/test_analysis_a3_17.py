...
def test_ready_publishes_two_artifacts_and_only_pending_delivery(tmp_path: Path):
    ...
    assert {
        item["key"] for item in service.context_builder.features
...
def test_every_daily_primary_item_template_is_accepted_by_safety_policy():
    ...
    assert contract["allowed_activity_kinds"] == ["running", "rest"]
    assert "climbing_template" not in contract
...
def test_active_acute_injury_fact_forces_rest_and_rejects_running_model_safety_claim():
    ...
    output = {"schema_version": "1", "run_key": "analysis:subject:daily:2026-07-23:one", "mode": "daily", "subject_id": 1, "status": "accepted", "artifacts": [
        {"artifact_kind": "daily_summary", "period": {"start_local_date": "2026-07-23", "end_local_date": "2026-07-23"}, "structured_content": {"overall_state": "恢复证据存在安全阻断", "decision_factors": ["用户报告急性伤情"], "activity_evidence": "unconfirmed", "plan_evidence": "unavailable", "data_completeness": "partial"}, "user_visible_text": "昨日恢复情况需要谨慎看待。用户报告的急性伤情是当前最重要的因素，活动记录尚未确认。"},
        {"artifact_kind": "daily_training_advice", "period": {"start_local_date": "2026-07-24", "end_local_date": "2026-07-24"}, "structured_content": {"primary_item": running, "configured_difficulty_level": 2, "selected_session_difficulty_level": 2, "difficulty_adjustment_reason": None, "confidence": "一般", "confidence_reason": "活动记录尚未确认，但伤情信息足以支持保守判断。", "data_limitation": None}, "user_visible_text": "今日跑步。完成包含热身、轻松主训练和放松的30分钟课程，全程保持能说完整句子。如有急性疼痛、胸痛、晕厥或异常呼吸困难，请立即停止。\n判断置信度：一般——活动记录尚未确认，但伤情信息足以支持保守判断。"},
