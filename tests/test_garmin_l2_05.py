...
def test_strict_json_bytes_reject_ambiguity_nonfinite_encoding_and_trailing_data_without_artifacts(
    ...
    with repository.connect() as connection:
        with pytest.raises(GarminError, match="provider_json_invalid") as caught:
            repository.archive(
                connection, "fixture_json", "strict-object", payload,
        ...
        assert connection.execute("SELECT count(*) FROM source_field_catalog").fetchone()[0] == 0
...
def test_provider_json_depth_width_node_and_byte_limits_have_exact_boundaries(
    ...
    with monkeypatch.context() as scoped:
        scoped.setattr("trainlab.garmin.PROVIDER_JSON_MAX_DEPTH", 2)
    ...
    with monkeypatch.context() as scoped:
        scoped.setattr("trainlab.garmin.PROVIDER_JSON_MAX_CONTAINER_ITEMS", 2)
    ...
    with monkeypatch.context() as scoped:
        scoped.setattr("trainlab.garmin.PROVIDER_JSON_MAX_NODES", 3)
        assert canonical_provider_json([1, 2]) == b"[1,2]"
        with pytest.raises(GarminError, match="provider_json_invalid"):
            canonical_provider_json([1, 2, 3])

    with monkeypatch.context() as scoped:
        scoped.setattr("trainlab.garmin.PROVIDER_JSON_MAX_BYTES", 7)
        with repository.connect() as connection:
            repository.archive(
                connection, "fixture_json", "byte-limit",
                b'{"a":1}', "json", "application/json",
            )
            with pytest.raises(GarminError, match="provider_json_invalid"):
                repository.archive(
                    connection, "fixture_json", "byte-limit-over",
                    b'{"a":1} ', "json", "application/json",
                )
...
def test_python_provider_json_rejects_unsupported_types_keys_nonfinite_and_cycles() -> None:
...
def test_changed_payload_increments_revision_and_keeps_one_current(tmp_path: Path) -> None:
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
    connection = repository.connect()
    repository.archive(
...
def test_create_only_publication_failure_leaves_no_partial_or_temp_file(
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", fail_link)
...
def test_raw_write_fsyncs_file_and_parent_directory(
    ...
    monkeypatch.setattr("trainlab.garmin.os.fsync", observe_fsync)
...
def test_raw_rejects_insecure_directory_or_file_permissions(tmp_path: Path) -> None:
    ...
    raw_id, _ = repository.store_raw("fixture_json", b'{"value":1}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        relative = connection.execute("SELECT relative_path FROM raw_objects WHERE id=?", (raw_id,)).fetchone()[0]
    os.chmod(config.raw_root.parent / relative, 0o644)
    with pytest.raises(ValueError, match="raw_object_corrupt"):
        repository.store_raw("fixture_json", b'{"value":1}', "json", "application/json")


def test_raw_create_only_race_and_directory_fsync_failure_fail_closed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", competing_link)
    raw_id, _ = repository.store_raw("fixture_json", b'{"race":1}', "json", "application/json")
    assert raw_id > 0
    ...
    monkeypatch.setattr("trainlab.garmin.os.fsync", fail_directory_fsync)
    with pytest.raises(OSError, match="directory fsync failed"):
        repository.store_raw("fixture_json", b'{"fsync":1}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects WHERE sha256!=?", ("",)).fetchone()[0] == 1
...
def test_new_child_directory_requires_parent_fsync_before_any_db_row(
    ...
    monkeypatch.setattr("trainlab.garmin.os.fsync", fail_first_directory_fsync)
    with pytest.raises(OSError, match="new child parent fsync failed"):
        repository.store_raw("fixture_json", b'{"new_child":true}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 0
...
def test_cleanup_claim_preserves_replaced_temp_and_blocks_db_publish(
    ...
    def replace_old_temp(source: str, destination: str, **kwargs: object) -> None:
        ...
        if str(source).startswith(".tmp-"):
            ...
            fd = os.open(source, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=directory_fd)
            try:
    ...
    monkeypatch.setattr("trainlab.garmin.os.rename", replace_old_temp)
    with pytest.raises(ValueError, match="raw_cleanup_temp_replaced"):
        repository.store_raw("fixture_json", b'{"cleanup_race":true}', "json", "application/json")
    assert replacements
...
def test_cleanup_claim_swap_is_preserved_and_blocks_db_publish(
    ...
    def replace_claim(source: str, destination: str, **kwargs: object) -> None:
        ...
        if str(destination).startswith(".cleanup-"):
            ...
            fd = os.open(destination, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=directory_fd)
            try:
    ...
    monkeypatch.setattr("trainlab.garmin.os.rename", replace_claim)
    with pytest.raises(ValueError, match="raw_cleanup_claim_replaced"):
        repository.store_raw("fixture_json", b'{"claim_race":true}', "json", "application/json")
...
def test_loser_accepts_same_content_and_rejects_different_content(
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", same_content_loser)
    assert repository.store_raw("fixture_json", b'{"loser":1}', "json", "application/json")[0] > 0
    monkeypatch.undo()
    original_link = os.link

    def different_content_loser(source: str, destination: str, **kwargs: object) -> None:
        original_link(source, destination, **kwargs)
        directory_fd = int(kwargs["dst_dir_fd"])
        os.unlink(destination, dir_fd=directory_fd)
        fd = os.open(destination, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=directory_fd)
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", different_content_loser)
    with pytest.raises(ValueError, match="raw_object_corrupt"):
...
def test_winner_requires_final_device_and_inode_match_temp(
    ...
    monkeypatch.setattr("trainlab.garmin.os.write", capture_temp)
    monkeypatch.setattr("trainlab.garmin.os.link", publish)
    monkeypatch.setattr("trainlab.garmin.os.fstat", spoof_final_device)
    with pytest.raises(ValueError, match="raw_object_corrupt"):
        repository.store_raw("fixture_json", b'{"device":1}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 0
...
def test_db_transaction_failure_keeps_final_raw_but_no_raw_object_row(
    ...
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
    monkeypatch.setattr(repository, "_open_relative_rawfd", lambda _relative: (_ for _ in ()).throw(ValueError("verification failed")))
    with pytest.raises(ValueError, match="verification failed"):
        repository.store_raw("fixture_json", b'{"db_rollback":true}', "json", "application/json")
...
def test_raw_short_write_loops_and_zero_or_write_errors_leave_no_db_row(
    ...
    monkeypatch.setattr("trainlab.garmin.os.write", partial_write)
    repository.store_raw("fixture_json", b'{"partial":true}', "json", "application/json")
    assert calls >= 2

    monkeypatch.setattr("trainlab.garmin.os.write", lambda _fd, _data: 0)
    with pytest.raises(OSError, match="raw_short_write"):
        repository.store_raw("fixture_json", b'{"zero":true}', "json", "application/json")
    monkeypatch.setattr("trainlab.garmin.os.write", lambda _fd, _data: (_ for _ in ()).throw(OSError("write failed")))
    with pytest.raises(OSError, match="write failed"):
        repository.store_raw("fixture_json", b'{"write":true}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 1
    assert not list(config.raw_root.rglob(".tmp-*"))


def test_raw_file_fsync_failure_leaves_no_db_row(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
    original_fsync = os.fsync
    ...
    monkeypatch.setattr("trainlab.garmin.os.fsync", fail_file_fsync)
    with pytest.raises(OSError, match="file fsync failed"):
        repository.store_raw("fixture_json", b'{"fsync":true}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 0
    assert not list(config.raw_root.rglob(".tmp-*"))
...
def test_raw_rejects_final_name_swap_after_create_only_publish(
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", replace_winner)
    with pytest.raises(ValueError, match="raw_object_corrupt"):
        repository.store_raw("fixture_json", b'{"value":1}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 0
...
def test_raw_rejects_final_name_symlink_swap_after_create_only_publish(
    ...
    def replace_winner_with_symlink(source: str, destination: str, **kwargs: object) -> None:
        original_link(source, destination, **kwargs)
        directory_fd = int(kwargs["dst_dir_fd"])
        os.unlink(destination, dir_fd=directory_fd)
    ...
    monkeypatch.setattr("trainlab.garmin.os.link", replace_winner_with_symlink)
    with pytest.raises(ValueError, match="raw_object_corrupt"):
        repository.store_raw("fixture_json", b'{"value":1}', "json", "application/json")
    with repository.connect(readonly=True) as connection:
        assert connection.execute("SELECT count(*) FROM raw_objects").fetchone()[0] == 0
...
def test_dirfd_anchor_rejects_parent_symlink_swap_after_root_is_opened(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
...
def test_dirfd_anchor_rejects_root_swap_and_read_does_not_create_missing_children(
    ...
    with pytest.raises(ValueError, match="unsafe_raw_path"):
        repository.store_raw("fixture_json", b'{"root_swap":true}', "json", "application/json")
    assert not list(outside.iterdir())
...
def test_raw_repeated_reads_do_not_leak_file_descriptors(tmp_path: Path) -> None:
    ...
    for _ in range(32):
        repository.store_raw("fixture_json", b'{"same":true}', "json", "application/json")
...
def test_all_json_fields_are_cataloged_and_fit_never_creates_zip(tmp_path: Path) -> None:
    config, _ = _setup(tmp_path)
    repository = GarminRepository(config)
    connection = repository.connect()
    payload = {
    ...
    fit_bytes = (Path(__file__).parents[1] / "test_data/new/Running.fit").read_bytes()
    repository.archive(
        connection,
        "activity_fit",
        "activity-1",
...
def test_nonfinite_provider_values_fail_closed_before_raw_revision_or_json_projection(
    ...
    with repository_connection(config) as connection:
        ...
        assert connection.execute("SELECT count(*) FROM daily_health").fetchone()[0] == 0
        assert connection.execute(
            """SELECT error_code FROM garmin_sync_items
               WHERE resource_kind='user_summary' AND status='failed'"""
        ).fetchone()[0] == "provider_json_invalid"
        assert connection.execute(
        ...
        assert all("NaN" not in statement and "Infinity" not in statement for statement in json_columns)
    assert not list(config.raw_root.rglob(".tmp-*"))


def test_unsupported_provider_object_is_never_stringified_or_persisted(tmp_path: Path) -> None:
...
def test_health_project_failure_is_sanitized_and_leaves_repairable_raw(
    ...
    with repository_connection(config) as connection:
        ...
        assert tuple(connection.execute("SELECT is_current,parsed_at_utc FROM source_revisions WHERE resource_kind='user_summary'").fetchone()) == (0, None)
...
def test_activity_fit_archive_failure_keeps_fit_revision_but_no_samples(
    ...
    connection = repository.connect()
    fit_payload = (
        Path(__file__).parents[1] / "test_data" / "new" / "Running.fit"
    ).read_bytes()
    ...
    assert connection.execute("SELECT count(*) FROM activity_samples").fetchone()[0] == 0
    connection.close()
