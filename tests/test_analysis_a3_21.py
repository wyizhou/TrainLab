...
def request(run_key: str | None = None, subject: str = "subject_a") -> AnalysisRequest:
    return AnalysisRequest.from_dict({
        "schema_version": "1", "mode": "status", "subject_id": subject,
        "invocation_id": None, "run_key": run_key, "summary_local_date": None,
        "advice_local_date": None, "as_of_local_date": None, "plan_id": None,
        "reason_event_id": None, "effective_local_date": None, "artifact_id": None,
        "delivery_id": None, "regeneration_reason_code": None, "requested_at_utc": NOW,
    })
...
def test_status_is_select_only_and_returns_only_subject_safe_summary() -> None:
    ...
    assert snapshot == {
        "selected_run": {"id": "12", "run_key": "analysis:subject_a:weekly:2026-07-25:two", "status": "rejected", "started_at_utc": "2026-07-26T00:00:00Z", "completed_at_utc": "2026-07-26T00:01:00Z"},
        "current_artifacts": [
            {"id": "101", "kind": "daily_summary", "period_start_local_date": "2026-07-24", "period_end_local_date": "2026-07-24", "revision_no": 2},
            {"id": "102", "kind": "weekly_summary", "period_start_local_date": "2026-07-18", "period_end_local_date": "2026-07-24", "revision_no": 1},
        ],
        "current_plan": {"id": "301", "start_local_date": "2026-07-25", "end_local_date": "2026-07-31", "status": "active"},
        "latest_delivery": {"id": "502", "status": "delivery_unknown"},
        "delivery_counts": {"pending": 0, "delivery_unknown": 1, "failed": 1},
        "actionable_delivery_ids": {"retry_delivery": ["501"], "reconcile_delivery": ["502"]},
        "recent_terminal_at_utc": {"succeeded": "2026-07-25T00:01:00Z", "failed": None, "rejected": "2026-07-26T00:01:00Z", "deferred": None},
        "quality_blocker_codes": ["analysis_input_blocked"],
        "deferred_history_supported": False,
    }
    assert all(statement.lstrip().upper().startswith("SELECT") for statement in statements)
    encoded = json.dumps(payload, sort_keys=True)
    for secret in ("SECRET", "provider-", "thread-", "private", "other_subject", "details_json", "objective_json", "structured_content_json", "user_visible_text"):
...
def test_selected_absent_or_foreign_run_never_leaks_another_subject() -> None:
    ...
    assert absent.status_snapshot is not None and absent.status_snapshot["selected_run"] is None
...
def test_status_schema_requires_bounded_snapshot_and_failure_is_redacted() -> None:
    ...
    try:
        ...
        raise AssertionError("status snapshot contract accepted an unsupported deferred history")
    failed = AnalysisStatusQueryService(conn, clock=lambda: NOW).execute(request(subject="unknown"))
    assert failed.status == "failed" and failed.errors[0].summary == "analysis status is unavailable"
    assert failed.status_snapshot is not None and failed.status_snapshot["selected_run"] is None


def test_status_module_has_no_forbidden_runtime_capability_imports() -> None:
    source = open("src/trainlab/analysis/status.py", encoding="utf-8").read()
    for forbidden in ("runner", "gmail", "context", "harness", "lock", "Codex", "Gmail"):
