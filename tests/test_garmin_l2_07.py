...
_V4_DRIVER_FIXTURE = Path(__file__).with_name("fixtures") / "garmin_live_v4_driver.py"
_V5_DRIVER_FIXTURE = Path(__file__).with_name("fixtures") / "garmin_live_v5_driver.py"


def _isolated_driver(tmp_path: Path, version: str) -> Path:
    """Materialize a reviewed driver fixture without depending on host state."""

    source = _V4_DRIVER_FIXTURE if version == "v4" else _V5_DRIVER_FIXTURE
    destination = tmp_path / version / "driver.py"
...
def test_v4_driver_import_and_noarg_are_inert_with_exact_manual_gate(
    tmp_path: Path,
) -> None:
    driver = _isolated_driver(tmp_path, "v4")
...
def test_v5_driver_has_a_frozen_double_gate_without_entering_live_path(
    tmp_path: Path,
) -> None:
    driver = _isolated_driver(tmp_path, "v5")
    source = driver.read_text(encoding="utf-8")
    ...
    assert "FROZEN_CONTROL_MANIFEST_SHA256" in source
    assert "FROZEN_SOURCE_MANIFEST_SHA256" in source
    assert "DISTRIBUTION_RECORD_SHA256" in source
    assert "def _execute_once" in source
...
def test_v5_driver_pre_auth_failures_write_independent_zero_stop(
    ...
    namespace["_acquire_once_lock"] = lambda: os.open(
        marker / "execution.lock", os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600
    )
    namespace["_verify_static_gate"] = lambda: (_ for _ in ()).throw(
        namespace["LiveGateError"](stage)
    )
    with pytest.raises(namespace["LiveGateError"]):
...
def test_v5_driver_preflight_stop_persist_failure_never_reaches_import(
    tmp_path: Path,
) -> None:
    driver = _isolated_driver(tmp_path, "v5")
    ...
    namespace["_verify_static_gate"] = lambda: (_ for _ in ()).throw(
        namespace["LiveGateError"]("fixture")
    )
    namespace["_persist_preflight_stop"] = lambda _code: (_ for _ in ()).throw(
        OSError("fixture")
    )
    with pytest.raises(namespace["LiveGateError"]):
        namespace["_execute_once"]()
...
def test_v4_driver_low_level_allowlist_counts_only_reviewed_entries(
    tmp_path: Path,
) -> None:
    driver = _isolated_driver(tmp_path, "v4")
...
def test_v4_driver_drift_accepts_only_real_demote_promote_transition(
    tmp_path: Path,
) -> None:
    driver = _isolated_driver(tmp_path, "v4")
