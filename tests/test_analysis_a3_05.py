...
import trainlab.analysis.run_state as run_state
from trainlab.analysis.contracts import AnalysisRequest
from trainlab.analysis.run_state import (
...
def request(mode: str = "daily", *, subject: str = "subject1", invocation: str | None = "invoke1", **override: str | None) -> AnalysisRequest:
    values: dict[str, str | None] = {
        "run_key": None, "summary_local_date": None, "advice_local_date": None, "as_of_local_date": None,
        "plan_id": None, "reason_event_id": None, "effective_local_date": None, "artifact_id": None,
        "delivery_id": None, "regeneration_reason_code": None,
    }
    if mode == "daily": values.update(summary_local_date="2026-07-22", advice_local_date="2026-07-23")
    if mode == "weekly": values.update(as_of_local_date="2026-07-23")
    if mode == "revise_plan": values.update(plan_id="plan1", reason_event_id="event1", effective_local_date="2026-07-23")
    if mode == "regenerate": values.update(artifact_id="artifact1", regeneration_reason_code="operator_review")
    values.update(override)
    return AnalysisRequest(mode=mode, subject_id=subject, invocation_id=invocation, requested_at_utc=UTC, **values)  # type: ignore[arg-type]


def connection(database: str | Path = ":memory:") -> sqlite3.Connection:
    conn = sqlite3.connect(database, isolation_level=None)
    conn.execute("PRAGMA foreign_keys=ON")
    conn.execute("CREATE TABLE data_subjects(id INTEGER PRIMARY KEY, subject_key TEXT NOT NULL UNIQUE, is_active INTEGER NOT NULL)")
    conn.executemany("INSERT INTO data_subjects(id,subject_key,is_active) VALUES(?,?,1)", [(1, "subject1"), (2, "subject2")])
    ...
    conn.execute("CREATE TABLE analysis_artifacts(id INTEGER PRIMARY KEY, generated_by_run_id INTEGER NOT NULL REFERENCES analysis_runs(id))")
    return conn
...
@pytest.mark.parametrize(
    ...
    [
        ("daily", {"invocation": "invoke:daily"}, "analysis:subject1:daily:2026-07-22:invoke%3Adaily"),
        ("weekly", {"invocation": "invoke:weekly"}, "analysis:subject1:weekly:2026-07-23:invoke%3Aweekly"),
        ("revise_plan", {"invocation": "invoke:plan", "plan_id": "plan:one", "reason_event_id": "event:one"}, "analysis:subject1:revise_plan:plan%3Aone:event%3Aone:invoke%3Aplan"),
        ("regenerate", {"invocation": "invoke:regen", "artifact_id": "artifact:one"}, "analysis:subject1:regenerate:artifact%3Aone:invoke%3Aregen"),
    ],
)
def test_colon_identifiers_are_reversibly_canonicalized(mode: str, override: dict[str, str], expected: str) -> None:
...
@pytest.mark.parametrize("value", ["analysis:subject1:daily:2026-01-01:inv:extra", "analysis:subject1:daily:not-a-date:inv", "analysis:subject1:retry_delivery:x:inv", "analysis:subject1:daily:2026-01-01:bad/inv", "analysis:subject1:daily:2026-01-01:invoke%3adaily", "analysis:subject1:daily:2026-01-01:"])
...
def test_schema_incompatible_and_missing_invocation_column_are_rejected() -> None:
    ...
    conn.execute("CREATE TABLE data_subjects(id INTEGER PRIMARY KEY, subject_key TEXT, is_active INTEGER)")
    conn.execute("CREATE TABLE analysis_runs(id INTEGER PRIMARY KEY, run_key TEXT UNIQUE, subject_id INTEGER, analysis_kind TEXT, status TEXT, started_at_utc TEXT, invocation_id TEXT)")
    with pytest.raises(AnalysisRunStateError, match="analysis_runs_schema_incompatible"):
...
def test_same_invocation_is_unique_and_receipt_loss_recovers_started_run() -> None:
    ...
    assert (first.action, recovered.action, first.run_id, recovered.run_id) == ("started", "recovered", first.run_id, first.run_id)
...
def test_legacy_run_rows_are_ignored_for_new_invocation_and_current_status() -> None:
    conn = connection()
    conn.execute("INSERT INTO analysis_runs(run_key,subject_id,analysis_kind,status,started_at_utc) VALUES('daily-run',1,'daily','succeeded','2099-01-01T00:00:00Z')")
    conn.execute("INSERT INTO analysis_runs(run_key,subject_id,analysis_kind,status,started_at_utc) VALUES('weekly-run',1,'weekly','succeeded','2099-01-02T00:00:00Z')")
    ...
    assert created.action == "started" and current is not None and current.run_id == created.run_id


def test_succeeded_is_unchanged_and_terminal_failures_are_not_reopened(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection())
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_terminal_run_requires_new_invocation"):
        repo.prepare_write(request(invocation="invoke2"))


def test_failed_transaction_leaves_no_half_run() -> None:
    conn = connection(); repo = AnalysisRunRepository(conn)
...
def locks(tmp_path: Path) -> SubjectLockManager:
    root = tmp_path / "locks"; root.mkdir(mode=0o700); root.chmod(0o700)
...
def test_coordinator_mutually_excludes_same_subject_but_not_other_subject(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection())
    coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    ...
    other.release(); first.release()


def test_succeeded_preflight_bypasses_other_subject_lock_but_new_work_does_not(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    old = coordinator.begin(request(invocation="old")); coordinator.finish(old, "succeeded")
...
def test_coordinator_acquires_before_write_and_releases_after_failed_prepare(tmp_path: Path) -> None:
    conn = connection(); repo = AnalysisRunRepository(conn); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    with pytest.raises(AnalysisRunStateError, match="analysis_subject_not_active"):
...
def test_coordinator_finish_requires_matching_prepared_lock(tmp_path: Path) -> None:
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_prepared_run_unauthorized"):
        coordinator.finish(PreparedAnalysisRun(prepared.decision, None), "failed")


def test_repository_has_no_public_finish_or_unbound_terminal_write(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection())
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_finish_unauthorized"):
        repo._finish_authorized(prepared.run_id, prepared.run_key, "succeeded", object())
    assert repo.status(request("status", invocation=None, run_key=prepared.run_key)).status == "started"  # type: ignore[union-attr]
    # Binding and finishing are only established by a Coordinator instance.
    AnalysisRunCoordinator(repo, locks(tmp_path))


def test_finish_accepts_only_same_coordinator_same_prepared_identity(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
...
def test_finish_rejects_mutated_prepared_scope_and_second_coordinator(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection()); manager = locks(tmp_path); coordinator = AnalysisRunCoordinator(repo, manager)
    with pytest.raises(AnalysisRunStateError, match="analysis_repository_already_bound"):
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_prepared_run_lock_required"):
        coordinator.finish(prepared, "succeeded")
    held.release()
...
def test_finish_db_failure_restores_canonical_lock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="dbfail"))
    original_finish = repo._finish_authorized
    monkeypatch.setattr(repo, "_finish_authorized", lambda *args: (_ for _ in ()).throw(sqlite3.OperationalError("db")))
...
def test_actual_sqlite_update_failure_rolls_back_and_restores_lock(tmp_path: Path) -> None:
    conn = connection(); repo = AnalysisRunRepository(conn); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="trigger-update"))
    conn.execute("CREATE TRIGGER fail_analysis_finish BEFORE UPDATE ON analysis_runs BEGIN SELECT RAISE(ABORT, 'finish denied'); END")
    with pytest.raises(sqlite3.DatabaseError, match="finish denied"):
        coordinator.finish(prepared, "succeeded")
    row = conn.execute("SELECT status FROM analysis_runs WHERE id=?", (prepared.decision.run_id,)).fetchone()
    assert row[0] == "started" and (tmp_path / "locks/analysis.lock.subject1.lock").exists()


def test_actual_sqlite_commit_failure_rolls_back_and_restores_lock(tmp_path: Path) -> None:
    conn = connection(); repo = AnalysisRunRepository(conn); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="deny-commit"))

    def deny_commit(action: int, arg1: str | None, arg2: str | None, db: str | None, source: str | None) -> int:
...
def test_commit_success_cleanup_failure_is_status_visible_and_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_finish_cleanup_required"):
        coordinator.finish(prepared, "succeeded")
    status = repo.status(request("status", invocation=None, run_key=prepared.decision.run_key))
    assert status is not None and status.status == "succeeded"
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        coordinator.begin(request(invocation="after-cleanup"))


def test_committed_finish_unlink_failure_keeps_claim_and_blocks(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="unlink-fail")); assert prepared.lock is not None
    ...
    status = repo.status(request("status", invocation=None, run_key=prepared.decision.run_key))
    assert status is not None and status.status == "succeeded"
    assert list((tmp_path / "locks").glob("*.claim")) or list((tmp_path / "locks").glob(".*.claim"))
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        coordinator.begin(request(invocation="after-unlink-fail"))


def test_committed_finish_parent_fsync_failure_rebuilds_blocker(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="fsync-fail")); assert prepared.lock is not None
    parent_fd = prepared.lock.parent_fd; original_fsync = os.fsync; parent_calls = 0
...
def test_invalid_finish_status_keeps_prepared_authorization_for_abort(tmp_path: Path) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
...
def test_cross_coordinator_prepared_is_rejected_but_origin_can_abort(tmp_path: Path) -> None:
    ...
    peer = sqlite3.connect(database, isolation_level=None); peer.execute("PRAGMA foreign_keys=ON")
...
def test_finish_claim_swap_never_authorizes_database_transition(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    prepared = coordinator.begin(request(invocation="claim-swap"))
    filename = "analysis.lock.subject1.lock"
    replacement = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:other", "started_at_utc": UTC}
    original_rename = os.rename

    def swap_finish_claim(src: str, dst: str, *, src_dir_fd: int | None = None, dst_dir_fd: int | None = None) -> None:
        ...
        if dst.startswith(f".{filename}.finish."):
            ...
            fd = os.open(dst, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=dst_dir_fd)
            try:
                os.write(fd, json.dumps(replacement).encode("utf-8")); os.fchmod(fd, 0o600)
            finally:
                os.close(fd)
...
def test_finish_canonical_rebuild_never_authorizes_database_transition(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = AnalysisRunRepository(connection()); coordinator = AnalysisRunCoordinator(repo, locks(tmp_path))
    ...
    def rebuild_canonical(src: str, dst: str, *, src_dir_fd: int | None = None, dst_dir_fd: int | None = None) -> None:
        original_rename(src, dst, src_dir_fd=src_dir_fd, dst_dir_fd=dst_dir_fd)
        if dst.startswith(f".{filename}.finish."):
            fd = os.open(filename, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=src_dir_fd)
            try:
                os.write(fd, json.dumps(replacement).encode("utf-8")); os.fchmod(fd, 0o600)
            finally:
                os.close(fd)
...
def test_active_stale_and_malicious_lock_records(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    path = tmp_path / "locks/analysis.lock.subject1.lock"
    payload = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:invoke1", "started_at_utc": UTC}
    ...
    recovered = manager.acquire("subject1", payload["run_key"]); recovered.release()
    path.write_text("{broken", encoding="utf-8"); path.chmod(0o600)
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        manager.acquire("subject1", payload["run_key"])


def test_stale_claim_preserves_same_payload_replacement_inode(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    filename = "analysis.lock.subject1.lock"
    ...
    def replace_before_claim(src: str, dst: str, *, src_dir_fd: int | None = None, dst_dir_fd: int | None = None) -> None:
        if src == filename and dst.startswith(f".{filename}.stale."):
            os.unlink(filename, dir_fd=src_dir_fd)
            replacement = os.open(filename, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600, dir_fd=src_dir_fd)
            try:
                os.write(replacement, json.dumps(payload).encode("utf-8")); os.fchmod(replacement, 0o600)
            finally:
...
def test_failed_create_leaves_no_owned_lock_record(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, short_write: bool) -> None:
    manager = locks(tmp_path)
...
def test_post_create_replacement_is_quarantined_not_deleted(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    filename = "analysis.lock.subject1.lock"
    replacement = {"pid": 999, "run_key": "analysis:subject1:daily:2026-07-22:invoke9", "started_at_utc": UTC}
...
def test_release_pre_read_swap_preserves_replacement_and_closes_parent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    held = manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")
    replacement = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:invoke2", "started_at_utc": UTC}
    original_reader = run_state._read_private_lock_at
...
def test_release_claim_swap_quarantines_unknown_and_blocks_future_acquire(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    held = manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")
    replacement = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:invoke2", "started_at_utc": UTC}
    original_rename = os.rename

    def swap_claim(src: str, dst: str, *, src_dir_fd: int | None = None, dst_dir_fd: int | None = None) -> None:
        original_rename(src, dst, src_dir_fd=src_dir_fd, dst_dir_fd=dst_dir_fd)
...
def test_release_does_not_overwrite_third_party_canonical_rebuild(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    held = manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")
    filename = "analysis.lock.subject1.lock"
    replacement = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:invoke2", "started_at_utc": UTC}
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        held.release()
    assert json.loads((tmp_path / "locks" / filename).read_text(encoding="utf-8")) == replacement
    monkeypatch.setattr(os, "kill", lambda pid, signal: None)
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        manager.acquire("subject1", replacement["run_key"])
...
@pytest.mark.skipif(not Path("/dev/fd").exists(), reason="platform has no fd view")
def test_release_closes_parent_once_and_is_explicitly_nonrepeatable(tmp_path: Path) -> None:
    manager = locks(tmp_path)
    before = len(os.listdir("/dev/fd"))
    for index in range(12):
        held = manager.acquire("subject1", f"analysis:subject1:daily:2026-07-22:invoke{index}")
        held.release()
        with pytest.raises(AnalysisRunStateError, match="analysis_lock_already_released"):
            held.release()
    assert len(os.listdir("/dev/fd")) == before
...
@pytest.mark.skipif(not Path("/dev/fd").exists(), reason="platform has no fd view")
def test_acquire_paths_do_not_leak_file_descriptors(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path)
    path = tmp_path / "locks/analysis.lock.subject1.lock"
    active = {"pid": 4242, "run_key": "analysis:subject1:daily:2026-07-22:invoke1", "started_at_utc": UTC}
    path.write_text(json.dumps(active), encoding="utf-8"); path.chmod(0o600)
    monkeypatch.setattr(os, "kill", lambda pid, signal: None)
    before = len(os.listdir("/dev/fd"))
    for _ in range(12):
        with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
...
def test_lock_creation_failure_removes_only_its_own_partial_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = locks(tmp_path); path = tmp_path / "locks/analysis.lock.subject1.lock"
    ...
    def replace_then_fail(fd: int) -> None:
        ...
        path.write_text('{"replacement":true}', encoding="utf-8"); path.chmod(0o600)
        raise OSError("fsync")
...
def test_lock_rejects_symlink_and_wide_permissions(tmp_path: Path) -> None:
    ...
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")
    path.unlink(); path.write_text(json.dumps({"pid": 1, "run_key": "analysis:subject1:daily:2026-07-22:invoke1", "started_at_utc": UTC}), encoding="utf-8"); path.chmod(0o666)
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_busy"):
        manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")


def test_lock_parent_requires_owner_only_mode_and_owner(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = tmp_path / "locks"; root.mkdir(); root.chmod(0o755)
    manager = SubjectLockManager(root / "analysis.lock")
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_parent_unsafe_permissions"):
        manager.acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")
    root.chmod(0o700)
...
def test_lock_rejects_symlink_at_each_trusted_boundary_component(tmp_path: Path, depth: int) -> None:
    real = tmp_path / "real"; real.mkdir(mode=0o700); real.chmod(0o700)
    if depth == 0:
        link = tmp_path / "link"; link.symlink_to(real, target_is_directory=True); base = link / "analysis.lock"; trusted = tmp_path
    elif depth == 1:
        (real / "locks").mkdir(mode=0o700); (real / "locks").chmod(0o700); link = tmp_path / "link"; link.symlink_to(real, target_is_directory=True); base = link / "locks/analysis.lock"; trusted = tmp_path
    else:
        (real / "one").mkdir(mode=0o700); (real / "one").chmod(0o700); (real / "one/locks").mkdir(mode=0o700); (real / "one/locks").chmod(0o700); link = tmp_path / "link"; link.symlink_to(real, target_is_directory=True); base = link / "one/locks/analysis.lock"; trusted = tmp_path
    with pytest.raises(AnalysisRunStateError, match="analysis_lock_path_escape"):
        SubjectLockManager(base, trusted_root=trusted).acquire("subject1", "analysis:subject1:daily:2026-07-22:invoke1")


def test_open_lock_parent_fd_stays_bound_after_path_swap(tmp_path: Path) -> None:
    root = tmp_path / "state"; root.mkdir(mode=0o700); root.chmod(0o700)
    parent = root / "locks"; parent.mkdir(mode=0o700); parent.chmod(0o700)
    manager = SubjectLockManager(parent / "analysis.lock", trusted_root=root)
    fd, info, name = manager._open_lock_parent("subject1")
    moved = root / "locks-old"; parent.rename(moved)
    replacement = root / "locks"; replacement.mkdir(mode=0o700); replacement.chmod(0o700)
    try:
        assert name == "analysis.lock.subject1.lock" and os.fstat(fd).st_ino == info.st_ino
...
def test_status_is_pure_read_only_for_database_and_lock_files(tmp_path: Path) -> None:
    conn = connection(); repo = AnalysisRunRepository(conn)
    decision = repo.prepare_write(request()); key = decision.run_key
    manager = locks(tmp_path); coordinator = AnalysisRunCoordinator(repo, manager); existing_files = sorted(path.name for path in (tmp_path / "locks").iterdir())
    ...
    assert sorted(path.name for path in (tmp_path / "locks").iterdir()) == existing_files
    assert manager  # Explicitly proves status has no need to acquire a lock manager.


def test_status_without_run_key_selects_latest_subject_run_read_only(tmp_path: Path) -> None:
    conn = connection(); repo = AnalysisRunRepository(conn)
    first = repo.prepare_write(request(invocation="one"))
    second = repo.prepare_write(request(invocation="two"))
    before = conn.total_changes; files = list((tmp_path / "locks").glob("*"))
    result = repo.status(request("status", invocation=None))
    assert result is not None and result.run_id == second.run_id and result.run_id != first.run_id
    assert conn.total_changes == before and list((tmp_path / "locks").glob("*")) == files
