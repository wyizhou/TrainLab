...
def test_provider_fixture_matches_public_invocation_date_receipt_and_exit_contract() -> (
    None
):
    value = _fixture()
    call = _call(value)
    ...
    # receipt field.
    assert runner._argv(call)[-len(value["expected_argv_suffix"]) :] == tuple(
        value["expected_argv_suffix"]
    )
    accepted, receipt_hash = runner._validate_receipt(
        call, receipt, value["expected_exit_code"]
    )  # type: ignore[arg-type]
    assert accepted is receipt and re.fullmatch(r"[0-9a-f]{64}", str(receipt_hash))
    assert receipt["mode"] == call.mode
    assert receipt["requested_range"] == {
        "from": None,
        "through": call.through_local_date,
    }
    assert re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9:_-]{0,127}", str(receipt["run_id"]))
    assert (
        runner._EXIT_BY_LAYER["garmin"][str(receipt["status"])]
...
def test_orchestration_has_no_private_garmin_state_write_path() -> None:
    ...
    assert sources
    assert all(
        not forbidden.search(path.read_text(encoding="utf-8")) for path in sources
    )
    ...
    boundary = (ROOT / "src/trainlab/orchestration/subprocess_runner.py").read_text(
        encoding="utf-8"
    )
...
def test_analysis_has_no_garmin_provider_import_and_uses_its_stable_read_boundary() -> (
    ...
    stable = (ROOT / "src/trainlab/analysis/stable_views.py").read_text(
        encoding="utf-8"
    )
...
def test_shadow_report_rejects_unknown_difference_or_payload_shaped_data() -> None:
    with pytest.raises(LegacyReconciliationError, match="difference_invalid"):
        build_legacy_reconciliation_report(
            ...
            end_local_date="2026-07-21",
            generated_at_utc="2026-07-24T00:00:00Z",
            legacy_counts=ReconciliationCounts(0, 0),
            garmin_counts=ReconciliationCounts(0, 0),
            legacy_snapshot_sha256="a" * 64,
            garmin_snapshot_sha256="b" * 64,
            differences=(
                ReconciliationDifference("contains_payload", "activity", 0, 0),
            ),
        )
    with pytest.raises(LegacyReconciliationError, match="count_invalid"):
        build_legacy_reconciliation_report(
            report_id="legacy-shadow-003",
            start_local_date="2026-07-15",
            end_local_date="2026-07-21",
            generated_at_utc="2026-07-24T00:00:00Z",
            legacy_counts=ReconciliationCounts(-1, 0),
            garmin_counts=ReconciliationCounts(0, 0),
            legacy_snapshot_sha256="a" * 64,
            garmin_snapshot_sha256="b" * 64,
        )
