...
def test_catalog_is_versioned_complete_and_lints_against_pinned_client() -> None:
    ...
    expected = {
        "user_profile", "user_profile_settings", "devices", "primary_device", "device_settings", "device_last_used", "personal_records",
        "user_summary", "steps", "floors", "heart_rates", "rhr", "hydration", "respiration", "spo2", "intensity_minutes", "stress", "all_day_events", "sleep", "lifestyle", "hrv", "body_battery", "body_battery_events", "body_composition", "weigh_ins", "blood_pressure", "training_readiness", "max_metrics", "lactate_threshold", "training_status", "running_tolerance", "endurance_score", "hill_score", "race_predictions", "fitness_age", "cycling_ftp", "menstrual_day", "menstrual", "pregnancy", "nutrition_food_log", "nutrition_meals", "nutrition_settings", "activity_inventory", "activity_summary", "activity_fit", "activity_details_fallback", "activity_splits", "activity_typed_splits", "activity_split_summaries", "activity_exercise_sets", "activity_hr_zones", "activity_power_zones", "activity_weather", "activity_gear",
    }
...
def test_catalog_methods_and_argument_shapes_are_staticly_compatible() -> None:
    ...
    for spec in RESOURCE_CATALOG.values():
        ...
        if spec.scope in {"daily", "range", "account"} and spec.resource_kind != "device_settings":
            args, kwargs = health_call_arguments(spec, "2026-07-22")
            signature.bind(None, *args, **kwargs)
            continue
        values = {
            "offset": 0, "limit": 100, "activity_id": "7", "ORIGINAL": Garmin.ActivityDownloadFormat.ORIGINAL,
            "maxchart": 2000, "maxpoly": 4000, "device_id": "device-1",
        }
...
def test_ignored_aliases_and_exclusions_never_plan_duplicate_requests() -> None:
    reasons = {
        "get_stats", "get_stats_and_body", "get_stress_data", "get_morning_training_readiness",
        "weekly_steps", "weekly_stress", "weekly_intensity_minutes", "progress_summary",
        "last_activity", "activities_for_date", "badges", "challenges", "social", "goals",
        "workout_templates", "scheduled_workouts", "training_plans", "activity_upload_edit_delete",
        "golf_community", "device_alarms", "device_solar",
    }
...
def test_mode_plans_cursor_semantics_and_field_drift_capability_are_deterministic() -> None:
    ...
    assert endpoint_capability(RESOURCE_CATALOG["menstrual"], empty=True) == "not_enabled"
    assert endpoint_capability(RESOURCE_CATALOG["blood_pressure"], http_status=404) == "not_available"
    assert endpoint_capability(RESOURCE_CATALOG["activity_fit"], adapter_has_method=False) == "not_supported"
    assert endpoint_capability(RESOURCE_CATALOG["get_stats"]) == "ignored_with_reason"


def test_catalog_change_requires_coverage_audit_and_extra_roles_are_catalog_derived() -> None:
    assert catalog_change_audit(CATALOG_VERSION)["coverage_audit_required"] is False
    audit = catalog_change_audit("garmin-v1")
    assert audit == {"previous_version": "garmin-v1", "current_version": CATALOG_VERSION, "coverage_audit_required": True}
    assert EXTRA_ROLES == (
        "splits_json", "typed_splits_json", "split_summaries_json", "exercise_sets_json",
...
def test_every_pinned_get_method_is_planned_or_has_an_explicit_ignore_reason() -> None:
    """Prevent silent 0.3.6 endpoint drift or accidental duplicate fetches."""
    methods = {name for name in dir(Garmin) if name.startswith("get_") and callable(getattr(Garmin, name))}
    planned = {spec.method for spec in RESOURCE_CATALOG.values() if spec.requestable}
    ignored = {name for name, spec in RESOURCE_CATALOG.items() if spec.disposition == "ignored" and spec.ignored_with_reason}
    assert methods <= planned | ignored


def test_transport_uses_catalog_reviewed_health_shapes_with_fake_client(tmp_path: Path) -> None:
    class Client:
        ...
        def get_body_composition(self, start: str, end: str):
            self.calls.append(("body", (start, end), {})); return {"ok": True}

        def get_lactate_threshold(self, *, latest: bool, start_date: str, end_date: str, aggregation: str):
            self.calls.append(("lactate", (), {"latest": latest, "start_date": start_date, "end_date": end_date, "aggregation": aggregation})); return {"ok": True}

        def get_cycling_ftp(self):
            self.calls.append(("ftp", (), {})); return {"ok": True}
    ...
    assert client.calls == [
        ...
        ("lactate", (), {"latest": False, "start_date": "2026-07-22", "end_date": "2026-07-22", "aggregation": "daily"}),
        ("ftp", (), {}),
    ]
...
def test_transport_uses_reviewed_account_and_per_device_shapes(tmp_path: Path) -> None:
    class Client:
        ...
        def get_primary_training_device(self): self.calls.append(("primary", ())); return {"deviceId": "opaque"}
        def get_device_settings(self, device_id: str): self.calls.append(("settings", (device_id,))); return {"softwareVersion": "1"}
        def get_device_last_used(self): self.calls.append(("last", ())); return {"deviceId": "opaque"}
        def get_personal_record(self): self.calls.append(("records", ())); return {"vo2Max": 42}
        def get_cycling_ftp(self): self.calls.append(("ftp", ())); return {"ftp": 200}
        def get_pregnancy_summary(self): self.calls.append(("pregnancy", ())); return {"pregnancyWeek": 12}
    client = Client()
    transport = GarminConnectTransport(None, None, TokenStore(tmp_path / "tokens"), client=client)
    assert transport.fetch_account("primary_device") == {"deviceId": "opaque"}
    assert transport.fetch_account("device_settings", "opaque") == {"softwareVersion": "1"}
    ...
    assert client.calls == [("primary", ()), ("settings", ("opaque",)), ("last", ()), ("records", ()), ("ftp", ()), ("pregnancy", ())]


def test_transport_uses_bounded_range_signatures_without_daily_fanout(tmp_path: Path) -> None:
    class Client:
        class Session:
            def request(self, *args, **kwargs): return object()
        def __init__(self): self.cs, self._api_session, self.calls = self.Session(), self.Session(), []
        def get_lactate_threshold(self, *, latest=True, start_date=None, end_date=None, aggregation="daily"):
            self.calls.append(("lactate", latest, start_date, end_date, aggregation)); return []
        def get_menstrual_calendar_data(self, start: str, end: str):
            self.calls.append(("menstrual", start, end)); return []
    client = Client()
    transport = GarminConnectTransport(None, None, TokenStore(tmp_path / "tokens"), client=client)
