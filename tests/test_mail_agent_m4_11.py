...
from trainlab.mail_agent.recovery import (
    DeliveryCandidate,
    MailRecoveryError,
    RetryPolicy,
    decide_retry,
...
def test_retry_uses_injected_jitter_with_bounded_exponential_backoff() -> None:
    decision = decide_retry(
        error_kind="network", attempt_count=2, now=NOW,
...
def test_429_respects_inline_wait_and_deadline(
    ...
    decision = decide_retry(
        error_kind="rate_limited", attempt_count=1, now=NOW,
...
def test_401_refreshes_once_then_requires_auth_and_403_is_rejected() -> None:
    initial = decide_retry(error_kind="auth", attempt_count=1, now=NOW, deadline=NOW + timedelta(seconds=1))
    repeated = decide_retry(error_kind="auth", attempt_count=1, now=NOW, deadline=NOW + timedelta(seconds=1), auth_refresh_attempted=True)
    forbidden = decide_retry(error_kind="forbidden", attempt_count=1, now=NOW, deadline=NOW + timedelta(seconds=1))
...
def test_deadline_and_budget_stop_retry_without_looping() -> None:
    deadline = decide_retry(error_kind="timeout", attempt_count=1, now=NOW, deadline=NOW)
    budget = decide_retry(error_kind="server", attempt_count=5, now=NOW, deadline=NOW + timedelta(minutes=1))
...
def test_unknown_delivery_requires_exactly_one_run_and_thread_match_and_never_resends() -> None:
    one = reconcile_unknown_delivery(
        expected_run_id="run-1", expected_thread_id="thread-1",
    ...
    assert (one.action, one.provider_message_id, one.resend_allowed) == ("record_sent", "message-1", False)
    assert (none.action, none.code, none.resend_allowed) == ("operator_review", "delivery_not_found", False)
    assert (many.action, many.code, many.resend_allowed) == ("operator_review", "duplicate_delivery_conflict", False)
...
def test_label_only_recovery_never_sends_again() -> None:
    ...
    unknown = recover_label_only(delivery_status="delivery_unknown", provider_message_id="message-1")
    assert (sent.action, sent.code, sent.resend_allowed) == ("apply_label", "label_pending", False)
    assert (unknown.action, unknown.code, unknown.resend_allowed) == ("operator_review", "label_recovery_evidence_missing", False)
...
def test_invalid_jitter_is_rejected_before_scheduling() -> None:
    with pytest.raises(MailRecoveryError, match="mail_recovery_jitter_invalid"):
        decide_retry(error_kind="network", attempt_count=1, now=NOW, deadline=NOW + timedelta(minutes=1), jitter=lambda: 1.0)
