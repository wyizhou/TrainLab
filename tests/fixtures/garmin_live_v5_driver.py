# ruff: noqa
"""One manually gated, redacted Garmin v5-a1 acceptance execution.
...
WORKTREE = Path("/home/dev/Project/.orchestration/worktrees/n14-garmin-live-v5")
PRODUCTION_ROOT = Path("/home/dev/Project")
MARKER_ROOT = Path(__file__).resolve().parent
TASK_PATH = (
    PRODUCTION_ROOT
    / ".orchestration/runs/trainlab-reliability-20260808-v5/tasks/t14-garmin-live.json"
)
RUN_ROOT = TASK_PATH.parent.parent
APPROVED_TASK_SHA256 = (
    "76da4e168b58352679a52278dad6cee33da6b0a59b2ffe94ab41ecf0ceb1c396"
)
...
T12_ARTIFACT_PRODUCER = {
    ...
    "node_run_id": "run-n12-integration-tests-cf-v5",
    "attempt": 1,
...
CANONICAL_PLAN_HASH = (
    "sha256:94562869382e87b00a6417225cba607750914e01b22fd86c78f94ef85f8ecac9"
)
CANONICAL_RUN_ID = "trainlab-reliability-20260808-v5"
FROZEN_CONTROL_MANIFEST_SHA256 = (
    "3a2bd33da720fa0e1f3e8956678a76832f8ae6fadc5c13d481cbf40ae89d9459"
)
MARKER_SHA256 = "2256a40d025ed3a63fb1d11dc067d583837e0569922e08db285cd085c072fcbc"
FROZEN_SOURCE_MANIFEST_SHA256 = (
    "67f9c19bad925cbd411075b9c2c5085f8ffce979276b5eeb42ae27ac16f66923"
)
FROZEN_HEAD_SHA1 = "192a247f997c14e64b668a3aa910a320818325de"
...
def _sha256_file(path: Path) -> str:
    return _stable_stream_sha256(path)
...
def _stable_stream_sha256(path: Path) -> str:
    """Stream a no-follow authority file; never buffer its full contents."""
    ...
    try:
        ...
        while remaining:
            ...
            if not block:
                raise LiveGateError("stable_file_short_read")
            digest.update(block)
            remaining -= len(block)
        if os.read(descriptor, 1):
            raise LiveGateError("stable_file_growth")
...
def _verify_marker(*, execution_locked: bool = False) -> None:
    ...
    siblings = sorted(
        MARKER_ROOT.parent.glob("trainlab-reliability-21-v5-n14-garmin-live-a1")
    )
    if (
...
def _production_quiescence_snapshot(
    ...
    _require_foundation_ready(
        foundation_tool.execute(
            FoundationRequest(
                "status", "garmin-live-v5-a1-quiescence", "2026-08-08T00:00:00Z"
            )
        ),
        ("ready",),
    )
...
def _persist_preflight_stop(failure_code: str) -> None:
    """Persist the only evidence available before TrainLab/auth imports work.

    This writer deliberately has no dependency on TrainLab, auth evidence, a
    token, or an authority pathname.  It is called only after the O_EXCL lock.
    """
    allowed = {
        "preflight_static_gate",
        "preflight_import",
        "preflight_quiescence",
        "preflight_authority",
    }
    if failure_code not in allowed:
        failure_code = "preflight_authority"
    document: dict[str, Any] = {
        "schema_version": "1",
        ...
        "plan_hash": CANONICAL_PLAN_HASH.removeprefix("sha256:"),
        "attempt_id": "trainlab-reliability-21-v5-n14-garmin-live-a1",
        "authority_file_count": 0,
        "authority_byte_count": 0,
        "authority_aggregate_sha256": hashlib.sha256(b"").hexdigest(),
    ...
    unsigned.pop("stop_sha256")
    document["stop_sha256"] = hashlib.sha256(
        json.dumps(
            unsigned,
            allow_nan=False,
            ensure_ascii=False,
    ...
    temporary = ".preflight-stop.tmp"
    descriptor: int | None = None
    try:
        if os.path.exists(MARKER_ROOT / "preflight-stop.json"):
            raise LiveGateError("preflight_stop_exists")
        descriptor = os.open(
            temporary,
        ...
        while written < len(payload):
            ...
            if count <= 0:
                raise OSError("preflight_stop_short_write")
            written += count
        os.fsync(descriptor)
        os.close(descriptor)
        descriptor = None
...
def _execute_once() -> None:
    ...
    preflight_stop_written = False
    preflight_code = "preflight_static_gate"
    ...
    try:
        ...
        try:
            ...
            preflight_code = "preflight_import"
            raise
        # The same-host controlled-execution assumption narrows import TOCTOU:
        # immediately rebind source/RECORD and reject a module outside WORKTREE
        # before any production, token or provider boundary.
        preflight_code = "preflight_static_gate"
        _verify_static_gate()
        module_path = Path(
            sys.modules[GarminCollectionTool.__module__].__file__ or ""
        ).resolve()
        if WORKTREE not in module_path.parents:
            raise LiveGateError("module_origin_invalid")

        preflight_code = "preflight_quiescence"
        ...
        preflight_code = "preflight_authority"
        authority_probe = GarminCollectionTool(production_config)
        try:
            authority_probe._run_live_acceptance_preflight(
                authority_paths={"database": production_config.database_path},
                stop_destination=MARKER_ROOT / "preflight-stop.json",
                plan_id=CANONICAL_PLAN_ID,
                plan_version=CANONICAL_PLAN_VERSION,
                plan_hash=CANONICAL_PLAN_HASH.removeprefix("sha256:"),
                attempt_id="trainlab-reliability-21-v5-n14-garmin-live-a1",
            ...
            preflight_stop_written = (MARKER_ROOT / "preflight-stop.json").exists()
            raise
        preflight_done = True
        facade = Garmin(
            None,
        ...
        try:
            ...
            try:
                _require_foundation_ready(
                    foundation_tool.execute(
                        FoundationRequest(
                            "init", "garmin-live-v5-a1", "2026-08-08T00:00:00Z"
                        )
                    ),
            ...
            requests = (
                SyncRequest("auth", invocation_id="v5-a1-auth"),
                SyncRequest(
                    "incremental",
                    through_local_date="2026-08-06",
                    invocation_id="v5-a1-incremental-1",
                ),
                SyncRequest(
                    "incremental",
                    through_local_date="2026-08-06",
                    invocation_id="v5-a1-incremental-2",
                ),
                SyncRequest(
                    "snapshot",
                    snapshot_local_date="2026-08-07",
                    invocation_id="v5-a1-snapshot",
                ),
                SyncRequest(
                    "audit",
                    health_from_local_date="2026-08-06",
                    through_local_date="2026-08-06",
                    invocation_id="v5-a1-audit",
                ),
            )
            gate.phase = "data"
            previous = "0" * 64
        ...
        if not preflight_done and not preflight_stop_written:
            try:
                _persist_preflight_stop(preflight_code)
                preflight_stop_written = True
            except Exception:
                pass
        raise
    finally:
