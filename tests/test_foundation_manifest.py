...
def request(mode: str) -> FoundationRequest:
    return FoundationRequest(mode=mode, invocation_id="manifest-test", requested_at_utc="2026-07-23T00:00:00Z")


def manifest() -> dict:
    return json.loads((Path(__file__).resolve().parents[1] / "harness/schemas/foundation_schema_manifest.json").read_text())
