...
def create_published_v1_database(root: Path) -> FoundationTool:
    ...
    try:
        ...
        conn.execute("INSERT INTO foundation_state VALUES(1,'ready',?,?,?,?,?)",(1,LEGACY_V1_MANIFEST_SHA256,UTC,UTC,"foundation-v1"))
        conn.execute("INSERT INTO schema_migrations VALUES(1,?,?,?,?)",("foundation_v1",UTC,"foundation-v1",LEGACY_V1_MANIFEST_SHA256))
        conn.commit()
        assert FoundationTool._schema_objects_sha256(conn) == LEGACY_V1_SCHEMA_OBJECTS_SHA256
    finally:
        conn.close()
    db_path.chmod(0o600)
