...
def accepted(run_key: str = "analysis:1:daily:2026-07-24:one") -> dict[str, object]:
    return {
        "run_key": run_key, "subject_id": 1, "mode": "daily", "status": "accepted",
        "artifacts": [
...
def manifest() -> list[dict[str, object]]:
    return [{
        ...
        "source_window": {"start_local_date": "2026-07-23", "end_local_date": "2026-07-24"},
...
def test_daily_pair_inputs_and_relations_publish_together():
    ...
    receipt = AnalysisPublisher(conn, clock=lambda: "2026-07-24T00:00:00Z").publish(
        run_id=1, accepted=accepted(), input_manifest=manifest(),
        run_evidence=evidence(),
    )
    assert set(receipt.artifact_ids) == {"daily_summary", "daily_training_advice"}
    assert conn.execute("SELECT count(*) FROM analysis_artifact_inputs").fetchone()[0] == 1
    ...
    assert conn.execute("SELECT count(*) FROM analysis_artifacts WHERE is_current=1").fetchone()[0] == 2
    assert conn.execute("SELECT status FROM analysis_runs WHERE id=1").fetchone()[0] == "started"
    ...
    stored_input = conn.execute("SELECT source_entity_id,source_revision_id FROM analysis_artifact_inputs").fetchone()
...
def test_publisher_accepts_every_analysis_context_trust_class(trust_class: str):
    ...
    assert conn.execute(
        "SELECT trust_class FROM analysis_artifact_inputs"
    ).fetchone()[0] == trust_class
...
def test_revisions_are_immutable_and_current_switches_with_supersedes():
    ...
    conn.execute("INSERT INTO analysis_runs(id,run_key,subject_id,analysis_kind,status) VALUES(2,'analysis:1:daily:2026-07-24:two',1,'daily','started')")
    conn.commit()
    second = publisher.publish(
        run_id=2, accepted=accepted("analysis:1:daily:2026-07-24:two"),
        input_manifest=manifest(), run_evidence=evidence(),
    )
    assert second.revisions == {"daily_summary": 2, "daily_training_advice": 2}
    assert conn.execute("SELECT is_current FROM analysis_artifacts WHERE id=?", (first.artifact_ids["daily_summary"],)).fetchone()[0] == 0
    assert conn.execute("SELECT count(*) FROM analysis_artifact_relations WHERE relation_type='supersedes'").fetchone()[0] == 2


@pytest.mark.parametrize("stage", ["after_run_evidence", "after_inputs", "after_artifact:daily_summary", "after_artifact:daily_training_advice", "after_relations"])
def test_failure_at_every_transaction_stage_rolls_back(stage: str):
    ...
    with pytest.raises(RuntimeError):
        AnalysisPublisher(conn).publish(
            run_id=1, accepted=accepted(), input_manifest=manifest(),
            run_evidence=evidence(), failpoint=fail,
        )
    assert conn.execute("SELECT count(*) FROM analysis_artifacts").fetchone()[0] == 0
    assert conn.execute("SELECT count(*) FROM analysis_artifact_inputs").fetchone()[0] == 0
...
def test_non_daily_and_bad_manifest_fail_without_side_effects():
    conn = database()
    payload = accepted(); payload["mode"] = "weekly"
    with pytest.raises(AnalysisPublishError):
        AnalysisPublisher(conn).publish(
            run_id=1, accepted=payload, input_manifest=[],
            run_evidence=evidence([]),
        )
    assert conn.execute("SELECT count(*) FROM analysis_artifacts").fetchone()[0] == 0
