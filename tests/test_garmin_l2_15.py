...
from trainlab.garmin_quality import collection_integrity_facts, week_readiness
...
def test_new_completed_coverage_replaces_snapshot_partial(tmp_path):
    path = _database(tmp_path)
    conn = sqlite3.connect(path)
    conn.execute("INSERT INTO resource_coverage(subject_id,provider,resource_kind,local_date,availability_state,record_count,observed_at_utc) VALUES(1,'garmin','steps','2026-07-20','partial',0,'2026-07-20T01:00:00Z')")
    conn.execute("INSERT INTO resource_coverage(subject_id,provider,resource_kind,local_date,availability_state,record_count,observed_at_utc) VALUES(1,'garmin','steps','2026-07-20','fetched',1,'2026-07-21T01:00:00Z')")
    conn.commit(); conn.close()
...
def test_gap_and_activity_stage_block_without_payload_leak(tmp_path):
    path = _database(tmp_path)
    conn = sqlite3.connect(path)
    conn.execute("INSERT INTO garmin_sync_gaps VALUES(1,1,'steps','PRIVATE-HEALTH-PAYLOAD','2026-07-20','2026-07-20','open')")
    conn.execute("INSERT INTO activities VALUES(9,1,'garmin','2026-07-20','active')")
    conn.execute("INSERT INTO source_revisions VALUES(1,'activity_summary',1,'2026-07-20T00:00:00Z')")
    conn.execute("INSERT INTO activity_source_revisions VALUES(9,1,'summary_json',1)")
    conn.commit(); conn.close()
    result = week_readiness(str(path), 1, "2026-07-20")
    assert result.state == "blocked"
    assert {reason.code for reason in result.blockers} >= {"gap_open", "activity_fit_or_fallback_incomplete"}
...
def test_optional_capability_warns_but_required_unavailable_blocks(tmp_path):
    path = _database(tmp_path)
    conn = sqlite3.connect(path)
    conn.execute("UPDATE garmin_resource_capabilities SET capability_state='not_supported' WHERE resource_kind='floors'")
    conn.commit(); conn.close()
    assert week_readiness(str(path), 1, "2026-07-20").state == "ready_with_warnings"
    conn = sqlite3.connect(path)
    conn.execute("UPDATE garmin_resource_capabilities SET capability_state='not_supported' WHERE resource_kind='steps'")
    conn.commit(); conn.close()
...
def test_subject_isolation_and_integrity_facts(tmp_path):
    ...
    conn.execute("INSERT INTO sleep_sessions VALUES(1,1,'2026-07-20T00:00:00Z','2026-07-20T08:00:00Z')")
    conn.execute("INSERT INTO sleep_stages VALUES(1,'2026-07-19T23:59:00Z','2026-07-20T01:00:00Z')")
    conn.commit(); conn.close()
    assert week_readiness(str(path), 1, "2026-07-20").state == "ready"
...
def test_database_failure_is_conservative_and_sanitized(tmp_path):
    ...
    conn = sqlite3.connect(path); conn.execute("DROP TABLE resource_coverage"); conn.commit(); conn.close()
    result = week_readiness(str(path), 1, "2026-07-20")
    assert result.state == "blocked"
