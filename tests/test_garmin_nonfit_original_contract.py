...
def _fit_start() -> str:
    with fitdecode.FitReader(FIT_PATH, check_crc=True) as reader:
        for frame in reader:
            if isinstance(frame, fitdecode.FitDataMessage) and frame.name == "session":
                return next(
                    field.value for field in frame.fields if field.name == "start_time"
...
class NonFitOriginalTransport:
    ...
    def activity_extra(self, _activity_id: str, role: str) -> Any:
        ...
        start_ms = datetime.fromisoformat(self.start.replace("Z", "+00:00")).timestamp() * 1000
        return {
            "activityId": 1,
            "points": [
...
def _setup(
    ...
    tool = GarminCollectionTool(
        GarminConfig(
            ...
            "2026-01-01", request_min_interval_ms=0,
        ),
        transport,
...
def test_nonfit_original_is_retained_without_a_fit_gap_and_later_fit_metadata_can_promote(
    ...
    first = _sync(tool, f"{format_key}-nonfit-first", ("activity_fit", "activity_details_fallback"))
    ...
    repeated = _sync(tool, f"{format_key}-nonfit-repeat", ("activity_fit", "activity_details_fallback"))
    ...
    transport.original = FIT_PATH.read_bytes()
    promoted = _sync(tool, f"{format_key}-fit-promote-after-metadata-change", ("activity_fit",))
