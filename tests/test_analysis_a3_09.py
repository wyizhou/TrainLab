...
from trainlab.analysis.safety_rules import (
    SafetyRuleError,
...
def zone_evidence(
    ...
    *,
    ...
    return {
        ...
        "heart_rate_bpm": None,
        "measurement_method": "provider_profile" if kind == "garmin_sport_zones" else "user_verified",
    }


def baseline(kind, bpm, revision, *, method="provider_profile", reliable="reliable", current=True):
    return {
        "evidence_id": f"e-{revision}",
        "source_kind": kind,
        "source_revision_id": revision,
...
def substitution(*, planned_minutes=60, actual_minutes=30, planned_seconds=3600, actual_seconds=1800):
    return {
...
def test_static_policy_and_schemas_are_versioned_strict_and_self_validating():
    root = Path(__file__).parents[1] / "src/trainlab/analysis"
    policy = json.loads((root / "training_safety_policy.json").read_text())
    schema = json.loads((root / "schemas/training_safety_policy.schema.json").read_text())
...
def test_garmin_beats_user_and_hrr_with_exact_revision_lineage():
    ...
    result = evaluate_training_safety(
        request(running(target_zone=2), zones=evidence)
    )
    ...
    assert set(result["input_revision_ids"]) == {"garmin-zones", "user-zones", "max", "rest"}
...
def test_expired_and_future_garmin_fall_through_to_current_user_zones():
    evidence = (
        ...
        zone_evidence("garmin_sport_zones", "future-garmin", effective="2026-08-01T00:00:00Z"),
        zone_evidence("user_zones", "user-zones", offset=-5),
    )
    ...
    assert {"zone_evidence_expired", "zone_evidence_future", "exact_zone_source_user"} <= reason_codes(result)
...
def test_same_priority_zone_conflict_fails_closed_to_rpe_fallback():
    ...
    result = evaluate_training_safety(
        request(running(course_type="intervals", target_zone=4, prescribed_rpe=8), zones=evidence)
    )
    item = result["primary_items"][0]
...
def test_every_exact_zone_boundary_is_inclusive_contiguous_and_non_overlapping():
    ...
    assert all(left["maximum_bpm"] + 1 == right["minimum_bpm"] for left, right in zip(hrr_zones, hrr_zones[1:]))
...
def test_explicit_red_flags_suspend_training_and_return_formal_rest(kind):
    result = evaluate_training_safety(
        request(running(target_zone=2), zones=(zone_evidence(),), signals=(signal(kind, "red"),))
    )
    assert result["status"] == "modified_for_safety"
    ...
    assert {"red_flag_training_suspended", "professional_evaluation_recommended"} <= reason_codes(result)
    assert result["constraints"]["medical_diagnosis_forbidden"] is True
    assert result["primary_items"][0]["uncertainty"] == "safety_rule_not_medical_diagnosis"


def test_device_anomaly_only_warns_without_changing_or_diagnosing():
    result = evaluate_training_safety(
        request(climbing(), signals=(signal("device_anomaly", "device", origin="device_fact"),))
    )
...
@pytest.mark.parametrize(
    ("item", "code"),
    [
        (running(course_type="steady", target_zone=4, prescribed_rpe=8), "zone45_requires_intervals"),
        (
            interval_running(
                work_intervals=[
                    {
...
def test_zone5_single_interval_and_recovery_are_stricter():
    excessive = interval_running(
        5,
        work_intervals=[
            {"work_seconds": 180, "recovery_seconds": 180, "repetitions": 2, "target_zone": 5}
        ],
    )
...
def test_hidden_high_zone_interval_cannot_hide_behind_lower_or_null_target():
    for item in (
        running(
            ...
            target_zone=2,
            work_intervals=[
                {"work_seconds": 60, "recovery_seconds": 60, "repetitions": 2, "target_zone": 5}
            ],
        ),
        running(
            course_type="intervals",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        running(
            ...
            prescribed_rpe=9,
            work_intervals=[
                {"work_seconds": 60, "recovery_seconds": 60, "repetitions": 2, "target_zone": 3}
            ],
        ),
    ],
)
...
def test_quality_frequency_and_minimum_recovery_fail_closed():
    ...
    frequency = evaluate_training_safety(
        request(interval_running(), zones=(zone_evidence(),), quality=(earlier, another))
    )
...
def test_running_fields_source_revision_stop_conditions_and_bpm_mismatch():
    result = evaluate_training_safety(request(running(target_zone=2), zones=(zone_evidence(),)))
    item = result["primary_items"][0]
    required = {
        "course_type",
        "warmup",
...
def test_climbing_is_conservative_and_recovery_signal_changes_to_rest():
    ...
    blocked = evaluate_training_safety(
        request(climbing(), signals=(signal("forearm_recovery_insufficient", "forearm"),))
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        strength(rationale="Perform three sets of ten repetitions with fifty kilograms"),
...
def test_substitution_same_sport_different_dose_is_not_false_conflict():
    ...
    assert exact["substitution_assessment"]["status"] == "conflicts_with_exact_activity_evidence"
...
def test_deterministic_pure_evaluation_does_not_mutate_log_or_read_runtime(monkeypatch, capsys):
    engine = TrainingSafetyRuleEngine()
    payload = request(
    ...
    monkeypatch.setattr(Path, "read_bytes", lambda *_: (_ for _ in ()).throw(AssertionError("runtime io")))
...
def test_result_validator_rejects_token_shaped_free_text_bypasses(item, field, value):
    ...
    rest_result["primary_items"][0]["evidence"] = [
        "take_ibuprofen_400mg_as_treatment"
    ]
    with pytest.raises(SafetyRuleError, match="controlled_token_invalid"):
