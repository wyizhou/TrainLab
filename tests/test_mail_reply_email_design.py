...
def test_reply_design_binds_only_published_values_and_removes_unset_optional_blocks() -> None:
    value = render()
...
def test_design_fails_closed_for_structured_content_type_errors(structured: object) -> None:
    with pytest.raises(MailRenderError, match="mail_response_structured_content_invalid"):
        render(structured_content=structured)


def test_subject_removes_crlf_replies_and_internal_markers() -> None:
    value = render(original_subject="Re: RE: [TrainLab idempotency: x] run-id: old \r\n  训练安排")
