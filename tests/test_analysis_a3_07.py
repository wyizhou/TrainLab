...
from trainlab.analysis.quality_gate import (
...
POLICY = json.loads(
    (ROOT / "src/trainlab/analysis/quality_policy.json").read_text(encoding="utf-8")
)
        ...
        capabilities = tuple(
            {
                ...
                "resource_kind": kind,
                "capability_state": "supported" if kind in REQUIRED else "not_supported",
        ...
        coverage = tuple(
            {
                "resource_kind": kind,
                "local_date": day,
                "availability_state": "fetched" if kind in REQUIRED else "not_supported",
...
def request(
    ...
    **kwargs: object,
    ...
    return QualityGateRequest(
        route, 1, start, end, as_of_utc, **kwargs  # type: ignore[arg-type]
    )
...
def test_request_requires_canonical_as_of_and_uses_hong_kong_date_at_23z() -> None:
    ...
    assert gate.evaluate(request(as_of_utc=crossing_as_of), source).state == "ready_with_warnings"
...
def test_optional_unavailable_requires_matching_coverage_for_every_day() -> None:
    ...
    assert gate.evaluate(
        request("weekly_summary", week[0], week[-1]), base
    ).state == "ready_with_warnings"
    ...
    missing = replace(
        ...
        coverage=tuple(
            ...
            if not (
                row["resource_kind"] == "sleep"
                and row["local_date"] == week[3]
            )
        ),
    )
...
def test_required_activity_coverage_and_catalog_fail_closed() -> None:
    ...
    assert {"coverage_missing"}.issubset(blocker_codes(gate.evaluate(request(), forged)))
...
def test_future_coverage_with_malformed_local_date_is_ignored_before_date_validation() -> None:
    base = snapshot()
...
def test_every_activity_requires_summary_core_or_fallback_and_inventory_proof() -> None:
    ...
    no_summary = replace(
        base,
        activity_stages=tuple({**row, "summary_ready": 0} for row in base.activity_stages),
    )
...
def test_every_fetched_sleep_date_requires_matching_singapore_end_date() -> None:
    ...
    assert gate.evaluate(
        request("weekly_summary", week[0], week[-1]), complete
    ).state == "ready_with_warnings"
...
def test_plan_reason_lineage_shape_and_subject_ownership_are_strict() -> None:
    ...
    revoked_shape = replace(
        source,
        plan_reasons=tuple(
            {**row, "revoked": True} for row in source.plan_reasons
        ),
    )
    assert "plan_reason_invalid" in blocker_codes(
        gate.evaluate(req, revoked_shape)
    )
    ...
    cross_subject_reason = replace(
        source,
        plan_reasons=tuple(
            {**row, "subject_id": 2} for row in source.plan_reasons
        ),
    )
    assert "subject_identity_invalid" in blocker_codes(
...
def test_reasons_are_sorted_deduplicated_bounded_and_fail_closed_on_overflow() -> None:
    ...
    assert all(len(reason.entity) <= POLICY["max_entity_length"] for reason in result.blockers)
    result.validate()
