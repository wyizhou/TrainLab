...
// AC-007b-1 (C-7): mobile hides the min-width:960px table and renders a card
// list; desktop shows the table with no card list. Neither viewport overflows
// horizontally (the table's in-panel horizontal scroll must not escape the page).
test('mobile shows the activity card list; desktop shows the table', async ({ page }) => {
  await page.setViewportSize(MOBILE)
  await page.goto('/activities')
  await expect(page.getByTestId('page-activities')).toBeVisible()

  // Mobile: the min-width:960px table container is display:none.
  const tableDisplay = await page
    .getByTestId('activity-table')
  ...
  // Desktop: the table is visible and the card list is not rendered visually.
  await page.setViewportSize(DESKTOP)
  const tableDisplayDesktop = await page
    .getByTestId('activity-table')
    .evaluate((el) => getComputedStyle(el).display)
  expect(tableDisplayDesktop).not.toBe('none')
  await expect(page.getByTestId('activity-card').first()).toBeHidden()
