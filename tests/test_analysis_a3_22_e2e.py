...
from trainlab.analysis.status import AnalysisStatusQueryService
...
class _DeliveryRepository:
    ...
    def __init__(self, status: str = "pending") -> None:
        self.state = SimpleNamespace(
            delivery_id=1, subject_id=1, analysis_run_id=1, status=status,
            provider_message_id=None, provider_thread_id=None, error_code=None,
        )
        self.calls: list[str] = []

    def load_pending(self, delivery_id: int):
        assert delivery_id == 1
        return SimpleNamespace(delivery_id=1, subject_id=1, idempotency_key="analysis-delivery:v1:synthetic")
    ...
    def load_rendered(self, delivery_id: int, subject_id: int):
        assert (delivery_id, subject_id) == (1, 1)
        return SimpleNamespace(subject="[TrainLab] synthetic", plain_text="safe", html="<p>safe</p>")
    ...
    def record_delivery_unknown(self, _subject_id: int, _delivery_id: int, **values: object):
        self.calls.append("unknown")
        return self._set("delivery_unknown", **values)

    def record_search_match(self, _subject_id: int, _delivery_id: int, **values: object):
...
class _Gateway:
    def __init__(self, *, failure: Exception | None = None, reconciliation=None) -> None:
...
def _status_request() -> AnalysisRequest:
    return AnalysisRequest.from_dict({
        "schema_version": "1", "mode": "status", "subject_id": "synthetic",
        "invocation_id": None, "run_key": None, "summary_local_date": None,
        "advice_local_date": None, "as_of_local_date": None, "plan_id": None,
        "reason_event_id": None, "effective_local_date": None, "artifact_id": None,
...
def _status_database() -> sqlite3.Connection:
    ...
    INSERT INTO analysis_deliveries VALUES(21, 1, 'delivery_unknown', '2026-07-26T00:02:00Z', 'provider-message', 'provider-thread', 'PRIVATE ERROR');
...
def test_synthetic_unknown_delivery_reconciles_without_a_second_send_or_background_worker():
    ...
    gateway.reconciliation = GmailDeliveryReceipt("already_sent", "synthetic-message", None, "label", NOW)
...
def test_synthetic_status_is_select_only_and_redacts_provider_and_artifact_bodies():
    ...
    receipt = AnalysisStatusQueryService(conn, clock=lambda: NOW).execute(_status_request())
    payload = receipt.as_dict()
    assert receipt.status == "succeeded"
    assert all(statement.lstrip().upper().startswith("SELECT") for statement in statements)
    encoded = json.dumps(payload, sort_keys=True)
    for secret in ("PRIVATE", "provider-message", "provider-thread", "structured_content_json", "user_visible_text"):
...
def test_synthetic_publish_fault_rolls_back_before_delivery_seed():
    ...
    """)
    accepted = {"run_key": "analysis:synthetic:daily:2026-07-25:one", "subject_id": 1, "mode": "daily", "status": "accepted", "artifacts": [
        {"artifact_kind": "daily_summary", "period": {"start_local_date": "2026-07-25", "end_local_date": "2026-07-25"}, "structured_content": {"summary": "ok"}, "user_visible_text": "摘要"},
        {"artifact_kind": "daily_training_advice", "period": {"start_local_date": "2026-07-26", "end_local_date": "2026-07-26"}, "structured_content": {"advice": "rest"}, "user_visible_text": "建议"},
    ]}
    manifest = [{"ordinal": 0, "input_role": "quality_state", "source_entity_type": "data_quality_issue", "source_entity_id": "quality:1", "source_revision_id": "quality:synthetic", "source_window": {"start_local_date": "2026-07-25", "end_local_date": "2026-07-26"}, "input_sha256": "a" * 64, "trust_class": "derived_statistic"}]
    snapshot = {"input_manifest": manifest}
    canonical = json.dumps(snapshot, sort_keys=True, separators=(",", ":"))
    evidence = {"harness_version": "synthetic", "input_schema_version": "1", "output_schema_version": "1", "context_snapshot_json": snapshot, "context_snapshot_sha256": __import__("hashlib").sha256(canonical.encode()).hexdigest(), "generator_metadata_json": {"adapter": "fake"}}
    with pytest.raises(RuntimeError, match="fault"):
        AnalysisPublisher(conn).publish(run_id=1, accepted=accepted, input_manifest=manifest, run_evidence=evidence, failpoint=lambda stage: (_ for _ in ()).throw(RuntimeError("fault")) if stage == "after_artifact:daily_summary" else None)
