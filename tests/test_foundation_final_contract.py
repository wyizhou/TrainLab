...
def test_delivery_trust_plan_and_reconcile_contracts(tmp_path: Path) -> None:
    ...
    with pytest.raises(sqlite3.IntegrityError): db.execute("INSERT INTO training_plans(subject_id,analysis_artifact_id,plan_start_local_date,plan_end_local_date,timezone,status,created_at_utc) VALUES(?,?,?,?,?,?,?)", (subject, daily, "2026-01-01", "2026-01-07", "Asia/Hong_Kong", "proposed", "2026-01-01T00:00:00Z"))
    db.execute("INSERT INTO training_plans(subject_id,analysis_artifact_id,plan_start_local_date,plan_end_local_date,timezone,status,created_at_utc) VALUES(?,?,?,?,?,?,?)", (subject, weekly, "2026-01-01", "2026-01-07", "Asia/Hong_Kong", "proposed", "2026-01-01T00:00:00Z"))
