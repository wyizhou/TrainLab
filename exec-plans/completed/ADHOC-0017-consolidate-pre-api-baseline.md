# 执行计划：CONSOLIDATE-20260914：单目录整合与 API 改造前旧版基线
...
| 阶段编号 | 状态 | 预期成果 | 依赖 |
| --- | --- | --- | --- |
| HISTORY | [completed] | 本次执行步骤（唯一现行状态）；保留原逐项成果/未完成边界 | 原前置要求及总计划同编号依赖；未另列的依赖不新增 |

| 任务编号 | 所属阶段 | 状态 | 输入输出及错误边界 | 依赖 | 检查方法 | 结果与证据（检查命令或人工方式、退出结果及关键证据） |
| --- | --- | --- | --- | --- | --- | --- |
| D-01 | HISTORY | [completed] | D-01 数据/令牌收尾；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L45](#原定义-l45)；[本任务原定义L115](#原定义-l115) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L45](#原定义-l45)；[本任务原定义L115](#原定义-l115) | 原记录：范围：Main已单独保全11份现用/旧版区分的令牌和配置，生成data/README及清单；核既有FIT目标和主DB保护索引，不改旧代码；原完成范围不扩大；对应原执行计划 L34 |
| D-02 | HISTORY | [completed] | D-02 数据收尾独立核对；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L45](#原定义-l45)；[本任务原定义L54](#原定义-l54) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L45](#原定义-l45)；[本任务原定义L54](#原定义-l54) | 原记录：范围：全新只读Validator 9a5c7d35按VC-002返回PASS，七项标准与80项最终检查通过；不外推旧产品/数据库一致性/在线认证；原完成范围不扩大；对应原执行计划 L35 |
| D-03 | HISTORY | [completed] | D-03 交付；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L54](#原定义-l54) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L54](#原定义-l54) | 原记录：范围：Main复核21项受审私有文件与11对源/目标，保存独立证据、更新记录及归档；不提交/删除；原完成范围不扩大；对应原执行计划 L36 |
...
| --- | --- | --- | --- | --- |
| ISSUE-L123 | 原要求/观察/裁决逐条记录；见本行源原文，不将规则或下一动作当发生事实 | 原行未单列修法则不推断；全部尝试/动作保留于右列及原上下文 | 原行未给统一累计值；保留原次数表述，不置0或按索引条数计次数 | &#124; C-06V1 保护缺口独立复核 &#124; done &#124; 全新只读Validator返回INCONCLUSIVE：保护目标SHA/权限适用检查通过、未见仅副本丢对象，但有限Git候选、DB安全/分类/链接载体边界未闭合；非C-04全产品验收 &#124;；原件L123 |
| ISSUE-L149 | 原要求/观察/裁决逐条记录；见本行源原文，不将规则或下一动作当发生事实 | 原行未单列修法则不推断；全部尝试/动作保留于右列及原上下文 | 原行未给统一累计值；保留原次数表述，不置0或按索引条数计次数 | - blocker_type：`EVIDENCE_PENDING`；诊断状态：`not_triggered`。首次保护结果存在恢复和分类未知，C-06V1已独立复现为INCONCLUSIVE且无新增FAIL；尚无两种修法/三轮不收敛，不触发Failure Analyst。任何清理/覆盖/提交仍暂停。；原件L149 |
| ISSUE-L176 | 原要求/观察/裁决逐条记录；见本行源原文，不将规则或下一动作当发生事实 | 原行未单列修法则不推断；全部尝试/动作保留于右列及原上下文 | 原行未给统一累计值；保留原次数表述，不置0或按索引条数计次数 | - 首次完整派发key `consolidate-validator-c06v1`；workflow `09dfa15d-db7c-4002-acca-0cc46b09292e`、child `4cba20c5-39d6-41e1-b223-cc0acdf1b131` 已完成；报告 `/Users/lucas/.pi/agent/sessions/--Volumes-DiskOther-Code-TrainLab--/subagent-artifacts/outputs/09dfa15d-db7c-4002-acca-0cc46b09292e/consolidate-validator-c06v1.md`。裁决 INCONCLUSIVE：71,046保护目标SHA匹配、data权限/ignore通过；主Git fsck异常源/raw/restore一致且有有限当前候选；DB安全分类、链接/载体边界仍未知。报告不是C-06完成、不是清理或提交授权。；原件L176 |
...
以下原VC-001范围已由用户批准的VC-002收窄替代。它的未完成项不计入VC-002完成范围，也不自动恢复；保留原编号、原状态含义、结果和失败，不能把旧未完成任务标成已通过。

| 阶段编号 | 状态 | 预期成果 | 依赖 |
| --- | --- | --- | --- |
| ORIGINAL-2 | [exec] | 已被VC-002替代的原VC-001范围（非当前适用阶段）；保留原逐项成果/未完成边界 | 原前置要求及总计划同编号依赖；未另列的依赖不新增 |

| 任务编号 | 所属阶段 | 状态 | 输入输出及错误边界 | 依赖 | 检查方法 | 结果与证据（检查命令或人工方式、退出结果及关键证据） |
| --- | --- | --- | --- | --- | --- | --- |
| C-01 | ORIGINAL-2 | [completed] | C-01 全新 Planner 首次拆解；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：child f4ee7d58-2917-4139-af84-cdcf7f7dc82c 完成；实际报告已完整读取，规划不是实施验收；原完成范围不扩大；对应原执行计划 L119 |
| C-02 | ORIGINAL-2 | [completed] | C-02 Main 审核及冻结；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：本节 VC-001；接受先保护/合成回归/保全/状态/产品/退役/验证/清理/提交的串行依赖，不接受额外产品命令或自动换治理要求；原完成范围不扩大；对应原执行计划 L120 |
| C-03 | ORIGINAL-2 | [plan] | C-03 保护及串行实施（汇总）；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：C-06–12全部完成后更新，不作为第二写入任务；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L121 |
| C-06 | ORIGINAL-2 | [exec] | C-06 来源冻结及保护副本；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L164](#原定义-l164) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收)；[本任务原定义L164](#原定义-l164) | 原记录：输入、产物与门禁：Developer已返回PARTIAL；已复制原件和Git元数据，主隔离fsck异常、DB/分类/恢复语义未闭合；所有源仍留原地；仅保留已开展但未完成完整交付的原范围；暂停/取消不恢复；对应原执行计划 L122 |
| C-06V1 | ORIGINAL-2 | [completed] | C-06V1 保护缺口独立复核；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：全新只读Validator返回INCONCLUSIVE：保护目标SHA/权限适用检查通过、未见仅副本丢对象，但有限Git候选、DB安全/分类/链接载体边界未闭合；非C-04全产品验收；原完成范围不扩大；对应原执行计划 L123 |
| C-06R1 | ORIGINAL-2 | [completed] | C-06R1 有限保护缺口补齐；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：Developer返回PARTIAL：只写ignored data/recovery新批次，补齐有限缺失blob候选、6041项DB/sidecar字节保护和分类、嵌套Git形态证据；仍不声称DB一致性/整体C-06完成；原完成范围不扩大；对应原执行计划 L124 |
| C-06R1V | ORIGINAL-2 | [completed] | C-06R1V 有限补齐独立复核；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：全新只读Validator对C-06R1局部证据PASS；PASS仅限ignored证据根记录与字节层证据，不放行C-06/C-04完成、清理或提交；原完成范围不扩大；对应原执行计划 L125 |
| C-07 | ORIGINAL-2 | [completed] | C-07 合成迁移回归先行；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：已新增合成保护helper、fixture和contract测试；py_compile/manual harness/diff-check通过，pytest/ruff/mypy因当前解释器缺模块未运行；C-07V局部PASS；原完成范围不扩大；对应原执行计划 L126 |
| C-07V | ORIGINAL-2 | [completed] | C-07V 合成迁移回归独立复核；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：全新只读Validator对C-07三文件局部PASS；完整pytest/ruff/mypy因环境缺模块仍未知，不冒充全质量门；原完成范围不扩大；对应原执行计划 L127 |
| C-08 | ORIGINAL-2 | [exec] | C-08 有界载体清查及FIT保全；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：Developer返回COMPLETE：13116 FIT候选/566唯一SHA保全到ignored data/fit，manifest和audit已生成；待全新只读Validator复核，不表示业务导入完成；仅保留已开展但未完成完整交付的原范围；暂停/取消不恢复；对应原执行计划 L128 |
| C-08V | ORIGINAL-2 | [completed] | C-08V FIT保全独立复核；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：全新只读Validator对C-08保全证据PASS：13116候选/566唯一SHA、data/fit目标、manifest、权限/ignore和源边界可复核；仅限字节保全，不是业务导入；原完成范围不扩大；对应原执行计划 L129 |
| C-09 | ORIGINAL-2 | [exec] | C-09 认证/配置/状态搬迁；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | C-08 | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原记录：输入、产物与门禁：VC-001整套搬迁停止，数据收尾转VC-002/D-01； 依赖C-08；按用户澄清优先保全 data/verification、必要令牌/配置/goal/email和状态账本事实；不合并冲突库，不重新签旧SHA或恢复真实调用；仅保留已开展但未完成完整交付的原范围；暂停/取消不恢复；对应原执行计划 L130 |
| C-10 | ORIGINAL-2 | [plan] | C-10 选择性产品整合；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：历史目标不再续跑； 用户澄清旧代码不重要；除非后续明确要求，不再做旧产品逐项修复/整合，只保留必要数据依赖和事实记录；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L131 |
| C-11 | ORIGINAL-2 | [plan] | C-11 场景映射及退役文档；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：历史目标不再续跑； 旧代码收尾降级为必要记录；不再反复验证旧流程，必要时只记录哪些旧能力/资料已被数据保护覆盖；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L132 |
| C-12 | ORIGINAL-2 | [plan] | C-12 自检与无外围依赖副本；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：历史目标不再续跑； 聚焦 data/FIT/令牌/配置保护、ignore/权限/manifest和无提交/无服务调用事实；不宣称旧产品完整质量门通过；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L133 |
| C-04 | ORIGINAL-2 | [plan] | C-04 全新独立验证及Main验收；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | C-12 | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：依赖C-12；只给冻结合同/规则/当前快照，不给Planner推理或实施辩护；完整适用门与恢复核验；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L134 |
| C-13 | ORIGINAL-2 | [plan] | C-13 精确清理；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | C-04 | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：依赖C-04；Main逐路径再核源与目标，健康/必要恢复资料留主项目，语义改变重新验证；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L135 |
| C-14 | ORIGINAL-2 | [plan] | C-14 精确本地基线；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | C-13 | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：依赖C-13；Main白名单暂存与本地提交，不推送，核对真实commit/tree；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L136 |
| C-05 | ORIGINAL-2 | [plan] | C-05 清理及交付（汇总）；边界不超出原目标/授权；定义：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 原表未单列；沿用原功能前置条件，不按编号新增依赖；[原明示前置/依赖及对应时点](#原任务定义与验收) | 核对原记录的验证方式/实际结果及源文件同编号验收；本轮不执行历史命令；验收依据：[原目标、输入输出、排除范围及错误/验收定义](#原任务定义与验收) | 本项待实施/验收；原栏：输入、产物与门禁：C-13/14事实完成后归档，不预写删除/提交/PASS；原待实施项；按原文区分历史已发生结果与待办，不把预期写成通过；对应原执行计划 L137 |
...
来源标题：2026-09-14 本次边界与恢复偏差；原件L38–L44。
...
来源标题：D-01 实际结果与D-02派发；原件L45–L53。
...
来源标题：D-02/D-03 最终交付事实（2026-09-14）；原件L54–L63。
...
来源标题：历史：VC-001 已确认目标与边界（已被收窄，非本轮指令）；原件L64–L73。
...
来源标题：历史冻结合同 VC-001（superseded，以下原文不改写）；原件L74–L114。
...
来源标题：历史工作分解（停止驱动本轮；以D-01–03为准）；原件L115–L138。
...
来源标题：C-06 实际结果与待复核缺口；原件L164–L183。
...
来源标题：Main 审核决策与分步边界；原件L184–L193。
...
- [不可变原件快照](../evidence/ADHOC-0029/history.zip)，条目`docs/exec-plans/completed/ADHOC-0017-consolidate-pre-api-baseline.md`，原SHA-256：`9d49196475440256f20d32835cd2db304d2829e51d128726863df22c0f6858fe`。
