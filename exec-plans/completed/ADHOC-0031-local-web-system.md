...
- 不新增通用聊天或永久历史留存；用户已批准真实请求及活动每日4点自动生成，不再视为未获收费许可。尚未决定的选数/补跑/周触发策略不得猜测；合成与真实检查分别记录。
...
| D31-03A | 每日4点的活动选数范围，以及错过/迟到/失败如何补做 | 只暂停自动批次选取和补跑，不影响合成底座/存储 | 待用户，4点时刻不重问 |
| D31-03B | 周报显式还是自动时点；有活动但缺活动总结时等待或先补齐 | 只暂停周自动触发/缺报策略，不影响空周存储及齐全输入服务 | 待用户，无活动固定结果不重问 |
| D31-02A | 不限请求是否也取消旧records周累计20次；若保留其统计周口径 | 暂停未明确配额政策及真实接线，不影响全量查询核心/合成解析 | 待用户，不重问AI收费/数据许可 |
...
| 0031-T2 | B0 | [completed] | 已确认设计/决策→统一DTO、JSON、错误、时间/重复、授权/预算/容量合同；可新增配置表记录系统参数；不能新加未授权业务列、静默截断或改验收 | 0031-T1；在各旧T1实施前冻结 | 准则逐项对齐、正常/错误/边界预期与接口一致性核对 | 身份/时间一致性及公共合同缺口在第二轮修复后复验通过；关联V31-B0-006的历史停止及累计保留。D31-03A/B、D31-02A仍显式未配置，只暂停后续业务政策，不影响B1消费合同。Validator独立探针覆盖时间证据、消息身份、默认123、报告保护、路径边界；主Agent复核通过。 |
...
| 0031-T5 | B5 | [completed] | 角色/两索引/事实/运动总结/历史/当前对话→三模式上下文和可信权限→报告服务；日报无业务副作用 | B2/B3、0031-T4、D31-03 | 上下文快照、资料白名单、会话隔离、统一T与缺报、预算/缓存一致性 | B5修复复验通过；D31-03A/B自动策略仍不猜 |
...
- 下一动作：本地/合成范围已完成并可等待用户决定是否授权真实AI/Garmin/浏览器人工8080检查、提交/推送/PR；当前不自动执行。
- 暂停原因：无当前产品暂停；D31-03A/B、02A仍为后续自动批次/周触发/真实records配额政策的待决范围，本次未实现且未阻塞本地/合成验收。
- 恢复条件：已满足。B1修复尝试2、公共合同累计尝试3、环境阻塞1/安装恢复1均保留；本轮不增加产品失败轮次，不授权额外代码修复或Git交付。
- PR与交付情况：本功能仅创建本地功能分支，未暂存/提交/推送/创建PR。旧PR #20合并是已完成的前一轮操作，不因此授权本功能自动合并。
...
2026-09-16：Planner-v2及workflow已结束，主Agent采纳模块所有权、目录A、统一Schema、时间/身份/报告/权限/一致视图协议并记录[分条主审及首轮修复范围](../evidence/ADHOC-0031/parent-contract-review-v2.md)。更正先前“全部决策闭合”的宽泛表述：已确认事项不撤销，只保留D31-03A/B、02A剩余分支；允许B1在其实际前置受验后推进，不能虚标整个B0/T2完成。准备派发新Developer，未开始修复时计数仍为首次1、修复失败0。
