...
| 阶段编号 | 状态 | 预期成果 | 依赖 |
| --- | --- | --- | --- |
| S1 | [completed] | 按本文目标准备/实施，业务边界不变 | 原用户目标及对应前置条件；ADHOC-0024/0025/0026已完成 |

| 任务编号 | 所属阶段 | 状态 | 输入输出及错误边界 | 依赖 | 检查方法 | 结果与证据（检查命令或人工方式、退出结果及关键证据） |
| --- | --- | --- | --- | --- | --- | --- |
| T1 | S1 | [completed] | 落实主键/重复保存、时间格式/缺失、文本空值/容量及宿主写入边界；不超出本文目标/错误边界 | ADHOC-0024/0025/0026已完成 | 保持三列及全文本目标，按新版AGENTS审核实施拆解 | Developer实现并经独立复验通过 |
| T2 | S1 | [completed] | 先固定合成Schema及报告往返用例；不超出本文目标/错误边界 | T1 | 正常/错误/边界输入输出明确 | 合成测试与Validator探针通过 |
| T3 | S1 | [completed] | 实现建表和受控保存/读取，配套本地测试/文档；不超出本文目标/错误边界 | T2 | 与FIT底座共存，不新增AI工具 | 固定62文件快照独立验证通过 |
| T4 | S1 | [completed] | 完整适用门禁与全新独立验证；不超出本文目标/错误边界 | T3 | 存储交付，不冒称真实报告生成已完成 | [B3 Validator](../evidence/ADHOC-0031/validator-b3.md)通过；不代表真实AI生成/调度接线 |

## 当前检查点

- 工作目录与分支：项目根；当前分支`work/adhoc-0031-local-web-system`，HEAD `9db8bb1dda5922e85fe42581e27d2428dbc9f195`，原迁移分支仅见历史证据。
- 验收要求与受验版本及未提交改动：ADHOC-0024/0025/0026已完成；当前存在未提交产品实现和协调记录，B3 Developer须先核对并保护无关改动。不得把迁移HEAD当作原产品受验版本。
- 最近完成：B3 Validator通过，固定62文件快照`3b476b4bb973df281bfd3f7143d7414f599fa2f09f67150a0ddef0e208bd8c95`；pytest 171通过、ruff/mypy/compile/架构门及独立报告探针通过。报告见[ADHOC-0031 validator-b3](../evidence/ADHOC-0031/validator-b3.md)。
- 下一动作：本功能完成，随[ADHOC-0031](ADHOC-0031-local-web-system.md)进入B4；活动总结生成与每日4点自动触发接线随B5/B4，不在本次冒称完成。
...
- 本任务聚焦表及完整报告的保存/读取边界。ADHOC-0031已授权本地AI生成、Prompt/Provider配置、宿主接线和活动总结按中国时区每日凌晨4点自动触发；邮件/PDF不在本轮执行授权内。用合成文本验存储不等于真实AI闭环已交付，真实AI检查单列证据。
