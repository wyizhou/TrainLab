...
| 阶段编号 | 状态 | 预期成果 | 依赖 |
| --- | --- | --- | --- |
| S1 | [completed] | 按本文目标准备/实施，业务边界不变 | 原用户目标及对应前置条件；ADHOC-0024已完成 |

| 任务编号 | 所属阶段 | 状态 | 输入输出及错误边界 | 依赖 | 检查方法 | 结果与证据（检查命令或人工方式、退出结果及关键证据） |
| --- | --- | --- | --- | --- | --- | --- |
| T1 | S1 | [completed] | 落实目录、规则/本地检查对齐、JSON细则、容量与宿主边界；不超出本文目标/错误边界 | ADHOC-0024已完成 | 保持已确认接口，按新版AGENTS审核实施拆解 | Developer实现并经独立复验通过 |
| T2 | S1 | [completed] | 合成测试固定全量与拒绝预期；不超出本文目标/错误边界 | T1 | 测试先行，不读取正式数据 | 新增测试和Validator语义探针通过 |
| T3 | S1 | [completed] | 实现唯一按ID只读工具；不超出本文目标/错误边界 | T2 | 仅一个工具、仅activity_id参数、全量返回 | 固定59文件快照独立验证通过 |
| T4 | S1 | [completed] | 全面检查与全新独立验证；不超出本文目标/错误边界 | T3 | 离线能力交付，实际API另授权 | [B2 Validator](../evidence/ADHOC-0031/validator-b2.md)通过；不代表真实Provider接线 |

## 当前检查点

- 工作目录与分支：项目根；当前分支`work/adhoc-0031-local-web-system`，HEAD `9db8bb1dda5922e85fe42581e27d2428dbc9f195`，原迁移分支仅见历史证据。
- 验收要求与受验版本及未提交改动：ADHOC-0024已在56文件快照上通过；当前存在未提交产品实现和协调记录，B2 Developer须先核对并保护无关改动。不得把迁移HEAD当作原产品受验版本。
- 最近完成：B2 Validator通过，固定59文件快照`8b3f7c0349cf2194abef6bb2e39913f3fa6dbd8ce2924bf378ea6ecf96864114`；pytest 163通过、ruff、B2相关mypy、compile/架构门、前端回归和语义安全探针通过。报告见[ADHOC-0031 validator-b2](../evidence/ADHOC-0031/validator-b2.md)。
- 下一动作：本功能完成，随[ADHOC-0031](ADHOC-0031-local-web-system.md)进入B3；真实AI/API接线仍属后续阶段。
...
工具名确定为get_running_records，Python脚本按ADHOC-0031目录放在source/skills相关运行脚本或共享基础设施中，并由source/tools/README.md索引。外部参数**只有activity_id**；返回该受授权跑步活动的全部采样记录。
