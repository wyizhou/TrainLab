...
- 迁移说明：设计已确认；B1首轮修复复验及主复现失败（12失败/79通过）的历史保留。用户授权的一轮001～003有界修复已返回，56文件独立复验曾因安装超时无法判断；用户用`resume`授权仅恢复验证环境后，全新Validator复验同一56文件通过，主Agent核快照并复跑pytest 157通过及架构门通过。当前协作规则仅以[AGENTS.md](../../AGENTS.md)为准；原合同及旧规范在快照中仅作历史来源，不继续生效为第二套流程。

## 阶段与任务

| 阶段编号 | 状态 | 预期成果 | 依赖 |
| --- | --- | --- | --- |
| S1 | [completed] | 按本文目标准备/实施，业务边界不变 | 原用户目标及对应前置条件；ADHOC-0031 B0目录/JSON缺口与B1同批修复并复验通过 |

| 任务编号 | 所属阶段 | 状态 | 输入输出及错误边界 | 依赖 | 检查方法 | 结果与证据（检查命令或人工方式、退出结果及关键证据） |
| --- | --- | --- | --- | --- | --- | --- |
| T1 | S1 | [completed] | 落实目录、规则/本地检查对齐、JSON细则、容量与重解析边界；不超出本文目标/错误边界 | ADHOC-0031 B0底座与T2公共合同 | 保持已确认设计，按新版AGENTS审核实施拆解 | 第二轮目录门/身份与时间一致性修复后，全新Validator复验通过；主Agent核快照、pytest及架构门通过 |
| T2 | S1 | [completed] | 合成测试先行；不超出本文目标/错误边界 | T1 | 正常/缺失/重复时间/长活动/原生分段与采样分别验证 | Validator按原锁文件恢复环境后运行157项pytest通过/2警告，并用独立FIT/SQLite探针覆盖正常、错误、边界；主Agent复跑157项通过 |
| T3 | S1 | [completed] | 实现建库、解析与两表整场写入，退出受影响旧路径；不超出本文目标/错误边界 | T2 | 代码/Schema/入口/规则/本地测试/文档同批 | 完整解析与跨层校验第二轮修复已通过复验；默认123、幂等、维护/冲突、报告保护和config共存回归通过 |
| T4 | S1 | [completed] | 完整门禁及全新独立验收；不超出本文目标/错误边界 | T3 | 绑定当前快照，未接API不能宣称业务上线 | 首次失败历史保留；56文件恢复复验通过，报告见[ADHOC-0031恢复复验](../evidence/ADHOC-0031/validator-b1-fix-r2-resume.md)。主Agent复核通过；本项完成不等于B2采样工具或Web/API上线 |

## 当前检查点

- 工作目录与分支：项目根；`work/adhoc-0031-local-web-system`。旧格式迁移分支仅为历史来源。
- 验收要求与受验版本及未提交改动：当前HEAD `9db8bb1dda5922e85fe42581e27d2428dbc9f195` 加未提交产品；[B1第二轮修复清单](../evidence/ADHOC-0031/b1-fix-r2-review-snapshot.json)记录56文件及SHA `5cb22a1f1950c1e73c0624c2ac2981f090c246738dc53f62eb13f9a5996149d8`。旧迁移基准不是本次产品受验版本。
- 前次复验：首轮修复[独立复验](../evidence/ADHOC-0031/validator-b1-fix-r1.md)和[主Agent另副本复现](../evidence/ADHOC-0031/parent-b1-fix-r1-repro.json)一致：12失败/79通过，退出1；53文件SHA未变。原92项/静态通过与首次18失败等证据不改写。
- 第二轮修复交接：workflow `1ed6a9eb-a527-4b39-9d28-e0a1d3b79a2b`及Developer均结束，[主交接收据](../evidence/ADHOC-0031/parent-b1-fix-r2-intake.md)核56文件、28份安装内容一致，主复跑157项与适用静态门通过；旧编译缓存导致的初次架构失败和隔离后恢复记录均保留。
- 最近完成：用户用`resume`授权仅恢复验证环境后，全新Validator workflow `921c5752-55db-4b80-9272-9730a8a3c927`、child `528d8a7b-8f40-4999-a973-3ebb5717f65e`完成，[结论通过](../evidence/ADHOC-0031/validator-b1-fix-r2-resume.md)：隔离依赖安装退出0，pytest 157通过/2警告，ruff/mypy/compile/架构门、前端抽查和8个FIT/SQLite探针通过；56文件快照聚合SHA一致。
- 下一动作：ADHOC-0024当前功能验收完成；按ADHOC-0031串行顺序可准备B2（ADHOC-0025/0026）。当前不改产品、不提交/推送/创建PR。
- 暂停原因：无当前ADHOC-0024暂停；E31-B1-001已恢复。D31-03A/B、02A仍属后续业务政策待决，不阻塞本功能已验收范围。
- 恢复条件：已满足。B1修复2、公共合同累计3、环境阻塞1/安装恢复1保留；不新增产品失败轮次，也不授权额外代码修复。
- PR 与交付情况：当前功能验收完成但仍未提交/推送/创建本功能PR；用户新授权的旧CI退役及当前公开工作区Git/PR交付由[ADHOC-0030](ADHOC-0030-retire-ci-and-publish.md)记录实际结果。原迁移及历史Git事实不改写。

## 问题记录

| 稳定问题编号 | 对应要求与实际问题 | 已尝试修法 | 累计失败次数 | 证据及处理结果 |
| --- | --- | --- | --- | --- |
| E31-B1-001 | 第二轮验证依赖安装被外层200秒超时中断 | 用户`resume`后恢复验证环境并新派全新只读Validator | 环境阻塞1、安装恢复1，不计产品失败 | [恢复复验](../evidence/ADHOC-0031/validator-b1-fix-r2-resume.md)通过 |
| V31-B1-001～006 | 首轮001/002/003剩余问题、004/005/006已测通过；第二轮恢复复验覆盖关联回归 | 首轮修复已复验失败；第二轮有界修复并恢复复验通过 | B1首次失败1、修复2、首轮复验失败1；003关联B0-006累计修复3，不清零；各项细分见0031 | [恢复复验](../evidence/ADHOC-0031/validator-b1-fix-r2-resume.md)及主复核通过；稳定编号及主记录由0031持续维护 |

## 当前唯一有效设计

- 本任务负责states/data.db的FIT基础层：activities十二列、records四列，解析及入库；分段在activities.segments_json，不另建分段表。全库还规划activties_report、weekly_report两张报告表，分别由[ADHOC-0027](../active/ADHOC-0027-activity-ai-report.md)与[ADHOC-0028](../active/ADHOC-0028-weekly-ai-report.md)负责，不再限制全库只能两表。
...
- 新目标替代旧A-021中不批量秒级落库/窗口细读等冲突条款；旧编号只记录批准来源。当前FIT基础层已通过本轮功能验收；后续采样工具、报告、同步、AI与Web/API仍按ADHOC-0031后续阶段验收。
...
目录已由ADHOC-0031主审核采用：产品在source/，运行脚本在source/skills/<skill>/scripts/，共享基础设施在source/skills/_shared/scripts/，前端source/frontend，默认输出source/skills/local-web/web，能力索引source/tools/README.md。不安装到全局或仓库.venv；产品依赖随实施合同和本地检查同批落实。
...
segments_json承载默认给AI的第3类原生分段；sensors_json保留设备/字段定义等非采样说明，不重复存全量record数组。以下是已确认设计；当前Schema、合成数据库及解析入库已通过本功能验收，正式业务接线仍属ADHOC-0031后续阶段。
