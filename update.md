...
## 2026-08-07：下游进程启动瞬时失败保护

- 09:00 morning workflow 的采集与当日快照成功，但 Garmin audit 在子进程尚未创建时遇到 `process_start_failed`。下游边界现仅对进程创建前的瞬时 OSError 做两次短延迟重试；路径缺失、权限不足等永久错误仍立即失败，已启动的同步、分析或邮件进程绝不因此重试。

## 2026-08-09：新增长期协商约定与 Garmin 在线验收边界

- 新增根目录 `AGREEMENTS.md`，用于只追加地记录用户与 Agent 已明确确认、后续工作必须遵守的长期操作约定；禁止记录凭据、个人健康数据或其他敏感内容。
- `AGENTS.md` 现在要求所有 Agent 在开始任务前同时读取 `harness/shared/HARNESS.md` 和 `AGREEMENTS.md`，并禁止未经用户确认擅自弱化、删除或改写约定。
- 首条约定 A-001 记录本次 Garmin v5 在线验收事故：空隔离数据库中的请求缺少开始日期，导致同步范围从预期短窗口回退到 `2022-01-01` 历史起点。
- 后续 Garmin 在线验收必须在审批前明确冻结起止日期、资源白名单、Provider entry 上限、墙钟时限及适用的数据对象预算；任何缺失、越界或历史回退必须在首次数据 Provider entry 前失败关闭。
- 正式 FIT、raw 和 SQLite 继续作为离线处理与分析的数据源；在线验收不得以测试为由重新获取全部历史数据，也不得替换或删除正式数据。
