...
## A-021: M12 FIT-only 每周跑步系统与分模块交付

- 确认日期：2026-09-05；依据用户批准 M12 与逐模块测试、提交和推送的补充。
- 仅对新 M12 替代旧规则中健康采集、日报、七份健康日报输入、高保真邮件、运行时 AI
  Harness、六表限制、统一 Python 入口禁令及 M12 禁止提交/推送的要求。旧合同和失败证据
  保留，不追溯改变历史批次。详细对应关系见 source/docs/legacy-retirement.md。

必须遵守：

1. 仅采集 2022-01-01 起运动 FIT，优先逐字复制已有 FIT；新 SQLite 独立建立。旧代码、库和
   非核心数据统一归档，归档删除后新系统仍可运行；凭据与私人配置不复制入归档。
2. Python 管理 inventory、同步状态/缺口、解析、每周 AI job、输出和动作账本。允许在
   source/skills/_shared/fit_weekly/ 建立模块入口，不恢复 source/src、source/index.py 或
   根产品包。路径由 source/ 或明确的实例根解析，不绑定本机绝对位置。
3. 每日香港时间 22:00 同步当日、昨日及已知缺口，发送简单同步邮件；星期日 15:00 同步并
   冻结本期，活动按结束时间属于前后周日 15:00 的半开窗口。完整分页才可证明无活动；
   当日状态未结束只能 provisional。停机缺口在晚间补跑，多周错过仅补最新周报并标注补发。
4. 周 AI 无状态，只读本期全部活动摘要/分段、最多前四份完整结构化周报、目标快照和当前
   计划背景。必要细读使用本地受限工具；不传 FIT 字节、GPS、路线、活动名称、标识、凭据、
   任意路径或 SQL。不以运动表现代替健康诊断，不恢复健康 API 或每日分析。
5. Easy/未知跑步 2 分钟、明确 SOS work/recovery 圈段或连续主训练 1 分钟，其他运动 5 分钟。
   不猜间歇类型或心率区间；设备历史分区时长及展示占比可用。固定七日跑步计划使用 RPE、
   步骤和技术备注，硬负荷最多三次、日期差至少三天，不补课、不做每日动态替代。
6. 同一验证结果生成 Markdown/PDF、简单邮件和 Garmin 合同。Gmail 继续官方 REST、正常
   原子 Token 刷新、单次发送和 RAW 核验；Garmin 只管理账本精确记录的自有 ID，保留重复组
   与日期核验。unknown 只读对账，成功项不随其他失败重复执行，不继承旧测试授权。
7. 调度由用户手动启动 Python daemon；不安装 cron、launchd 或开机项。离线验证通过后才
   执行获批在线范围，首轮真实周任务等待下个正常星期日 15:00，不提前生成历史发布。
8. 每个独立模块完成适用测试与全新只读验证后正常提交、推送现有 origin/main，并核对远端；
   不强推，不混入私人数据、未验收工作或历史失败产物。旧测试按取消业务逐项映射，适用安全
   测试保留或迁移，不靠删除/弱化/skip/xfail 制造通过。

旧 state 的正式切换、在线调用和全系统完成仍以 M12 冻结合同的前置门与实际验证为准。
