...
| --- | --- | --- | --- | --- | --- |
| ADHOC-0034 | [exec] | 升级最新agentsmd并行Harness及项目技能，保留业务记录 | H34-01～07；来源/固定组合独立验证及主验收，不恢复业务 | 用户2026-09-20要求；[客观要求](exec-plans/evidence/ADHOC-0034/requirements.md) | [执行计划](exec-plans/active/ADHOC-0034-agentsmd-parallel.md) |
...
- 当前唯一任务（2026-09-20）：ADHOC-0034升级官方37520a5脚手架。8文件候选已正式交回并由主核对，主已构造C34/Cuse及各自最小记录增量，完整交接材料已由全新Validator复验及主亲验通过，同版Harness已受控落到原实际目录；当前仅推进本功能独立分支Git/PR交付。原用户工作区的ADHOC-0033及其未提交产品/证据继续暂停保护，不将其源码或整份计划带入本分支。此分支基于实际已合并PR21的origin/main af09e3f；下方旧关注/状态保留基准当时记录，不是继续执行或重复合并的指令。详细进度以本功能执行计划为准。
