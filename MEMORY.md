...
| 事项 | 已确认的决定及理由 | 来源与日期 |
| --- | --- | --- |
| 当前脚手架 | 采用agentsmd固定提交a313e6319b249c2c78e92079f3c0b0b013929660；README及项目.pi六文件采用上游原字节，完整替换旧自定义角色/模型配置。既有AGENTS、通用角色/执行计划模板及网页验收技能与该版一致，索引保留项目边界。允许符合隔离和就绪条件的流水线/跨功能并行，每目录仍最多一名Developer；共享项唯一修改、固定交接，最终组合独立及主验收不可省略。业务计划/记忆不由空模板替换，升级不恢复暂停业务、不改调度器或全局配置。 | 用户再次升级及完全替换.pi要求，2026-09-20；安装事实复核2026-09-21；[本轮来源与范围](exec-plans/evidence/ADHOC-0035/requirements.md)。此前[并行规则升级](exec-plans/evidence/ADHOC-0034/requirements.md)及[原迁移](exec-plans/completed/ADHOC-0029-agentsmd-upgrade.md)保留。 |
